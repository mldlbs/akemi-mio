...
| **CI 复跑**（`Run 67` / `4175641`，35731831643） | ✅ `completed successfully` |

> ⚠️ **但要说清证据强度**：修复后的 CI 样本目前是 **2/2 绿**。
> 在旧的 ≈40% 红率下，「连续两笔都不红」的概率约 **0.36** ——
> 所以这**与修复一致，但还不能证明修复**。
> 真正强得多的是**机制层证据**：该测试的测试体从 1374ms 降到 **96ms**，
> 对 5000ms 的余量从 3.6× 变成 **52×**；超时预算被吃掉的机制（冷 import）已不在窗口内。
> 结论按「机制已消除 + 样本尚未足够」记录，后续运行自然累积。
