...
* ✅ ~~**待拍板**：是否把自检从「报数」升级成判红~~ —— **09-23 用户拍板 A，已实施**
  （`--selfcheck-min=5`，下限对 CI 的 ≈147 有约 **29 倍**余量）。
  这一步属「改门禁语义」，按惯例先给选项。
...
#### ✅ v2：升级成判红（09-23，用户拍板 A）

下限取 **`--selfcheck-min=5`**（默认值）。它不是性能门槛，只用来把
「仪器死了（≈0）」和「仪器活着（CI ≈147 / 本机夹具 ≈42）」分开 —— 两者差三个数量级。

判红分**两条**分支，理由不同，缺一不可：

| 情形 | 判红？ | 理由 |
|---|---|---|
| 注入的动画**没有在运行** | ✅ 红 | 本次**完全没有证据**说明这条链是好的 |
| 注入生效、但 GPU 差值 < 5 | ✅ 红 | 「动画 → GPU」这条链在这台机器上没响应 ⇒ 另两条判据**都不会触发**，绿是空绿 |
| 清理不干净（有残留） | ❌ **不红**，只 notice | 清理发生在**所有测量之后**，不影响任何数字的可信度；它只是本脚本自身的缺陷信号。判红只会白添一条抖动来源 |

⚠️ **只判第二条是不够的**：那样只要注入代码一坏，`自检动画 0/0` 就会让自检变成装饰，
而「注入坏了」本身没有任何后果 —— 这正是 **FM-6（能跑但不可能红）**的形状。
所以**两条都要判**，且用 `if / else if` 保证互斥：两者的差值都是 0，
但对后者说「这条链断了」才是对的，对前者说就是**假话**。

⚠️ 判红条件用的是 **`running`（`playState==='running'`）而不是 `animCount`**：
动画存在但处于 `paused` 时同样造不出 GPU 负载、同样是「没有证据」，
而 `animCount` 会把它错报成「注入生效了」，进而落到第二条分支、
说出「这条链在这台机器上没有响应」这句假话。**这个区别是本轮变异检验实测出来的**（MUT-4）。

#### 变异检验（受控夹具：headless Edge + 零动画页）

每轮都是真实端到端运行（CDP 连真浏览器），不是单元模拟：

| # | 变异 | 期望 | 实测 | 退出码 |
|---|---|---|---|---|
| 基线 | 无 | 通过 | 自检差值 **44.5**，动画 2/2，残留 `0\|el0\|style0` | 0 ✅ |
| MUT-1 | 注入的 div **不挂上去** | 红·分支一 | 动画 `0/0` → 「注入的合成动画没有在运行」+ `::error::` | 1 ✅ |
| MUT-2 | 差值恒为 0（`sc.gpu - sc.gpu`） | 红·分支二 | 动画 2/2、GPU 22.2% 但差值 0.0 → 「链没有响应」 | 1 ✅ |
| MUT-3 | 清理时**留下 div** | **绿** + notice | 残留 `0\|el1\|style0` → notice 触发，**未判红** | 0 ✅ |
| MUT-4 | 挂上去但 **`animation-play-state:paused`** | 红·分支一 | 动画 **`0/2`** → 被 `running` 条件抓住 | 1 ✅ |

MUT-3 与 MUT-4 是这一轮最有价值的两条：前者证明「不该红的没红」，
后者证明「`running` vs `animCount`」这个改动是**承重的**而不是顺手改的。

#### ✅ CI 复跑确认：没有误红（`Run 77` / `Run 78` / `7158566`）

两笔都 `completed successfully`，注解逐字：

| run | 自检 GPU | 差值（下限 5） | 自检动画 | 清理残留 | GPU 进程 | error 注解 |
|---|---|---|---|---|---|---|
| #77 | 148.6% | **148.6** | 2/2 | `0\|el0\|style0` | 1 | **无** ✅ |
| #78 | 148.3% | **148.3** | 2/2 | `0\|el0\|style0` | 1 | **无** ✅ |

CI 上的自检样本累积到 **6 笔**：147.3 / 147.2 / 146.9 / 147.6 / 148.3 / 148.6
（极差 **1.7** 个点，≈1%）。下限 5 对应 **≈29 倍**余量，且这 6 笔里最低的 146.9
也是下限的 29 倍 —— 「判红」这个语义**不会因为正常波动误触发**。

⚠️ **过程中踩到一个夹具陷阱**：MUT-4 第一次跑是**假阴性**（动画 2/2、GPU 41.9%、通过）。
原因是 `SELFCHECK_ON` 是**幂等**的（`if(!d)` 才创建），而 MUT-3 故意留下了那个 div
⇒ 那一轮复用了上一轮的残留元素，变异根本没生效。
**夹具页面跨运行保留状态**，所以每轮变异必须换一个**全新浏览器实例**。
（CI 上每个 run 都是新起的浏览器，不受影响；但这条要记下来，
否则下次复跑这套变异会得到「全都通过」的错误结论。）
...
#### ✅ 扩大覆盖：门禁现在也量 `pet` / `chat` / `wallpaper`（09-23，用户拍板「现在做」）

`scripts/check-idle-gpu.cjs` 新增 `checkForms()`，复用主窗口同一套测量（每形态窗口各
`phase('基线')` + `phase('自检')`，各自一组 CDP WebSocket）。每个形态窗口上做**两件事**：

| 检查 | 判红？ | 理由 |
|---|---|---|
| **空闲动画数**（`getAnimations()` 里 `running.length > 0`） | ✅ **红**（承重） | 形态窗口在空闲时若有*常驻 running 动画* = 正是要抓的回归。走 JS 的 `getAnimations()`，**不依赖 GPU 合成** → 可靠，不会因后台窗口节流而误判 |
| **自检差值**（注入重负载动画后的 GPU 差值） | ❌ **永久只报数**（notice），**不**判红 | 见下「为什么不强判红」——CI 实测形态窗口差值仅 0~1.6 个点（GPU 进程 CPU 全机共享，单窗口注入被淹没），**结构性不可承重** |

⚠️ **为什么不强判红（且永远不会）**：一开始以为是 headless 多标签假象（本机实测
主窗口单标签 44.5% → 多标签掉到 0.1%；pet/chat 0.0、wallpaper 23.2）。但 **CI 复跑
（`#81`/`#82`，真实 Electron）证明形态窗口自检差值也是 0.4~1.6 个点** —— 因为
`phase()` 量的是 **GPU 进程的 CPU**，而 GPU 进程是**全机共享**的：注入单窗口动画对它
的影响被所有窗口的总负载淹没，永远到不了阈值 5。所以这条判据在形态窗口上**结构性不可承重**，
不是「标定前别判红」，而是「标定后也永远不该判红」。**它只剩一个存在性信号的价值**：
自检那一行报了 `自检动画 N/N` = 这个窗口的 target 确实被找到、注入确实落进去了、
清理确实干净（残留 `0|el0|style0`）；报 `⚠️ 注入没在运行` = 这个窗口的测量链没被证明能动，
值得人工看一眼。形态窗口的**承重**全靠上面「空闲动画数」那条（per-window、走 JS）。

⚠️ **拉不起来必须显式报「未覆盖」**：`toggleForm(kind)` 后若 15 秒内没有对应的
`pet.html`/`chat.html`/`wallpaper.html` 调试 target，发一条 `⚠️ 形态窗口 X 未覆盖` 的
notice 并 `continue`，**绝不**静默通过 —— 否则又落回 FM-1（只量主窗口，形态窗口的回归从眼皮下溜走）。

**受控夹具路径验证**（headless Edge + 4 个 target：零动画页 + pet/chat/wallpaper 各一）：
三个形态窗口都找到了 target、量了基线、跑了自检、清理干净（残留 `0|el0|style0`），
自检差值以 notice 发出（`pet` 0.0 / `chat` 0.0 / `wallpaper` 23.2，下限 5）；
空闲动画 0/0 → **不误红**。完整路径已走通。

⚠️ **夹具留下的已知假象**：同一轮里**主窗口**自检在 headless 多标签下也掉到 0.0，
触发 §10.22 的判红（EXIT=1）。这是 headless 多标签 GPU 记账不可信的假象，
**真实 CI 上主窗口自检 delta ≈147**（见 §10.22 的 6 笔样本），所以这道门禁在 CI 上是绿的。
结论：**本机 headless 多标签夹具只能用于走通代码路径，不能用于最终判定**；
扩大覆盖的「不误红 / 能红」最终必须由 CI 复跑确认。

**变异检验状态（诚实记录）**：
* 形态窗口**空闲动画数**判红是承重断言，逻辑与主窗口已证的 `running.length > 0` 同构；
  但一条**专门**的变异检验（让某个形态窗口带一个 running 的常驻空闲动画、删掉判红后应不红）
  需要真实 app 形态窗口（带真实 idle 动画）做夹具，headless 零动画页构造不出来 → **留作 CI 标定阶段的跟进项**（见下 CI 结果里的 pet `4/4` 线索）。
* 形态窗口**自检**升级成判红（v2-for-forms）已被 CI 数据**证伪**：`#81`/`#82` 显示差值
  系统级 ≈0~1.6，结构性不可承重 → **不再计划判红**，保持 notice-only（见上「为什么不强判红」）。

#### ✅ CI 复跑确认：不误红，且拿到形态窗口真实 delta（`Run 81` / `Run 82`）

两笔都 `completed successfully`。⚠️ 关键：形态窗口自检差值在真实 Electron 上**也是 0~1.6 个点**
（不是 headless 假象），证明它是全机共享 GPU 指标的结构性结果 → 自检对形态窗口**不可承重**（见上）。

| run | 窗口 | 自检 GPU | 自检差值（下限 5） | 自检动画 | 空闲动画 | 清理残留 |
|---|---|---|---|---|---|---|
| #81 | pet | 1.8% | **1.6** | 4/4 | 0/0 | `0\|el0\|style0` |
| #81 | chat | 2.2% | **-0.2** | 2/2 | 0/0 | `0\|el0\|style0` |
| #81 | wallpaper | 2.4% | **0.4** | 2/2 | 0/0 | `0\|el0\|style0` |
| #82 | pet | 1.5% | **1.4** | 4/4 | 0/0 | `0\|el0\|style0` |
| #82 | chat | 1.7% | **-0.1** | 2/2 | 0/0 | `0\|el0\|style0` |
| #82 | wallpaper | 2.3% | **0.5** | 2/2 | 0/0 | `0\|el0\|style0` |

主窗口自检（对照）：#81 **147.3** / #82 **147.9** 个点（承重判红照常工作，余量 ~29 倍）。
三形态窗口空闲动画 **全部 0/0** → 承重守卫没误红；自检差值全部 < 5 → 只报数不误红。✅

⚠️ **pet 的 `4/4` 是个该查的线索**：自检注入的是 2 个动画，但 pet 报了 4/4 —— 说明 pet
窗口里**另有 2 个动画在运行**。可对照的是：pet 的「空闲动画数」那条在 `基线` 量到的是 **0/0**。
两种可能：① pet 真有 2 个 idle 动画，但 `基线` 测量发生在窗口刚创建、动画还没起的瞬间，
漏抓了；② 那 2 个是 self-check 注入后 pet 自身的某些继发动画。无论哪种，都说明
「空闲动画数」那条在**窗口刚拉起时**可能量不到已存在的 idle 动画 → **待查**
（属于「扩大覆盖」的承重守卫会不会漏抓真正回归的范畴）。

#### ★★ 09-23 续查 pet `4/4`：**「空闲动画数」判据与产品现实冲突**（判据修正待拍板）

顺着 pet 自检 `4/4` 查下去，挖出一个比「漏抓」更根本的问题——**当前判据的语义是错的**：

**CSS/组件静态证据**（不需要跑浏览器即可确证）：
* `src/renderer/src/forms/pet/styles.css:35-52`：`.pet-svg` 挂 **`pet-breathe` + `pet-float`
  两条 infinite**，`.pet-glow` 挂 `pet-glow-pulse`，左右手各挂 `pet-arm-sway-l/r`；
  `PetAvatar.tsx:107` **无条件**渲染 `<svg class="pet-svg">`。⇒ pet 渲染完成后，
  空闲 running 动画 **≥ 2，至多 5 条** —— 呼吸/漂浮是**产品意图**（宠物就是要动）。
* `src/renderer/src/forms/chat/ChatForm.tsx:77`：`<span class="chat-avatar-dot">` 在标题栏
  **无条件渲染**，挂 `chat-dot-pulse` infinite ⇒ chat 渲染完成后空闲 running ≥ 1。
* wallpaper：0 条 infinite（三条已移除，§10.17 起因）⇒ 空闲 0/0 是真实健康。

**与 CI 数据互证**：
* pet 自检 `4/4` = **2 条注入 + 2 条产品动画**（breathe/float）⇒ pet 的 React 在自检时
  **已渲染完成**，而基线时 `0/0` ⇒ **基线测量发生在 React 渲染完成之前**（`#root` 还空着）。
* chat 自检 `2/2`（纯注入，无产品动画）⇒ chat 的 React 在**整个测量期间都没渲染完**。

**结论**：判据「空闲 running>0 = 红」**只对 wallpaper 成立**。对 pet/chat，它迟早出事：
基线测量若哪天落在 React 渲染完成之后（target 出现更慢、module 缓存更热、机器更快），
...
CI 第一次真正量到形态窗口（`#81`/`#82`），两类问题一次曝光。

#### 仍未做

* ✅ ~~**把自检从「报数」升级成「判红」**~~ —— **09-23 已做**（见上）：
  `--selfcheck-min=5`，两条互斥分支（注入没生效 / 链没响应）各判一次，
  清理不干净**不**判红只 notice；四项变异检验（含「不该红的没红」与 `running` vs
  `animCount` 的承重性）全部通过。✅ **CI 复跑已确认不会误红**（`#77`/`#78`：
  148.6 / 148.3，六笔样本极差 1.7 个点，最差的也有 29 倍余量）。
* ✅ ~~**扩大覆盖**：让门禁也量形态窗口~~ —— **09-23 代码已落地 + CI 确认**（`#81`/`#82` 两笔绿）：
  `checkForms()` 让 pet/chat/wallpaper 各量空闲动画数[判红] + 自检差值[notice-only]；
  拉不起来显式报「未覆盖」。✅ 三窗口在真实 Electron 上都被量到、空闲动画 0/0 → **不误红**；
  ✅ 主窗口自检 delta 147.3/147.9（承重判红仍正常）。
  ✅ **跟进已查完**（见上「判据与产品现实冲突」）：pet 自检 `4/4` = 2 注入 + 2 产品动画
  （breathe/float）⇒ 基线 0/0 是「React 未渲染完」的时序巧合；**判据修正待拍板**（A 白名单 / B 收窄 / C 保持）。

---

### 10.24 ★★★ 形态窗口动画判据在 CI 上**结构性为空**的真根因：runner 的 reduced-motion（`2d29f28`）

**症状**（CI #89/#90 起）：pet / chat / wallpaper 三形态窗口的「空闲动画」恒 `0/0`，
等 React 渲染锚点也没用；而主窗口自检 147% 正常，形态窗口自检差值只有 0.5 / 0.2 / 1.4 个点
（下限 5）—— 门禁自己在注解里说「本次绿可能是空绿」。⇒ §10.22 那个白名单棘轮
**从未在 CI 上被真正求值过**。

**根因**：CI runner 系统级开着「减少动态效果」。`forms/shared.css` 的可达性规则把 `.form-root`
内的产品动画压成 `0.001ms × 1`；finished 且未 fill 的 CSS 动画不再是 relevant，
`getAnimations()` 恒空。⚠️ 而自检注入的 div 挂在 `documentElement`、在 `.form-root` **之外**，
不受该规则影响 —— 这正好解释了「主窗口自检 147% 而产品基线恒 0」这个此前很费解的组合。

**修法**（改前提，不改判据）：CDP `Emulation.setEmulatedMedia({features:[{name:'prefers-reduced-motion',
value:'no-preference'}]})`，主窗口与每个形态窗口各调一次，并把 `matchMedia` 回读进 notice。
门禁要量的是**设计意图**的动画集，不是可达性模式下的**有意**空集。

#### 09-25 补做的验证：原改动只有注释，没有证据

⚠️ **先量到一件必须先知道的事**：本机**原生 `reduce = false`**（探针实测 `matchMedia` +
`document.getAnimations().length = 6`）。⇒ 在本机这个修复**是 no-op**，
「跑一遍门禁通过」**根本不能**证明它有用。所以改用变异检验（把 emulated 值改成 `reduce`）：

| 观测点 | 修复版（`no-preference`） | 变异版（`reduce`） |
|---|---|---|
| 主窗口 基线动画 | `0/6` | **`0/0`** |
| pet-基线 空闲动画 | **`5/5`**（全在白名单） | **`0/0`** |
| chat-基线 空闲动画 | **`1/1`**（chat-dot-pulse） | **`0/0`** |

变异版**逐字复现了 CI #89/#90 的 `0/0` 签名** ⇒ 这行 emulation 是**承重的**。

⚠️ **但变异版门禁仍然退 0（绿）**：`animCount === 0` 时白名单判据被 `if (base.animProbeOk)`
跳过、主窗口只发 notice。⇒ 对这道门禁，**判红/判绿不是变异检验的可观测信号**，
要看**量到的动画集合** —— 这正是「空绿」的定义。

#### 顺带：第一次用实测数据验证了 §10.22 的白名单

白名单当初是**按 CSS 静态分析列的**（因为基线恒 0/0，从未被真实集合检验过）。本次实测：
pet 基线 5 个 running（`pet-breathe / pet-float / pet-glow-pulse / pet-arm-sway-l / pet-arm-sway-r`）
**全在白名单**、chat 1 个（`chat-dot-pulse`）**在白名单** ⇒ **无陌生人、不误红**。
`pet-deco-float` 只在自检相出现（`12/12`），印证注释里「delay 残留」的说法。

#### 一并做的小修

`reduce` 回读原先**只进 notice**，而 `notice()` 只在 `GITHUB_ACTIONS` 下输出 ⇒ 本地跑这道门禁时
这个前提**完全不可见**（我就是在本地撞上这堵墙、才不得不改用变异检验的）。按本文件
「每个事实 console + notice 双通道」的既有惯例，补上本地 console 输出。

#### 未修（记录）

- ⚠️ `wallpaper-基线 动画 探测失败`（三次运行都出现）⇒ wallpaper 的空闲动画判据
  **没有量到任何东西**；同处的 `[wallpaper] 残留 undefined` 也是假警报
  （`SELFCHECK_OFF` 求值超时 → 被当成「清理不干净」）。两者均**非本次引入**。
- ⚠️ 三形态里 `wallpaper: []` 是**空**白名单 —— 前提修好之后，若 wallpaper 其实有产品动画
  会立刻判红。当前它因探测失败而未参与判定，属于「已知未覆盖」而非「已验证安全」。
