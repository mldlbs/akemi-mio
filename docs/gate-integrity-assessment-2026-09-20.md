...
* ✅ ~~**`check:idle-gpu` 的主判据可能在 CI 上永远是死的**~~
  ...
  v1 **只报数不判红**（实测两次差值 43.3 / 19.9，约 2 倍波动，紧阈值会变成 FM-8）；
  **升级成判红**待 CI 收几轮数据。
...
⇒ **A′ 之前 8 个 run 里 4 个红（50%）；A′ 之后 6 个 run 全绿。**
在 50% 红率下连续 6 绿的概率是 `0.5^6 ≈ 1.6%` —— 「红率没变」在 α=0.05 上可以被拒绝。
...
> ⚠️ 但**不要把 6/6 读成「已证明修好」**：0/6 的 95% 单侧上界仍有 **≈39%**
> （解 `(1-p)^6 = 0.05`），也就是「红率降低但没有归零」这个假设**还没有被排除**。
...
标定实测：同一个零动画夹具上两次自检差值分别是 **43.3** 和 **19.9** ——
**约 2 倍波动**。一个拍脑袋定的紧阈值（比如「自检差值必须 ≥ 20」）在第二次就会红，
那就是 FM-8（抖动红）的制造机。

所以顺序是：**先把这个数字在 CI 上收几轮 → 再看它能不能支撑一条判据**。
判它该不该判红的条件：只有当 CI 上的自检差值**稳定地**明显大于 0，才能把它写成
「差值 ≈ 0 ⇒ 仪器失灵 ⇒ 判红」；如果 CI 上它本身就是 0（软件渲染下 GPU 计数不响应），
那么正确的结论不是加判据，而是**承认这两条判据在 runner 上不可用**，
改去用 `document.getAnimations()` 的存在性 + 总 CPU 做替代判据。
