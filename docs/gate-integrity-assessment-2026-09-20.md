...
| **自检差值**（注入重负载动画后的 GPU 差值） | ❌ **暂只报数**（notice），待 CI 标定后升级成判红 | 见下「为什么暂不强判红」 |

⚠️ **为什么暂不强判红**：headless 多标签下，后台窗口**不合成动画、GPU 不响应**，
自检差值会是 0 —— 本机实测：主窗口单标签 44.5% → 多标签掉到 0.1%；pet/chat 直接 0.0、
wallpaper 23.2（wallpaper 那次恰好在前台）。这正是 §10.22 主窗口 v1→v2 走过的节奏：
**先报数、在 CI 上收几轮真实 Electron 窗口的 delta、确认它不是 0 之后，再升级成判红。**
在「标定前」就判红 = 把夹具假象变成 FM-8（抖动红）。
...
* 形态窗口**空闲动画数**判红是承重断言，逻辑与主窗口已证的 `running.length > 0` 同构；
  ...
  需要真实 app 形态窗口（带真实 idle 动画）做夹具，headless 零动画页构造不出来 → **留作 CI 标定阶段的跟进项**。
* 形态窗口**自检**升级成判红（v2-for-forms）的前提是先在 CI 上看到非零 delta；
  届时与 §10.22 一样补两套变异（注入没挂上 / 差值恒 0）。
...
* ✅ ~~**扩大覆盖**：让门禁也量形态窗口~~ —— **09-23 代码已落地**（`checkForms()`，
  pet/chat/wallpaper 各量空闲动画数[判红] + 自检差值[暂报数]；拉不起来显式报「未覆盖」）。
  ⬜ **待 CI 复跑确认不误红**，并**待标定后**把形态窗口自检升级成判红（v2-for-forms）。
