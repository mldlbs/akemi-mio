...
export function createElectronAPI(ipc: IpcRenderer) {
  return {
    ...
    recordBehaviorRetraction: (): Promise<{ success: boolean; error?: string }> => ipc.invoke('tts:behaviorEmotion:recordRetraction'),
    ...
    cancelEmotionalNarrative: (): Promise<{ success: boolean }> => ipc.invoke('emotional-narrative:cancel'),
    ...
    onEmotionalNarrativeStart: (callback: (data: { trend: string; totalSegments: number; contextSummary: string }) => void) => {
      const handler = (
        _event: Electron.IpcRendererEvent,
        data: {
          trend: string
      ...
      return () => {
        ipc.removeListener('emotional-narrative:start', handler)
      }
    },

    onEmotionalNarrativeSegment: (
      callback: (data: { text: string; index: number; total: number; label: string; durationMs: number }) => void,
    ) => {
      const handler = (
        _event: Electron.IpcRendererEvent,
        data: {
      ...
      return () => {
        ipc.removeListener('emotional-narrative:segment', handler)
      }
    },

    onEmotionalNarrativeEnd: (callback: (data: { totalSegments: number; totalDurationMs: number; success: boolean }) => void) => {
      const handler = (
        _event: Electron.IpcRendererEvent,
        data: {
          totalSegments: number
      ...
      return () => {
        ipc.removeListener('emotional-narrative:end', handler)
      }
    },
    ...
    onEmotionalNarrativeCancel: (callback: () => void) => {
      ...
      return () => {
        ipc.removeListener('emotional-narrative:cancel', handler)
      }
    },
    ...
    onToolInvoked: (callback: (data: { tool: string; args: Record<string, any>; id: string }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:tool_invoked', handler)
      }
    },
    ...
    onToolCompleted: (callback: (data: { tool: string; result: string; id: string; latencyMs: number }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:tool_completed', handler)
      }
    },
    ...
    onToolFailed: (callback: (data: { tool: string; error: string; id: string; latencyMs: number }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:tool_failed', handler)
      }
    },
    ...
    onPlanCreated: (callback: (data: { planId: string; title: string }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:plan_created', handler)
      }
    },
    ...
    onPlanStep: (callback: (data: { planId: string; stepIndex: number; status: string }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:plan_step', handler)
      }
    },
    ...
    onPlanCompleted: (callback: (data: { planId: string }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:plan_completed', handler)
      }
    },
    ...
    onPlanFocusSwitched: (
      ...
      return () => {
        ipc.removeListener('agent:plan_focus_switched', handler)
      }
    },
    ...
    onAgentObserve: (
      ...
      return () => {
        ipc.removeListener('agent:observe', handler)
      }
    },
    ...
    onAgentThink: (callback: (data: { requestId: string; step: number; toolCallCount: number; strategyPrompted: boolean }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:think', handler)
      }
    },
    ...
    onAgentReflect: (
      ...
      return () => {
        ipc.removeListener('agent:reflect', handler)
      }
    },
    ...
    onInputReceived: (callback: (data: { text: string; requestId: string; source: string }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:input_received', handler)
      }
    },
    ...
    onResponseGenerated: (callback: (data: { text: string; requestId: string; source: string }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:response_generated', handler)
      }
    },
    ...
    onGuardrail: (callback: (data: { type: string } & Record<string, any>) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:guardrail', handler)
      }
    },
    ...
    onBudgetExhausted: (callback: (data: { resource: string; utilization: number }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:budgetExhausted', handler)
      }
    },
    ...
    onBudgetRestored: (callback: (data: { resource: string; utilization: number }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:budgetRestored', handler)
      }
    },
    ...
    onAgentError: (callback: (data: { error: string; requestId: string }) => void) => {
      ...
      return () => {
        ipc.removeListener('agent:error', handler)
      }
    },
    ...
    onWorkflowRunCreated: (callback: (data: { runId: string; workflowDefId: string }) => void) => {
      ...
      return () => {
        ipc.removeListener('workflow:run_created', handler)
      }
    },

    // 契约以 main 侧发射端为准：WorkflowStoreV2.updateRun() 只发 { runId, status }，
    // 失败原因当前并未随事件下发，故 error 为可选（renderer 侧回退为 'unknown'）。
    onWorkflowRunUpdated: (callback: (data: { runId: string; status: string; error?: string }) => void) => {
      const handler = (_event: Electron.IpcRendererEvent, data: any) => callback(data)
      ipc.on('workflow:run_updated', handler)
      return () => {
        ipc.removeListener('workflow:run_updated', handler)
      }
    },

    // 契约以 main 侧发射端为准：
    //   WorkflowStoreV2.updateStep() → eventBus.emit('workflow.run.step',
    //     { runId, stepId, status, error, agentResult: result })
    // 两个附加字段在原声明里被漏掉，导致 renderer 读 agentResult 时被 TS 判为不存在。
    onWorkflowRunStep: (
      callback: (data: { runId: string; stepId: string; status: string; agentResult?: string; error?: string }) => void,
    ) => {
      const handler = (_event: Electron.IpcRendererEvent, data: any) => callback(data)
      ipc.on('workflow:run_step', handler)
      return () => {
        ipc.removeListener('workflow:run_step', handler)
      }
    },
    ...
    onWorkflowDefCreated: (callback: (data: { workflowDefId: string }) => void) => {
      ...
      return () => {
        ipc.removeListener('workflow:def_created', handler)
      }
    },
    ...
    // ── TTS 语音字幕 ──
    onTtsSubtitle: (callback: (data: { text: string; estimatedDurationMs: number; id: string; startTime: number }) => void) => {
      const handler = (
        _event: Electron.IpcRendererEvent,
        data: {
          text: string
    ...
    setSubtitleEnabled: (enabled: boolean): Promise<{ success: boolean; enabled: boolean }> =>
      ipc.invoke('tts:subtitle:setEnabled', enabled),
    ...
    // ── 桌面悬浮任务面板（TaskPanel） ──
    getTaskPanelState: (): Promise<{ success: boolean; state?: any; error?: string }> => ipc.invoke('taskPanel:getState'),

    toggleTaskPanelVisibility: (): Promise<{ success: boolean; visible: boolean }> => ipc.invoke('taskPanel:toggleVisibility'),

    setTaskPanelVisibility: (visible: boolean): Promise<{ success: boolean; visible: boolean }> =>
      ipc.invoke('taskPanel:setVisibility', visible),

    invokeTaskPanelQuickAction: (
      actionId: string,
      args?: Record<string, string>,
    ): Promise<{ success: boolean; result?: string; error?: string }> => ipc.invoke('taskPanel:invokeQuickAction', actionId, args || {}),

    recordTaskPanelAction: (action: {
      tool: string
      status: 'running' | 'success' | 'error'
      summary: string
    }): Promise<{ success: boolean }> => ipc.invoke('taskPanel:recordAction', action),
    ...
    recordToolParamFeedback: (toolName: string, args: Record<string, any>, rating: number): Promise<{ success: boolean; error?: string }> =>
      ipc.invoke('tool:recordParamFeedback', toolName, args, rating),
    ...
    getWallpaperInteractiveConfig: (): Promise<{ enabled: boolean; shortcut: string }> => ipc.invoke('wallpaper:interactive:getConfig'),
    ...
    refreshBlogKanban: (): Promise<{ success: boolean }> => ipc.invoke('wallpaper:blogKanban:refresh'),
    ...
    onOrganizerProgress: (
      callback: (data: {
        // 与生产端 packages/platform/src/wallpaper/FileOrganizerProgressService.ts 的
        // SessionStatus 对齐。原声明为 string，导致 renderer 侧同名联合类型无法匹配。
        ...
        recentMoves: Array<{
          ...
          ruleId: string
          ruleSource: string
          // 与生产端 FileOrganizerProgressService 的 FileMoveStatus 对齐
          // （原先声明为 string，与 renderer 侧同名的联合类型不匹配）。
    ...
    quickTaskExecute: (taskId: string): Promise<{ success: boolean; task?: any; error?: string }> =>
      ipc.invoke('quickTask:execute', taskId),

    quickTaskDismiss: (taskId: string): Promise<{ success: boolean; task?: any; error?: string }> =>
      ipc.invoke('quickTask:dismiss', taskId),

    quickTaskSnooze: (taskId: string): Promise<{ success: boolean; task?: any; error?: string }> => ipc.invoke('quickTask:snooze', taskId),

    quickTaskEdit: (
      taskId: string,
      steps: Array<{ tool: string; label: string; args?: Record<string, unknown> }>,
    ): Promise<{ success: boolean; task?: any; error?: string }> => ipc.invoke('quickTask:edit', taskId, steps),

    getQuickTaskSnapshot: (): Promise<{ success: boolean; snapshot?: any; error?: string }> => ipc.invoke('quickTask:getSnapshot'),

    quickTaskSetSensitivity: (threshold: number): Promise<{ success: boolean; error?: string }> =>
      ipc.invoke('quickTask:setSensitivity', threshold),
    ...
    voiceBookmarkCreate: (
      summary: string,
      // role 用 string：main 侧 handler 收的是 any[]，实际值来自数据库自由字符串列。
    ...
    voicenoteSetDisplayMode: (mode: 'fixed' | 'scroll'): Promise<{ success: boolean }> => ipc.invoke('voicenote:setDisplayMode', mode),
    ...
    onOrchestrationProgress: (
      callback: (data: {
        ...
        stepStatuses: Array<{
          id: string
          name: string
          // 与生产端 packages/intelligence/src/orchestrator/types.ts 的 StepStatus 对齐
          // （原先声明为 string，与 renderer 侧的联合类型不匹配）。
    ...
    // ── 项目工作区（多个授权目录） ──
    getProjectRoots: (): Promise<{ roots: string[]; active: string }> => ipc.invoke('workspace:getProjectRoots'),
    addProjectRoot: (path: string): Promise<{ success: boolean; error?: string }> => ipc.invoke('workspace:addProjectRoot', path),
    removeProjectRoot: (path: string): Promise<{ success: boolean; error?: string }> => ipc.invoke('workspace:removeProjectRoot', path),
    setActiveProjectRoot: (path: string): Promise<{ success: boolean; error?: string }> =>
      ipc.invoke('workspace:setActiveProjectRoot', path),
    selectProjectRootDialog: (): Promise<string | null> => ipc.invoke('workspace:selectProjectRootDialog'),
  }
}
