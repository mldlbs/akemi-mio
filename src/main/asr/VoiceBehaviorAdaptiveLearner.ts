...
export class VoiceBehaviorAdaptiveLearner {
  ...
  constructor(options?: {
    ...
    minInteractionsForOptimize?: number
    minOptimizeIntervalMs?: number
  }) {
  ...
  ): AdaptiveOptimizationResult | null {
  ...
  getAccentProfile(): AccentProfile {
    ...
    // 偏度计算
    const pitchSkew = n >= 3
      ? this.pitchValues.reduce((sum, v) => sum + Math.pow((v - avgPitch) / Math.max(pitchStd, 0.001), 3), 0) / n
      : 0
    ...
    return {
      ...
      pitchRange: n >= 2
        ? Math.round((Math.max(...this.pitchValues) - Math.min(...this.pitchValues)) * 10) / 10
        : 0,
  ...
  getBayesianVocab(): WordBayesianState[] {
    return Array.from(this.bayesianVocab.values())
      .sort((a, b) => b.correctionProbability - a.correctionProbability)
  }
  ...
  getCorrectionRate(): number {
    return this._totalRecognitions > 0
      ? this._totalCorrections / this._totalRecognitions
      : 0
  }
  ...
  private updateBayesianSuccess(word: string): void {
    ...
    if (existing) {
      ...
      existing.correctionProbability = this.computeBetaProbability(
        existing.successCount,
  ...
  private runOptimization(): AdaptiveOptimizationResult {
    ...
    if (bayesianHotwords.length > 0) {
      for (const word of bayesianHotwords) {
        // 将贝叶斯建议的热词以较高频次注入
        const state = this.bayesianVocab.get(word)
        const repeatCount = state && state.correctionProbability >= BAYESIAN_HIGH_PROB_THRESHOLD
          ? 3
          : 1
