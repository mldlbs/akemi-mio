...
export class AsrAcousticEnvironmentClassifier {
  ...
  private determineEnvironment(features: AudioFeatures): AcousticEnvironment {
    ...
    if (
      ...
      zcr < 0.06 &&
      silenceRatio < 0.4 &&
      pitchHz >= 80 && pitchHz <= 300 &&
      speechRate >= 2 && speechRate <= 10
    ) {
      return 'quiet'
    }
    ...
    if (
      energy < 0.1 &&
      zcr < 0.05 &&
      (pitchHz === 0 || (pitchHz >= 80 && pitchHz <= 250)) &&
      silenceRatio > 0.3
    ) {
      return 'far_field'
    }
    ...
    if (
      zcr > 0.08 &&
      (pitchHz === 0 || (pitchHz > 0 && pitchHz < 60) || pitchHz > 400) &&
      (energy < 0.15 || silenceRatio > 0.5)
    ) {
      return 'noisy'
    }
    ...
    if (
      zcr >= 0.04 && zcr <= 0.1 &&
      energy >= 0.05 && energy <= 0.4 &&
      silenceRatio >= 0.3 && silenceRatio <= 0.6 &&
