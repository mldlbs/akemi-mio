...
export class MemoryAsrHypothesisGenerator {
    ...
    if (this.memoryService) {
      try {
        ...
        if (context.topics.length > 0) {
          try {
            const predictions = this.memoryService.topicTransitionPredictor.predictNextTopics(
              context.topics.slice(0, 3),
              5,
            )
  ...
  private buildPredictedText(keywords: string[], context: AsrConversationContext): string {
    ...
    if (context.recentUserText) {
      parts.push('。最近提到：')
      const snippet = context.recentUserText.length > 40
        ? context.recentUserText.slice(0, 40) + '…'
