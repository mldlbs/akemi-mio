...
import { DevPlan, PlanStep, PlanStoreData } from './types'
...
export class PlanManager {
  ...
  updateStep(planId: string, stepIndex: number, status: PlanStep['status'], result?: string): boolean {
    ...
    if (!plan || stepIndex >= plan.steps.length) return false
  ...
  abandonPlan(planId: string, reason?: string): boolean {
    ...
    plan.updatedAt = Date.now()
    if (reason) plan.reflection = reason
    this.save()
  ...
  cleanupOldPlans(completedCutoff: number, abandonedCutoff: number): number {
    ...
    this.data.plans = this.data.plans.filter((p) => {
      if (p.status === 'completed' && p.createdAt < completedCutoff) return false
      if (p.status === 'abandoned' && p.createdAt < abandonedCutoff) return false
      return true
    })
