...
import { ResponseValidator, formatValidationSummary } from '../ResponseValidator';
...
describe('ResponseValidator', () => {
    ...
    it('stopAndValidate analyze 模式检测 write_file', () => {
        validator.startListening();
        eventBus.emit('agent.tool.invoked', { tool: 'read_file', args: { path: 'x' } });
        eventBus.emit('agent.tool.invoked', { tool: 'write_file', args: { path: 'y' } });
    ...
    it('analyze 模式 edit_file 算违规', () => {
        validator.startListening();
        eventBus.emit('agent.tool.invoked', { tool: 'edit_file', args: {} });
        const result = validator.stopAndValidate('analyze');
        expect(result.passed).toBe(false);
    });
    it('analyze 模式 create_plugin 算违规', () => {
        validator.startListening();
        eventBus.emit('agent.tool.invoked', { tool: 'create_plugin', args: {} });
        const result = validator.stopAndValidate('analyze');
        expect(result.passed).toBe(false);
    });
    it('execute 模式 create_dev_plan 算违规', () => {
        validator.startListening();
        eventBus.emit('agent.tool.invoked', { tool: 'create_dev_plan', args: {} });
        const result = validator.stopAndValidate('execute');
        expect(result.passed).toBe(false);
    });
    it('execute 模式 write_file 不算违规', () => {
        validator.startListening();
        eventBus.emit('agent.tool.invoked', { tool: 'write_file', args: { path: 'x' } });
        const result = validator.stopAndValidate('execute');
    ...
    it('错误过多触发警告', () => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            eventBus.emit('agent.tool.failed', { tool: 'read_file', error: 'err' });
        }
        const result = validator.stopAndValidate('analyze');
