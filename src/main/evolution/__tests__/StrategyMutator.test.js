...
function createMutator() {
    ...
    const strategies = mutator.strategies;
    const scores = mutator.scores;
...
describe('StrategyMutator', () => {
    it('targetDimension planQuality 变异 promptMode', () => {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (m && (m.reason === 'promptMode' || m.reason === 'timeoutMs'))
                foundTargeted = true;
        }
    ...
    it('targetDimension analysisDiversity 变异 degenerationThreshold', () => {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (m && (m.reason === 'degenerationThreshold' || m.reason === 'maxHistoryEntries'))
                foundTargeted = true;
        }
        expect(foundTargeted).toBe(true);
    });
