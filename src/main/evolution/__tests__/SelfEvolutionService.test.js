...
describe('SelfEvolutionService — 集成测试', () => {
    ...
    function warmupReady(service) {
        ;
        service.firstRunComplete = true;
    }
    describe('detectPlanMode（通过 tryRun 间接验证）', () => {
        ...
        it('有活跃计划时，prompt 应包含计划上下文注入', async () => {
            ...
            const planMode = service.analyzer.detectPlanMode();
    ...
    describe('错误计数器与步骤重试修复', () => {
        it('错误计数器不应在 executor 入口重置', async () => {
            ...
            service.executor.planExecConsecutiveErrors = 0;
            warmupReady(service);
            const execCtx = {
        ...
        it('连续 3 步失败应触发自动放弃计划', async () => {
            ...
            await service.executor.executeNextStep(execCtx(0));
            await service.executor.executeNextStep(execCtx(1));
            await service.executor.executeNextStep(execCtx(2));
            expect(mockPlan._abandonedPlans().length).toBeGreaterThanOrEqual(1);
        });
    ...
    describe('状态持久化 — saveState/loadState 闭环', () => {
        it('saveState + loadState 应完整保持 currentAnalysisTimeoutMs / promptTrimMode / historyMaxEntries', async () => {
            ...
            service.analyzer.currentAnalysisTimeoutMs = 250000;
            service.analyzer.promptTrimMode = false;
            service.analyzer.historyMaxEntries = 10;
            service.saveState();
            // 创建新实例，加载同一文件
            ...
            expect(service2.analyzer.currentAnalysisTimeoutMs).toBe(250000);
            expect(service2.analyzer.promptTrimMode).toBe(false);
            expect(service2.analyzer.historyMaxEntries).toBe(10);
        });
        it('旧状态文件（不包含新字段）应优雅降级为默认值', async () => {
            ...
            // 应使用默认值
            expect(service.analyzer.currentAnalysisTimeoutMs).toBe(120000);
            expect(service.analyzer.promptTrimMode).toBe(false);
            expect(service.analyzer.historyMaxEntries).toBe(5);
        });
    });
    describe('步骤 2: buildLivingPlanContext — 上下文裁剪优化', () => {
        it('无 living plan 文件时应返回模板上下文', async () => {
            ...
            const context = service.analyzer.buildLivingPlanContext();
    ...
    describe('步骤 2b: getHistorySummary — 历史摘要缩减', () => {
        it('历史摘要应只取最近 1 条记录', async () => {
            ...
            const summary = service.analyzer.getHistorySummary();
