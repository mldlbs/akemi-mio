...
describe('EvolutionStrategizer', () => {
  ...
  it('select 返回策略配置', () => {
    const config = strategizer.select({ consecutiveFailures: 0, isFirstRun: true, isRecovering: false, hoursSinceLastRun: 0, isDegenerate: false })
    expect(config.name).toBe('full')
  })

  it('select 退化时返回 review', () => {
    const config = strategizer.select({ consecutiveFailures: 0, isFirstRun: false, isRecovering: false, hoursSinceLastRun: 24, isDegenerate: true })
    expect(config.name).toBe('review')
  })

  it('select 多次失败时返回 review', () => {
    const config = strategizer.select({ consecutiveFailures: 5, isFirstRun: false, isRecovering: false, hoursSinceLastRun: 24, isDegenerate: false })
    expect(config.name).toBe('review')
  })

  it('evaluate 更新策略分', () => {
    strategizer.evaluate('balanced', { success: true, durationMs: 5000, planCreated: true, stepsPlanned: 2, hadTimeout: false, hadRetry: false, promptTrimmed: false })
