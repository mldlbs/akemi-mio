...
describe('EvolutionGitOps', () => {
    ...
    it('autoGitCommit 调用 git add 和 commit', async () => {
        ...
        expect(execAsync).toHaveBeenCalledWith('git add -A', { timeout: 15000 });
        expect(execAsync).toHaveBeenCalledWith('git commit -m "[evolution] 测试计划"', { timeout: 15000 });
    });
    it('autoGitCommit 失败时不抛出', async () => {
        ;
        execAsync.mockRejectedValueOnce(new Error('nothing to commit'));
    ...
    it('workspacePreCheck 在 dirty 文件多时 stash', async () => {
        ;
        execAsync.mockResolvedValueOnce(' M f1\n M f2\n M f3\n M f4\n M f5\n M f6\n');
    ...
    it('workspacePostRestore 调用 stash pop', async () => {
        ...
        expect(execAsync).toHaveBeenCalledWith('git stash pop', { timeout: 15000 });
    });
    it('workspacePostRestore 失败时不抛出', async () => {
        ;
        execAsync.mockRejectedValueOnce(new Error('nothing to pop'));
        await expect(gitOps.workspacePostRestore()).resolves.toBeUndefined();
    });
    it('collectChangedFiles 解析 git status', async () => {
        ;
        execAsync.mockResolvedValueOnce(' M src/test.ts\n?? new_file.ts\nA  added.ts\nR  old.ts -> new.ts\n');
