...
// =============================================================================
// ResponseValidator — LLM 响应合规性验证
// =============================================================================
//
// 问题背景:
// SelfEvolutionService 发送 prompt 给 LLM 后，通过 runSelfTask 调用，
// 返回结果只有 { success, summary }，不包含 LLM 实际调用了哪些工具。
// 这导致分析模式（禁止 write_file/edit_file）与执行模式之间的约束无法验证。
//
// 解决方案:
// 通过 eventBus 监听 agent.tool.invoked/completed/failed 事件，
// 在分析/执行模式结束后校验 LLM 是否遵守了模式约束。
//
// 验证规则:
// - 分析模式 (ANALYZE): 禁止 write_file, edit_file, create_plugin
// - 执行模式 (EXECUTE): 鼓励写文件，但禁止 create_dev_plan（应更新已有计划）
// - 两种模式都禁止: 在 project_root 之外猜测路径
// =============================================================================
...
export class ResponseValidator {
  ...
  startListening(): void {
    ...
    const onInvoked = (data: { tool: string; args: Record<string, any> }) => {
    ...
    const onCompleted = (data: { tool: string; result: string }) => {
    ...
    const onFailed = (data: { tool: string; error: string }) => {
      const last = this.toolCallBuffer[this.toolCallBuffer.length - 1]
  ...
  stopAndValidate(mode: EvolutionMode): ValidationResult {
    ...
    for (const call of this.toolCallBuffer) {
      ...
      if (WRITE_TOOLS.has(call.name)) write++
