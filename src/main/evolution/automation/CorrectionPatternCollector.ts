...
const CORRECTION_KEYWORDS = [
  '不满', '纠正', '重复提问', 'error_pattern', 'short_response',
  '不正确', '错了', '不对', '不是这样', '重做', '不对吧',
  '重复错误', '同样的错误', '又错了', '修复失败', '回滚',
]
...
export class CorrectionPatternCollector implements SignalCollector {
  ...
  async collect(): Promise<Problem[]> {
    try {
      ...
      for (const entry of entries) {
        ...
        const matchedKeyword = CORRECTION_KEYWORDS.find((kw) =>
      ...
      for (const [topic, data] of significantTopics) {
        ...
        problems.push({
          ...
          lastSeen: Date.now(),
          occurrenceCount: isSeen
            ? (this.seenTopics.get(topicId) ?? 0) + 1
            : data.count,
          context: {
            raw: [
              `重复纠正模式分析:`,
  ...
  private buildDescription(
    topic: string,
    data: { count: number; samples: string[] },
    module: string,
  ): string {
    const lines: string[] = [
