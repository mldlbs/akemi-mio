...
const CANDIDATE_STOPWORDS = new Set([
  '一个', '没有', '我们', '你们', '他们', '什么', '怎么', '为什么',
  '因为', '所以', '但是', '可是', '不过', '而且', '或者', '如果',
  '虽然', '然而', '于是', '因此', '可以', '可能', '应该', '必须',
  '需要', '能够', '已经', '曾经', '正在', '将要', '一直', '还是',
  '就是', '只是', '不是', '不用', '不能', '不会', '不行', '不要',
  '这个', '那个', '这些', '那些', '这里', '那里', '哪里',
  '自己', '大家', '别人', '所有', '一些', '一点', '很多',
  '时候', '时间', '地方', '东西', '事情', '问题', '方法',
  '今天', '昨天', '明天', '现在', '以前', '以后',
  ...
  async execute(problem: AssignedProblem): Promise<FixResult> {
    ...
    try {
      ...
      const similarWord = existingVocab.find(
        (v) => levenshteinDistance(v.word.toLowerCase(), cacheKey) <= CLUSTER_EDIT_DISTANCE_THRESHOLD,
      )
      if (similarWord) {
        return {
          problemId: problem.id,
