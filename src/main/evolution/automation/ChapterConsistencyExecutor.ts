...
JSON 结构：
{
  ...
  private extractContext(fullContent: string, originalText: string): string {
    ...
    context =
      context.slice(0, issueStart) +
      '【问题片段开始】' +
      context.slice(issueStart, issueEnd) +
      '【问题片段结束】' +
      context.slice(issueEnd)
   ...
   * 优先选择低风险、高评分方案。
   */
  private selectBestPatch(
    allPatches: { issue: any; candidates: PatchCandidate[] }[],
  ): { issue: any; candidate: PatchCandidate } | null {
    if (allPatches.length === 0) return null

    // 展平所有方案
    const flat = allPatches.flatMap((p) =>
      p.candidates.map((c) => ({ issue: p.issue, candidate: c })),
    )
