...
import { behaviorAdjustmentJournal } from '../../behavior/BehaviorAdjustmentJournal'
import { behaviorUsagePatternAnalyzer } from '../../behavior/BehaviorUsagePatternAnalyzer'
import { behaviorRuleEngine } from '../../behavior/BehaviorRuleEngine'
import type { AdjustmentType } from '../../behavior/BehaviorAdjustmentJournal'
...
export class BehaviorParamAdjustmentExecutor implements FixExecutor {
   ...
   */
  private async handleTtsVoiceSoften(
    ...
    try {
      ...
      const { ttsConfigManager } = await import('../../tts/TtsConfigManager')
    ...
    return {
      ...
      success: true,
      summary: success
        ? `✅ 深夜模式：TTS 音色已切换到柔和模式（${SOFT_TTS_VOICE}）`
        : `✅ 深夜模式：TTS 参数无需变更（可能与当前一致）`,
      durationMs: Date.now() - startTime,
   ...
   * 使系统在对话中能主动提供相关信息。
   */
  private async handleMemoryEnhance(
    problem: AssignedProblem,
    metadata: Record<string, string>,
    startTime: number,
  ): Promise<FixResult> {
    ...
    try {
      const { getMemoryService } = await import('../../tool/deps')
    ...
    this.lastSummary = memoryCreated
      ? '✅ Memory 摘要已增强'
      : '⚠️ Memory 摘要增强未生效（MemoryService 不可用）'
   ...
   */
  private async handleResponseWarmth(
    problem: AssignedProblem,
    metadata: Record<string, string>,
    startTime: number,
  ): Promise<FixResult> {
