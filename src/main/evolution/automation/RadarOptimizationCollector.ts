...
/** 观察者采集器目录 */
const COLLECTORS_DIR = 'src/main/observer/collectors'
...
function detectHardcodedUrls(content: string, lines: string[]): ScanFinding[] {
...
function detectMissingTimeout(content: string, lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = []
...
function detectMissingResOkCheck(content: string, lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = []
...
/** 缺少备用 API 端点检测 */
function detectMissingFallbackApis(lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = []
  // 查找类属性中定义 apis 数组的
...
/** 检测是否有重试逻辑 */
function detectMissingRetryLogic(content: string, lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = []
...
function detectMissingMetrics(content: string, lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = []
...
/** 检测硬编码密钥/Token */
function detectHardcodedSecrets(content: string, _lines: string[]): ScanFinding[] {
  const findings: ScanFinding[] = []
  // 检测常见的密钥模式
...
export class RadarOptimizationCollector implements SignalCollector {
  ...
  async collect(): Promise<Problem[]> {
    ...
    const files = readdirSync(collectorsAbsDir).filter(
    ...
    for (const file of files) {
      ...
      for (const rule of SCAN_RULES) {
        const findings = rule.check(content, lines)
