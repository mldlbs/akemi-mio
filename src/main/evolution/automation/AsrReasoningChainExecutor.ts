...
export class AsrReasoningChainExecutor implements FixExecutor {
  ...
  private generateReasoningChain(problem: AssignedProblem): ReasoningChain {
    ...
    // Step 2: 生成热词优化方案
    if (corrected) {
      const hotwordDetail = corrected.length >= 2
        ? `将 "${corrected}" 加入热词列表以提升后续识别率`
        : `评估是否需将 "${corrected}" 加入热词`
      steps.push(this.makeStep(stepIndex++, timestamp,
        '生成热词优化方案',
        hotwordDetail,
      ))
    ...
    steps.push(this.makeStep(stepIndex++, timestamp,
      '验证补丁兼容性',
      `检查 "${corrected}" 是否已存在于热词表，避免重复添加`,
    ))

    // Step 4: 执行 ASR 配置补丁
    steps.push(this.makeStep(stepIndex++, timestamp,
      '应用 ASR 配置补丁',
      `通过 AsrEvolutionManager 提交 "${corrected}" 热词配置`,
    ))
  ...
  private stepValidate(
    ...
    const similarWord = existingVocab.find(
      (v) => levenshteinDistance(v.word.toLowerCase(), cacheKey) <= similarThreshold,
    )
