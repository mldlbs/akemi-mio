...
export class BehaviorUsageCollector implements SignalCollector {
      ...
      for (const adj of activeAdjustments) {
        ...
        switch (adj.type) {
          case 'tts_voice_soften': {
            ...
            const positiveDesc = adj.context.lateNightRatio > report.timePattern.lateNightRatio
              ? '深夜活跃占比下降，调整效果正面'
              : '深夜活跃占比稳定或上升'
          ...
          case 'memory_summary_enhance': {
            ...
            const positiveDesc = currRepeatCount < prevRepeatCount
              ? '重复话题数减少，调整效果正面'
              : '重复话题数未减少或增加'
            behaviorAdjustmentJournal.recordEffect(adj.id, {
          ...
          case 'response_warmth_increase': {
            // 评估：负面情感占比是否下降
            const positiveDesc = report.sentiment.negativeRatio < adj.context.negativeRatio
              ? '负面情感占比下降，调整效果正面'
              : '负面情感占比未改善'
            behaviorAdjustmentJournal.recordEffect(adj.id, {
  ...
  async collect(): Promise<Problem[]> {
    ...
    try {
      ...
      // ── 1. 深夜 TTS 模式检测 ──
      if (
        report.timePattern.isCurrentlyLateNight &&
        report.timePattern.lateNightRatio >= 0.2 &&
        !activeTypes.has('tts_voice_soften')
      ) {
...
24 小时分布: ${report.timePattern.hourlyCounts.join(',')}`,
      ...
      // ── 2. 重复提问记忆增强检测 ──
      if (
        report.repeatedPattern.hasSignificantRepeat &&
        !activeTypes.has('memory_summary_enhance')
      ) {
...
窗口交互数: ${report.repeatedPattern.totalInteractions}`,
      ...
      // ── 3. 负面情感提示 ──
      if (
        report.sentiment.negativeRatio >= 0.3 &&
        !activeTypes.has('response_warmth_increase')
      ) {
        problems.push({
