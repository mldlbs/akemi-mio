...
describe('EvidenceCollector — severity 映射', () => {
  it('passRate = 1.0 不产生 Problem', () => {
    const c = new EvidenceCollector()
    expect(c.convert(mockReport({ summary: { total: 44, passed: 44, failed: 0, skipped: 0, durationMs: 1500, passRate: 1.0, status: 'pass' } }))).toHaveLength(0)
  })

  it('passRate = 0.9（>= 0.8）不产生 Problem', () => {
    const c = new EvidenceCollector()
    expect(c.convert(mockReport({ summary: { total: 44, passed: 40, failed: 4, skipped: 0, durationMs: 1500, passRate: 0.9, status: 'pass' } }))).toHaveLength(0)
  })

  it('passRate = 0.68（< 0.8）产生 warning Problem', () => {
    const c = new EvidenceCollector()
    const problems = c.convert(mockReport({ summary: { total: 44, passed: 30, failed: 14, skipped: 0, durationMs: 1500, passRate: 0.68, status: 'fail' } }))
  ...
  it('passRate = 0.45（< 0.5）产生 error Problem', () => {
    const c = new EvidenceCollector()
    const problems = c.convert(mockReport({ summary: { total: 44, passed: 20, failed: 24, skipped: 0, durationMs: 1500, passRate: 0.45, status: 'fail' } }))
    expect(problems.length).toBeGreaterThanOrEqual(1)
...
describe('EvidenceCollector — 能力退化映射', () => {
  it('单 capability regression 产生一个 Problem', () => {
    const c = new EvidenceCollector()
    const problems = c.convert(mockReport({
      summary: { total: 44, passed: 40, failed: 4, skipped: 0, durationMs: 1500, passRate: 0.9, status: 'pass' },
      capability: { regressed: [{ capability: 'tool.execution', failedCount: 3, totalCount: 10, affectedCaseIds: ['T01', 'T02', 'T03'] }], intact: ['cause_effect'] },
    }))
  ...
  it('混合：低 passRate + 能力退化 = 两者共存', () => {
    ...
    const problems = c.convert(mockReport({
      ...
      capability: { regressed: [{ capability: 'tool.execution', failedCount: 5, totalCount: 10, affectedCaseIds: ['T01'] }], intact: ['cause_effect'] },
    }))
...
describe('EvidenceCollector — 去重 & 置信度', () => {
  ...
  it('不同 report（不同 sha）不触发去重', () => {
    const c = new EvidenceCollector()
    const r1 = mockReport({ _unique: 'abc123', summary: { total: 44, passed: 20, failed: 24, skipped: 0, durationMs: 1500, passRate: 0.45, status: 'fail' } })
    const r2 = mockReport({ _unique: 'def456', summary: { total: 44, passed: 20, failed: 24, skipped: 0, durationMs: 1500, passRate: 0.45, status: 'fail' } })
  ...
  it('适量样本产生合理置信度', () => {
    const c = new EvidenceCollector()
    const problems = c.convert(mockReport({
      capability: { regressed: [{ capability: 'tool.execution', failedCount: 5, totalCount: 10, affectedCaseIds: ['T01', 'T02'] }], intact: [] },
    }))
    expect(problems).toHaveLength(1)
  ...
  it('confidence 上限 0.95', () => {
    const c = new EvidenceCollector()
    const problems = c.convert(mockReport({
      capability: { regressed: [{ capability: 'tool.execution', failedCount: 20, totalCount: 20, affectedCaseIds: Array.from({ length: 20 }, (_, i) => `T${i}`) }], intact: [] },
    }))
    expect(problems).toHaveLength(1)
