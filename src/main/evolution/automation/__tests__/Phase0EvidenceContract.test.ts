...
import type { Problem, ProblemSource, Severity, AssignedProblem, FixResult, SignalCollector, FixExecutor } from '../automation/types'
import { ReportGenerator } from '../../../reasoning/golden/ReportGenerator'
import type { RegressionReport, ReplayResult } from '../../../reasoning/golden/types'
...
describe('Phase 0: Problem 扩展兼容性', () => {
  ...
  it('tryFix 执行器匹配不受新 source 影响', () => {
    ...
    const matching = executors.filter((e) =>
      (e.supportedSources as string[]).includes('evidence'),
    )
...
describe('Phase 0: ReportGenerator 架构边界', () => {
  ...
  function mockResult(overrides?: Partial<ReplayResult>): ReplayResult {
    return {
      ...
      durationMs: 1523,
      failures: [
        { caseId: 'Q01', expected: { pattern: 'cause_effect', goals: ['g1'], constraints: [], outputStyle: 'json' }, actual: { pattern: 'hypothesis_verification', goals: ['g1'], constraints: [], outputStyle: 'json' }, diff: 'p: cause_effect vs hypothesis_verification' },
      ],
      runnerVersion: '0.1',
      executedAt: '2026-07-21T19:00:00.000Z',
      ...overrides,
...
describe('Phase 0: ProposalValidator 边界', () => {
  it('Evidence 派生的 Proposal 可满足 checkScope 要求', () => {
    ...
    const evidenceProposal = {
      ...
      targetFiles: ['src/main/tool/definitions/MemoryTools.ts'],
  ...
  it('assessRegressionRisk 将 evidence 类默认为 low risk', () => {
    // assessRegressionRisk 检查路径是否包含 core/eventbus/scheduler/lifecycle/constitution
    // Evidence 类问题默认不修改这些路径
    const safePaths = ['src/main/tts/piper/', 'src/main/evolution/automation/', 'src/main/config/']
    const riskyPaths = ['src/main/core/', 'src/main/scheduler/']

    const assessRisk = (files: string[]): 'low' | 'medium' | 'high' => {
      const risky = files.filter((f) =>
        ['core/', 'eventbus', 'scheduler', 'lifecycle', 'constitution'].some((p) => f.toLowerCase().includes(p)),
      )
    ...
    // evidence 默认指向非核心路径
    expect(assessRisk(safePaths)).toBe('low')
    // 如果 proposal 涉及核心路径
    expect(assessRisk(riskyPaths)).toBe('high')
  })
})
