...
import type { ConfigChangeType, ToolTunableConfig } from '../../tool/ToolConfigManager'
...
export class ToolConfigOptimizationExecutor implements FixExecutor {
  ...
  async execute(problem: AssignedProblem): Promise<FixResult> {
    ...
    try {
      ...
      // ── 根据优化类型生成配置变更 ──
      const { changeType, newValues, reason } = this.buildConfigChange(
        suggestion,
        metadata,
      )
      ...
      const { snapshotId, changed } = toolConfigManager.applyChange(
        toolName,
        changeType,
        newValues,
        reason,
