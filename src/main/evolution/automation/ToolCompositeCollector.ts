...
import { patternMiner } from '../../behavior/PatternMiner'
...
export class ToolCompositeCollector implements SignalCollector {
  ...
  async collect(): Promise<Problem[]> {
    ...
    try {
      ...
      const existingFingerprints = new Set(
        existingProposals.map((p) => p.toolSequence.join('→')),
      )
        ...
        const problem: Problem = {
          ...
          context: {
            raw: [
              ...
              ...candidate.steps.map(
                (s, i) =>
                  `  ${i + 1}. ${s.toolName} - ${s.description}${s.optional ? ' (可选)' : ''}`,
              ),
            ].join('\n'),
            metadata: {
  ...
  private buildDescription(candidate: {
    ...
    const sequenceDesc = candidate.toolSignature.join(' → ')
    const stepsDesc = candidate.steps
      .map((s, i) => `  ${i + 1}. ${s.toolName}: ${s.description}`)
      .join('\n')
