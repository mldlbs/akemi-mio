...
export class AutoPatchExecutor implements FixExecutor {
  ...
  private applyPatch(
    filePath: string,
    action: FixAction,
  ): { success: boolean; error?: string; diff?: string } {
  ...
  private injectMkdirGuard(lines: string[], writeLineIdx: number): string {
    ...
    if (writeLine.includes('existsSync') || lines.slice(Math.max(0, writeLineIdx - 3), writeLineIdx).some((l) => l.includes('existsSync'))) {
      return lines.join('\n')
    }
  ...
  private injectJsonTryCatch(lines: string[], parseLineIdx: number): string {
    ...
    let safeParseLine = parseLine.trim()
  ...
  private applyNullGuard(content: string, action: FixAction): string {
    ...
    for (let i = scanStart; i < scanEnd; i++) {
      ...
      if (chainMatch) {
        ...
        const isGuarded = lines.slice(Math.max(0, i - 3), i).some(
          (l) => l.includes(`${varName} != null`) || l.includes(`${varName} !== null`) || l.includes(`${varName} !== undefined`),
        )
        if (isGuarded) continue

        // 将 . 替换为 ?.
        lines[i] = line.replace(
          new RegExp(`(\\b${varName})\\.(\\w+)`),
          `$1?.$2`,
        )
        modified = true
