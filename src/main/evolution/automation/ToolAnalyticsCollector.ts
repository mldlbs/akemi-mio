    ...
    try {
      ...
      for (const report of summary.reports) {
        ...
        if (
          ...
          const boost = Math.min(Math.floor(
            (1 - report.errorRate) * 3 +
            (report.latency.avg < 50 ? 1 : 0)
          ), 5)
          priority = boost
        ...
        if (
          !problemType &&
          report.latency &&
          report.latency.avg > COLLECTOR_CONFIG.HIGH_LATENCY_MS &&
        ...
        // ── 条件 E: 极高错误率（> 50%）→ 建议禁用 ──
        if (
          report.errorRate >= 0.50 &&
          report.totalCalls >= COLLECTOR_CONFIG.MIN_SAMPLES
        ) {
          priority = -10
        ...
        if (problemType && priority !== 0) {
          ...
          problems.push({
            ...
            source: 'tool',
            severity: report.errorRate >= 0.50 ? 'error' : 'warning',
            title,
            description,
          ...
          // 标记冷却
          this.toolCooldowns.set(
            report.toolName,
            Date.now() + COLLECTOR_CONFIG.TOOL_COOLDOWN_MS,
          )
        }
      }
      ...
      const prioritizedProblems = candidates
        ...
        .map((c) => problems.find((p) =>
          p.context.metadata?.toolName === c.tool.toolName,
        )!)
        .filter(Boolean)
      ...
      if (prioritizedProblems.length > 0) {
        log('INFO', 'tool_analytics_collector_problems', {
          ...
          tools: prioritizedProblems.map((p) =>
            `${p.context.metadata?.toolName}(${p.context.metadata?.suggestion})`,
          ).join(', '),
        })
      }
  ...
  private buildReduceDescription(report: any): string {
    return [
      ...
      report.failurePatterns.length > 0
        ? `- 主要失败模式: ${report.failurePatterns[0].description}`
        : '',
      ``,
