...
const TOOL_SOURCE_MAP: Record<string, string> = {
  read_file: 'src/main/tool/definitions/ReadFileTool.ts',
  write_file: 'src/main/tool/definitions/WriteFileTool.ts',
  edit_file: 'src/main/tool/definitions/EditFileTool.ts',
  grep: 'src/main/tool/definitions/GrepTool.ts',
  list_files: 'src/main/tool/definitions/ListFilesTool.ts',
  run_command: 'src/main/tool/definitions/RunCommandTool.ts',
  remember_fact: 'src/main/tool/definitions/RememberFactTool.ts',
  analyze_codebase: 'src/main/tool/definitions/AnalyzeCodebaseTool.ts',
  generate_image: 'src/main/tool/definitions/ImageGenerationTool.ts',
  writing_system: 'src/main/tool/definitions/WritingTool.ts',
  cicd_trigger: 'src/main/tool/definitions/CicdTools.ts',
  memory_search: 'src/main/tool/definitions/MemoryLoadTool.ts',
  memory_save: 'src/main/tool/definitions/MemorySaveTool.ts',
  telegram_send: 'src/main/tool/definitions/TelegramGatewayTools.ts',
  workflow_run: 'src/main/tool/definitions/WorkflowTools.ts',
}
...
function buildLearningCurvePrompt(
  toolName: string,
  errorPatterns: string[],
  sourceCode: string,
  userLevel: string,
): string {
  const errorPatternGuide = errorPatterns.length > 0
...
export class LearningCurveExecutor implements FixExecutor {
  ...
  private async executeSuggest(
    ...
    try {
      ...
      // 发出通知事件（供 UI/用户确认使用）
      eventBus.emit('evolution.learning_curve.patch_ready' as any, {
        patchId,
        toolName,
        sourceFile: filePath,
        patchFile,
    ...
    try {
      ...
      try {
        await this.gitOps.autoGitCommit(
          `[learning-curve] improve error handling for ${toolName} based on user error patterns`,
        )
      } catch {
        log('WARN', 'learning_curve_apply_commit_skip', { toolName })
      }

      eventBus.emit('evolution.learning_curve.patch_applied' as any, {
        toolName,
        sourceFile: filePath,
        snapshotTag,
        errorPatterns,
  ...
  private checkImportIntegrity(original: string, improved: string): boolean {
    ...
    const originalImports = new Set([
      ...(original.match(importRegex) || []),
      ...(original.match(requireRegex) || []),
    ])

    const improvedImports = new Set([
      ...(improved.match(importRegex) || []),
      ...(improved.match(requireRegex) || []),
    ])
