...
import type { AssignedProblem, FixResult, FixExecutor, ProblemSource } from '../automation/types'
...
export class TypeRefactorExecutor implements FixExecutor {
  ...
  async execute(problem: AssignedProblem): Promise<FixResult> {
    ...
    try {
      ...
      // 4. 生成重构代码（使用 LLM）
      const refactorResult = await this.generateRefactor(
        category,
        suggestion,
        filePath,
        contextLines,
        targetLineIdx - contextStart,
      )
  ...
  private refactorAddParamType(
    ...
    // 为没有类型的参数添加 : unknown
    const result = targetLine.replace(
      /\(([^)]*)\)/,
      (match, params: string) => {
