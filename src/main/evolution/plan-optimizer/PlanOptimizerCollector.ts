...
export class PlanOptimizerCollector implements SignalCollector {
   ...
   * 将优化建议转换为 Problem 条目。
   */
  private suggestionToProblem(
    suggestion: OptimizationSuggestion,
    analyzedAt: number,
  ): Problem | null {
    const planTitles = [
      ...new Set(suggestion.affectedTasks.map((t) => t.planTitle)),
    ]
    ...
    const description = [
      ...
      ``,
      ...suggestion.affectedTasks.map(
        (t) => `  [${t.planTitle}] 步骤 ${t.stepIndex}: ${t.description.slice(0, 80)}`,
      ),
      ``,
