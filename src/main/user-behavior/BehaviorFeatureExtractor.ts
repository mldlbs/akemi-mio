...
import type { ToolCallRecord } from '../agent/UserBehaviorAnalyzer'
...
export class BehaviorFeatureExtractor {
  ...
  private generateSuggestions(
    ...
    // 4. 极高频率工具 → 缓存/优先级优化
    const topFreq = frequentTools
      .filter((t) => t.callCount >= 5)
      .slice(0, 2)
    for (const ft of topFreq) {
      suggestions.push({
        ...
        description:
          `${ft.name} 被调用了 ${ft.callCount} 次（高频工具）。` +
          `建议对 ${ft.name} 的结果添加 LRU 缓存，避免重复计算。`,
        target: ft.name,
