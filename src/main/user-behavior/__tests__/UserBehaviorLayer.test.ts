  ...
  describe('预处理钩子', () => {
    it('按注册顺序执行预处理钩子', async () => {
      ...
      const layer = new UserBehaviorLayer(mock, {
        ...
        preHooks: [
          async (ctx) => { order.push(1); return ctx },
          async (ctx) => { order.push(2); return ctx },
          async (ctx) => { order.push(3); return ctx },
        ],
      })
    ...
    it('单个钩子失败不阻断后续钩子', async () => {
      ...
      const layer = new UserBehaviorLayer(mock, {
        ...
        preHooks: [
          async (ctx) => { order.push(1); return ctx },
          async () => { order.push(2); throw new Error('hook_error') },
          async (ctx) => { order.push(3); return ctx },
        ],
      })
  ...
  describe('后处理钩子', () => {
    ...
    it('缓存最后一次后处理结果', async () => {
      ...
      const layer = new UserBehaviorLayer(mock, {
        features: ['summary_enhance'],
        postHooks: [
          async () => ({ enhancedSummary: 'cached_result' }),
        ],
      })
