...
export class MCPFeedbackLoopService {
  ...
  private rebuildToolSnapshots(): void {
    ...
    for (const [toolName, qualities] of perTool) {
      ...
      const effectiveLatencies = latencies.length > 0
        ? latencies
        : qualities.map(() => this.estimateLatency(toolName))

      const avgLatency = effectiveLatencies.length > 0
        ? effectiveLatencies.reduce((s, v) => s + v, 0) / effectiveLatencies.length
        : 0
      const sorted = [...effectiveLatencies].sort((a, b) => a - b)
      const medianLatency = sorted.length > 0
        ? sorted[Math.floor(sorted.length / 2)]
        : 0
      const p95Latency = sorted.length > 0
        ? sorted[Math.floor(sorted.length * 0.95)]
        : 0

      // 延迟标准差
      const variance = effectiveLatencies.length > 0
        ? effectiveLatencies.reduce((s, v) => s + (v - avgLatency) ** 2, 0) / effectiveLatencies.length
        : 0
      ...
      const avgOutputLength = outputLengths.length > 0
        ? outputLengths.reduce((s, v) => s + v, 0) / outputLengths.length
        : 0
  ...
  private adjustmentTypeToParamKey(type: ParameterAdjustmentType): keyof BehaviorParameterSet | null {
    switch (type) {
      case 'window_size': return 'analysisWindow'
      case 'frequency_threshold': return 'highFreqToolThreshold'
      case 'confidence_threshold': return 'preloadConfidenceThreshold'
      case 'confidence_weights': return 'confidenceRecencyScale'
      case 'pause_threshold': return 'pauseThresholdMs'
      case 'sequence_frequency': return 'minSequenceFrequency'
      default: return null
    }
  }
