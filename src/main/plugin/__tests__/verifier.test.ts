...
function makeSignedPlugin(code: string): string {
  const sig = sign(null, Buffer.from(code, 'utf-8'), privateKey)
  return code + '\n' + SIGNATURE_COMMENT_PREFIX + sig.toString('base64') + '\n'
}

function stripSignatureLine(content: string): string | null {
  ...
  if (!lastLine.startsWith(SIGNATURE_COMMENT_PREFIX)) return null
  const contentLines = lines.slice(0, lastLineIndex).join('\n')
  return lines[lastLineIndex - 1] === '' ? contentLines + '\n' : contentLines
}

function getSignatureAndContent(signed: string): { signature: Buffer; contentBytes: Buffer } | null {
  const lines = signed.split('\n')
  let lastIdx = lines.length - 1
  while (lastIdx >= 0 && lines[lastIdx].trim() === '') lastIdx--
  if (lastIdx < 0) return null
  const sigLine = lines[lastIdx].trim()
  if (!sigLine.startsWith(SIGNATURE_COMMENT_PREFIX)) return null
