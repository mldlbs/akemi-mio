...
import { ToolRegistry, Permission, expandPermissions } from '../registry'
...
describe('ToolRegistry 权限执行器', () => {
  ...
  describe('权限检查生效', () => {
    ...
    it('调用者缺少权限 → throw [PermissionDenied]', async () => {
      ...
      await expect(
        registry.execute('delete_system_file', {}, READ_ONLY_PERMS),
    ...
    it('错误消息应包含缺失的权限名', async () => {
      ...
      await expect(
        registry.execute('exec_cmd', {}, READ_ONLY_PERMS),
  ...
  describe('ServerManager 集成路径', () => {
    it('通过 toolRegistry.execute() 执行 → 自动权限检查', async () => {
      ...
      // 无权限 → 拒绝
      await expect(registry.execute('restart_server', {}, ['filesystem:read'])).rejects.toThrow(
        /PermissionDenied/,
      )
    })
  })
