...
import { getPlanManager } from '../tool/deps'
import { songgeCorrectionAdapter } from './PlanSonggeCorrectionAdapter'
import { issuePatternAnalyzer } from './IssuePatternAnalyzer'
import type { FixExecutor, AssignedProblem, FixResult } from '../evolution/automation/types'
import type { ReasoningChain, ReasoningStep } from '../evolution/automation/types'
import type { CorrectionInput, CorrectionResult, CorrectionIssue, CorrectionSeverity } from './PlanSonggeCorrectionAdapter'
...
export class SonggeReasoningChainExecutor implements FixExecutor {
  ...
  async executeDirect(input: CorrectionInput): Promise<{
    ...
    if (!extracted) {
      return { success: false, summary: '无法提取修正输入', chain: { problemId: '', title: '', steps: [], createdAt: Date.now(), allSucceeded: false } }
    }
  ...
  private buildIdempotencyKey(
    ...
    input: { chapterStart: number; chapterEnd: number; designDoc: string },
  ): string {
  ...
  private generateReasoningChain(
    ...
    // Step 0: 分析设计文档需求
    steps.push(this.makeStep(stepIndex++, timestamp,
      '分析设计文档需求',
      input.designDoc
        ? `从设计文档中提取修正标准：${designDocHint}`
        : '未提供设计文档，使用默认修正标准',
    ))
    ...
    steps.push(this.makeStep(stepIndex++, timestamp,
      '生成逐章修正方案',
      '基于评分结果，为每个需要修正的章节生成具体修正问题和建议',
    ))

    // Step 3: 验证修正兼容性
    steps.push(this.makeStep(stepIndex++, timestamp,
      '验证修正兼容性',
      '检查跨章节修正是否一致，以及修正方案与风格指南的兼容性',
    ))

    // Step 4: 执行章节修正
    steps.push(this.makeStep(stepIndex++, timestamp,
      '执行章节修正',
      `批量应用修正结果到第${input.chapterStart}-${input.chapterEnd}章`,
    ))

    // Step 5: 创建评估快照
    steps.push(this.makeStep(stepIndex++, timestamp,
      '创建修正评估快照',
      '记录修正后的评分基线，标记需要后续关注的问题',
    ))

    // 根据问题严重度裁剪步骤
    const trimmed = problem.severity === 'info'
  ...
  private async executeStep(
    ...
    switch (step.index) {
      case 0: return this.stepAnalyzeDesignDoc(input)
      case 1: return this.stepEvaluateChapters(input)
      case 2: return this.stepGenerateCorrections(input)
      case 3: return this.stepValidateCompatibility(input)
      case 4: return this.stepExecuteCorrections(input)
      case 5: return this.stepEvaluateSnapshot(input)
      default: return { success: true, summary: `步骤 ${step.index} 无操作，已跳过` }
    }
  }
   ...
   */
  private async stepEvaluateChapters(
    input: { chapters: Record<number, string>; designDoc: string; styleGuide?: string; chapterStart: number; chapterEnd: number },
  ): Promise<{ success: boolean; summary: string; output?: string }> {
    ...
    // 构建可读摘要
    const chapterSummaries = result.snapshots.map((snap) => {
   ...
   */
  private stepGenerateCorrections(
    input: { chapters: Record<number, string>; designDoc: string; styleGuide?: string; chapterStart: number; chapterEnd: number },
  ): { success: boolean; summary: string; output?: string } {
    ...
    for (const snap of result.snapshots) {
      for (const issue of snap.issues) {
        switch (issue.severity) {
          ...
          case 'major': majorIssues.push(issue); break
          default: minorIssues.push(issue)
        }
      }
    }
   ...
   */
  private stepValidateCompatibility(
    input: { chapters: Record<number, string>; designDoc: string; styleGuide?: string; chapterStart: number; chapterEnd: number },
  ): { success: boolean; summary: string; output?: string } {
    const { chapters, designDoc, styleGuide, chapterStart, chapterEnd } = input
    ...
    // 检查风格指南兼容性
    const styleIssues = styleGuide
      ? result.snapshots.filter((s) => s.scores.styleConsistency < 0.5).length
      : 0
   ...
   */
  private stepExecuteCorrections(
    input: { chapters: Record<number, string>; designDoc: string; styleGuide?: string; chapterStart: number; chapterEnd: number },
  ): { success: boolean; summary: string; output?: string } {
    const { chapters, designDoc, styleGuide, chapterStart, chapterEnd } = input
  ...
  private stepEvaluateSnapshot(
    ...
    const scoreSummary = baselineResult.snapshots.map((s) => {
  ...
  private buildReasoningSummary(
    ...
    return [
      ...
      chain.allSucceeded
        ? '推理链全部完成，章节修正方案已就绪。'
        : `推理链部分完成（${succeeded}/${totalSteps}），请检查失败步骤。`,
      chain.conclusion ? `\n结论: ${chain.conclusion}` : '',
    ].join('\n')
  }
  ...
  private buildConclusion(
    ...
    const failedSteps = stepResults
      .map((r, i) => ({ ...r, desc: chain.steps[i]?.description || `步骤${i}` }))
      .filter((r) => !r.success)
    ...
    try {
      ...
      return {
        snapshots: (parsed.snapshots || []).map((s: any) => ({
          ...
          issues: [],
          severity: s.score < 0.4 ? 'critical' as CorrectionSeverity : s.score < 0.6 ? 'major' as CorrectionSeverity : s.score < 0.8 ? 'minor' as CorrectionSeverity : 'info' as CorrectionSeverity,
          needsCorrection: s.needsCorrection,
          timestamp: Date.now(),
        })),
