...
export function registerVoiceBookmarkHandlers({ voiceBookmarkRef }: HandlerContext): void {
  ...
  ipcMain.handle('voice-bookmark:create', async (_event, summary: string, conversationContext: any[], options?: any) => {
    try {
      ...
      return { success: true, bookmark: { id: bookmark.id, summary: bookmark.summary, audioPath: bookmark.audioPath, createdAt: bookmark.createdAt, isFavorite: bookmark.isFavorite } }
    } catch (err: any) { log('ERROR', 'voice_bookmark_create_failed', { error: String(err) }); return { success: false, error: String(err) } }
  })
  ...
  ipcMain.handle('voice-bookmark:get', async (_event, id: string) => {
    ...
    } catch (err: any) { log('ERROR', 'voice_bookmark_get_failed', { error: String(err) }); return { success: false, error: String(err) } }
  })
  ...
  ipcMain.handle('voice-bookmark:delete', async (_event, id: string) => {
    ...
    } catch (err: any) { log('ERROR', 'voice_bookmark_delete_failed', { error: String(err) }); return { success: false, error: String(err) } }
  })
  ...
  ipcMain.handle('voice-bookmark:getAudioPath', async (_event, id: string) => {
    try {
      const svc = voiceBookmarkRef.current
      if (!svc) return { success: false, error: 'VoiceBookmarkService not initialized' }
      const audioPath = svc.getBookmarkAudioPath(id)
      if (!audioPath) return { success: false, error: '音频文件未找到' }
      return { success: true, audioPath }
    } catch (err: any) { log('ERROR', 'voice_bookmark_audiopath_failed', { error: String(err) }); return { success: false, error: String(err) } }
  })
  ...
  ipcMain.handle('voice-bookmark:toggleFavorite', async (_event, id: string) => {
    ...
    } catch (err: any) { log('ERROR', 'voice_bookmark_toggle_favorite_failed', { error: String(err) }); return { success: false, error: String(err) } }
  })
  ...
  ipcMain.handle('voice-bookmark:favorites', async () => {
    ...
    } catch (err: any) { log('ERROR', 'voice_bookmark_favorites_failed', { error: String(err) }); return { success: false, bookmarks: [], error: String(err) } }
  })
  ...
  ipcMain.handle('blog:saveAudio', async (_event, audio: ArrayBuffer, type: string, options?: any) => {
    try {
      ...
      const entry = await blogVoiceService.saveAudio(audio, type as 'dictation' | 'annotation', {
        sessionId: options?.sessionId, paragraphIndex: options?.paragraphIndex, durationSec: options?.durationSec, transcribedText: options?.transcribedText, label: options?.label,
      })
      if (entry) return { success: true, entry: { id: entry.id, type: entry.type, durationSec: entry.durationSec, transcribedText: entry.transcribedText, createdAt: entry.createdAt } }
      return { success: false, error: '保存失败' }
    } catch (err: any) { log('ERROR', 'blog_save_audio_failed', { error: String(err) }); return { success: false, error: String(err) } }
  })
  ...
  ipcMain.handle('blog:listAudio', async (_event, type?: string, limit?: number) => {
    ...
    } catch (err: any) { log('ERROR', 'blog_list_audio_failed', { error: String(err) }); return [] }
  })
