...
export function registerDesktopHandlers({ restoreRef }: HandlerContext): void {
  ...
  ipcMain.handle('desktop:invokeTool', async (_event, toolName: string, args: Record<string, any>) => {
    ...
    } catch (err: any) { log('ERROR', 'desktop_invoke_tool_failed', { toolName, error: String(err) }); return { success: false, error: err.message || String(err) } }
  })
  ...
  ipcMain.handle('runtime:restore', async (_event, checkpointId: string) => {
    ...
    } catch (err: any) { log('ERROR', 'runtime_restore_failed', { checkpointId, error: String(err) }); return { success: false, error: String(err), status: 'failed' } }
  })
}
