...
import { EventBus, eventBus as defaultEventBus } from '../core/EventBus'
...
export class FailureAnalyzer {
  ...
  start(): void {
    this.unsubscribers.push(
      this.eventBus.on('agent.tool.failed', (p) => {
        this.record({
          type: 'tool',
          name: p.tool,
          error: p.error,
    ...
    this.unsubscribers.push(
      this.eventBus.on('agent.error', (p) => {
        this.record({
          type: 'crash',
          name: 'agent_error',
          error: p.error,
  ...
  private updatePattern(failure: FailureRecord): void {
    ...
    if (!p.errors.includes(failure.error)) {
      ...
      if (p.errors.length > 5) p.errors = p.errors.slice(-5)
    }
  }

  private static readonly DECAY_HALF_LIFE_MS = 24 * 60 * 60 * 1000 // 24 hours

  /** Apply time-decay to pattern weight: weight = count * 0.5^(age / halfLife) */
  private applyDecayWeight(p: FailurePattern): number {
    const ageMs = Date.now() - p.lastSeen
    if (ageMs <= 0) return p.count
    const halfLives = ageMs / FailureAnalyzer.DECAY_HALF_LIFE_MS
    return p.count * Math.pow(0.5, halfLives)
  }

  /** 获取热点失败模式（按时间衰减加权频次排序） */
  getHotPatterns(topK = 5): FailurePattern[] {
  ...
  getStats(): { total: number; patterns: number; topFailures: string[] } {
    return {
