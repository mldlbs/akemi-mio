...
import { getRecentMessages, getMessagesBySession, getSessions, type StoredMessage, type SessionItem } from '../../db/messages'
import {
...
export class PatternExtractor {
  ...
  loadFromHistory(options?: PatternExtractOptions): ToolCallSequence[] {
    ...
    for (const session of sessions) {
      ...
      if (tools.length >= (options?.minSequenceLength ?? DEFAULT_MIN_SEQUENCE_LENGTH)) {
        const intentLabel = intentLabels.size > 0
          ? Array.from(intentLabels).join(',')
          : 'general'
        sequences.push({
    ...
    // 合并到内部缓存
    this.recentSequences = [...this.recentSequences, ...sequences]
      .slice(-this.maxSequences)
  ...
  clusterSequences(options?: PatternExtractOptions): SequenceCluster[] {
    ...
    for (const [key, sequences] of groups) {
      ...
      // 找共同工具模式（取交集，即完整匹配）
      const commonToolPattern = this._findCommonPattern(
        sequences.map((s) => s.toolNames),
      )
    ...
    for (const tmpl of templates) {
      ...
      if (matchedKeywords.length > 0) {
        ...
        if (tmpl.lastUsedAt) {
          ...
          if (daysSinceLastUse < 1) score += 0.1      // 今天用过
  ...
  buildTemplateSuggestionPrompt(matches: TemplateMatch[]): string {
    ...
    lines.push(
      `如适用，可直接执行模板。若需修改后执行，请调整后执行。`,
      `可使用 list_task_templates 查看所有模板。`,
    )
  ...
  private _generateDescription(cluster: SequenceCluster): string {
    const toolNames = cluster.commonToolPattern
      .map((t) => `\`${t}\``)
      .join(' → ')
  ...
  private _isStopWord(word: string): boolean {
    const stopWords = new Set([
      ...
      '没有', '看', '好', '自己', '这', '那', '些', '吧', '吗', '啊', '呢',
      'the', 'a', 'an', 'is', 'are', 'was', 'were', 'be', 'to', 'of', 'in',
      'for', 'on', 'with', 'at', 'by', 'from', 'as', 'into', 'and', 'or',
      'but', 'not', 'so', 'if', 'than', 'that', 'this', 'these', 'those',
