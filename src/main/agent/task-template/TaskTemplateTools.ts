...
import { buildTool, formatToolResult, formatToolError } from '../../tool/types'
...
function formatTemplate(t: TaskTemplate): string {
  const keywordStr = t.triggerKeywords.length > 0
    ? `触发关键词: ${t.triggerKeywords.join(', ')}`
    : '无触发关键词'

  const toolSteps = t.toolSequence
    .map((s, i) => `  ${i + 1}. ${s.toolName}${s.description ? ' - ' + s.description : ''}`)
    .join('\n')

  const successRate = t.useCount > 0
    ? ` (成功率: ${((t.successCount / t.useCount) * 100).toFixed(0)}%)`
    : ''
