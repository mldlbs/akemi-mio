 ...
 * 基于关键词匹配和写作习惯
 */
export function evaluateTopicComplexity(
  topic: string,
 ...
 * 评估输入特征，用于模式推荐
 */
export function evaluateInputFeatures(
  topic: string,
  userInput: string,
  profile?: WritingHabitProfile,
): InputFeatureProfile {
  ...
  return {
    ...
    prefersInteractive:
      /逐段|一步一步|分步|确认|问我|交互|interactive|step by step/i.test(lower) ||
      !profile || profile.totalSessions < 3,
    hasStructuredRequirements:
      /大纲|结构|章节|目录|outline|structure|chapter|toc/i.test(lower) ||
      (profile?.prefersOutline ?? true),
    ...
    crossPlatformPublishing:
      /多平台|全平台|公众号.*博客|多端|multiple platform|cross.publish/i.test(lower || '') ||
      false,
    urgencyLevel:
      /急|马上|尽快|urgent|asap|quick|immediately/i.test(lower) ? 0.8 : 0.3,
