...
import type { ReasoningStepStatus } from '../../evolution/automation/types'
...
const ACCUMULATED_ERROR_THRESHOLD = 0.20
...
export class BlogReasoningChainExecutor {
    ...
    try {
      ...
      if (planId && pm) {
        if (allSucceeded) {
          ...
          pm.freezePlan(planId, `博客推理链中断于步骤 ${stepResults.filter((r) => !r.success).length + 1}/${chain.steps.length}: ${conclusion}`)
        }
      }
  ...
  ): { description: string; detail: string; critical: boolean } {
    switch (stage) {
      case BlogStage.IntentAnalysis:
        return {
          description: '主题与意图分析',
          detail: `理解用户意图和目标：主题="${context.topic}"` +
      ...
      case BlogStage.MaterialCollection:
        return {
          description: '素材收集',
          detail: '围绕主题收集技术资料、代码示例、参考文献和相关数据' +
      ...
      case BlogStage.QualityReview:
        return {
          ...
          detail: '对初稿进行多维质量评估和内容安全审查' +
            (context.enableSecurityReview !== false ? '（含安全审查）' : '（安全审查已禁用）'),
          critical: false,
        }
  ...
  private stepOutline(context: BlogReasoningContext): BlogStepResult {
    ...
    const sectionLines = outlineSections.map(
      (s, i) => `  ${i + 1}. **${s.title}**\n     ${s.description}`,
    )
  ...
  private stepDraftWriting(context: BlogReasoningContext, currentContent: string): BlogStepResult {
    ...
    const summary = [
      ...
      '',
      hasInitialContent
        ? `📄 现有内容长度: ${(initialContent || currentContent).length} 字`
        : '📄 无初始内容，需要从头撰写',
      '',
      '✍️ 初稿写作要点:',
      '  - 保持技术准确性：所有代码示例应经过验证',
      ...
      '',
      '📏 建议篇幅:',
      this.evaluateTopicComplexity(topic) >= 0.7 ? '  3000-6000 字（深度技术文章）' :
        this.evaluateTopicComplexity(topic) >= 0.4 ? '  1500-4000 字（中等篇幅）' :
          '  800-2000 字（入门文章）',
      '',
      '⏩ 初稿生成完成后将进入质量审核阶段。',
    ].filter(Boolean).join('\n')
   ...
   */
  private async stepQualityReview(
    context: BlogReasoningContext,
    currentContent: string,
  ): Promise<BlogStepResult> {
    ...
    if (assessment) {
      ...
      if (assessment.totalIssues > 0) {
        assessmentLines.push(`🔍 发现 ${assessment.totalIssues} 个可改进项:`)
        for (const issue of assessment.issues) {
          const severityLabel = issue.severity === 'critical' ? '🔴' : issue.severity === 'major' ? '🟡' : issue.severity === 'minor' ? '🟢' : '🔵'
      ...
      assessmentLines.push(assessment.publishReady
        ? '✅ 内容已达到发布就绪标准'
    ...
    if (securityReport) {
      ...
      assessmentLines.push('')
      assessmentLines.push(`建议动作: ${
        securityReport.recommendedAction === 'block' ? '❌ 禁止发布' :
          securityReport.recommendedAction === 'fix_before_publish' ? '⚠️ 修复后发布' :
            '✅ 可以发布'
      }`)
    }
  ...
  private stepPublishingPlan(context: BlogReasoningContext): BlogStepResult {
    ...
    const platformAdvice: Array<{ platform: string; advice: string[] }> = [
      {
        platform: '博客园',
        advice: [
          '支持完整 Markdown，代码高亮效果好',
          '建议添加 "原创" 标签提高推荐权重',
          '注意首段要有足够文字（200字以上）作为摘要',
        ],
      },
      {
        platform: 'CSDN',
        advice: [
          '支持 Markdown，代码块需标注语言',
          '标题长度控制在 30 字以内',
          '添加合适的技术分类标签',
        ],
      },
    ...
    const platformSpecific = platformAdvice.find(
      (p) => platform.includes(p.platform) || p.platform.includes(platform),
    )
    ...
    const summary = [
      ...
      platformSpecific
        ? [
          `📌 ${platformSpecific.platform} 发布注意事项:`,
          ...platformSpecific.advice.map((a) => `  • ${a}`),
          '',
        ].join('\n')
        : '',
      '📅 发布时程建议:',
  ...
  private evaluateTopicComplexity(topic: string): number {
    ...
    // 高复杂度信号
    const highSignals = [
      '架构', '设计模式', '分布式', '微服务', '性能优化',
      '源码分析', '算法', '编译器', '内核', '数据库',
      'architecture', 'design pattern', 'distributed', 'microservice',
      'deep dive', 'under the hood', 'internals', 'optimization',
      '系统设计', '高并发', '高可用', '容错', '一致性',
      '原理', '机制', '实现', '底层', '框架设计',
    ]
    ...
    const lowSignals = [
      '入门', '介绍', '初探', '笔记', '心得',
      '教程', '指南', '快速', '简单', 'hello world',
      'introduction', 'getting started', 'beginner', 'tutorial',
      'guide', 'quick', 'simple', 'basic',
    ]
  ...
  private buildConclusion(
    ...
    if (chain.allSucceeded) {
      ...
      return `博客推理链全部 ${total} 步成功完成（总耗时 ${(totalDuration / 1000).toFixed(1)}s）。` +
        `博客「${chain.topic}」从主题分析到发布规划的全流程已完成。` +
        (chain.finalContent
          ? `最终内容 ${chain.finalContent.length} 字符。`
          : '')
    }
   ...
   */
  private enrichChainWithEvolutionContext(
    chain: BlogReasoningChain,
    context: BlogReasoningContext,
  ): void {
    try {
      ...
      if (publishStep && evoCtx.scheduler) {
        publishStep.detail += `\n\n【Evolution 调度器状态】\n  ` +
          ...
          (evoCtx.scheduler.isHealthy
            ? '  提示: Evolution 运行正常，可按计划发布'
            : '  提示: Evolution 出现故障，建议等待系统恢复后再发布')
      }
