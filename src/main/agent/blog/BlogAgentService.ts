...
export class BlogAgentService {
  ...
  startSession(topic: string, platform?: string, userInput?: string): BlogSessionState {
    ...
    const state: BlogSessionState = {
      ...
      stageStatuses: Object.fromEntries(
        BLOG_STAGE_ORDER.map((s) => [s, 'pending']),
      ) as Record<BlogStage, 'pending' | 'running' | 'completed' | 'skipped' | 'failed'>,
      topic,
  ...
  advanceStage(sessionId: string): BlogSessionState | null {
    ...
    while (nextIdx < BLOG_STAGE_ORDER.length) {
      ...
      if (!s.skippedStages.includes(nextStage)) {
        ...
        s.lastActivityAt = Date.now()
        log('INFO', 'blog_stage_advance', { sessionId, from: BLOG_STAGE_LABELS[BLOG_STAGE_ORDER[currentIdx]], to: BLOG_STAGE_LABELS[nextStage] })
        return s
      }
      nextIdx++
    }
  ...
  generateSuggestions(): string[] {
    ...
    if (profile.totalSessions > 2) {
      ...
      if (profile.commonTopics.length > 3) {
        suggestions.push(
          `你常写的主题有：${profile.commonTopics.slice(0, 3).join('、')}。是否需要围绕这些主题规划系列博客？`,
        )
      }

      if (profile.commonPlatforms.length > 0) {
        suggestions.push(
          `你常用的发布平台是 ${profile.commonPlatforms[0]}，我已在工作流中优先适配该平台格式。`,
        )
      }
  ...
  private handleSkip(s: BlogSessionState, command: ParsedBlogCommand): string {
    // 如果指定了目标阶段
    const targetStage = command.targetStage
      ? this.fuzzyMatchStage(command.targetStage as string)
      : s.currentStage
  ...
  private extractCommandParams(type: BlogCommandType, input: string): ParsedBlogCommand {
    ...
    const contentMatch = input.match(
      /(?:跳过|修改|改成|改为|调整为|调整|补充|加(?:一)?个|删掉|减少|不要)\s*(.+)/i,
    )
    if (contentMatch) {
