...
export const COMMAND_PATTERNS: Array<{
...
}> = [
  {
    type: 'skip',
    patterns: [
      /跳过|不要|不需要|取消|不用.*了|跳过.*步|省略|略过/i,
      /skip|不需要|不用(做|写|分析)/i,
    ],
  },
  {
    type: 'override',
    patterns: [
      /改成|改为|换(一)?个|不要.*要|我觉得|我认为|按我的|听我说/i,
      /change.*to|instead|override|改用/i,
    ],
  },
  {
    type: 'modify',
    patterns: [
      /修改|调整|改一下|优化|补充|完善|增加.*内容|删掉|减少/i,
      /modify|adjust|update|improve/i,
    ],
  },
  {
    type: 'restart',
    patterns: [
      /重来|重新(开始|来)|从头|回到|从.*开始|回退到/i,
      /restart|redo|go back|rollback/i,
    ],
  },
  {
    type: 'approve',
  ...
  },
  {
    type: 'feedback',
    patterns: [
      /觉得|感觉|意见|反馈|建议|问题|不足|改进|加(一)?个|补充(一)?下/i,
      /feedback|suggestion|opinion|think|feel/i,
    ],
  },
  {
    type: 'status',
    patterns: [
      /进度|状态|到哪(了|里)|进行得|怎么样了|当前(步骤|阶段)/i,
      /status|progress|where|current/i,
    ],
  },
]
