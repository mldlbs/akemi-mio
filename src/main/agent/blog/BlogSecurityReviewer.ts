...
export class BlogSecurityReviewer {
  ...
  review(
    ...
    // 对每个检测模式执行匹配
    const patternsToRun = criticalOnly
      ? ALL_PATTERNS.filter((p) => p.riskLevel === 'critical' || p.riskLevel === 'high')
      : ALL_PATTERNS
  ...
  autoFix(content: string): { fixed: string; changes: number } {
    ...
    result = result.replace(
      ...
      () => { changes++; return '<internal-ip>' },
    )

    // 替换 localhost 引用
    result = result.replace(
      /https?:\/\/localhost(:\d+)?/gi,
      () => { changes++; return 'http://localhost<port>' },
    )

    // 替换文件路径
    result = result.replace(
      /[A-Z]:\\(?:Users|home|tmp|temp|var|etc|opt|app)[^"'\s)]*/gi,
      (match) => { changes++; return match.replace(/[^\\]+$/, '<user-path>') },
    )
        ...
        if (pi.length > 80 && pj.length > 80) {
          ...
          if (common.length > Math.min(pi.length, pj.length) * 0.7) {
            ...
            return {
              ...
              suggestion:
                '如为引用他人内容，请标注来源和授权方式。如为原创内容，确保没有粘贴重复',
              autoFixable: false,
              confidence: 0.6,
            }
          }
  ...
  private getRecommendedAction(
    riskLevel: SecurityRiskLevel,
  ): 'proceed' | 'fix_before_publish' | 'block' {
    switch (riskLevel) {
      case 'info':
      case 'low':
        return 'proceed'
