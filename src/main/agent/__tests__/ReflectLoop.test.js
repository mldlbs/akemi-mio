...
describe('ReflectLoop', () => {
    ...
    beforeEach(() => {
        ...
        loop.setDeps(mockLlm, mockEngineering);
        loop.setDecisionStore(mockDecisionStore);
        loop.setResourceBudget(mockBudget);
    });
    afterEach(() => {
        vi.useRealTimers();
    });
    ...
    it('调用 chatJson 并存储结果', async () => {
        ...
        expect(mockEngineering.store).toHaveBeenCalledWith(expect.objectContaining({ type: 'failure_pattern', source: 'reflect_loop', confidence: 0.85 }));
