...
import { Scratchpad, AttentionSet } from '../WorkingMemory';
describe('Scratchpad 压力测试', () => {
    ...
    it('flushNew 精确区分新旧条目', () => {
        const pad = new Scratchpad();
        for (let i = 0; i < 10; i++)
            pad.add('observe', `old_${i}`);
        const mark = pad.size;
        for (let i = 0; i < 5; i++)
            pad.add('think', `new_${i}`);
...
describe('AttentionSet 压力测试', () => {
    ...
    it('1000 个独立实体 getActive 排序正确', () => {
        ...
        for (let i = 0; i < 1000; i++)
            attn.add(`entity_${i}`, 'concept', Math.random());
        const active = attn.getActive(0);
        expect(active.length).toBeGreaterThan(0);
        for (let i = 1; i < active.length; i++)
            expect(active[i - 1].relevance).toBeGreaterThanOrEqual(active[i].relevance);
    });
    it('大量旧实体通过 tick 正确清理', () => {
        const attn = new AttentionSet();
        for (let i = 0; i < 200; i++)
            attn.add(`old_${i}`, 'concept', 0.5);
        vi.advanceTimersByTime(31 * 60 * 1000);
        for (let i = 0; i < 200; i++)
            attn.add(`fresh_${i}`, 'file', 1.0);
        attn.tick();
        const active = attn.getActive(0);
    ...
    it('大量同名实体去重取最高 relevance', () => {
        const attn = new AttentionSet();
        for (let i = 0; i < 100; i++)
            attn.add('dedup_key', 'concept', i / 100);
