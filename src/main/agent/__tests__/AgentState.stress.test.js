...
describe('AgentState 压力测试', () => {
    ...
    it('10000 次 lifecycle 交替后状态一致', () => {
        ...
        const cycle = ['ready', 'running', 'interrupted', 'running', 'completed', 'ready'];
        for (let i = 0; i < 2000; i++) {
            for (const target of cycle)
                s.transitionLifecycle(target);
    ...
    it('所有 AgentMode 各跑 1000 次 transition', () => {
        for (const mode of ['chat', 'task', 'evolution', 'background']) {
    ...
    it('随机游走 5000 步不进入无效状态', () => {
        ...
        const stages = ['idle', 'observe', 'think', 'plan', 'act', 'reflect', 'sleep'];
        for (let i = 0; i < 5000; i++) {
            const from = s.stage;
