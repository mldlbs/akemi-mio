...
import { ScopedAgent } from '../ScopedAgent';
...
describe('ScopedAgent - 纯逻辑', () => {
    it('初始状态为 pending', () => {
        const agent = new ScopedAgent('sk_1', 'test-skill', makeAgentDef(), {}, null, '', '');
        expect(agent.status).toBe('pending');
    });
    it('构造时存储 skillName 和 id', () => {
        const agent = new ScopedAgent('sk_custom', 'my-skill', makeAgentDef(), {}, null, '', '');
    ...
    it('toResult 返回结构化结果（未执行时）', () => {
        const agent = new ScopedAgent('sk_1', 'test-skill', makeAgentDef(), { foo: 'bar' }, null, '', '');
    ...
    it('toResult 始终包含 required 字段', () => {
        const agent = new ScopedAgent('sk_2', 'test-skill', makeAgentDef(), {}, null, '', '');
