...
export const AGENT_SUB_MODULE_MAPPINGS: AgentSubModuleMapping[] = [
  ...
  // ──────────── 1. ContentClassifier ────────────
  {
    ...
    migrationPlan: '将分类函数注册为 WallpaperPluginRegistry 的 content_classifier 能力插件。ChatExecutor 改为通过 WallpaperPluginRegistry 获取分类结果，保留回退路径。',
  ...
  // ──────────── 2. SleepCycle ────────────
  {
    ...
    migrationPlan: 'SleepCycle 的调度由 Wallpaper 接管，作为 periodic plugin 在系统空闲时触发。AgentService 不再维护 SleepCycle 实例，改为查询 WallpaperPluginRegistry 获取 sleep_maintenance 插件。',
  ...
  // ──────────── 5. ExecutionGovernor ────────────
  {
    ...
    prereqs: [
      '需要定义 GovernorDecision 的标准化 Wallpaper 事件',
      'AgentWallpaperBridge 需要支持决策型数据流（不仅是行为数据）',
    ],
  ...
  // ──────────── 6. ProgressGuardrail ────────────
  {
    ...
    migrationPlan: '通过 WallpaperEventBridge 订阅 agent.progress 事件，状态机在 Wallpaper 侧维护停滞计数。当检测到停滞时通过 EventBus 通知 ChatExecutor。',
  ...
  // ──────────── 9. ReflectLoop ────────────
  {
    ...
    integrationPoints: [
      'ChatExecutor.run() → 每轮对话后 trigger()',
      'AgentService.refreshMemoryInContext() 依赖',
    ],
  },
  ...
  // ──────────── 10. UserBehaviorAnalyzer ────────────
  {
    ...
    migrationPlan: 'Agent 端的 UserBehaviorAnalyzer 逐步迁移为消费 Wallpaper 的行为数据。先做只读迁移：UserBehaviorAnalyzer 改为通过 WallpaperPluginRegistry.getBehaviorProvider() 获取行为快照。后续 Agent 端的行为分析由 Wallpaper 统一承担。',
    integrationPoints: [
  ...
  // ──────────── 11. Guardrail ────────────
  {
    ...
    migrationPlan: 'Guardrail 规则引擎包装为 Wallpaper 插件，但核心执行路径保持在内联（避免 IPC 延迟）。Wallpaper 插件仅做配置管理和规则热更新。',
  ...
  // ──────────── 12. PersonaStateManager ────────────
  {
    ...
    migrationPlan: '包装为 Wallpaper 插件，人格状态通过 EventBus 发布到 Wallpaper 渲染层。Persona changes 通过 agent.persona.updated 事件通知 Wallpaper。',
  ...
  // ──────────── 20. AgentPluginRegistry ────────────
  {
    ...
    wallpaperSuitability: 'not_suitable',
    priority: 5,
    migrationPlan: '保留作为 Agent 侧插件注册表。通过 AgentWallpaperBridge 与 WallpaperPluginRegistry 建立发现通道。不会被 Wallpaper 替换。',
  ...
  // ──────────── 22. Cognitive Stage Plugins ────────────
  {
    ...
    wallpaperSuitability: 'not_suitable',
    priority: 5,
    migrationPlan: '保留在 AgentPluginRegistry 中。认知阶段是 Agent 运行时的核心环节，不适合抽象到 Wallpaper 层。但阶段结果可通过 EventBus 广播到 Wallpaper 展示。',
...
export function getPriorityReplacementQueue(): AgentSubModuleMapping[] {
  return AGENT_SUB_MODULE_MAPPINGS
    .filter((m) => m.priority <= 2 && m.wallpaperSuitability !== 'not_suitable')
