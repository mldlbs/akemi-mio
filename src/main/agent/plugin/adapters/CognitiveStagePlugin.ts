...
export class ObserveStagePluginAdapter implements ObserveStagePlugin {
  readonly manifest: AgentPluginManifest = {
    ...
    version: '1.0.0',
    description:
      'OTPAR Observe 阶段：查询 ProceduralMemory 和 FailureAnalyzer ' +
      '获取与当前工具相关的上下文，注入提示防止重复失败',
  ...
  async observe(
    ...
    return {
      ...
      summary: result.injected
        ? `发现 ${result.proceduresFound} 个相关流程和 ${result.patternsFound} 个失败模式`
        : '无相关上下文',
...
export class ThinkStagePluginAdapter implements ThinkStagePlugin {
  readonly manifest: AgentPluginManifest = {
    ...
    version: '1.0.0',
    description:
      'OTPAR Think 阶段：当 LLM 提出大量工具调用且无文本回复时，' +
...
export class ReflectStagePluginAdapter implements ReflectStagePlugin {
  readonly manifest: AgentPluginManifest = {
    ...
    version: '1.0.0',
    description:
      'OTPAR Reflect 阶段：在工具执行结果推入 messages 后注入执行反馈，' +
      '包括成功/失败统计和超时告警',
    capability: 'cognitive_stage',
    priority: 30,
    author: 'akemi-mio',
