...
function extractDiffs(original: string, recognized: string): DiffSegment[] {
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && original[i - 1] === recognized[j - 1]) {
      ...
      if (currentType !== newType) { flush(); currentType = newType; endPos = i }
      ...
      if (currentType !== newType) { flush(); currentType = newType; endPos = endPos || 0 }
      origAccum = ' ' + origAccum
      recogAccum = recognized[j - 1] + recogAccum
      j--
    }
...
export class TypographyVerificationService {
  ...
  async verify(formattedText: string): Promise<VerificationReport> {
    ...
    const totalDiffRate = sentences.length > 0
      ? sentences.reduce((sum, s) => sum + s.diffRate, 0) / sentences.length
      : 0
