...
import { eventBus, type EventPayload, type SubscriptionTracker } from '../core/EventBus'
...
export class TtsEventNotificationService {
  ...
  start(): void {
    ...
    // 订阅工具完成事件
    const dis1 = eventBus.on(
      'agent.tool.completed',
      (p) => this.handleEvent('tool_completed', p),
      { priority: 'low', label: 'tts_notif:tool_completed' },
    )
    this.disposers.push(dis1)

    // 订阅工具失败事件
    const dis2 = eventBus.on(
      'agent.tool.failed',
      (p) => this.handleEvent('tool_failed', p),
      { priority: 'high', label: 'tts_notif:tool_failed' },
    )
