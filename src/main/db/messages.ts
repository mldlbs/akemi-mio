...
const FAILURE_KEYWORDS = [
  ...
  '重新', '再来', '再试', '重试', '没用', '无效', '不工作', '坏了', '有问题',
  '听不懂', '不理解', '没懂', '不明白', '没明白',
  'timeout', '超时', 'error', '失败', 'failed', 'exception', '异常',
  '无法', '不能', '抱歉', 'sorry',
...
export function getConversationAnalytics(limit = 200): ConversationAnalytics {
  ...
  // 初始化分类计数器
  const catStats: Record<IntentCategory, { sessions: Set<string>; msgCount: number; failures: number; assistantMsgs: number; issues: Map<string, number> }> = {
      ...
      if (msg.role === 'assistant') {
        ...
        for (const kw of FAILURE_KEYWORDS) {
          if (content.includes(kw.toLowerCase())) {
            ...
            catStats[primaryCategory].issues.set(
              issueKey,
              (catStats[primaryCategory].issues.get(issueKey) || 0) + 1,
            )
            break
          }
        }
  ...
  // 构建分类分布
  const categoryDistribution: CategoryAnalytics[] = (Object.entries(catStats) as [IntentCategory, typeof catStats[keyof typeof catStats]][]).map(
    ([category, stats]) => ({
      category,
  ...
  const totalAssistantMsgs = (Object.values(catStats) as typeof catStats[keyof typeof catStats][]).reduce((s, c) => s + c.assistantMsgs, 0)
...
function inferSessionCategory(msgs: StoredMessage[]): IntentCategory {
  ...
  const scores: Record<IntentCategory, number> = {
    prompt: 0, tool: 0, response: 0, speed: 0, accuracy: 0, general: 0,
  }
