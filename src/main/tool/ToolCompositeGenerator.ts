...
import type { BehaviorPattern, PatternStep } from '../behavior/types'
...
export class ToolCompositeGenerator {
  ...
  ): Promise<CompositeToolProposal | CompositeRegistrationResult> {
    const toolName = this.buildToolName(pattern.toolSequence)
  ...
  async reviewProposal(
    ...
    approved: boolean,
    note?: string,
  ): Promise<CompositeRegistrationResult> {
    this.loadProposals()
   ...
   */
  private async registerCompositeTool(
    proposal: CompositeToolProposal,
  ): Promise<CompositeRegistrationResult> {
    try {
  ...
  private buildDescription(pattern: BehaviorPattern): string {
    ...
    return `复合工具: ${seq}（历史出现 ${freq} 次，成功率 ${conf}%）。将 ${pattern.toolSequence.length} 步操作合并为一步调用。`
  }
