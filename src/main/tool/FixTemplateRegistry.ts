...
const retryWrapperTemplate: FixTemplate = {
  ...
  generate(sourceCode: string, toolName: string, _pattern: FailurePattern): string | null {
    ...
    const handlerMatch = sourceCode.match(
      /(handler:\s*(async)?\s*\([^)]*\)\s*(:\s*Promise[^\{]*)?\s*\{)/,
    )
    if (!handlerMatch) {
    ...
    // 构建重试包装
    const importBlock = `import { createTimeoutSignal } from '../../utils/async'
...
const paramValidatorTemplate: FixTemplate = {
  ...
  generate(sourceCode: string, toolName: string, pattern: FailurePattern): string | null {
    ...
    // 构建参数校验代码
    const affectedParams = pattern.affectedParams.length > 0
      ? pattern.affectedParams
      : ['path']
    ...
    const importBlock = `import { formatToolResult, formatToolError } from '../types'`

    const modifiedCode =
      sourceCode.slice(0, bodyStart) +
      validationBlock +
      sourceCode.slice(bodyStart)
...
const errorHandlerTemplate: FixTemplate = {
  ...
  generate(sourceCode: string, toolName: string, _pattern: FailurePattern): string | null {
    ...
    const importBlock = `import { classifyToolError, ToolErrorType } from '../ToolErrorType'
import { formatToolResult, formatToolError } from '../types'`
...
`
    ...
    // 确保有 log 导入
    if (!modifiedCode.includes("from '../../logger/Logger'") && !modifiedCode.includes("from '../logger/Logger'")) {
      const importBlockLog = `import { log } from '../../logger/Logger'`
