...
import { WritingDecisionService } from '../../creativity/WritingDecisionMemory'
...
export const writingRewriteStructureTool = buildTool({
  ...
  handler: async (args: { chapterPath: string; detailLevel?: string }) => {
    try {
      ...
      const dialogueRatio = nonEmptyLines.length > 0
        ? Math.round((dialogueMarkers.length / nonEmptyLines.length) * 100)
        : 0
      ...
      const avgSentenceLen = sentences.length > 0
        ? Math.round(sentences.reduce((a, s) => a + s.replace(/\s/g, '').length, 0) / sentences.length)
        : 0
      ...
      lines_output.push(`  长句子(>80字): ${longSentences} 句 (${sentences.length > 0 ? Math.round((longSentences / sentences.length) * 100) : 0}%)`)
      ...
      recordToMemory('structure_check', `file="${args.chapterPath}" paragraphs=${totalParagraphs} dialogue=${dialogueRatio}% score=${rhythmScore}`)
...
function calculateRhythmScore(
  ...
  else if (cv > 1.2) score += 10 // 变化大但可能过激
  else score -= 10 // 过于均匀
    ...
    try {
      ...
      if (allChanges.length === 0) {
        return formatToolResult(
          `✅ 未找到任何匹配的替换内容。\n\n` +
          `检查以下替换规则是否存在于目标文件中：\n${
            pairs.map((p, i) => `  ${i + 1}. "${p.old}" → "${p.new}"${p.description ? ` (${p.description})` : ''}`).join('\n')
          }`,
        )
      }
