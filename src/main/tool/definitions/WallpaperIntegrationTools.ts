...
import { getWallpaperToolBridge } from '../deps'
...
export const wallpaperSetConfigTool = buildTool({
  ...
  description: '通过 MCP 调整 Wallpaper 叠加层配置。可建议模式切换、调整透明度、切换显示元素。配置变化通过 EventBus 广播到 Wallpaper 系统。',
  inputJSONSchema: {
    type: 'object',
    properties: {
      opacity: {
  ...
  handler: async (args: {
    ...
    showEvolutionDashboard?: boolean
  }) => {
    try {
      ...
      const summary = changes.length > 0
        ? `Wallpaper 配置已更新 ${changes.length} 项`
        : '未指定任何配置项'
