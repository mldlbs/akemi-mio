...
import { getMemoryService, getMemoryResourceProvider } from '../deps'
...
export const summarizeMemoryTool = buildTool({
  ...
  description:
    '按主题分组记忆并生成摘要。只读操作，不修改记忆存储。' +
    '支持按查询过滤（可选），返回分组结果和每组摘要，便于模型快速了解记忆全貌。',
  inputJSONSchema: {
    type: 'object',
    properties: {
      query: {
  ...
  handler: async (args: {
    ...
    try {
      ...
      // 1. 获取待分析的记忆条目
      let entries = ms.getEntries()

      // 过滤：只分析 user_fact 类型，排除 permanent（已固定不压缩）
      let candidates = entries.filter(
        (e) => e.type === 'user_fact' && e.tier !== 'permanent' && !e.isPinned,
      )
      ...
      if (groups.length === 0) {
        return formatToolResult(
          `没有找到可分组的话题（最小分组大小=${minGroupSize}）。` +
            '共 ' + candidates.length + ' 条记忆，但它们的主题相似度不足以形成分组。',
        )
      }
      ...
      for (let i = 0; i < topGroups.length; i++) {
        const g = topGroups[i]
        const topicLabel = g.commonTopics.length > 0
          ? `【${g.commonTopics.slice(0, 3).join('、')}】`
          : '【未分类】'
        const summary = generateGroupSummary(g.contents, g.commonTopics)
        lines.push(`\n分组 ${i + 1}: ${topicLabel}`)
        lines.push(`  条目数: ${g.entries.length} | 平均置信度: ${g.avgConfidence.toFixed(2)} | 最高行为分: ${g.maxBehaviorScore.toFixed(2)}`)
...
export const configureMemoryTool = buildTool({
  ...
  description:
    '配置记忆系统。支持启用/禁用、设置隐私级别、保留天数和自动压缩。' +
    '不传参数时返回当前配置。',
  inputJSONSchema: {
    type: 'object',
    properties: {
  ...
  handler: async (args: {
    ...
    try {
      ...
      // 没有变更参数时返回当前配置
      const hasChanges = args.enabled !== undefined || args.privacy_level !== undefined ||
        args.retention_days !== undefined || args.compression !== undefined
...
function formatEntryDetail(entry: {
  id: string; type: string; tier: string; confidence: number;
  behaviorScore: number; reinforceCount: number; accessCount: number;
  isPinned: boolean; createdAt: number; updatedAt: number;
  lastAccessedAt?: number; content: string; topics?: string[];
