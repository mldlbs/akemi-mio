  ...
  inputJSONSchema: {
    ...
    properties: {
      ...
      output_path: {
        type: 'string',
        description:
          '可选，输出 MP3 文件路径。默认为临时目录。建议指定到博客资源目录以便嵌入。' +
          '如: D:\\blog\\audio\\my-post.mp3',
      },
      speed: {
        type: 'number',
        description:
          '全局语速覆盖（0.5-2.0）。1.0=正常，0.8=较慢（适合教程），1.2=较快（适合资讯）。' +
          '不指定则每个段落使用模型推荐值。',
      },
      pitch: {
        type: 'number',
        description:
          '全局音调覆盖（0.5-2.0）。1.0=正常，>1=偏高（活泼），<1=偏低（沉稳）。' +
          '不指定则每个段落使用模型推荐值。',
      },
      fade_in: {
        type: 'number',
  ...
  handler: async (args: {
    ...
    if (result.segmentDetails && result.segmentDetails.length > 0) {
      ...
      for (const detail of result.segmentDetails) {
        ...
        const snippet = detail.textSnippet.length > 50
          ? detail.textSnippet.slice(0, 50) + '...'
          : detail.textSnippet
...
export const generatePodcastPreviewTool = buildTool({
  ...
  handler: async (args: { markdown: string }) => {
    ...
    lines.push(`--- 共 ${segments.length} 段，${assignments.filter((a) => a.model === 'zh_CN-ling_ling-medium').length} 段使用玲玲·温柔女声, ${assignments.filter((a) => a.model === 'zh_CN-tx_mati-medium').length} 段使用马提·沉稳男声, 其余使用花颜·女声 ---`)
