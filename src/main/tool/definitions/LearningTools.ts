...
import { buildTool, formatToolResult, formatToolError, type Tool } from '../types'

// ── 学习查询工具 ──

export const learningQueryTool: Tool = buildTool({
  name: 'learning_query',
    ...
    try {
      ...
      if (!result.matched) {
        return formatToolResult(
          `未找到匹配的知识点。${result.explanation || '你可以尝试说：什么是条件类型、解释映射类型。'}`
        )
      }
...
export const oralCodeGenerateTool: Tool = buildTool({
...
export const learningSummaryTool: Tool = buildTool({
...
export const typeChallengeNewTool: Tool = buildTool({
  ...
  handler: async (args: { concept?: string }) => {
    try {
      const { typeChallengeGenerator } = await import('../../learning/TypeChallengeGenerator')
      const { learningVocabularyManager } = await import('../../learning/LearningVocabularyManager')
...
export const typeChallengeSubmitTool: Tool = buildTool({
  ...
  handler: async (args: { challengeId: string; code: string }) => {
    try {
      const { typeChallengeGenerator } = await import('../../learning/TypeChallengeGenerator')
      const { typeScriptCompilerService } = await import('../../learning/TypeScriptCompilerService')
      const { learningVocabularyManager } = await import('../../learning/LearningVocabularyManager')
      ...
      // 构建完整代码
      const fullCode = typeChallengeGenerator.buildFullCode(
        stored.starterCode,
        args.code,
        stored.verifierCode,
      )
      ...
      if (result.success) {
        ...
        const diagText = typeScriptCompilerService.formatDiagnostics(result.diagnostics)
          .split('\n')
...
export const typeChallengeSolutionTool: Tool = buildTool({
