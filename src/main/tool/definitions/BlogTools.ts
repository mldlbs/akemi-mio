...
import { blogSecurityReviewer } from '../../agent/blog/BlogSecurityReviewer'
    ...
    try {
      ...
      if (report.findings.length > 0) {
        ...
        for (const finding of report.findings) {
          const severityIcon = finding.riskLevel === 'critical' ? '🚫' : finding.riskLevel === 'high' ? '🔴' : finding.riskLevel === 'medium' ? '🟡' : '🟢'
          lines.push(
...
export const blogRecordPerformanceTool = buildTool({
  ...
  description:
    '【BlogAgent】登记已发布博客文章的效果数据（阅读量、互动等）。用于后续分析文章表现和调整选题策略。支持单条或批量登记',
  inputJSONSchema: {
    type: 'object',
    properties: {
  ...
  handler: async (args: {
    postId?: string; title?: string; platform?: string; views?: number;
    likes?: number; comments?: number; shares?: number;
    publishedAt?: number; publishHour?: number; publishDay?: number;
    batch?: Array<{ postId: string; title: string; platform: string; views: number; likes?: number; comments?: number; shares?: number; publishedAt?: number }>;
  }) => {
    try {
      // 批量模式
...
export const blogAnalyticsReportTool = buildTool({
  ...
  description:
    '【BlogAgent】生成博客文章效果分析报告。基于已登记的效果数据，按类别、平台、发布时段汇总分析，提供数据洞察',
  inputJSONSchema: {
    type: 'object',
    properties: {},
    required: [],
    ...
    try {
      ...
      if (report.platformPerformance.length > 0) {
        ...
        for (const pf of report.platformPerformance) {
          lines.push(
            `  • ${pf.platform}: ${pf.postCount}篇 | 均阅读 ${pf.avgViews} | 互动率 ${(pf.avgEngagementRate * 100).toFixed(1)}%`,
          )
        }
        lines.push('')
      }
    ...
    try {
      ...
      if (strategy.recommendedTopics.length > 0) {
        ...
        for (let i = 0; i < strategy.recommendedTopics.length; i++) {
          const adj = strategy.adjustments.find(
            (a) => a.topic === strategy.recommendedTopics[i],
          )
          if (adj) {
      ...
      if (strategy.adjustments.length > 0) {
        ...
        for (const adj of strategy.adjustments) {
          const dirLabel = adj.direction === 'double_down' ? '📈加大投入' : adj.direction === 'explore' ? '🔍探索' : adj.direction === 'shift' ? '🔄调整方向' : '📉减少'
...
export const blogSetModeTool = buildTool({
  ...
  handler: async (args: { sessionId: string; mode: 'mcp' | 'plan_chain'; reason?: string; force?: boolean }) => {
    try {
      const { getBlogModeService } = await import('../deps')
