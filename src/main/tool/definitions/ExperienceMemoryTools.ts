...
import type { BlogMemoryCategory } from '../../memory/plan-memory-blog/types'
...
export const blogMemorySearchTool = buildTool({
  ...
  inputJSONSchema: {
    ...
    properties: {
      ...
      category: {
        type: 'string',
        description: '按分类筛选：analysis（代码分析）、design_decision（设计决策）、test_result（测试结果）、refactoring（重构记录）、bug_fix（Bug修复）、performance（性能优化）、architecture（架构决定）',
      },
      blogId: {
        type: 'string',
  ...
  handler: async (args: { query: string; limit?: number; category?: string; blogId?: string }) => {
    try {
      ...
      const lines: string[] = [
        `🔍 找到 ${results.length} 条相关经验（按相似度排序）：`,
        '',
      ]
...
export const blogMemoryListTool = buildTool({
  ...
  description:
    '【体验记忆】列出指定博客的所有经验记录。可按分类、时间范围筛选。如果不传 blogId，列出所有包含经验的博客',
  inputJSONSchema: {
    type: 'object',
    properties: {
      blogId: {
  ...
  handler: async (args: { blogId?: string; category?: string; pinnedOnly?: boolean; limit?: number }) => {
    try {
      ...
      const category = args.category
        ? (args.category.split(',').map((c) => c.trim()) as BlogMemoryCategory[])
        : undefined
      ...
      const lines: string[] = [
        `📝 博客「${args.blogId}」的经验记录 (${results.length} 条):`,
        '',
      ]
...
export const blogMemoryPinTool = buildTool({
  ...
  description:
    '【体验记忆】标记/取消标记一条经验为重要（固定）。被固定的经验不会被自动清理，在检索时优先展示',
  inputJSONSchema: {
    type: 'object',
    properties: {
      memoryId: {
...
export const blogMemoryDeleteTool = buildTool({
  ...
  description:
    '【体验记忆】删除一条经验记录。删除后不可恢复。如果经验已被固定（标记重要），需要先取消固定才能删除',
  inputJSONSchema: {
    type: 'object',
    properties: {
      memoryId: {
...
export const blogMemoryClearTool = buildTool({
  ...
  description:
    '【体验记忆】清空指定博客的所有经验记录。保留被固定的经验（除非设置 includePinned=true）',
  inputJSONSchema: {
    type: 'object',
    properties: {
      blogId: {
...
export const blogMemoryStatsTool = buildTool({
  ...
  description:
    '【体验记忆】查看经验记忆系统的统计信息：总条目数、已固定数、涉及博客数、各分类分布',
  inputJSONSchema: {
    type: 'object',
    properties: {},
    required: [],
...
export const blogMemoryRecordTool = buildTool({
  ...
  description:
    '【体验记忆】手动记录一条经验到记忆库。适用于 Agent 在写作过程中主动记录重要发现、决策、代码分析结论等',
  inputJSONSchema: {
    type: 'object',
    properties: {
      content: {
      ...
      category: {
        type: 'string',
        description: '分类：analysis（代码分析）、design_decision（设计决策）、test_result（测试结果）、refactoring（重构记录）、bug_fix（Bug修复）、performance（性能优化）、architecture（架构决定）、other（其他）',
