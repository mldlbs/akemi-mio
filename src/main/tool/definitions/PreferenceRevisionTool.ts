...
import { getPreferenceRevisionMemory } from '../../creativity/PreferenceRevisionMemory'
  ...
  inputJSONSchema: {
    ...
    properties: {
      ...
      chapterId: {
        type: 'string',
        description:
          '章节标识（如 "ch19" 或 "第19章"）。record_preference / record_revised_chapter 需要。',
      },
      suggestion: {
        type: 'string',
        description:
          'Agent 给出的修订建议摘要（300 字以内）。record_preference 需要。',
      },
      feedbackType: {
        type: 'string',
        enum: ['accept', 'reject'] as any,
        description:
          '用户反馈类型。record_preference 需要。\n' +
          '- accept: 用户接受了该建议\n' +
          '- reject: 用户拒绝了该建议',
      },
      userComment: {
        type: 'string',
        description:
          '用户可选评论文本，可用于后续偏好分析（500 字以内）。record_preference 可选。',
      },
      category: {
        type: 'string',
      ...
      summary: {
        type: 'string',
        description:
          '修订内容摘要（500 字以内）。record_revised_chapter 需要。',
      },
      minConfidence: {
        type: 'number',
        description:
          '偏好提示的最低置信度过滤（0-1，默认 0）。值越高返回越可靠的提示但数量越少。get_hints 可选。',
      },
    },
    required: ['action'],
  },
      ...
      switch (action) {
        ...
        case 'get_preferences': {
          ...
          const formatted = preferences.slice(0, 20).map((p) => {
            const catLabel: Record<string, string> = {
              style: '文风', detail: '详略', plot: '情节',
              dialogue: '对话', character: '角色', other: '其他',
            }
        ...
        case 'get_revision_progress': {
          ...
          if (progress.totalPreferences === 0 && progress.revisedCount === 0) {
            return formatToolResult(
              `《${storyName}》暂无修订活动。可使用 record_preference 记录修订反馈，` +
              'record_revised_chapter 记录已修订章节。',
            )
          }
          ...
          return formatToolResult(
            `📊 《${storyName}》修订进度\n` +
              ...
              (progress.revisedChapters.length > 0
                ? `📖 已修订章节：${progress.revisedChapters.join('、')}\n`
                : '') +
        ...
        case 'clear_story': {
          ...
          return formatToolResult(
            `✓ 已清空《${storyName}》的 ${removed} 条偏好与修订记录。`,
          )
        }
