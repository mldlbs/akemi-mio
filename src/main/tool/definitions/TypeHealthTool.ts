...
import { typeHealthScanner } from '../../evolution/typehealth/TypeHealthScanner'
import { learningVocabularyManager } from '../../learning/LearningVocabularyManager'
import type { TypeHealthCategory, TypeHealthScanConfig } from '../../evolution/typehealth/TypeHealthIssue'
...
export const typeHealthTool = buildTool({
  name: 'type_health',
  description: '代码库类型健康度扫描与重构。支持子命令：scan(执行扫描), report(查看摘要), issues(查看问题), apply(应用重构), apply_all(应用全部)',
  inputJSONSchema: {
    type: 'object',
    properties: {
      action: {
