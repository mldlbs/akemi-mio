...
export const saveTaskStateTool = buildTool({
  ...
  description:
    '保存当前任务进度，以便下次对话恢复。当执行多步骤任务时，每完成一步或暂停时调用。系统会自动追踪任务ID、步骤进度和中间结果',
  inputJSONSchema: {
    type: 'object',
    properties: {
...
export const queryTasksTool = buildTool({
  name: 'query_tasks',
  description:
    '查询未完成的任务列表，用于对话开始时恢复上次的工作。返回任务ID、标题、进度、上次更新时间等信息',
  inputJSONSchema: {
    type: 'object',
    properties: {},
    required: [],
...
export const saveUserPreferenceTool = buildTool({
  ...
  description:
    '保存用户的个人偏好到记忆系统。当用户透露了风格、语言、详略度等偏好时主动调用，用于跨对话保持一致的交互体验',
  inputJSONSchema: {
    type: 'object',
    properties: {
...
export const getUserPreferencesTool = buildTool({
  ...
  description:
    '获取已保存的用户画像和偏好信息。在对话开始时自动调用，帮助了解用户习惯和偏好',
  inputJSONSchema: {
    type: 'object',
    properties: {},
    required: [],
  ...
  handler: async () => {
    try {
      ...
      const labels: Record<string, string> = {
        style: '风格', detail: '详略', language: '语言', preference: '偏好', identity: '身份', other: '其他',
      }
