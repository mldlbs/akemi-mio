...
import { streamingAudioBuffer } from '../../tts/StreamingAudioBuffer'
import { ttsRouter } from '../../tts/TtsRouter'
import { networkMonitor } from '../../tts/NetworkMonitor'
...
export const ttsSpeakTool = buildTool({
  ...
  inputJSONSchema: {
    ...
    properties: {
      ...
      rate: {
        type: 'string',
        description:
          '语速（Edge TTS 云端引擎）。格式如 "+10%"（加快 10%）或 "-5%"（减慢 5%）。' +
          '不指定时由情感标签自动选择。',
      },
      pitch: {
        type: 'string',
        description:
          '音调（Edge TTS 云端引擎）。格式如 "+8Hz"（升高）或 "-4Hz"（降低）。' +
          '不指定时由情感标签自动选择。',
      },
      ...
      speed: {
        type: 'number',
        description:
          '语速因子（本地引擎，0.5-2.0）。1.0=正常，0.7-0.9=较慢，1.1-1.3=较快。' +
          '不指定时由情感标签或模型默认值决定。',
      },
      pitch_factor: {
        type: 'number',
        description:
          '音调因子（本地引擎，0.5-2.0）。1.0=正常，>1=偏高（活泼），<1=偏低（沉稳）。' +
          '不指定时由情感标签或模型默认值决定。',
      },
    },
    required: ['text'],
  ...
  handler: async (args: {
    ...
    if (emotion && !EMOTION_LABELS.includes(emotion as any) && !['欢快', '严肃', '温柔', '中性', '温暖', '活力', '沉稳', '俏皮', '开心', '高兴', '生气', '难过', '平静', '热情', '活泼'].includes(emotion)) {
    ...
    if (effectiveStreaming) {
      ...
      const modelDisplayName = resolvedPiperModel
        ? PIPER_MODEL_CATALOG[resolvedPiperModel]?.displayName || resolvedPiperModel
        : ''
