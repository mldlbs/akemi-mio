...
import { getToolChainOrchestrator } from '../deps'
...
export const orchestrateTaskTool = buildTool({
  ...
  description: '【工具链编排】自动将复杂请求分解为子步骤，发现可用 MCP 工具，按依赖关系编排执行，实时展示进度。适用于需要多个工具协作的场景，如：「帮我查天气并设置闹钟」「搜索某个主题并保存到文件」「分析代码后发送报告」。传入自然语言描述即可，系统会自动分解、匹配工具、按序执行并汇总结果',
  inputJSONSchema: {
    type: 'object',
    properties: {
      task: {
    ...
    try {
      ...
      if (result.success) {
        return formatToolResult(
          `✅ 工具链编排完成（${result.totalDurationMs}ms）\n\n${result.summary}`,
        )
      }

      return formatToolResult(
        `⚠️ 工具链编排部分完成（${result.totalDurationMs}ms）\n\n${result.summary}`,
      )
    } catch (err: any) {
 ...
 */
export const toolChainOrchestratorTools = [
  orchestrateTaskTool,
