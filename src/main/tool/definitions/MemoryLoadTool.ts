  ...
  handler: async (args: {
    ...
    try {
      ...
      const recentEntries = allEntries
        .filter((e) => e.type === 'user_fact' && e.topics && e.topics.length > 0)
        .slice(-50)
      ...
      // 格式化输出
      const formatted = this.formatLoadResult(result)
  ...
  if (result.preferences.length > 0) {
    ...
    for (const [cat, items] of Object.entries(byCategory)) {
      const catLabels: Record<string, string> = {
        style: '风格偏好', detail: '详略偏好', language: '语言偏好',
        preference: '个人偏好', identity: '身份信息', other: '其他',
      }
