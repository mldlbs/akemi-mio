...
import { memoryEntityExtractor } from '../../asr/MemoryEntityExtractor'

// ══════════════════════════════════════════
//  工具：注入 ASR 领域词汇
// ══════════════════════════════════════════

export const injectAsrVocabularyTool: Tool = {
  name: 'inject_asr_vocabulary',
  description: '将一组领域词汇注入 ASR 语音识别的热词系统，提升对这些词汇的识别准确率。用于在 ASR 启动时或检测到新对话话题时调用，使 ASR 更准确地识别专有名词、人名、项目名和地名。',
  inputJSONSchema: {
    type: 'object',
    properties: {
      words: {
  ...
  handler: async (args: Record<string, any>) => {
    ...
    if (!words || words.length === 0) {
      return { success: false, error: '词汇列表为空', injected: 0 }
    }
...
export const queryAsrVocabularyTool: Tool = {
...
export const clearAsrVocabularyTool: Tool = {
