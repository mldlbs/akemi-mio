...
export class ToolConfigManager {
  ...
  applyChange(
    ...
    const changes = Object.keys(newValues).filter(
      (k) => (newValues as any)[k] !== (before as any)[k],
    )
  ...
  evaluateAndMaybeRollback(snapshotId: string): {
    ...
    if (decision.action === 'rollback') {
      // 查找该快照关联的所有变更
      const relatedChanges = this.history.filter(
  ...
  canOptimize(toolName: string, minNewCalls: number = MIN_NEW_CALLS_BEFORE_RE_EVAL): boolean {
    ...
    if (snapshotId) {
      ...
      if (snapshot) {
        ...
        const baselineCalls = snapshot.beforeMetrics.byTool.find(
          (bt) => bt.toolName === toolName,
        )?.totalCalls ?? 0
  ...
  getPendingEvaluations(): ConfigChangeRecord[] {
    ...
    return this.history.filter(
      (h) => h.evalResult === undefined || h.evalResult === null,
    )
  }
  ...
  private extractChangedFields(
    before: ToolTunableConfig,
    newValues: Partial<ToolTunableConfig>,
  ): Partial<ToolTunableConfig> {
