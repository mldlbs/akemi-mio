...
export class GoalEngine {
  getActiveGoals(category?: Goal['category']): Goal[] {
    ...
    return rows ? rows.values.map((v: any[]) => this.rowToGoal(rows.columns, v)) : []
  }
  ...
  create(input: GoalInput): Goal {
    ...
    const now = Date.now()
    db.run(
      `INSERT INTO goals (id, title, description, priority, status, category, parent_goal_id, progress, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`,
      [id, input.title, input.description, input.priority, input.status, input.category, input.parentGoalId || null, now, now],
    )
  ...
  adjustPrioritiesByToken(tokenBalance: number): void {
    ...
    if (tokenBalance < 5000) {
      for (const g of lowCost) dbUpdatePriority(g.id, Math.max(g.priority, 5))
      for (const g of highCost) dbUpdatePriority(g.id, Math.min(g.priority, 3))
    } else {
      for (const g of [...lowCost, ...highCost]) dbUpdatePriority(g.id, g.priority)
    }
  }

  private estimateTokenCost(goal: Goal): number {
    const costMap: Record<Goal['category'], number> = {
      mission: 5000,
      long_term: 3000,
      short_term: 800,
      initiative: 2000,
  ...
  private rowToGoal(columns: string[], values: any[]): Goal {
    const obj: any = {}
...
function dbUpdatePriority(id: string, priority: number): void {
  const db = getRawDb()
