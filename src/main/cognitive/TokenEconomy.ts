...
const TASK_COST_ESTIMATES: Record<string, number> = {
  analysis: 2000,
  plan_execution: 5000,
...
export class TokenAccount {
  ...
  private applyAutoAllowance(): void {
    const db = getRawDb()
  ...
  getWealthLevel(): 'poor' | 'moderate' | 'rich' {
    if (this.cachedBalance < 3000) return 'poor'
    if (this.cachedBalance < 30000) return 'moderate'
    return 'rich'
  }
  ...
  getLifetimeStats(): { earned: number; spent: number } {
    const db = getRawDb()
...
export class CostEstimator {
  ...
  estimateTask(taskType: string, complexity: 'low' | 'medium' | 'high' = 'medium'): number {
  ...
  isWorthwhile(earnings: number, cost: number, threshold: number = 1.5): boolean {
