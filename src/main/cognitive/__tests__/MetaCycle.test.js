...
import { ProceduralMemory } from '../../agent/ProceduralMemory';
import { EngineeringMemory } from '../../memory/EngineeringMemory';
const TEST_DB = join(process.cwd(), 'akemi-mio.db');
...
describe('MetaCycle', () => {
    ...
    beforeEach(async () => {
        ...
        process.env.AKEMI_MIO_DB_PATH = TEST_DB;
        await initDatabase();
    ...
    afterEach(() => {
        ...
        if (existsSync(TEST_DB))
            unlinkSync(TEST_DB);
        if (existsSync(TEST_CONSTITUTION))
            unlinkSync(TEST_CONSTITUTION);
    });
    it('should initialize with deps', () => {
        cycle.initialize({ identity, engineeringMemory: engMem, proceduralMemory: procMem, llmService: null });
    ...
    it('should load latest review from DB on re-init', () => {
        ...
        const cycle2 = new MetaCycle();
        cycle2.initialize({ identity, engineeringMemory: engMem, proceduralMemory: procMem, llmService: null });
