...
import { LLM_API_URL, LLM_CHAT_MODEL } from '../config'
import { getCredentialsManager } from '../tool/deps'
import { createTimeoutSignal } from '../utils/async'
import { WritingMemoryContinuation } from '../creativity/WritingMemoryContinuation'
import type { StoryAtmosphere } from '../audio/types'
...
export class VoiceContinuationService {
  ...
  private getApiKey(): string | null {
    const envKey = process.env.LLM_KEY
    if (envKey) return envKey
    try {
      const cm = getCredentialsManager()
      return cm?.get('llm_key') || null
    } catch {
      return null
    }
  }
  ...
  async initializeContext(storyName: string, chapterNum: number): Promise<ContinuationContext> {
    ...
    try {
      ...
      const story = stories.find(
        (s: any) => s.title === storyName || s.title?.includes(storyName) || storyName.includes(s.title || ''),
      )
      if (story) {
        ...
        let prevScene = Array.isArray(scenes)
          ? scenes.find((s: any) => Number(s.order) === prevChapterNum)
          : null
  ...
  private buildContinuationPrompt(
    ...
    if (atmosphere && atmosphere.confidence > 0.2) {
      ...
      if (atmosphere.tension > 0.3) activeDims.push(`- 紧张度 ${(atmosphere.tension * 100).toFixed(0)}%：节奏紧凑，悬念适度，让读者有压迫感`)
   ...
   * 调用 LLM 进行续写生成
   */
  private async callLlmForContinuation(
    prompt: string,
    storyName: string,
    chapterNum: number,
  ): Promise<string> {
