...
/** 严格模式阈值（低于此值视为需要重试） */
const DEFAULT_STRICT_THRESHOLD = 0.50
...
export class ASRCrossValidationStage implements StageExecutor {
  ...
  async execute(
    ...
    const audioFile: string =
      (piperStage?.data?.audioFile as string) ?? ''
    const synthesisSuccess: boolean =
      (piperStage?.data?.success as boolean) ?? false
    ...
    const modelUsed: string =
      (piperStage?.data?.model as string) ?? ''
    const emotionLabel: string =
      (piperStage?.data?.emotionLabel as string) ?? ''
    const synthesisDurationMs: number =
      (piperStage?.data?.synthesisDurationMs as number) ?? 0
    ...
    try {
      ...
      // ── 仲裁逻辑 ──
      const arbitrationResult = this.arbitrate(
        similarity,
        cer,
        validationMode,
        threshold,
      ...
      if (enableAutoRetry && arbitrationResult === 'retry') {
        ...
        try {
          ...
          const retryResult = await this.retrySynthesis(
            originalText,
            modelUsed,
            ctx.traceId,
          )
          ...
          if (retryResult.success && retryResult.audioFile) {
            // 对重试结果再次 ASR
            const retryAsr = await this.transcribeAudio(
              retryResult.audioFile,
              `${ctx.traceId}_retry`,
            )
