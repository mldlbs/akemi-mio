...
import type { RadarScanInput } from '../../startup-radar/types'
...
import type {
  CorrectionInput,
  CorrectionSeverity,
} from '../../gongye-songge/PlanSonggeCorrectionAdapter'

// Blog Analysis (博客分析)
import { blogAnalyticsTracker } from '../../agent/blog/BlogAnalyticsTracker'
import type { AnalyticsReport, StrategyOutput } from '../../agent/blog/types'
...
export class PlanParallelAdvancementStage implements StageExecutor {
  ...
  async execute(
    ...
    const getDuration = (key: string): number =>
      settled.find((t) => t.key === key)?.durationMs ?? 0
 ...
 */
async function runRadarMerge(
  _asrText: string,
  keywords: string[],
  traceId: string,
): Promise<RadarMergeOutput> {
  const t0 = Date.now()
 ...
 * - 返回备份分析结果
 */
async function runSonggeBackup(
  asrText: string,
  traceId: string,
): Promise<SonggeBackupOutput> {
  const t0 = Date.now()
...
async function runBlogAnalysis(
  ...
  const plannedTopics = keywords.length > 0
    ? [...keywords.slice(0, 5)]
    : undefined
...
async function timedTask<T>(
  key: string,
  fn: () => Promise<T>,
): Promise<{ key: string; result: T; durationMs: number }> {
  const t0 = Date.now()
