...
export class TtsParameterStage implements StageExecutor {
  ...
  async execute(
    ...
    const dominantLabel: string =
      (emotionStage?.data?.dominantLabel as string) ?? 'neutral'
    const rateDeltaFromEmotion: number =
      (emotionStage?.data?.rateDelta as number) ?? 0
    const pitchDeltaFromEmotion: number =
      (emotionStage?.data?.pitchDelta as number) ?? 0
    const trend: string =
      (emotionStage?.data?.trend as string) ?? 'stable'
    const interestTopics: string[] =
      (memoryStage?.data?.interestTopics as string[]) ?? []
    ...
    const blendedRate = Math.round(
      baseRate * (1 - emotionWeight) + (baseRate + rateDeltaFromEmotion) * emotionWeight,
    )
    const blendedPitch = Math.round(
      basePitch * (1 - emotionWeight) + (basePitch + pitchDeltaFromEmotion) * emotionWeight,
    )
