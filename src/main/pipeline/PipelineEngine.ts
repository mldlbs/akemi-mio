...
import { cacheManager, computeInputHash } from './CacheManager'
import type {
  PipelineDefinition,
  PipelineResult,
  StageDefinition,
...
export class PipelineEngine {
    ...
    for (const stageDef of sortedStages) {
      ...
      if (upToStage && stageDef.id === upToStage) {
        await this.executeSingleStage(stageDef, stageOutputs, pipelineInput, traceId, options)
          .then(output => { stageOutputs.set(stageDef.id, output) })
          .catch(err => { stageErrors[stageDef.id] = String(err) })
        break
      }

      try {
        const output = await this.executeSingleStage(
          stageDef,
          stageOutputs,
          pipelineInput,
          traceId,
  ...
  private async executeSingleStage(
    ...
    if (!executor) {
      throw new Error(
        `No executor registered for stage type "${stageDef.stageType}". ` +
