...
describe('ASRCrossValidationStage — 综合相似度', () => {
  ...
  it('非常相似的文本', () => {
    const sim = computeCombinedSimilarity(
      '你好，今天天气真好',
      '你好今天天气真好',
    )
    // 标点差异不应该大幅降低相似度
...
describe('ASRCrossValidationStage — 仲裁逻辑', () => {
  ...
  it('等于阈值 → pass', () => {
    expect(arbitrate(0.70, 0.30, 'normal', threshold, strictThreshold)).toBe('pass')
  })

  it('低于阈值但高于严格阈值 → warning', () => {
    expect(arbitrate(0.60, 0.40, 'normal', threshold, strictThreshold)).toBe('warning')
  })

  it('低于严格阈值 → retry', () => {
    expect(arbitrate(0.30, 0.70, 'normal', threshold, strictThreshold)).toBe('retry')
  })

  it('等于严格阈值 → retry（严格阈值为不合格分界）', () => {
    expect(arbitrate(0.50, 0.50, 'normal', threshold, strictThreshold)).toBe('retry')
  })
  ...
  describe('strict 模式', () => {
    ...
    it('边界过高阈值被 clamp', () => {
      const result = arbitrate(0.98, 0.02, 'strict', 0.90, 0.50)
  ...
  describe('relaxed 模式', () => {
    it('阈值降低 0.15', () => {
      // normal 模式下 0.55 是 retry, relaxed 下应变为 warning
      expect(arbitrate(0.55, 0.45, 'normal', threshold, strictThreshold)).toBe('retry')
      expect(arbitrate(0.55, 0.45, 'relaxed', threshold, strictThreshold)).toBe('warning')
    })

    it('边界过低阈值被 clamp', () => {
      const result = arbitrate(0.05, 0.95, 'relaxed', 0.10, 0.05)
...
describe('ASRCrossValidationStage — 流水线定义完整性', () => {
  it('hybrid-tts-asr-pipeline.json 文件存在', async () => {
    // 验证定义被正确导入
    const def = await import('../definitions/hybrid-tts-asr-pipeline.json')
