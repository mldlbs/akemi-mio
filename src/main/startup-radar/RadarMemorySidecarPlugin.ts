...
import { startupRadarAdapter } from './PlanStartupRadarAdapter'
import type {
  ISidecarPlugin,
  AddEntryContext,
  AddEntryResult,
...
export class RadarMemorySidecarPlugin implements ISidecarPlugin {
  ...
  preAddEntry(ctx: AddEntryContext): AddEntryResult {
    ...
    if (
      ctx.type === 'user_fact' &&
      ctx.content.startsWith(RADAR_FEEDBACK_PREFIX) &&
      ctx.confidence < 0.5
    ) {
    ...
    // 过短的雷达扫描日志 → 过滤
    if (
      ctx.content.startsWith(RADAR_SCAN_PREFIX) &&
      ctx.content.length < 20
    ) {
  ...
  postGetEntries(entries: MemoryEntry[]): MemoryEntry[] {
    ...
    const radarFeedbackEntries = entries.filter(
      (e) => e.type === 'user_fact' && e.content.startsWith(RADAR_FEEDBACK_PREFIX),
    )
  ...
  preGetFormattedContext(): GetFormattedContextResult {
    // 检查雷达上下文是否仍然有效
    if (
      this.lastRadarContext &&
      Date.now() - this.lastRadarContextTimestamp < this.RADAR_CONTEXT_TTL_MS
    ) {
