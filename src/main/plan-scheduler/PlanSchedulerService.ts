...
import type { ToolResult, ToolScheduler } from '../agent/ToolScheduler'
...
class ToolSchedulerAdapter implements ToolExecutor {
  ...
  async executeTool(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    const results = await this.toolScheduler.executeAll([
      { id: `sched_${Date.now()}`, name: toolName, arguments: args },
    ])
    return results[0] ?? {
      id: '',
      name: toolName,
...
export class PlanSchedulerService {
  ...
  async loadAndSchedule(options?: {
    ...
    const plan = options?.planId
      ? this.deps.planManager.getPlan(options.planId)
      : this.deps.planManager.getActivePlan()
  ...
  async schedulePlan(
    ...
    const analysisResults = await this.analyzer.analyzePlan(
      plan.id,
      plan.title,
      plan.description,
      stepInputs,
  ...
  private buildExecutionPlan(
    plan: DevPlan,
    analysisResults: import('./types').StepAnalysisResult[],
  ): PlanExecutionPlan {
    ...
    // 估计总时长
    const estimatedTotalSec = analysisResults.reduce(
      (s, a) => s + (a?.estimatedDurationSec ?? 120),
      0,
    )
