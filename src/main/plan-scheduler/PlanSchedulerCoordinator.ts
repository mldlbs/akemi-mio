...
import type { ToolScheduler } from '../agent/ToolScheduler'
import type { PlanManagerLike, DevPlan } from '../evolution/types'
...
export class PlanSchedulerCoordinator {
  ...
  private async heartbeat(): Promise<void> {
    for (const [planId, scheduler] of this.schedules) {
      ...
      const hasReady = engine.getAnalysisReadyTasks().length > 0
        || engine.getReadyTasks().length > 0
