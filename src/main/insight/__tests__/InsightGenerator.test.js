...
describe('InsightScorer', () => {
    ...
    describe('score', () => {
        ...
        it('多个 detection 按得分降序排列', () => {
            const detections = [
                { detector: 'a', type: 'hot_topic', severity: 'low', title: '低分', description: '', evidence: [], novelty: 10, impact: 10, actionability: 10 },
                { detector: 'b', type: 'conflict', severity: 'high', title: '高分', description: '', evidence: ['e1', 'e2', 'e3'], novelty: 90, impact: 90, actionability: 80 },
            ];
            const results = scorer.score(detections, ctx);
    ...
    describe('calculateConfidence', () => {
        ...
        it('零值 novelty/impact/actionability 得分为 0', () => {
            const detections = [{
                    detector: 'a', type: 'conflict', severity: 'low',
                    title: '零分', description: '', evidence: ['e'],
                    novelty: 0, impact: 0, actionability: 0,
                }];
        ...
        it('满分 novelty/impact 得分最高', () => {
            const detections = [{
                    ...
                    title: '满分', description: '', evidence: ['e1', 'e2'],
                    novelty: 100, impact: 100, actionability: 100,
                }];
            const [result] = scorer.score(detections, ctx);
...
describe('InsightGenerator', () => {
    ...
    beforeEach(() => {
        chatJson = vi.fn();
        generator = new InsightGenerator({ chatJson: chatJson });
        ctx = {
    ...
    it('无内存和摘要时返回空数组', async () => {
        const emptyCtx = { memoryEntries: [], summaries: [], interactionCount: 0, plans: [], eventCount: 0 };
    ...
    it('LLM 返回多条结果时全部参与评分和过滤', async () => {
        chatJson.mockResolvedValue({
            data: [
                { title: '高分洞察', description: 'a', detector: 'conflict', severity: 'high', evidence: ['e1', 'e2', 'e3'], novelty: 90, impact: 85, actionability: 80 },
                { title: '低分洞察', description: 'b', detector: 'hot_topic', severity: 'low', evidence: [], novelty: 10, impact: 10, actionability: 10 },
                { title: '边缘洞察', description: 'c', detector: 'conflict', severity: 'medium', evidence: ['e1'], novelty: 50, impact: 40, actionability: 30 },
            ],
        });
        const results = await generator.generate(ctx);
    ...
    it('LLM 有返回时触发 candidate.generated 事件', async () => {
        const { eventBus } = await import('../../core/EventBus');
        chatJson.mockResolvedValue({
            data: [{ title: 't', description: 'd', detector: 'conflict', severity: 'medium', evidence: ['e1'], novelty: 50, impact: 50, actionability: 50 }],
        });
        await generator.generate(ctx);
