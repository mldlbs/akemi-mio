...
function createMockStore() {
    ...
    return {
        ...
        addMany: vi.fn((arr) => insights.push(...arr)),
        getUnreported: vi.fn(() => insights.filter(i => !reportedIds.includes(i.id))),
        getHighValueUnreported: vi.fn((threshold = 50, confidenceThreshold = 0.7) => insights.filter(i => !reportedIds.includes(i.id) && i.score >= threshold && i.confidence >= confidenceThreshold)),
        markReported: vi.fn((id) => { if (!reportedIds.includes(id))
            reportedIds.push(id); }),
        markAllReported: vi.fn(() => { insights.forEach(i => { if (!reportedIds.includes(i.id))
            reportedIds.push(i.id); }); }),
...
describe('InsightService', () => {
    ...
    beforeEach(() => {
        ...
        chatJson = vi.fn();
        presence = new PresenceService(0.1, mockBus); // 6秒 idle timeout
        service = new InsightService(store, presence, {
            getMemoryEntries: () => [{ type: 'chat', content: 'test', createdAt: Date.now() }],
            getSummaries: () => ['summary'],
            getInteractionCount: () => 10,
            getPlans: () => [],
        }, { chatJson: chatJson }, { minIdleMinutes: 0.1, minNewMessages: 0, minNewEvents: 0 }, mockBus);
    });
    afterEach(() => {
        service.stop();
        vi.useRealTimers();
    ...
    describe('start / stop', () => {
        it('多次 start 不重复创建 timer', () => {
            ...
            service.start();
            const timer2 = service.checkTimer;
    ...
    describe('forceAnalysis', () => {
        it('LLM 返回洞察时存入 store', async () => {
            ...
            const addedInsights = store.addMany.mock.calls[0][0];
    ...
    describe('tryAnalyze 门控条件', () => {
        it('away 状态不满足时跳过分析', async () => {
            ...
            const runAnalysisSpy = vi.spyOn(service, 'runAnalysis');
            service.tryAnalyze();
            expect(runAnalysisSpy).not.toHaveBeenCalled();
        });
        it('minIdleMinutes 不满足时跳过', () => {
            const longIdleService = new InsightService(store, presence, { getMemoryEntries: () => [], getSummaries: () => [], getInteractionCount: () => 0, getPlans: () => [] }, { chatJson: vi.fn() }, { minIdleMinutes: 9999, minNewMessages: 0, minNewEvents: 0 }, mockBus);
            longIdleService.messagesAtLastAnalysis = 0;
            longIdleService.eventsAtLastAnalysis = 0;
            // 让 presence 进入 away
            vi.advanceTimersByTime(10000);
            presence.tick();
            expect(presence.isAway()).toBe(true);
            const spy = vi.spyOn(longIdleService, 'runAnalysis');
            longIdleService.tryAnalyze();
