...
describe('ToolRegistry', () => {
    ...
    it('rejects duplicate registration', () => {
        ...
        expect(() => toolRegistry.register({ name: 'dup', description: 'Second', parameters: {}, required: [], handler: () => 'b', pluginName: 'p2' })).toThrow('already registered');
    });
...
describe('Management plugin', () => {
    ...
    it('create_plugin rejects invalid names', () => {
        expect(() => managementPlugin.handle('create_plugin', { name: '', content: 'x' })).toThrow('无效');
    });
    it('create_plugin validates content has manifest', () => {
        expect(() => managementPlugin.handle('create_plugin', { name: 'test', content: 'console.log(1)' })).toThrow('manifest');
    });
    it('list_plugins returns string output', async () => {
        const result = await managementPlugin.handle('list_plugins', {});
