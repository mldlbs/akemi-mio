...
import type { TopicPrediction } from '../memory/TopicTransitionPredictor'
...
export class BehaviorPredictiveMemoryPrewarmer {
  ...
  prewarmForSession(forceRefresh = false): PrewarmResult {
    ...
    // 对每条候选记忆按话题匹配度评分
    const scoredMemories: Array<{ entry: MemoryEntry; score: number; matchedTopic: string; scoreDetail: TopicScoreDetail }> = []
  ...
  private computeTopicScores(
    ...
    for (const [topic, data] of topicMap) {
      ...
      // resumedBoost: 中断续接场景下，最后话题获得额外 boost
      const resumedBoost = (intervalAnalysis.hasGap && lastTopics.includes(topic))
        ? RESUMED_TOPIC_BOOST
        : 0

      // 综合评分
      const combinedScore = Math.min(1, Math.round(
        (baseConfidence * recencyWeight + transitionBoost + resumedBoost) * 100,
      ) / 100)
  ...
  private getSceneDescription(
    scene: 'continue' | 'new' | 'resume_after_gap',
    _analysis: InteractionIntervalResult,
  ): string {
    switch (scene) {
      case 'continue':
