...
export class BehaviorBlogBridge {
  ...
  getBehaviorSummary(): BehaviorSummary {
    try {
      ...
      for (const t of sceneResult.dominantTopics) {
        if (!topicSet.has(t)) { topicSet.add(t); topicList.push(t) }
      }
   ...
   * 数据不足时返回平坦的默认分布。
   */
  private computeActiveTimeDistribution(
    totalInteractions: number,
  ...
  private formatSummary(
    ...
    if (!hasData || totalInteractions < MIN_INTERACTIONS_FOR_ACTIVITY) {
      return (
        '【行为分析】用户行为数据不足（交互数不足），无法生成个性化风格建议。' +
        '建议使用通用友好的技术博客风格。'
      )
    }
    ...
    if (avgLength > 0) {
      lines.push(`- 用户平均消息长度：${Math.round(avgLength)} 字${avgLength > 80 ? '（偏好详细内容）' : avgLength > 30 ? '（偏好中等深度）' : '（偏好简洁内容）'}`)
    }
    lines.push(`- 当前交互场景：${scene}`)
    if (sentiment !== 'unknown') {
      lines.push(`- 用户情绪基调：${sentiment === 'positive' ? '积极正面' : sentiment === 'negative' ? '消极负面' : '中性'}`)
    }
    lines.push(`- 推荐发布时间：${['周日', '周一', '周二', '周三', '周四', '周五', '周六'][publishRec.recommendedDay]} ${publishRec.recommendedHour}:00（置信度 ${(publishRec.confidence * 100).toFixed(0)}%）`)
