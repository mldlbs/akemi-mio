...
export class BehaviorUsagePatternAnalyzer {
  ...
  analyze(options: AnalyzeOptions = {}): UsagePatternReport {
    ...
    // 阶段 3：情感倾向分析
    const sentiment = skipSentiment
      ? this.getDefaultSentiment()
      : this.analyzeSentiment(texts)
  ...
  private classifyQuestionTypes(texts: string[]): QuestionTypeAnalysis {
    const counts: Record<QuestionType, number> = {
      debug: 0, code_gen: 0, query: 0, chat: 0, command: 0, creative: 0, other: 0,
    }
  ...
  private analyzeSentiment(texts: string[]): SentimentAnalysis {
    const counts: Record<SentimentLabel, number> = {
      urgent: 0, frustrated: 0, satisfied: 0, neutral: 0,
    }
  ...
  private detectRepeatedPatterns(
    ...
    for (let i = 0; i < texts.length - 1; i++) {
      ...
      const currentWords = new Set(
        (current.match(/[一-鿿]{2,}/g) || []).slice(0, 10),
      )
      const nextWords = new Set(
        (next.match(/[一-鿿]{2,}/g) || []).slice(0, 10),
      )
