...
const STOP_WORDS = new Set([
  ...
  '吧', '吗', '啊', '呢', '哦', '嗯', '哈', '然后', '因为', '所以', '但是',
  '如果', '虽然', '可以', '这个', '那个', '什么', '怎么', '为什么', '哪个',
  'the', 'a', 'an', 'is', 'are', 'was', 'were', 'be', 'been', 'being',
  'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'can', 'could',
  'may', 'might', 'shall', 'should', 'to', 'of', 'in', 'for', 'on', 'with',
  'at', 'by', 'from', 'as', 'into', 'through', 'during', 'before', 'after',
  'above', 'below', 'between', 'and', 'or', 'but', 'not', 'so', 'if', 'than',
...
export class BehaviorDrivenMemoryAnalyzer {
  ...
  start(intervalMs: number = BEHAVIOR_MEMORY_ANALYZER_INTERVAL): void {
    ...
    setTimeout(() => {
      this.runAnalysis().catch((err) =>
        log('WARN', 'behavior_memory_analyzer_init_run_failed', { error: String(err) }),
      )
    }, 30_000)

    this.timer = setInterval(() => {
      this.runAnalysis().catch((err) =>
        log('WARN', 'behavior_memory_analyzer_cycle_failed', { error: String(err) }),
      )
    }, intervalMs)
  }
  ...
  analyzeTimePatterns(records: InteractionRecord[]): TimePattern {
    ...
    const activeHours = hourCounts
      .map((count, hour) => ({
        hour,
        count,
        score: hourStats.stdDev > 0
          ? (count - hourStats.mean) / hourStats.stdDev
          : 0,
      }))
    ...
    // 如果没有活跃小时，取最高的几个
    const finalActiveHours = activeHours.length > 0
      ? activeHours
      : hourCounts
    ...
    const activeDays = dayCounts
      .map((count, day) => ({
        day,
        count,
        score: dayStats.stdDev > 0
          ? (count - dayStats.mean) / dayStats.stdDev
          : 0,
      }))
      .filter((d) => d.score >= PEAK_Z_SCORE * 0.7) // 宽松阈值
      .sort((a, b) => b.score - a.score)

    const finalActiveDays = activeDays.length > 0
      ? activeDays
      : dayCounts
    ...
    const daySpan = timespans.length >= 2
      ? Math.max(1, (Math.max(...timespans) - Math.min(...timespans)) / 86_400_000)
      : 1
  ...
  private generateMemories(
    ...
    if (topTerms.length >= 3) {
      // 构建偏好话题列表
      const topTopics = topTerms
        .filter((t) => t.documentFrequency >= 2)
    ...
    if (clusters.length >= 2 && created < MAX_MEMORIES_PER_CYCLE) {
      const dominantClusters = clusters
        .filter((c) => c.size >= 2 && c.confidence >= 0.3)
        .slice(0, 2)

      for (const cluster of dominantClusters) {
        const topicStr = cluster.dominantTopics.length > 0
          ? cluster.dominantTopics.join('、')
          : '综合类'
  ...
  private hasExistingMemory(content: string): boolean {
    const ms = this.memoryService
    if (!ms) return false
    return ms.getEntries().some(
      (e) =>
        e.type === 'user_fact' &&
        e.structuredData &&
        e.structuredData.includes(STRUCTURED_SOURCE_KEY) &&
        e.content === content,
    )
  }
  ...
  private removePreviousBehaviorMemories(): void {
    ...
    for (const entry of entries) {
      if (
        entry.type === 'user_fact' &&
        entry.structuredData &&
        entry.structuredData.includes(STRUCTURED_SOURCE_KEY)
      ) {
  ...
  private loadInteractions(limit: number): InteractionRecord[] {
    ...
    try {
      ...
      const logResult = db.exec(
        `SELECT id, user_text, topics, timestamp FROM interaction_log ORDER BY timestamp DESC LIMIT ${limit}`,
      )
