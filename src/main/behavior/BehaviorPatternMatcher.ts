...
import {
  BEHAVIOR_PATTERN_MATCH_MIN_CONFIDENCE,
  BEHAVIOR_PATTERN_MATCH_MAX_RESULTS,
} from '../config'
...
export class BehaviorPatternMatcher {
  ...
  getPredictionContext(matches: MatchedPattern[]): string {
    ...
    for (const match of matches) {
      ...
      lines.push(
        `- 提到「${match.rule.consequent}」相关（概率 ${prob}%，置信度 ${confidence}%）`,
      )
    }

    lines.push(
      '你可以提前准备相关信息，或在适当时机主动询问是否需要帮助。',
      '---',
    )
...
export const behaviorPatternMatcher = new BehaviorPatternMatcher(
  _sharedStore,
)
