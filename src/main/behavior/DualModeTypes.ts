...
export const MODE_SPECIFICATIONS: Record<ModeType, ModeWorkingConditions> = {
  'user-behavior': {
    ...
    bestWhenDescription:
      '用户正常多任务操作、切换窗口、浏览网页、通讯交互。行为状态机持续跟踪窗口类别和空闲状态，快速响应交互变化。',
    notIdealWhenDescription:
      '用户进行深度专注学习（如编码练习）时，频繁的状态更新可能造成干扰；在长空闲时段后恢复时可能有短暂的状态滞后。',
  },
  'plan-typescript': {
    inputCharacteristics: {
      appCategoryDistribution: {
    ...
    notIdealWhenDescription:
      '用户未在进行 TypeScript 相关工作时强制激活会导致上下文错配；多任务高负载期间不应切换到此模式。',
  },
}
