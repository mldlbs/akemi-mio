 ...
 */
export function matchContentBehavior(
  metrics: ContentBehaviorMetrics,
): ContentBehaviorAdjustment {
  for (const rule of CONTENT_BEHAVIOR_RULES) {
    const { conditions } = rule

    // 检查每个条件（-1 = 忽略）
    const shortMatch = conditions.minShortMessages < 0
      || metrics.consecutiveShortMessages >= conditions.minShortMessages
    const questionMatch = conditions.minConsecutiveQuestions < 0
      || metrics.consecutiveQuestions >= conditions.minConsecutiveQuestions
    const intervalMatch = conditions.maxIntervalSec < 0
      || metrics.recentMessageIntervalSec <= conditions.maxIntervalSec
    const ratioMatch = conditions.minQuestionRatio < 0
      || metrics.questionMarkRatio >= conditions.minQuestionRatio
 ...
 */
export function applyContentAdjustment(
  base: EmotionTtsParams,
  adjustment: ContentBehaviorAdjustment,
): EmotionTtsParams {
