...
export class BehaviorStateMachine {
  ...
  computeMode(
    ...
    if (newMode !== prevMode) {
      log('INFO', 'behavior_mode_changed', {
        ...
        stats: stats
          ? { total: stats.totalInteractions, switches: stats.switchCount }
          : null,
      })
  ...
  private computeStats(): BehaviorStats | null {
    ...
    const stdIntervalMs = intervals.length > 1
      ? Math.sqrt(intervals.reduce((sum, v) => sum + (v - meanIntervalMs) ** 2, 0) / intervals.length)
      : 0
  ...
  private adaptThresholds(): void {
    ...
    const newFocusThreshold = Math.max(
      30_000,
      Math.min(300_000,
        Math.round(stats.meanIntervalMs * 4 * codeRatio)
      )
    )
  ...
  private isMultitasking(stats: BehaviorStats | null): boolean {
    ...
    const notDeepFocus = stats.dominantAppCategory !== 'code' ||
      stats.meanIntervalMs < 10_000 // 即使 code 但操作太频繁也算多任务
  ...
  private isFocused(appCategory: string, focused: boolean): boolean {
    ...
    // 检查最近交互窗口中编码占比
    const codeSamples = this.interactionBuffer.filter(
      (r) => r.appCategory === 'code'
    ).length
    const codeRatio = this.interactionBuffer.length > 0
      ? codeSamples / this.interactionBuffer.length
      : 0
