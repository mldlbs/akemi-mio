...
import { BEHAVIOR_PATTERN_ANALYZER_INTERVAL, BEHAVIOR_PATTERN_MIN_INTERACTIONS, BEHAVIOR_PATTERN_WINDOW_SIZE } from '../config'
  ...
  start(intervalMs: number = BEHAVIOR_PATTERN_ANALYZER_INTERVAL ?? DEFAULT_ANALYZER_INTERVAL_MS): void {
    ...
    setTimeout(() => {
      this.runAnalysis().catch((err) =>
        log('WARN', 'behavior_pattern_miner_first_run_failed', { error: String(err) }),
      )
    }, 60_000)

    this.timer = setInterval(() => {
      this.runAnalysis().catch((err) =>
        log('WARN', 'behavior_pattern_miner_cycle_failed', { error: String(err) }),
      )
    }, intervalMs)
  }
