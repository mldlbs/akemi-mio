...
import { DrizzlePlanManager } from '../evolution/DrizzlePlanManager'
...
export class DualModeController {
  ...
  getCurrentModeLabel(): string {
    return this.currentMode === 'user-behavior'
      ? '用户行为模式'
      : 'TypeScript 高级类型学习计划模式'
  }
  ...
  evaluateConditions(behaviorState: UserBehaviorState): SwitchDecision | null {
    ...
    const interactionRate = behaviorState.idleTimeMs > 0
      ? Math.round(60000 / Math.max(behaviorState.idleTimeMs, 1000))
      : 0
    ...
    if (this.currentMode === 'user-behavior') {
      ...
      if (shouldSwitch) {
        return this.buildDecision('user-behavior', 'plan-typescript', 'context_match', shouldSwitch.confidence, metrics, shouldSwitch.debounceMs)
      }
    }
    ...
    if (this.currentMode === 'plan-typescript') {
      ...
      if (shouldSwitchBack) {
        return this.buildDecision('plan-typescript', 'user-behavior', 'context_mismatch', shouldSwitchBack.confidence, metrics, shouldSwitchBack.debounceMs)
      }
    }
  ...
  private evaluatePlanModeConditions(
    ...
    const titleHasTs = PLAN_TITLE_KEYWORDS.some((kw) => titleLower.includes(kw.toLowerCase())) ||
   ...
   */
  private evaluateUserBehaviorConditions(
    metrics: SwitchDecision['triggerMetrics'],
  ): { confidence: number; debounceMs: number } | null {
    let confidence = 0.0
  ...
  getDiagnostics(): Record<string, unknown> {
    return {
      ...
      lastBehaviorSnapshotAge: this.lastBehaviorSnapshot
        ? Date.now() - this.lastBehaviorSnapshot.savedAt
        : null,
      lastPlanSnapshotAge: this.lastPlanSnapshot
        ? Date.now() - this.lastPlanSnapshot.savedAt
        : null,
    }
  }
}
