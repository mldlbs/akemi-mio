...
import { toolCallChainStore } from '../tool/ToolCallChainStore'
import type { CompletedChain } from '../tool/ToolCallChainStore'
...
export class PatternMiner {
  ...
  private prefixSpan(sequences: Sequence[]): FrequentPattern[] {
    ...
    const filtered = results
      .filter((p) => p.toolSequence.length >= 2)
   ...
   * - 关联关键词
   */
  private convertToBehaviorPatterns(
    ...
    for (const fp of frequentPatterns) {
      ...
      // 查找关联类别
      const associatedCategories = this.findAssociatedCategories(
        fp.toolSequence,
        categoryMap,
      )
  ...
  private inferStepDescription(toolName: string, params: Record<string, string>): string {
    ...
    const paramHint = Object.values(params).filter((v) => v.length < 40).slice(0, 1).join(', ')
   ...
   * 查找与工具序列关联的类别。
   */
  private findAssociatedCategories(
    toolSequence: string[],
  ...
  private extractKeywords(toolSequence: string[], sequences: Sequence[]): string[] {
    ...
    for (const seq of sequences) {
      ...
      if (!this.containsSubsequence(seq.map((s) => s.toolName), toolSequence)) continue
