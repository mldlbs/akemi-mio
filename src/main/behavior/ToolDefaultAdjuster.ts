...
export class ToolDefaultAdjuster {
  ...
  private buildToolDefaultHints(snapshot: PreferenceSnapshot): ToolDefaultHint[] {
    ...
    for (const [key, value] of Object.entries(snapshot.toolDefaults)) {
      ...
      const confidence = source === 'correction'
        ? Math.min(0.7, 0.2 + correctionCount * 0.15)
        : 0.4
  ...
  private buildFormattedPrompt(
    ...
    // 用户偏好语言对（特定场景）
    const langPair = snapshot.toolDefaults['translate_text:target_language'] ||
