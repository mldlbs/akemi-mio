...
import type {
  ContentBehaviorMetrics,
  ContentBehaviorAdjustment,
} from './BehaviorTtsContentMap'
...
export class BehaviorTtsContentDetector {
  ...
  private computeMetrics(): ContentBehaviorMetrics {
    ...
    // 距上一条消息的间隔
    const recentMessageIntervalSec = window.length >= 2
      ? (now - window[window.length - 1].timestamp) / 1000
      : 0

    // ── 指标 3: 问号比例 ──
    const questionCount = window.filter(m => m.endsWithQuestion).length
    const questionMarkRatio = window.length > 0
