...
import { RuntimeRestoreServiceImpl, type RuntimeRestoreResult } from '../RuntimeRestoreService'
...
describe('RuntimeRestoreService — Scene 8: activation phase', () => {
  it('should call activator on successful restore', async () => {
    ...
    registry.register({
      id: 'workflow-runtime', version: '1.0',
      create: () => ({
        id: 'workflow-runtime', snapshot: vi.fn(),
    ...
    const cp = makeValidCheckpoint({
      componentStates: { 'workflow-runtime': { component: 'workflow-runtime', version: '1.0', data: { activeRuns: [{ runId: 'r1', status: 'running' }] }, createdAt: 0 } },
    })
    await cpMgr.save(cp)
