...
describe('Runtime Harness — Contract Validation', () => {
  ...
  beforeEach(() => {
    events = []
    unsub?.()
    unsub = eventBus.on(RUNTIME_EVENT as any, (e: any) => { events.push(e) })
  })
  ...
  describe('Scenario 3: Cancel (RUNNING → CANCELLED)', () => {
    it('cancel 可从多状态出发', () => {
      for (const from of [RuntimeState.RUNNING, RuntimeState.WAITING_TOOL, RuntimeState.WAITING_SUPERVISOR, RuntimeState.PAUSED, RuntimeState.INTERRUPTED]) {
