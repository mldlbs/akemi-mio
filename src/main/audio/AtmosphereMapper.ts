...
export class AtmosphereMapper {
  ...
  map(features: AudioFeatures): StoryAtmosphere {
    ...
    const confidence = sorted.length >= 2
      ? Math.min(1, Math.max(0, sorted[0][1] - sorted[1][1]) * 2)
      : 0.5
    ...
    const result: StoryAtmosphere = {
      ...
      description: dominantDef
        ? `${dominantDef.label}：${dominantDef.description}`
        : '中性',
    }
  ...
  private mergeWeights(
    defaults: AtmosphereMappingWeights,
    overrides: Partial<AtmosphereMappingWeights>,
  ): AtmosphereMappingWeights {
    const result = { ...defaults }
    for (const dim of Object.keys(defaults) as Array<keyof AtmosphereMappingWeights>) {
      if (overrides[dim]) {
        result[dim] = { ...defaults[dim], ...overrides[dim] }
      }
    }
    return result
  }
