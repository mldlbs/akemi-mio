...
export class BrowserAgentRuntime {
  ...
  async connect(endpoint?: CdpEndpoint): Promise<CdpConnectResult> {
    ...
    try {
      ...
      // 找一个已登录番茄的页面
      this.pwPage = availablePages.find((p: any) =>
        p.url().includes('fanqienovel') && !p.url().includes('login')
   ...
   */
  async act(action: {
    type: 'navigate' | 'click' | 'fill' | 'observe'
    ...
    try {
      ...
      switch (action.type) {
        ...
        case 'observe': {
          ...
          const lines = text.split('\n').filter((l: string) => l.trim().length > 3).slice(0, 30)
          ...
          return {
            ...
            observation: {
              ...
              loginState: url.includes('login') ? 'login_page' as const :
                          url.includes('writer') || url.includes('book') ? 'logged_in' as const :
                          'unknown' as const,
              observedAt: Date.now(),
            },
            durationMs: Date.now() - start,
  ...
  getSession(): BrowserSession | null { return this.session }
  ...
  private cleanup(): void {
    if (this.pwBrowser) {
      try { this.pwBrowser.close().catch(() => {}) } catch {}
      this.pwBrowser = null
    }
    this.pwPage = null
    this.session = null
