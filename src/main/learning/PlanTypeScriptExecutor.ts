...
import type { DevPlan, PlanManagerLike } from '../evolution/types'
...
export class PlanTypeScriptExecutor {
  ...
  createLearningPlan(planManager?: PlanManagerLike): DevPlan | null {
    ...
    try {
      ...
      // 创建初始评估快照
      learningProgressTracker.createSnapshot([
        `学习计划创建: ${unmastered.length} 个知识点待掌握`,
      ])
  ...
  autoAdjustStrategy(): void {
    ...
    // 检查是否需要调整
    const highFreqDifficulties = difficultyPatterns.filter(
      (d) => d.frequency >= DIFFICULTY_TRIGGER_THRESHOLD,
    )
  ...
  syncFocusContext(): LearningFocusContext | null {
    ...
    const currentStepDesc = currentPlan
      ? this.getCurrentStepDescription(currentPlan)
      : undefined

    this.currentFocusContext = buildFocusContext(
      learningVocabularyManager.getAllItems(),
      currentStepDesc,
    )
  ...
  getFocusSummary(): string {
    ...
    return this.currentFocusContext
      ? buildFocusSummary(this.currentFocusContext)
      : '【无活跃学习上下文】'
  }
