 ...
 * 支持中文和英文模糊匹配。
 */
function matchLearningItems(
  query: string,
  items: LearningItem[],
): { items: LearningItem[]; categories: LearningCategory[] } {
...
export class LearningAsrBridge {
  ...
  getLearningSummary(): string {
    ...
    if (progress.categorySummary.length > 0) {
      const activeCats = progress.categorySummary
        .filter(c => c.avgMastery < 0.8)
        .slice(0, 3)
  ...
  isLearningQuery(text: string): boolean {
    ...
    const learningKeywords = [
      'typescript', 'ts', '类型', 'type', '泛型', 'generic',
      '条件', 'conditional', '映射', 'mapped', '工具类型',
      'utility', '类型守卫', 'guard', '推断', 'infer',
      '学习', 'learn', '复习', 'review', '知识点',
      '什么是', '什么是', '解释', 'explain', 'what is',
      '怎么用', 'how to', '用法', '用法',
    ]
