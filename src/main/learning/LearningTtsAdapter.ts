...
import type {
  ...
  LearningTtsNeed,
  LearningTtsContent,
} from './LearningTtsContract'
import {
  CONTENT_TYPE_TTS_NEED_MAP,
  getContentTypeRoutingWeights,
} from './LearningTtsContract'
...
function buildProgressContent(progress: LearningProgress): LearningTtsContent {
  ...
  const weakCategories = progress.categorySummary
  ...
  if (weakCategories.length > 0) {
    parts.push(
      `需要加强的领域：${weakCategories.map((c) => c.category).join('、')}。`,
    )
  }
 ...
 */
function buildStepFeedbackContent(
  stepDescription: string,
  success: boolean,
  conceptName?: string,
): LearningTtsContent {
  const maxLen = 60
  const truncated = stepDescription.length > maxLen
    ? stepDescription.slice(0, maxLen) + '…'
    : stepDescription
...
function buildExplanationContent(matchResult: LearningMatchResult): LearningTtsContent {
  ...
  return {
    ...
    secondaryText: masteryPercent < 100
      ? `你当前掌握度为 ${masteryPercent}%，建议多练习这个知识点。`
      : undefined,
 ...
 * @returns 鼓励性 TTS 内容
 */
function buildEncouragementContent(
  conceptName: string,
  description: string,
  frequency: number,
): LearningTtsContent {
