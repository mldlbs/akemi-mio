...
export class HybridArbitrator {
  ...
  private resolveStepConflict(
    ...
    for (const step of evolution.output) {
      ...
      if (existing) {
        // 融合：优先级取加权平均
        existing.priority =
          existing.priority * this.config.originalPathWeight +
          step.priority * this.config.evolutionPathWeight
  ...
  private resolveStrategyConflict(
    ...
    for (const s of evolution.output) {
      ...
      if (existing) {
        existing.confidence =
          existing.confidence * this.config.originalPathWeight +
          s.confidence * this.config.evolutionPathWeight
