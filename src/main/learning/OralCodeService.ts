...
const PATTERNS: OralCodePatternDef[] = [
  // ── 1. 泛型函数 ──
  {
    ...
    name: '泛型函数',
    keywords: [
      '泛型函数', 'generic function', '泛型方法', 'generic method',
      '接受T', '接受类型参数', '类型参数T',
      '返回', 'Promise', 'type parameter',
      '<T>', '<T,',
    ],
    generate: (params) => {
  ...
  // ── 2. 泛型接口 ──
  {
    ...
    name: '泛型接口',
    keywords: [
      '泛型接口', 'generic interface', '接口', 'interface',
      '泛型类型', 'generic type',
    ],
    generate: (params) => {
      const tParam = params.typeParam || 'T'
  ...
  // ── 3. 泛型约束 ──
  {
    ...
    name: '泛型约束',
    keywords: [
      '泛型约束', 'generic constraint', 'extends', '约束',
      '类型参数约束', 'constrained',
    ],
    generate: (params) => {
      const tParam = params.typeParam || 'T'
  ...
  // ── 4. 条件类型 ──
  {
    ...
    name: '条件类型',
    keywords: [
      '条件类型', 'conditional type', 'extends ?', '三目',
      '条件判断', '类型选择',
    ],
    generate: (params) => {
    ...
    examples: [
      '条件类型，T extends string 返回 true 否则 false',
      '条件类型 IsString',
    ],
  },
  ...
  // ── 6. 映射类型 ──
  {
    ...
    name: '映射类型',
    keywords: [
      '映射类型', 'mapped type', 'keyof', 'in keyof',
      '遍历属性', 'transform', '转换类型',
      '所有属性', '每个属性',
    ],
    generate: (params) => {
  ...
  // ── 7. 映射类型 + 修饰符 ──
  {
    ...
    name: '映射类型修饰符',
    keywords: [
      '映射类型', '修饰符', 'modifier', 'readonly', '可选',
      '只读', '可选属性', '加readonly', '减readonly',
    ],
    generate: (params) => {
      const source = params.sourceType || 'T'
      return {
        ...
        label: '映射类型修饰符',
        explanation: '映射类型支持使用 +/- 符号添加或移除属性的 readonly 和可选标记。-readonly 移除只读，-? 移除可选标记。这让你可以灵活转换类型的属性修饰符。',
      }
    },
    examples: [
      '映射类型，去掉所有 readonly',
  ...
  // ── 8. 模板字面量类型 ──
  {
    ...
    name: '模板字面量类型',
    keywords: [
      '模板字面量', 'template literal', '模板字符串类型',
      '`${}`', '字符串拼接',
    ],
    generate: (params) => {
      ...
      return {
        ...
        explanation: '模板字面量类型在类型层面拼接字符串。你可以像 JavaScript 模板字符串一样，在类型中使用 ${} 嵌入其他类型，生成新的字符串字面量类型。配合 Capitalize 等内置字符串操作类型，可以构建强大的事件处理类型。',
      }
    },
    examples: [
      '模板字面量类型，event_加字符串',
      '模板字面量 on 加 Capitalize',
    ],
  },
  ...
  // ── 11. 类型守卫 ──
  {
    ...
    name: '类型守卫',
    keywords: [
      '类型守卫', 'type guard', 'is', '类型谓词',
      'type predicate', 'typeof', 'instanceof',
    ],
    generate: (params) => {
  ...
  // ── 12. 联合类型 ──
  {
    ...
    generate: (params) => {
      ...
      return {
        ...
        label: `联合类型 ${types}`,
        explanation: '联合类型表示一个值可以是几种类型之一。配合可辨识联合（discriminated union）模式，每个分支有一个字面量类型的 kind 字段，TypeScript 可以根据 kind 自动收窄类型，实现类型安全的条件分支。',
      }
    },
    examples: [
      '联合类型 string | number',
  ...
  // ── 13. 递归类型 ──
  {
    ...
    name: '递归类型',
    keywords: [
      '递归类型', 'recursive type', '递归', 'recursive',
      '树状结构', '嵌套', 'tree', '链表',
    ],
    generate: (params) => {
  ...
  // ── 14. 断言函数 ──
  {
    ...
    name: '断言函数',
    keywords: [
      '断言函数', 'assertion function', 'asserts',
      '断言', '类型断言',
    ],
    generate: (params) => {
      ...
      return {
        ...
        label: '断言函数 asserts',
        explanation: '断言函数使用 asserts 关键字告诉 TypeScript，如果函数没有抛出异常，参数的类型就被收窄。与类型守卫不同，断言函数不返回布尔值，而是在条件不满足时抛出错误，适用于前置条件检查。',
      }
    },
    examples: [
      '断言函数，断言为 string',
  ...
  // ── 15. 分布式条件类型 ──
  {
    ...
    generate: (params) => {
      return {
        ...
        label: '分布式条件类型',
        explanation: '分布式条件类型是指当条件类型的检查类型是泛型参数且传入联合类型时，条件类型会自动分发到联合的每个成员上。ToArray<string | number> 的结果是 string[] | number[] 而不是 (string | number)[]。用方括号包裹类型参数可以阻止分发行为。',
      }
    },
    examples: [
      '分布式条件类型 ToArray',
...
export class OralCodeService {
  ...
  isOralCodeQuery(text: string): boolean {
    ...
    const oralKeywords = [
      '写', '生成', 'create', 'generate', 'make',
      '代码', 'code', '类型', 'type',
      '一个', '示例', 'example', 'sample',
      '泛型', 'generic', '条件', 'conditional',
      '映射', 'mapped', '模板', 'template',
      '函数', 'function', '接口', 'interface',
    ]
