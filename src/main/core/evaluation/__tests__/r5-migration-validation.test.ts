...
describe('R5-C: Revert Protocol', () => {
  it('should revert DROP TABLE IF EXISTS and restore DROP TABLE', async () => {
    ...
    const { revertMigration } = await import('../../db/migration')
    ...
    const { MIGRATIONS } = await import('../../db/migration')
...
describe('R5-B: Backfill NULL Handling', () => {
  ...
  it('should not crash on rows without seq column in queryBySeq result', () => {
    // Verify the deserialize function handles missing seq gracefully
    const { QUERY_NO_LIMIT } = require('../EvaluationStore')
  ...
  return {
    checkSchemaOnly(sql: string): { pass: boolean; detail: string } {
      ...
      for (const line of lines) {
        ...
        const allowed = ALLOWED.some((pat) => upper.startsWith(pat))
