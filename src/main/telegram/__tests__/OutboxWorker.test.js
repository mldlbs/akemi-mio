...
describe('OutboxWorker', () => {
    ...
    const okJson = (data = { ok: true }) => new Response(JSON.stringify(data), { status: 200, headers: { 'Content-Type': 'application/json' } });
    ...
    it('HTTP 非 ok 标记失败', async () => {
        vi.mocked(fetch).mockResolvedValue(new Response(JSON.stringify({ ok: false }), { status: 400, headers: { 'Content-Type': 'application/json' } }));
        insertOutbox({ chatId: '123', msgType: 'reply', message: '失败' });
        const r = await worker.tick();
        expect(r.success).toBe(false);
    });
