...
import type { ParsedRuleIntent, PushFrequency, PushPeriod } from './types'
...
const CHINESE_CITIES = [
  ...
  '西安', '长沙', '重庆', '天津', '苏州', '青岛', '厦门', '大连',
  '宁波', '郑州', '沈阳', '济南', '昆明', '合肥', '福州', '贵阳',
  '海口', '三亚', '珠海', '佛山', '东莞', '中山', '惠州',
]

/** 国际城市/地区 */
const INTNL_LOCATIONS = [
  '硅谷', '旧金山', '纽约', '伦敦', '东京', '新加坡', '柏林',
  '硅谷', '华尔街', '湾区',
]
...
export class RadarNaturalLanguageParser {
  ...
  parse(text: string): ParsedRuleIntent {
    ...
    const description = this.buildDescription({ frequency, hour, minute, dayOfWeek, location, keywords, sources, rawText: trimmed, confidence })
   ...
   * 构建人类可读的描述文本（用于用户确认）
   */
  buildDescription(intent: ParsedRuleIntent): string {
    const parts: string[] = []
