...
import { TaskGraph } from '../core/tasks/TaskGraph'
import { ToolChainDecomposer, toToolSummaries } from './ToolChainDecomposer'
import type {
  OrchestrationPlan,
  OrchestrationStep,
...
import { DEFAULT_ORCHESTRATOR_CONFIG, ORCHESTRATION_EVENTS } from './types'
...
export class ToolChainOrchestrator {
  ...
  constructor(
    decomposer: ToolChainDecomposer,
  ...
  private async executeSteps(
    ...
    for (const layer of layers) {
      ...
      const layerResults = await Promise.all(
        layer.map((step) => this.executeSingleStep(plan, step, completedIds)),
      )
  ...
  private emitStepProgress(
    ...
    const successCount = plan.steps.filter(
      (s) => s.status === 'success' || s.id === step.id,
    ).length
  ...
  private buildSummary(steps: OrchestrationStep[], allSuccess: boolean): string {
    ...
    return [
      '部分步骤执行失败：',
      ...failed,
      ...(skipped.length > 0 ? ['', '跳过的步骤：', ...skipped] : []),
    ].join('\n')
  }
