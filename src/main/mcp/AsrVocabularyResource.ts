...
import { memoryEntityExtractor, type EntityCategory, type ExtractedEntity } from '../asr/MemoryEntityExtractor'
  ...
  async readResource(uri: string): Promise<AsrResourceContent> {
    ...
    if (!parsed) {
      return {
        uri,
        mimeType: 'text/plain',
        text: `无效的资源 URI: ${uri}\n\n可用 ASR 词汇资源：\n${this.getResourceDefinitions().map((d) => `  ${d.uri} — ${d.description}`).join('\n')}`,
      }
    }
