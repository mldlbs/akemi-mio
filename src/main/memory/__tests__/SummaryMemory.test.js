...
describe('SummaryMemory', () => {
    ...
    it('parseJsonField null/undefined', () => {
        expect(sm.parseJsonField(null, [])).toEqual([]);
        expect(sm.parseJsonField(undefined, 'def')).toBe('def');
    });
    it('parseJsonField Array 原样', () => {
        expect(sm.parseJsonField(['a'], [])).toEqual(['a']);
    });
    it('parseJsonField 有效 JSON', () => {
        expect(sm.parseJsonField('["x","y"]', [])).toEqual(['x', 'y']);
    });
    it('parseJsonField 无效字符串 wrap', () => {
        expect(sm.parseJsonField('not-json', 'fb')).toEqual(['not-json']);
    });
    it('DB 无 topics 列时自动 ALTER TABLE', () => {
        const db = getRawDb();
        db.run('ALTER TABLE memory_summaries RENAME TO memory_summaries_old');
        db.run('CREATE TABLE IF NOT EXISTS memory_summaries (id TEXT PRIMARY KEY, summary TEXT NOT NULL, turn_start INTEGER NOT NULL, turn_end INTEGER NOT NULL, created_at INTEGER NOT NULL)');
        const sm2 = new SummaryMemory();
    ...
    it('DB 写入失败不抛异常', () => {
        ...
        db.run("CREATE TABLE IF NOT EXISTS memory_summaries (id TEXT PRIMARY KEY, summary TEXT NOT NULL, turn_start INTEGER NOT NULL, turn_end INTEGER NOT NULL, topics TEXT DEFAULT '[]', decisions TEXT DEFAULT '[]', key_entities TEXT DEFAULT '[]', created_at INTEGER NOT NULL)");
    });
});
