...
describe('DecisionStore 压力测试', () => {
    ...
    it('200 条边界不触发 prune', () => {
        for (let i = 0; i < 200; i++)
            store.record({ agentId: 'a', category: 'tool_select', context: `ctx_${i}`, choice: `ch_${i}` });
        expect(store.query({ limit: 999 })).toHaveLength(200);
    });
    it('大量 updateOutcome 不影响', () => {
        const ids = [];
        for (let i = 0; i < 500; i++)
            ids.push(store.record({ agentId: 'a', category: 'tool_select', context: `ctx_${i}`, choice: `ch_${i}` }));
        for (let i = 0; i < 250; i++)
            store.updateOutcome(ids[i * 2], 'failure');
    ...
    it('多种 category 混合写入后按类别查询准确', () => {
        for (let i = 0; i < 500; i++)
            store.record({
                agentId: 'mix',
                category: ['tool_select', 'strategy', 'plan_route', 'goal_adjust', 'recovery'][i % 5],
                context: `ctx_${i}`,
                choice: `ch_${i}`,
            });
        for (const cat of ['tool_select', 'strategy', 'plan_route', 'goal_adjust', 'recovery']) {
