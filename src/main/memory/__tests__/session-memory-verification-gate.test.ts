...
interface FactChange {
  subject: string; type: 'new' | 'changed' | 'invalidated'; oldValue?: string; newValue: string; confidence: number
}
interface DecisionRecord {
  subject: string; decision: string; alternatives?: string[]; rationale?: string; confidence: number
}
interface EntityRecord {
  name: string; type: string; salience: number
}
type CompactionTriggerReason = 'production_threshold' | 'observation_threshold' | 'idle'
interface SessionCompaction {
  id: string; sessionId: string; source: 'electron' | 'telegram'; digest: string
  topics: string[]; entities: EntityRecord[]; facts: FactChange[]
  decisions: DecisionRecord[]; unresolved: string[]
  importanceScore: number; messageCount: number; tokenCount: number
  sessionStartAt: number; sessionEndAt: number; createdAt: number
  triggerReason: CompactionTriggerReason
}
interface MatchedFactors {
  recency: number; attention?: number; frequency: number; importance: number
}
interface RetrievalResult {
  compaction: SessionCompaction; score: number; matchedFactors: MatchedFactors
}
...
function computeScore(factors: MatchedFactors, hasAttention: boolean): number {
  if (hasAttention && factors.attention !== undefined) {
    return SCORE_RECENCY_WEIGHT * factors.recency +
  ...
  return FALLBACK_RECENCY_WEIGHT * factors.recency +
  ...
  it('INSERT succeeds with all columns', async () => {
    ...
    sqlite.run(
      `INSERT INTO session_compactions
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        'sc_st_001', 'session_st', 'electron', '摘要测试',
        ...
        0.75, 20, 120, 1000, 2000, Date.now(),
      ],
    )
  ...
  it('stores and retrieves JSON array columns correctly', async () => {
    ...
    sqlite.run(
      `INSERT INTO session_compactions
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        'sc_json_1', 'session_json', 'electron', 'json test',
        JSON.stringify(topics), JSON.stringify(entities), JSON.stringify(facts),
        '[]', '[]', 0.6, 15, 80, 1000, 2000, Date.now(),
      ],
    )
  ...
  it('INSERT with trigger_reason stores and retrieves correctly', async () => {
    ...
    sqlite.run(
      `INSERT INTO session_compactions
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ['sc_tr_1', 'session_tr', 'electron', 'obs test', '[]', '[]', '[]', '[]', '[]',
       0.5, 10, 50, 1000, 2000, 3000, 'observation_threshold'],
    )
    ...
    sqlite.run(
      `INSERT INTO session_compactions
       (id, session_id, source, digest, topics, entities, facts, decisions, unresolved,
        importance_score, message_count, token_count, session_start_at, session_end_at, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ['sc_tr_2', 'session_tr', 'electron', 'prod test', '[]', '[]', '[]', '[]', '[]',
       0.5, 50, 200, 4000, 5000, 6000],
    )
...
describe('V5 — Scoring Quality Events', () => {
  ...
  it('token utilization equals sum of selected compaction token counts', () => {
    const results = [
      { compaction: { tokenCount: 200 } },
      { compaction: { tokenCount: 150 } },
      { compaction: { tokenCount: 100 } },
    ]
