...
import { MetaController, DEFAULT_POLICY } from '../MetaController';
describe('MetaController', () => {
    ...
    // ─── 摘要内容 ───
    it('摘要内容包含用户消息和回复', () => {
        mc.onInteractionEnd(makeCtx({ userMessage: '测试消息', assistantReply: '测试回复', planActive: false, tokenUsed: 200, tokenBudget: 200 }));
        expect(mockSummary.addSummary).toHaveBeenCalledWith(expect.stringContaining('测试消息'), expect.any(Number), expect.any(Number), expect.objectContaining({ topics: [] }));
    });
    ...
    it('足够条目时触发 consolidateSummaries', async () => {
        mockSummary.getAll.mockReturnValue(Array.from({ length: 12 }, (_, i) => ({
            topics: [`topic${i}`],
            decisions: [],
            turnStart: i * 1000,
            turnEnd: (i + 1) * 1000,
    ...
    it('未调用 setDeps 时 onInteractionEnd 不抛异常', () => {
        const m = new MetaController();
        expect(() => m.onInteractionEnd({ userMessage: 'hi', assistantReply: 'hello', tokenUsed: 10, tokenBudget: 100, planActive: false, agentId: 'a' })).not.toThrow();
    });
