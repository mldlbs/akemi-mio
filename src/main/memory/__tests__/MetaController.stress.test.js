...
describe('MetaController 压力测试', () => {
    ...
    beforeEach(() => {
        ...
        mc.setDeps({ summary: mockSummary, decisions: mockDecisions, memory: mockMemory });
    });
    afterEach(() => {
        vi.useRealTimers();
    });
    ...
    it('5000 次 onInteractionEnd 统计正确', () => {
        for (let i = 0; i < 5000; i++)
            mc.onInteractionEnd(makeCtx({ tokenUsed: 50 + (i % 10) * 100, planActive: i % 7 === 0 }));
    ...
    it('高频 interaction 不触发 summary (token 比例很低)', () => {
        for (let i = 0; i < 1000; i++)
            mc.onInteractionEnd(makeCtx({ tokenUsed: 10, tokenBudget: 10000, planActive: false }));
        // summaryFrequency=5, 1000/5 = 200 次摘要
        expect(mockSummary.addSummary).toHaveBeenCalledTimes(200);
    });
    it('token 比例持续超过阈值持续触发摘要', () => {
        for (let i = 0; i < 20; i++)
            mc.onInteractionEnd(makeCtx({ tokenUsed: 800, tokenBudget: 1000, planActive: false }));
        // 每次 token 比例 0.8 > 0.7，每次都触发
        expect(mockSummary.addSummary).toHaveBeenCalledTimes(20);
    });
    it('planActive 比例影响决策类别', () => {
        for (let i = 0; i < 100; i++)
            mc.onInteractionEnd(makeCtx({ tokenUsed: 10, tokenBudget: 1000, planActive: i % 2 === 0 }));
        const planRouteCalls = mockDecisions.record.mock.calls.filter((c) => c[0].category === 'plan_route').length;
        const toolSelectCalls = mockDecisions.record.mock.calls.filter((c) => c[0].category === 'tool_select').length;
