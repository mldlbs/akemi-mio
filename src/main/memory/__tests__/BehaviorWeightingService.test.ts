...
describe('BehaviorWeightingService', () => {
  ...
  describe('getTopicRecencyMs', () => {
    ...
    it('从未提及返回 null', () => {
      const interactions = [
        makeInteraction({ userText: '写代码', topics: ['调试'] }),
      ]
  ...
  describe('computeCleanupMultiplier', () => {
    ...
    it('7天内被提及返回 1.0（中性）', () => {
      const now = Date.now()
      const interactions = [
        makeInteraction({ userText: '之前', topics: ['编程'], timestamp: now - 3 * 24 * 3600_000 }),
      ]
    ...
    it('超过24天最低为 0.5', () => {
      const now = Date.now()
      const interactions = [
        makeInteraction({ userText: '远古话题', topics: ['编程'], timestamp: now - 30 * 24 * 3600_000 }),
      ]
      const emptyProfile = svc.computeInterestProfile([])
      const mult = svc.computeCleanupMultiplier(['编程'], interactions, emptyProfile)
