...
export class MemorySleepManager {
    ...
    try {
      ...
      const lowFreqEntries = entries.filter(
        (e) =>
          ...
          !e.isPinned &&
          e.lastAccessedAt > 0 &&
          now - e.lastAccessedAt > sevenDaysMs,
      )
  ...
  /** 使用 LLM 生成分组摘要 */
  private async createLLMSummary(group: {
    contents: string[]
    commonTopics: string[]
  }): Promise<string> {
...
压缩摘要（只输出摘要文本，不要解释）：`
    ...
    try {
      const response = await this.summaryLLM.chatJson(prompt, {
        system:
          '你是记忆压缩专家。将多条相关记忆压缩为一条简洁准确的摘要。只输出纯文本，不要 JSON 格式。',
        temperature: 0.2,
        timeoutMs: 15000,
      })
  ...
  predictNextActivePeriod(): { hour: number; daysActive: number } | null {
    ...
    // 过滤出高频活跃时段
    const activePeriods = stats
      .filter((s) => s.activeDays >= HIGH_FREQUENCY_DAY_THRESHOLD)
  ...
  private populatePreloadCache(): void {
    ...
    const highFreqEntries = entries
      .filter(
        (e) =>
          e.type === 'user_fact' &&
          (e.tier === 'permanent' || e.behaviorScore >= 0.5),
      )
