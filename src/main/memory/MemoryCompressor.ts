...
export class MemoryCompressor {
  ...
  computeImportanceScore(entry: MemoryEntry): ImportanceScoreResult {
    ...
    // 4. 新鲜度维度（0-1）：近期访问的加分
    const daysSinceLastAccess = entry.lastAccessedAt > 0
      ? (now - entry.lastAccessedAt) / (1000 * 60 * 60 * 24)
      : 365 // 从未访问
  ...
  computeAllImportanceScores(
    entries: MemoryEntry[],
  ): Array<{ entry: MemoryEntry; score: ImportanceScoreResult }> {
    return entries
  ...
  private async createGroupSummary(group: CompressGroup): Promise<string | null> {
    ...
    if (this.summaryLLM) {
      try {
        const contentsStr = group.contents
...
压缩摘要（只输出摘要文本，不要解释）：`
        ...
        const summary = typeof response.data === 'string'
          ? response.data
          : response.data?.summary || response.data?.text || this.fallbackSummary(group)
  ...
  private fallbackSummary(group: CompressGroup): string {
    ...
    const topicPrefix = group.commonTopics.length > 0
      ? `【${group.commonTopics.slice(0, 3).join('、')}】`
      : ''
    ...
    try {
      ...
      db.run(
        `INSERT OR REPLACE INTO memories
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          entry.id, entry.type, entry.content, entry.confidence, entry.tier,
          entry.reinforceCount, entry.behaviorScore,
          entry.lastAccessedAt, entry.accessCount, entry.isPinned ? 1 : 0, entry.manualScoreOverride,
          entry.createdAt, entry.updatedAt, entry.structuredData ?? null,
          JSON.stringify(entry.topics || []),
          entry.utilityScore, entry.agentReferenceCount, entry.userConfirmedUsefulCount,
