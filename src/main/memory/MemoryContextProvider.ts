...
export class MemoryContextProvider {
  ...
  buildInjectionContext(spec: RetrievalSpec, traceId?: string): InjectionContext {
    try {
      ...
      const facts = results.flatMap(r =>
        r.compaction.facts.map(f => `[${f.type}] ${f.subject}: ${f.newValue}`),
      )
      const decisions = results.flatMap(r =>
        r.compaction.decisions.map(d => `${d.subject}: ${d.decision}`),
      )
      ...
      if (actualTokens > maxTokens && results.length > 0) {
        ...
        for (let i = results.length - 1; i >= 0; i--) {
          ...
          const keptFacts = keptResults.flatMap(r =>
            r.compaction.facts.map(f => `[${f.type}] ${f.subject}: ${f.newValue}`),
          )
          const keptDecisions = keptResults.flatMap(r =>
            r.compaction.decisions.map(d => `${d.subject}: ${d.decision}`),
          )
