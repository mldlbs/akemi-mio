...
export class AdaptiveOrchestrator {
  ...
  getFormattedContext(): string {
    ...
    const topPatterns = this.miner.getAllPatterns()
    ...
    const parts: string[] = [
      '---',
      '【记忆驱动的自适应编排】基于用户历史行为模式，以下为可参考的操作流程：',
    ]
  ...
  private getSourceLabel(source: string): string {
    switch (source) {
      case 'procedure': return '显式保存的流程'
      case 'task_template': return '自动挖掘的模板'
      case 'mined_topic_sequence': return '话题转移模式'
