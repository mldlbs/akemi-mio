...
export class KeywordFrequencyTracker {
    ...
    try {
      ...
      for (const keyword of keywords) {
        const existing = db.exec(
          `SELECT freq, created_at FROM keyword_freq WHERE keyword = ?`,
          [keyword],
        )
        ...
        if (existing && existing.length > 0 && existing[0].values.length > 0) {
          ...
          db.run(
            `UPDATE keyword_freq SET freq = ?, last_seen = ?, created_at = ? WHERE keyword = ?`,
            [currentFreq + 1, now, createdAt, keyword],
          )
        } else {
          // 新增关键字
          db.run(
            `INSERT OR REPLACE INTO keyword_freq (keyword, freq, last_seen, created_at) VALUES (?, 1, ?, ?)`,
            [keyword, now, now],
          )
        }
      }
      ...
      try { getRawDb().run('ROLLBACK') } catch {}
      ...
      for (const { keywords, timestamp } of entries) {
        ...
        for (const keyword of keywords) {
          ...
          if (existing && existing.length > 0 && existing[0].values.length > 0) {
            ...
            db.run(
              `UPDATE keyword_freq SET freq = ?, last_seen = ?, created_at = ? WHERE keyword = ?`,
              [currentFreq + 1, timestamp, createdAt, normalizedKeyword],
            )
          } else {
            db.run(
              `INSERT OR REPLACE INTO keyword_freq (keyword, freq, last_seen, created_at) VALUES (?, 1, ?, ?)`,
              [normalizedKeyword, timestamp, timestamp],
            )
          }
        }
      }
  ...
  getTopKeywords(k?: number): KeywordFreqRecord[] {
    ...
    try {
      ...
      const result = db.exec(
        `SELECT keyword, freq, last_seen, created_at FROM keyword_freq ORDER BY freq DESC LIMIT ?`,
        [limit],
      )
  ...
  getAllKeywords(): KeywordFreqRecord[] {
    try {
      const db = getRawDb()
      const result = db.exec(
        `SELECT keyword, freq, last_seen, created_at FROM keyword_freq ORDER BY freq DESC`,
      )
      if (!result || result.length === 0) return []
      const columns = result[0].columns
      return result[0].values.map((v: any[]) => {
  ...
  decayFrequencies(): number {
    ...
    try {
      ...
      // 1. 衰减所有频率
      db.run(
        `UPDATE keyword_freq SET freq = MAX(0, CAST(ROUND(freq * ?) AS INTEGER))`,
        [decayFactor],
      )
      decayed = Number(
        db.exec('SELECT changes() AS c')[0]?.values[0]?.[0] || 0,
      )

      // 2. 移除低于阈值的
      const toRemove = db.exec(
        `SELECT keyword FROM keyword_freq WHERE freq < ?`,
        [minFreq],
      )
  ...
  private pruneExcess(): void {
    try {
      const db = getRawDb()
      const count = Number(
        db.exec('SELECT COUNT(*) AS c FROM keyword_freq')[0]?.values[0]?.[0] || 0,
      )
  ...
  startDecayTimer(): void {
    ...
    }, Math.min(this.config.decayIntervalMs, 3600_000)) // 最多每小时检查一次
  ...
  private loadDecayTime(): void {
    try {
      const db = getRawDb()
      const result = db.exec(
        `SELECT last_seen FROM keyword_freq ORDER BY last_seen DESC LIMIT 1`,
      )
  ...
  private saveDecayTime(now: number): void {
    ...
    try {
      ...
      const existing = db.exec(
        `SELECT 1 FROM keyword_freq WHERE keyword = ?`,
        [key],
      )
      if (existing && existing.length > 0 && existing[0].values.length > 0) {
        db.run(
          `UPDATE keyword_freq SET freq = ?, last_seen = ? WHERE keyword = ?`,
          [0, now, key],
        )
      } else {
        db.run(
          `INSERT INTO keyword_freq (keyword, freq, last_seen, created_at) VALUES (?, 0, ?, ?)`,
          [key, now, now],
        )
      }
    } catch {
