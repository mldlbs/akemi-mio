...
export class EngineeringMemory {
  ...
  query(params: { types?: string[]; tags?: string[]; topK?: number } = {}): EngineeringEntry[] {
    const db = getRawDb()
    ...
    stmt.bind(values)
    const results: EngineeringEntry[] = []
    while (stmt.step()) {
  ...
  search(text: string, topK: number = 5): EngineeringEntry[] {
    const db = getRawDb()
    const like = `%${text}%`
    const stmt = db.prepare(
      `SELECT * FROM engineering_memory WHERE content LIKE ? OR source LIKE ? OR tags LIKE ? ORDER BY confidence DESC LIMIT ${topK}`,
    )
  ...
  getFormattedContext(topK: number = 5): string {
    const entries = this.query({ topK })
    if (entries.length === 0) return ''
    const parts = ['---', '【工程知识】']
    for (const e of entries) {
      parts.push(`[${e.type}] ${e.content.slice(0, 150)}`)
      if (e.relatedFiles.length > 0) parts.push(`  文件: ${e.relatedFiles.join(', ')}`)
    }
    parts.push('---')
    return parts.join('\n')
  }
  ...
  getRelatedFiles(filePath: string): string[] {
    ...
    for (const e of entries) {
      for (const f of e.relatedFiles) files.add(f)
    }
  ...
  private prune(): void {
    try {
      ...
      if (count > MAX_ENTRIES) {
        db.run(
          `DELETE FROM engineering_memory WHERE id IN (SELECT id FROM engineering_memory ORDER BY confidence ASC, created_at ASC LIMIT ?)`,
          [count - MAX_ENTRIES],
        )
      }
    } catch {
