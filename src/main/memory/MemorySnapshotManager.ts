...
export class MemorySnapshotManager {
  ...
  generateSnapshot(): MemorySnapshot | null {
    ...
    try {
      ...
      const openingSuggestion = this.isNewSession && consecutiveDayTopics.length > 0
  ...
  private compressSummaries(summaries: Array<{ summary: string; topics: string[] }>): string {
    ...
    // 4-5 条：提取关键信息
    const recent = summaries.slice(-3).map((s) => s.summary).filter(Boolean)
  ...
  detectConsecutiveDayTopics(
    ...
    try {
      ...
      for (const [topic, daySet] of topicDayMap) {
        ...
        const hasRecent = daySet.has(this.formatDate(todayMs)) ||
          daySet.has(this.formatDate(todayMs - 24 * 60 * 60 * 1000))
  ...
  suggestTools(
    topTopics: string[],
    consecutiveTopics: ConsecutiveDayTopic[],
    mappings?: TopicToolMapping[],
  ): string[] {
  ...
  generateOpeningSuggestion(
    ...
    if (consecutiveTopics.length > 0) {
      const topicNames = consecutiveTopics.slice(0, 3).map((ct) => ct.topic).join('、')
  ...
  getFormattedSnapshotContext(): string {
    ...
    if (snapshot.consecutiveDayTopics.length > 0) {
      const topicNames = snapshot.consecutiveDayTopics
        .map((ct) => `${ct.topic}（连续${ct.consecutiveDayCount}天）`)
        .join('、')
