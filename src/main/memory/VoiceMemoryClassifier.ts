...
export type VoiceMemoryIntent =
  | { type: 'write'; entity: string; confidence: number }
  | { type: 'read'; query: string; confidence: number }
  | { type: 'none' }
...
const WRITE_PATTERNS: WritePattern[] = [
  // "记住{entity}" → 存储 entity
  { regex: /记住[：:，,\s]*([\s\S].+)/, extract: (m) => m[1].trim(), minConfidence: 0.90 },
  // "记得{entity}" → 存储 entity
  ...
  { regex: /别忘了[：:，,\s]*([\s\S].+)/, extract: (m) => m[1].trim(), minConfidence: 0.90 },
  // "我叫{name}" → 存储 "用户的名字是{name}"
  { regex: /我(?:叫|是)(.{1,30})$/, extract: (m) => `用户的名字是${m[1].trim()}`, minConfidence: 0.80 },
  // "我喜欢{thing}" → 存储 "用户喜欢{thing}"
  { regex: /我喜欢([\s\S]{1,100})/, extract: (m) => `用户喜欢${m[1].trim()}`, minConfidence: 0.80 },
...
const READ_PATTERNS: ReadPattern[] = [
  ...
  // "上次{thing}"（无后缀问词，短文本）
  { regex: /^上次(.{1,30})$/, extractQuery: (m) => m[1].trim(), minConfidence: 0.80 },
  // "{thing}是什么" / "{thing}是啥"（需要去掉句末标点）
  { regex: /^(.{2,20})(?:是啥|是什么|是什么来着|是什么意思)$/, extractQuery: (m) => m[1].trim(), minConfidence: 0.70 },
  // "我之前说的{thing}" / "我刚才说的{thing}"
  { regex: /我(?:之前|刚才|以前)(?:说|讲|提)(?:过|的)?(.{1,30})/, extractQuery: (m) => m[1].trim(), minConfidence: 0.80 },
  // "关于{thing}的事|记忆|信息"
  { regex: /关于(.{1,30})(?:的|之事|的事|的记忆|的信息)/, extractQuery: (m) => m[1].trim(), minConfidence: 0.75 },
  // "我的{thing}是什么"（读己方属性）
  { regex: /我的(.{1,10})(?:是啥|是什么|是哪个|在哪里)/, extractQuery: (m) => m[1].trim(), minConfidence: 0.80 },
  // "还记得{query}吗"
 ...
 */
export function formatMemoryFeedback(
  intent: VoiceMemoryIntent,
  memoryResult?: string[],
): string | null {
  switch (intent.type) {
    case 'write':
