...
import { BLOG_STAGE_ORDER, BLOG_STAGE_LABELS } from '../agent/blog/types'
...
export class BlogKanbanBridge {
  ...
  private buildPayload(): BlogKanbanPayload {
    ...
    const sessionsData: BlogKanbanSessionData[] = sessions.map((s) => {
      ...
      const completedCount = BLOG_STAGE_ORDER.filter(
        (st) => s.stageStatuses[st] === 'completed',
      ).length
      ...
      return {
        ...
        stageProgress: completedCount > 0
          ? Math.round((currentIdx / (BLOG_STAGE_ORDER.length - 1)) * 100)
          : 0,
