...
describe('ModelHealthTracker', () => {
    ...
    it('100% 完成 → score=100', () => {
        ...
        t.start();
        for (let i = 0; i < 10; i++) {
            eventBus.emit('agent.tool.completed', { tool: 'test', result: 'ok' });
        }
        const h = t.getHealth();
    ...
    it('errorBreakdown 记录分类事件', () => {
        const t = new ModelHealthTracker();
        t.start();
        eventBus.emit('recovery.error.classified', { category: 'RETRYABLE' });
        eventBus.emit('recovery.error.classified', { category: 'RETRYABLE' });
        eventBus.emit('recovery.error.classified', { category: 'FATAL' });
        const h = t.getHealth();
    ...
    it('多样性惩罚降低 score', () => {
        ...
        eventBus.emit('recovery.error.classified', { category: 'A' });
        eventBus.emit('recovery.error.classified', { category: 'B' });
        eventBus.emit('recovery.error.classified', { category: 'C' });
