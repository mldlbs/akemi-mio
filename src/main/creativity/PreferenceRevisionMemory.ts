...
export class PreferenceRevisionMemory {
  ...
  recordRevisedChapter(storyName: string, chapterId: string, summary: string): void {
    // 去重：避免重复标记同一章节
    const existing = this.store.revisedChapters.find(
      (r) => r.storyName === storyName && r.chapterId === chapterId,
    )
    if (existing) {
    ...
    if (ms) {
      ms.addEntry(
        'revision_preference',
        `${REVISED_MEMORY_PREFIX}${storyName}:${chapterId}: ${summary.slice(0, 200)}`,
        0.9,
        { tier: 'semi' },
      )
    }
  ...
  isChapterRevised(storyName: string, chapterId: string): boolean {
    // 检查内存缓存
    const inJson = this.store.revisedChapters.some(
      (r) => r.storyName === storyName && r.chapterId === chapterId,
    )
    ...
    if (ms) {
      ...
      return entries.some(
        (e) =>
          e.type === 'revision_preference' &&
          e.content.startsWith(`${REVISED_MEMORY_PREFIX}${storyName}:${chapterId}:`),
      )
    }
  ...
  getStoryPreferences(storyName: string): RevisionPreferenceEntry[] {
    return this.store.preferences
      .filter((p) => p.storyName === storyName)
  ...
  getPreferenceHints(storyName?: string, minConfidence: number = 0): PreferenceHint[] {
    const preferences = storyName
      ? this.getStoryPreferences(storyName)
  ...
  getPreferenceContext(storyName: string): string {
    ...
    if (hints.length > 0) {
      for (const hint of hints) {
        ...
        lines.push(
          `- ${catLabel}：${hint.hint}（置信度 ${Math.round(hint.confidence * 100)}%，${hint.sampleCount} 条样本）`,
        )
      }
    }
    ...
    if (totalChapters > revised.length) {
      ...
      lines.push(
        `未修订章节：${remaining} 章 ← 请参考以上偏好调整后续修订方向`,
      )
    ...
    if (recentFeedback.length > 0) {
      ...
      for (const f of recentFeedback) {
        ...
        const truncatedSuggestion =
          f.suggestion.length > 50
            ? f.suggestion.slice(0, 50) + '…'
            : f.suggestion
  ...
  getRevisedChapters(storyName: string): RevisedChapterRecord[] {
    return this.store.revisedChapters
      .filter((r) => r.storyName === storyName)
      .sort((a, b) => a.revisedAt - b.revisedAt)
  }
  ...
  clearStoryData(storyName: string): number {
    ...
    this.store.preferences = this.store.preferences.filter(
      (p) => p.storyName !== storyName,
    )
    this.store.revisedChapters = this.store.revisedChapters.filter(
      (r) => r.storyName !== storyName,
    )
  ...
  private extractKeyPattern(entry: RevisionPreferenceEntry): string {
    ...
    if (entry.userComment) {
      const cleaned = entry.userComment
        .replace(/[喜欢讨厌不错可以好不行不要别不太非常很太更的了吧吗呢啊哦嗯]/g, '')
        .trim()
