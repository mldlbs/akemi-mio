...
export class PolishingMemoryManager {
   ...
   * - 当前持久规则库的关键规则
   */
  buildPolishingContext(
    storyId: string,
    chapterTitle?: string,
  ): PolishingContextHint {
    ...
    for (const d of recent) {
      const modSummary = d.modifications
        .map(
          (m) =>
            MODIFICATION_TYPE_LABELS[m.type] + ':' + m.description.slice(0, 30),
        )
        .join('; ')
      lines.push(
        `  [${d.chapterTitle} 段落#${d.paragraphIndex}] ${modSummary}`,
      )
    }
  ...
  savePersistentRule(rule: Omit<PersistentStyleRule, 'id' | 'createdAt'>): void {
    ...
    ms.addFact(
      `${MEMORY_KEY_PREFIX}:rule ${JSON.stringify(fullRule)}`,
      rule.confidence,
      { tier: 'permanent' },
    )
  }
   ...
   * 新规则与已有规则去重后写入持久层。
   */
  async extractRules(storyId?: string): Promise<PersistentStyleRule[]> {
    const ms = getMemoryService()
    if (!ms) return []

    // 收集待分析的决策
    const decisions = storyId
      ? this.queryHistory(storyId, { limit: 100 })
      : this.getAllDecisions(100)
  ...
  async compressHistory(storyId: string): Promise<number> {
    ...
    for (const [chapter, chapterDecisions] of chapterGroups) {
      ...
      const summary: ChapterPolishingSummary = {
        ...
        modifiedParagraphs: chapterDecisions.filter(
          (d) => d.modifications.length > 0,
        ).length,
  ...
  private buildSearchableContent(decision: PolishingDecision): string {
    ...
    const modReasons = decision.modifications
      .map((m) => m.reason.slice(0, 40))
      .join(';')
    return (
   ...
   * LLM 规则提取 — 从决策中提炼风格规则。
   */
  private async llmExtractRules(
    decisions: PolishingDecision[],
  ): Promise<Omit<PersistentStyleRule, 'id' | 'createdAt'>[]> {
    const key = LLM_TEXT_KEY || process.env.LLM_KEY || ''
    if (!key) return []
...
- 如果无法提炼出有意义的规则，返回空数组 []`
   ...
   */
  private ruleBasedExtractRules(
    decisions: PolishingDecision[],
  ): Omit<PersistentStyleRule, 'id' | 'createdAt'>[] {
    // 统计所有规则的出现频次
