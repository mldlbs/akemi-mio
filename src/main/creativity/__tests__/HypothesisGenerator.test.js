...
describe('HypothesisGenerator', () => {
    ...
    beforeEach(() => {
        chatJson = vi.fn();
        generator = new HypothesisGenerator(chatJson, 42);
    });
    ...
    describe('generate - 模板模式（fallback）', () => {
        ...
        it('相同输入产生稳定的模板选择（确定性）', async () => {
            chatJson.mockResolvedValue({ data: [] });
            const gen1 = new HypothesisGenerator(chatJson, 42);
            const gen2 = new HypothesisGenerator(chatJson, 42);
    ...
    describe('模式匹配 — 不同模板选择', () => {
        it('不同来源对映射到不同的模板标题格式', async () => {
            ...
            const formats = new Set([...titles].map((t) => {
                if (t.includes('驱动的'))
                    return 'driven';
                if (t.includes('增强'))
                    return 'enhanced';
    ...
    describe('通用模板降级', () => {
        it('chatJson 为 undefined 时正常降级到 TemplateLibrary', async () => {
            const gen = new HypothesisGenerator(undefined, 42);
    ...
    describe('模板评分', () => {
        ...
        it('相同 seed 产生相同评分', async () => {
            chatJson.mockResolvedValue({ data: [] });
            const genA = new HypothesisGenerator(chatJson, 123);
            const genB = new HypothesisGenerator(chatJson, 123);
        ...
        it('不同 seed 可能产生不同评分', async () => {
            ...
            for (let seed = 0; seed < 10; seed++) {
                const gen = new HypothesisGenerator(chatJson, seed);
                const combo = makeCombo({ sources: ['A', 'B'] });
