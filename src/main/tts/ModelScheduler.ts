...
export class ModelScheduler {
  ...
  recordFeedback(requestId: string, positive: boolean): void {
    ...
    if (!entry) {
      ...
      const slotKey = getCurrentSlotKey()
      const recentEntry = [...this.usageLogs]
        .reverse()
        .find((e) => e.slotKey === slotKey && e.feedbackScore === 0)
      if (recentEntry) {
  ...
  tryAutoSwitch(): { switched: boolean; from?: string; to?: string; reason: string } {
    ...
    if (prediction.confidence < this.config.autoSwitchConfidenceThreshold) {
      return { switched: false, reason: `置信度不足: ${(prediction.confidence * 100).toFixed(0)}% < ${(this.config.autoSwitchConfidenceThreshold * 100).toFixed(0)}%` }
    }
  ...
  getState(currentModel: string): ModelSchedulerState {
    ...
    return {
      ...
      learnedSlots: Object.keys(this.slotStats).length,
      totalPredictions: this.totalPredictions,
      acceptanceRate: this.totalPredictions > 0
        ? Math.round((this.acceptedPredictions / this.totalPredictions) * 100) / 100
        : 0,
      msSinceLastSwitch,
