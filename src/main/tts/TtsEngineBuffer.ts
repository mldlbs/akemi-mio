...
export type PreSynthesizeCallback = (
  text: string,
  engine: TtsEngine,
) => Promise<string | null> // 返回音频文件路径或 null
...
export class TtsEngineBuffer {
  ...
  consume(text: string, targetEngine: TtsEngine): string | null {
    ...
    const idx = this.entries.findIndex(
      (e) => e.engine === targetEngine && e.text === text && e.ready && !e.consumed,
    )
  ...
  private isAlreadyBuffered(text: string, engine: TtsEngine): boolean {
    return this.entries.some(
      (e) => e.engine === engine && e.text === text && !e.consumed,
    )
  }
   ...
   * 后台预生成备选引擎的音频。
   */
  private async preSynthesizeInBackground(
    text: string,
    engine: TtsEngine,
    dedupKey: string,
  ): Promise<void> {
