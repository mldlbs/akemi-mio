...
import { evolutionPiperBridge } from '../../evolution/piper/EvolutionPiperBridge'
...
export class PiperReasoningChainExecutor {
    ...
    try {
      ...
      if (planId && pm) {
        if (allSucceeded) {
          ...
          pm.freezePlan(planId, `Piper 推理链中断于步骤 ${stepResults.filter((r) => !r.success).length + 1}/${chain.steps.length}: ${conclusion}`)
        }
      }
  ...
  private buildReasoningChain(context: PiperReasoningContext): PiperReasoningChain {
    ...
    if (bestOption && bestOption.compatible) {
      steps.push(this.makeStep(
        stepIndex++, timestamp, true,
        '应用故障恢复操作',
        `执行 "${bestOption.name}": ${bestOption.description}`,
      ))
    }
    ...
    steps.push(this.makeStep(
      stepIndex++, timestamp, false,
      '验证恢复效果',
      '重新检查 Piper 状态，确认故障已解除或至少改善',
    ))

    // 根据严重度调整步骤深度
    const trimmed = context.severity === 'info'
  ...
  private async executeStep(
    step: PiperReasoningStep,
    context: PiperReasoningContext,
    _startedAt: number,
  ): Promise<PiperStepResult> {
    switch (step.index) {
      case 0:
