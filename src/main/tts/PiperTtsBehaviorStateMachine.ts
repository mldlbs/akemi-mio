...
export class PiperTtsBehaviorStateMachine {
  ...
  toBehaviorInput(): BehaviorSidecarInput {
    return {
      outputMode:
        this.currentConfig.outputMode === 'normal' ? null : this.currentConfig.outputMode,
  ...
  private decideState(input: AutoStateInput): { newState: PiperAutoState; reason: string } {
    ...
    const isBrightnessLow =
      environment.valid && environment.brightness >= 0
        ? environment.brightness < NIGHT_BRIGHTNESS_THRESHOLD
        : sceneName === 'late_night' // 降级：使用场景检测
