...
const FOCUS_COLORS: Record<FocusScoreState['level'], { label: string; color: string; gradientFrom: string; gradientTo: string }> = {
...
export function useFocusScore(behavior: BehaviorState | null, customConfig?: Partial<FocusScoreConfig>): FocusScoreState {
  ...
  return {
    ...
    restReminderText: scores.restReminder ? `已持续低专注超过 ${Math.round(config.restReminderAfterMs / 60_000)} 分钟，建议起身活动` : '',
    level: scores.level,
    config,
  }
}
