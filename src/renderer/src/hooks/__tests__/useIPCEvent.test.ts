...
describe('useIPCEvent', () => {
  it('calls the register function on mount', () => {
    const register = vi.fn<[(data: string) => void], () => void>()
  ...
  it('calls cleanup on unmount', () => {
    const cleanup = vi.fn()
    const register = vi.fn<[(data: string) => void], () => void>().mockReturnValue(cleanup)
  ...
  it('handles undefined cleanup gracefully', () => {
    const register = vi.fn<[(data: string) => void], () => void>().mockReturnValue(undefined as any)
    const { unmount } = renderHook(() => useIPCEvent(register, vi.fn()))
  ...
  it('re-subscribes when deps change', () => {
    const register = vi.fn<[(data: string) => void], () => void>().mockReturnValue(vi.fn())
