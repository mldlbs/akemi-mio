...
describe('useAIOutput', () => {
  ...
  it('handleResult calls chat and sets agentState to thinking', async () => {
    const { result } = renderHook(() => useAIOutput('sess-1', false, vi.fn()))
    await act(async () => {
      await result.current.handleResult('hello')
    })
    expect(result.current.transcribed).toBe('hello')
  ...
  it('handleResult passes noTts=true when voiceActive is false', async () => {
    const { result } = renderHook(() => useAIOutput('sess-1', false, vi.fn()))
    await act(async () => {
      await result.current.handleResult('hi')
    })
    expect(window.electronAPI.chat).toHaveBeenCalledWith('hi', undefined, 'sess-1', true)
  })

  it('handleResult passes noTts=false when voiceActive is true', async () => {
  ...
  it('transitions to replying on first chunk when thinking', () => {
    ...
    act(() => {
      result.current.handleResult('hi').catch(() => {})
  ...
  it('clears onError on successful handleResult start', async () => {
    const onError = vi.fn()
    const { result } = renderHook(() => useAIOutput('sess-1', false, onError))
    await act(async () => {
      await result.current.handleResult('hi')
  ...
  it('clears state when session switches', () => {
    ...
    act(() => {
      result.current.handleResult('test').catch(() => {})
    })
    // Switch session
  ...
  it('handles empty text in handleResult', async () => {
    const { result } = renderHook(() => useAIOutput('sess-1', false, vi.fn()))
    await act(async () => {
      await result.current.handleResult('')
    })
