...
export function useBehaviorAwareWallpaper(customConfig?: Partial<WallpaperConfig>): WallpaperState {
  ...
  // ── 计算透明度 ──
  const calcOpacity = useCallback((b: BehaviorState | null, cfg: WallpaperConfig): number => {
  ...
  // ── Break 模式透明度（隐私淡入） ──
  const calcBreakOpacity = useCallback((b: BehaviorState, cfg: WallpaperConfig): number => {
    // 基础 idle/away 透明度
  ...
  // ── 计算隐私淡入程度（用于 CSS 效果） ──
  const calcPrivacyFade = useCallback((b: BehaviorState | null, cfg: WallpaperConfig): number => {
  ...
  // ── 计算是否隐藏所有装饰 ──
  const calcHideDecoration = useCallback((b: BehaviorState | null, cfg: WallpaperConfig): boolean => {
    if (!b || !cfg.hideDecorationOnFocus) return false
    return b.mode === 'focus'
  }, [])

  // ── 计算是否显示任务切换器 ──
  const calcShowTaskSwitcher = useCallback((b: BehaviorState | null, cfg: WallpaperConfig): boolean => {
    if (!b || !cfg.modeSwitchingEnabled) return false
    return b.mode === 'multitasking' && b.recentSwitches.length > 0
  }, [])

  // ── 计算是否显示快捷键指南（coding 情境） ──
  const calcShowShortcutsGuide = useCallback((b: BehaviorState | null, cfg: WallpaperConfig): boolean => {
  ...
  const calcShowRssSummary = useCallback((b: BehaviorState | null, cfg: WallpaperConfig): boolean => {
    if (!b || !cfg.enabled) return false
    return b.context === 'browsing' && !cfg.hideDecorationOnFocus
  }, [])

  // ── 计算是否显示自然动画背景（resting 情境） ──
  const calcShowNatureAnimation = useCallback((b: BehaviorState | null, cfg: WallpaperConfig): boolean => {
    if (!b || !cfg.enabled) return false
    // resting 情境且在 break 模式或空闲时才显示
    return b.context === 'resting' && (b.mode === 'break' || b.activityState === 'idle' || b.activityState === 'away')
  }, [])

  // ── 计算是否需要显示 idle 覆层 ──
  const calcIdleOverlay = useCallback((b: BehaviorState | null, cfg: WallpaperConfig): boolean => {
  ...
  useEffect(() => {
    ...
    document.documentElement.style.setProperty('--wallpaper-overlay-opacity', String(targetOpacity))
    document.documentElement.style.setProperty('--wallpaper-transition-duration', `${transitionSec}s`)

    // 隐私淡入程度
    const fade = calcPrivacyFade(behavior, config)
    document.documentElement.style.setProperty('--wallpaper-privacy-fade', String(fade))

    // 当前模式（供 CSS 使用）
    if (behavior) {
      document.documentElement.style.setProperty('--wallpaper-mode', `"${behavior.mode}"`)
      document.documentElement.style.setProperty('--wallpaper-context', `"${behavior.context}"`)
    }

    // idle 文字颜色
    if (behavior && (behavior.activityState === 'idle' || behavior.activityState === 'away')) {
      document.documentElement.style.setProperty('--wallpaper-idle-text', 'oklch(0.98 0.002 12)')
    } else {
  ...
  // ── 模式中文标签 ──
  const modeLabel = behavior ? MODE_LABELS[behavior.mode] || MODE_LABELS.focus : MODE_LABELS.focus

  // ── 情境中文标签 ──
  const contextLabel = behavior ? CONTEXT_LABELS[behavior.context] || CONTEXT_LABELS.resting : CONTEXT_LABELS.resting
