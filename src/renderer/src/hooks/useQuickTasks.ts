...
export function useQuickTasks() {
  ...
  // ── 订阅主进程推送的推荐 ──
  useIPCEvent(
    window.electronAPI?.onQuickTaskRecommend,
    (data: { tasks: any[]; timestamp: number }) => {
  ...
  // ── 订阅主进程推送的任务更新 ──
  useIPCEvent(
    window.electronAPI?.onQuickTaskUpdate,
    (data: { task: any; action: string; timestamp: number }) => {
