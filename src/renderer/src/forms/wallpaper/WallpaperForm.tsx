 ...
 * 2. 画面是**静止**的：星点分布 + 两团模糊色晕，构成了这张壁纸。
 *    曾经有三条无限动画（极光漂移 ×2、星点闪烁 ×140），已全部移除 ——
 *    全屏透明窗口只要有任何动画在跑，合成器就每帧重光栅化整个表面，
 *    实测 GPU 常驻吃满一个核（136%），停掉后归零。壁纸是要长期盯着的
 *    静态画面，"不动的星空"比"烧掉一个核的流动"更符合它的定位。
 ...
 *
 *  注意：这里**故意**保留每颗星的 `d`（曾用于 animationDelay 的相位）。
 *  虽然闪烁动画已移除、`d` 不再被消费，但删掉它会改变伪随机数的取用次数，
 *  从而让整个星图重新排布 —— 那不是修复，是换了一张壁纸。
 *  保留取数次数 = 星点位置与设计师当时审阅的完全一致。 */
