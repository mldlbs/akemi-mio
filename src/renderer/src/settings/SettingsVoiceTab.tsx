...
export function SettingsVoiceTab({ values, onSetCredential }: SettingsTabProps) {
  ...
  const handleToggleVocab = useCallback(async (enabled: boolean) => {
    try {
    ...
    <>
      <div className="settings-section">
        ...
        <div className="settings-field">
          ...
          {ttsMode === 'auto' && <span className="settings-help">根据网络状况和内容需求自动选择云端/本地引擎</span>}
          {ttsMode === 'cloud' && <span className="settings-help">始终使用云端 TTS（高表现力，需联网）</span>}
          {ttsMode === 'local' && <span className="settings-help">始终使用本地 Piper TTS（低延迟，可离线）</span>}
        </div>
      </div>
      ...
      <div className="settings-section">
        ...
        <span className="settings-help">
          系统会从你的对话中自动学习高频词汇，逐步提升语音识别准确率。 已学习词汇跨会话持久保存，你可以随时查看和删除。
        </span>

        <div className="settings-field">
          <span className="settings-label">词汇学习</span>
          <div className="settings-toggle">
            <button type="button" className={`settings-toggle-btn${vocabEnabled ? ' active' : ''}`} onClick={() => handleToggleVocab(true)}>
              启用
            </button>
            <button
              type="button"
        ...
        {vocabMessage && <div className="settings-message">{vocabMessage}</div>}

        {vocabLoading && <div className="settings-loading">加载中...</div>}

        {vocab && (
          <>
            {/* 领域分布 */}
            {vocab.domainStats.length > 0 && (
              <div className="settings-domain-stats">
                <span className="settings-label">领域分布（共 {vocab.totalWords} 词）</span>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {vocab.domainStats.map((ds) => (
                    <span key={ds.label} className="settings-domain-tag">
                      {ds.label}: {ds.count}
                    </span>
                  ))}
                </div>
            ...
            <div className="settings-actions-row">
              <button type="button" className="settings-action-btn" onClick={handleRefreshContext}>
                刷新上下文
              </button>
              {vocab.totalWords > 0 && (
                <button
                  type="button"
                  className={`settings-action-btn settings-action-danger${confirmClear ? ' confirmed' : ''}`}
            ...
            {vocab.words.length > 0 ? (
              <div className="settings-vocab-list">
                {vocab.words.map((entry) => (
                  <div key={entry.word} className="settings-vocab-item">
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <span className="settings-vocab-word">{entry.word}</span>
                      <span className="settings-vocab-domain">{entry.domain}</span>
                      <span className="settings-vocab-meta">{entry.count}次</span>
                      <span className="settings-vocab-date">{formatDate(entry.lastSeen)}</span>
                    </div>
                    <button
                      type="button"
              ...
              <div className="settings-vocab-empty">暂无已学习词汇。开始使用语音对话后，系统会自动学习高频词汇。</div>
            )}
          </>
        )}
      </div>
