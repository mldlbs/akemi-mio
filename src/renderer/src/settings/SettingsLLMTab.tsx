...
export function SettingsLLMTab({ values, onSetCredential }: SettingsTabProps) {
  const isConfigured = (key: string) => !!values[key]?.trim()
  return (
    <>
      {LLM_SECTIONS.map((section) => {
        const urlKey = section.fields[0]?.key ?? ''
        const keyField = [...section.fields].reverse().find((f) => f.type === 'password')
        const ready = !!urlKey && !!keyField && isConfigured(urlKey) && isConfigured(keyField.key)
        return (
          <div key={section.title} className="settings-section">
            <div className="settings-section-head">
              <span className="settings-section-title">{section.title}</span>
              <span className={`settings-model-status${ready ? ' ready' : ''}`}>{ready ? '已配置' : '未配置'}</span>
            </div>
