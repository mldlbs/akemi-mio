...
function EvolutionStatusPanel({ monitoring }: WallpaperWidgetContext) {
  ...
  return (
    <div className="wp-evo-status">
      ...
      {/* 摘要 */}
      {evolution.summary && (
        <div className="wp-evo-summary">{evolution.summary}</div>
      )}

      {/* 统计 */}
      <div className="wp-evo-stats">
        {evolution.fixedCount > 0 && (
          <span className="wp-evo-stat wp-evo-stat--good">
            ✓ {evolution.fixedCount} 修复
          </span>
        )}
        {evolution.errorCount > 0 && (
          <span className="wp-evo-stat wp-evo-stat--bad">
            ✗ {evolution.errorCount} 错误
          </span>
        )}
        {evolution.consecutiveFailures > 0 && (
          <span className="wp-evo-stat wp-evo-stat--bad">
            ⚠ 连败 {evolution.consecutiveFailures}
          </span>
        )}
      </div>
    </div>
