...
function FileMoveRow({ move, index }: { move: FileMoveResult; index: number }) {
  ...
  return (
    <div className={`wp-org-move-row${isLatest ? ' wp-org-move-latest' : ''}`} style={{ animationDelay: `${index * 30}ms` }}>
      <span
        className="wp-org-move-icon"
        style={{ color: STATUS_COLORS[move.status] || '#94a3b8' }}
        title={move.status}
      >
      ...
      {move.status === 'failed' && move.error && (
        <span className="wp-org-move-error" title={move.error}>!</span>
      )}
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="wp-org-header">
        <span className="wp-org-title">
          {progress.isPaused ? '⏸ 整理已暂停' : isFinished ? '✅ 整理完成' : '📁 文件整理中'}
        </span>
        {!isFinished && (
          <span className="wp-org-percent">{progress.percentComplete}%</span>
        )}
      </div>
      ...
      {!isFinished && (
        <div className="wp-org-controls">
          {progress.isPaused ? (
            <button
              className="wp-org-btn wp-org-btn-resume"
              onClick={() => window.electronAPI.organizerResume()}
              title="继续整理"
            >
              ▶ 继续
            </button>
            ...
            <button
              className="wp-org-btn wp-org-btn-pause"
              onClick={() => window.electronAPI.organizerPause()}
              title="暂停整理"
            >
              ⏸ 暂停
            </button>
          ...
          {progress.currentFile && (
            <button
              className="wp-org-btn wp-org-btn-skip"
              onClick={() => window.electronAPI.organizerSkip()}
              title="跳过当前文件"
            >
              ⏭ 跳过
            </button>
      ...
      {/* ── 完成摘要 ── */}
      {isFinished && progress.summary && (
        <div className="wp-org-summary">{progress.summary}</div>
      )}
    </div>
  )
}
