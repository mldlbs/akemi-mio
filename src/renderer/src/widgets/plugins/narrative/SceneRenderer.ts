...
import type { SceneId, SceneVisualDefinition, SceneElement, ParticleSystemConfig, EffectType } from './types'
...
/**
 * 粒子系统缺省参数。
 * 场景定义里 particles 声明为 Partial<ParticleSystemConfig>（每个场景只覆盖关心的字段），
 * 而粒子系统需要完整参数，故用这份保守默认值补齐缺项。
 */
const PARTICLE_DEFAULTS: ParticleSystemConfig = {
  rate: 8,
  maxCount: 120,
  speed: 20,
  size: [1, 2],
...
export class SceneRenderer {
  ...
  private renderElement(ctx: CanvasRenderingContext2D, element: SceneElement, w: number, h: number, alpha: number, sceneId: SceneId): void {
    const x = element.x * w
    const y = element.y * h
  ...
  private updateParticles(dt: number): void {
    // 合并默认值补齐 Partial<ParticleSystemConfig> 的缺项，避免 maxCount / rate
    // 为 undefined 时算出 NaN 的生成数量。early-return 保留：未声明 particles
    // 的场景不应凭空出现粒子。
    const partial = this.currentVisual?.particles
    if (!partial) return
    const config: ParticleSystemConfig = { ...PARTICLE_DEFAULTS, ...partial }

    // 生成新粒子
    if (this.particles.length < config.maxCount) {
      const spawnCount = Math.min(Math.ceil(config.rate * dt), config.maxCount - this.particles.length)
    ...
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      p.opacity =
        (p.config.opacity[0] + (p.config.opacity[1] - p.config.opacity[0]) * lifeRatio) *
  ...
  private updateEffects(dt: number): void {
    for (let i = this.effects.length - 1; i >= 0; i--) {
      ...
      if (effect.particles.length < effect.config.maxCount) {
        const spawnCount = Math.min(Math.ceil(effect.config.rate * dt * effect.intensity), effect.config.maxCount - effect.particles.length)
  ...
  private getEffectColor(type: EffectType, intensity: number): string {
    switch (type) {
      case 'rain':
        return `rgba(136, 153, 187, ${0.3 + intensity * 0.4})`
      case 'snow':
        return `rgba(255, 255, 255, ${0.5 + intensity * 0.5})`
      case 'fireflies':
        return `rgba(200, 230, 160, ${0.3 + intensity * 0.5})`
      case 'torchlight':
        return `rgba(255, 200, 100, ${0.2 + intensity * 0.3})`
      case 'sparkle':
        return `rgba(255, 215, 0, ${0.5 + intensity * 0.5})`
      case 'stars_twinkle':
        return `rgba(255, 255, 255, ${0.5 + intensity * 0.5})`
      case 'leaves_falling':
        return `rgba(180, 130, 60, ${0.4 + intensity * 0.4})`
      case 'bubbles':
        return `rgba(200, 230, 255, ${0.2 + intensity * 0.3})`
    }
  }
  ...
  private getEffectConfig(type: EffectType, intensity: number): ParticleSystemConfig | null {
    switch (type) {
      case 'rain':
        return {
          rate: Math.round(80 * intensity),
          maxCount: Math.round(300 * intensity),
          size: [0.5, 2],
          color: '#8899bb',
          lifetime: 2,
          opacity: [0.2, 0.5],
          ...
          twinkle: false,
          gravity: true,
          gravityForce: 180 * intensity,
          direction: Math.PI / 2,
          spread: 0.3,
        }
      case 'snow':
        return {
          rate: Math.round(30 * intensity),
          maxCount: Math.round(150 * intensity),
          size: [1, 4],
          color: '#ffffff',
          lifetime: 6,
          opacity: [0.4, 0.9],
      ...
      case 'fireflies':
        return {
          rate: Math.round(3 * intensity),
          maxCount: Math.round(20 * intensity),
          size: [1.5, 3.5],
          color: '#c8e6a0',
          lifetime: 5,
          opacity: [0.2, 0.6],
      ...
      case 'torchlight':
        return {
          rate: Math.round(2 * intensity),
          maxCount: Math.round(15 * intensity),
          size: [1, 3],
          color: '#ffcc66',
          lifetime: 4,
          opacity: [0.3, 0.6],
      ...
      case 'sparkle':
        return {
          ...
          maxCount: Math.round(60 * intensity),
          size: [1, 3],
          color: '#ffd700',
          lifetime: 2,
          opacity: [0.6, 1.0],
      ...
      case 'stars_twinkle':
        return {
          rate: Math.round(3 * intensity),
          maxCount: Math.round(50 * intensity),
          size: [0.5, 2],
          color: '#ffffff',
          lifetime: 8,
          opacity: [0.4, 1.0],
      ...
      case 'leaves_falling':
        return {
          rate: Math.round(5 * intensity),
          maxCount: Math.round(30 * intensity),
          size: [3, 6],
          color: '#b4823c',
          lifetime: 5,
          opacity: [0.5, 0.8],
