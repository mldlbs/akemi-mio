...
import type { SceneId, SceneKeywordEntry, ActionEffectEntry, SceneRuntimeState, EffectType, ActionEffectInstance } from './types'
import { DEFAULT_SCENE_KEYWORDS, DEFAULT_ACTION_EFFECTS, getSceneVisual } from './types'
...
export function useNarrativeWallpaper(options?: UseNarrativeWallpaperOptions): UseNarrativeWallpaperResult {
  const {
   ...
   */
  const matchKeywords = useCallback((text: string): SceneKeywordEntry[] => {
    ...
    for (const entry of sceneKeywordsRef.current) {
      ...
      if (entry.compoundMatch && entry.compoundMatch.length > 0) {
        hasCompound = entry.compoundMatch.some((compoundGroup) => compoundGroup.every((kw) => trimmed.includes(kw)))
      }
   ...
   * 匹配动作效果关键词。
   */
  const matchActions = useCallback((text: string): ActionEffectEntry[] => {
    if (!text || text.trim().length === 0) return []
   ...
   * 触发瞬时动作效果。
   */
  const triggerAction = useCallback((effectType: EffectType, intensity = 0.8, durationMs = 3000) => {
    ...
    setState((s) => ({
      ...
      activeEffects: s.activeEffects.includes(effectType) ? s.activeEffects : [...s.activeEffects, effectType],
    }))
    ...
    setTimeout(() => {
      setState((s) => ({
        ...
        activeEffects: s.activeEffects.filter((e) => e !== effectType || s.activeActions.some((a) => a.type === e && a.id !== id)),
      }))
    }, durationMs)
  }, [])
  ...
  const processText = useCallback(
    (text: string) => {
      ...
      if (sceneMatches.length > 0) {
        ...
        // 去重：同一场景在去抖时间内不重复触发
        const isDuplicate = lastMatch && lastMatch.sceneId === bestMatch.sceneId && now - lastMatch.timestamp < MATCH_DEBOUNCE_MS

        // 最小场景持续时长检查
        const tooSoon = stateRef.current.currentScene !== null && now - sceneStartRef.current < MIN_SCENE_DURATION_MS
  ...
  useEffect(() => {
    // 待接线：main/preload 目前并没有 narrative ASR 文本桥（全仓搜不到 onNarrativeAsrText
    // 的实现，只有这里的消费点）。这里以"可选能力探测"的方式声明它 —— 桥补上之后本段
    // 无需改动即可生效；未接线的今天它是一次空操作，与原行为一致。
    type NarrativeAsrBridge = {
      onNarrativeAsrText?: (cb: (payload: { text: string }) => void) => () => void
    }
    const bridge = window.electronAPI as typeof window.electronAPI & NarrativeAsrBridge
    if (!bridge.onNarrativeAsrText) return

    const unsub = bridge.onNarrativeAsrText((payload) => {
      if (payload?.text) {
