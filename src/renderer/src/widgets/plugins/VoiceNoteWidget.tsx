...
function VoiceNoteContent(ctx: WallpaperWidgetContext) {
  ...
  /** 发送音频到 ASR */
  const transcribeAudio = useCallback(async (samples: Float32Array[], sampleRate: number) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'oklch(0.25 0.02 260 / 0.85)',
        backdropFilter: 'blur(12px)',
        WebkitBackdropFilter: 'blur(12px)',
        border: '1px solid oklch(0.6 0.04 260 / 0.3)',
        borderRadius: 12,
        padding: '14px 18px',
        color: '#e0e0e0',
        fontSize: 15,
    ...
    >
      ...
      <div style={{
        ...
        <span style={{ fontSize: 11 }}>
          {vnState.displayMode === 'fixed' ? '固定' : '滚动'}
        </span>
      </div>
      ...
      {showActions && vnState.text && (
        <div style={{
          ...
          <ActionButton
            label={vnState.displayMode === 'fixed' ? '切换滚动' : '切换固定'}
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 6,
        border: primary
          ? '1px solid oklch(0.55 0.12 260)'
          : '1px solid oklch(0.5 0.02 260 / 0.4)',
        background: primary
          ? 'oklch(0.45 0.1 260 / 0.5)'
          : 'oklch(0.35 0.02 260 / 0.4)',
