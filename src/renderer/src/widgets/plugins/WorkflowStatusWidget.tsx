  ...
  return (
    <div className={`wp-wf-step-row${isLatest ? ' wp-wf-step-latest' : ''}`} style={{ animationDelay: `${index * 30}ms` }}>
      ...
      {step.status === 'failed' && step.error && (
        <span className="wp-wf-step-error" title={step.error}>
          !
        </span>
      )}
    </div>
...
function WorkflowStatusPanel({ hideDecoration }: WallpaperWidgetContext) {
  ...
  useEffect(() => {
    const unsub = window.electronAPI.onWorkflowRunUpdated((data) => {
      setRuns((prev) =>
        prev.map((r) =>
          r.runId !== data.runId ? r : { ...r, status: data.status, completedAt: data.status !== 'running' ? Date.now() : undefined },
        ),
      )
      setLastActivity(Date.now())
      setStalled(false)
        ...
        if (active.length > 0) {
          setRuns(
            active.slice(0, 5).map((r) => ({
              ...
              status: r.status,
              // r 来自 listWorkflowRuns()，其契约是 Promise<any[]>，故 steps 也是 any；
              // 显式标注为本地 WorkflowStepStatus，让下方字段访问重新接受检查。
              steps: (r.steps || []).map((s: WorkflowStepStatus) => ({
                stepId: s.stepId,
  ...
  const currentStep = activeRuns.length === 1 ? activeRuns[0].steps.find((s) => s.status === 'running') : null
    ...
    <div className="wp-wf-panel">
      ...
      >
        <div className="wp-wf-header-left">
          ...
          />
          <span className="wp-wf-title">{stalled ? '⏸ 工作流中断' : hasActive ? '⚡ 工作流' : '📋 工作流'}</span>
        </div>
        <div className="wp-wf-header-right">
          {hasActive && <span className="wp-wf-percent">{overallPercent}%</span>}
      ...
      {hasActive && !expanded && (
        <>
          ...
          {/* 阶段标记点 */}
          {activeRuns.length === 1 && activeRuns[0].steps.length > 0 && <PhaseDots steps={activeRuns[0].steps} />}
          ...
          <div className="wp-wf-summary">
            {stalled ? (
              <div className="wp-wf-stalled-info">
                ...
                <span className="wp-wf-stalled-text">最后活动: {fmtTime(lastActivity)}</span>
                {checkpoint && <span className="wp-wf-checkpoint">检查点: {fmtTime(checkpoint.time)}</span>}
              </div>
            ) : currentStep ? (
              ...
              <div className="wp-wf-stats">
                ...
                {allSteps.filter((s) => s.status === 'failed').length > 0 && (
                  <span className="wp-wf-stat wp-wf-stat-fail">失败 {allSteps.filter((s) => s.status === 'failed').length}</span>
                )}
              </div>
            )}
          </div>
      ...
      {/* ── 多工作流摘要（展开前显示运行数量） ── */}
      {hasActive && !expanded && activeRuns.length > 1 && <div className="wp-wf-run-count">共 {activeRuns.length} 个工作流运行中</div>}

      {/* ── 空状态消息 ── */}
      {!hasActive && !expanded && runs.length > 0 && (
        <div className="wp-wf-idle-note">
          {runs.filter((r) => r.status === 'done').length > 0 ? `上次完成: ${runs.length} 个工作流` : '所有工作流已完成'}
        </div>
      )}
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  tabIndex={0}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') setDetailRunId(isExpanded ? null : run.runId)
                  }}
                >
                  <div className="wp-wf-run-title-group">
                    <span className="wp-wf-run-name">{run.workflowName}</span>
                    <span className={`wp-wf-run-badge wp-wf-run-badge-${run.status}`}>{RUN_STATUS_LABELS[run.status] || run.status}</span>
                  </div>
                  <div className="wp-wf-run-meta">
                    {elapsed > 0 && run.status === 'running' && <span className="wp-wf-run-elapsed">{fmtElapsed(elapsed)}</span>}
                    <span className="wp-wf-run-pct">{runPct}%</span>
                    <span className="wp-wf-expand-icon">{isExpanded ? '▾' : '▸'}</span>
                  </div>
                </div>
                ...
                {isExpanded && run.steps.length > 0 && (
                  <div className="wp-wf-step-list">
                    {run.steps.map((step, i) => (
                      <StepRow key={step.stepId} step={step} index={i} isLatest={i === run.steps.length - 1 && step.status === 'running'} />
                    ))}
                  </div>
                )}

                {/* 空步骤提示 */}
                {isExpanded && run.steps.length === 0 && <div className="wp-wf-step-empty">等待步骤数据…</div>}
              </div>
            )
          })}

          {/* 初始加载提示 */}
          {runs.length === 0 && <div className="wp-wf-step-empty">等待工作流数据…</div>}
        </div>
      )}
    </div>
  )
