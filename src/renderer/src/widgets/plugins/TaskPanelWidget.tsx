...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`wp-tp-badge${c.pulse ? ' wp-tp-pulse' : ''}`}
    ...
    <div className={`wp-tp-panel${expanded ? ' wp-tp-expanded' : ''}`}>
      ...
      >
        ...
        <div className="wp-tp-header-right">
          {hasActivePlan && (
            <span className="wp-tp-plan-pct">{plan.percentComplete}%</span>
          )}
          <button
      ...
      {hasActivePlan && (
        <div className="wp-tp-plan-section">
          ...
          <div className="wp-tp-plan-stats">
            <span>{plan.completedSteps}/{plan.totalSteps} 步</span>
            {plan.currentStep && (
      ...
      {!expanded && (
        <>
          {/* 反馈提示 */}
          {feedbackMsg && (
            <div className="wp-tp-feedback">{feedbackMsg}</div>
          )}
        </>
      )}
              ...
              <div className="wp-tp-recent-list">
                {recentActions.map((ra) => (
                  <div key={ra.id} className={`wp-tp-recent-item wp-tp-recent-${ra.status}`}>
                    <span className="wp-tp-recent-icon" style={{
                    ...
                    <span className="wp-tp-recent-time">
                      {formatTimeAgo(ra.timestamp)}
                    </span>
                  </div>
                ))}
              </div>
