...
function EventCardItem({
  ...
  return (
    <div
      className={`wp-event-card${visible ? ' wp-event-card-visible' : ''}${card.actionable ? ' wp-event-card-actionable' : ''}`}
      style={
        {
  ...
  return (
    <div className={`wp-event-card-container${!expanded ? ' wp-event-card-collapsed' : ''}`}>
        ...
        <div className="wp-event-card-detail-overlay" onClick={() => setDetailCard(null)}>
          <div className="wp-event-card-detail" onClick={(e) => e.stopPropagation()}>
            <div className="wp-event-card-detail-header">
              ...
              <button className="wp-event-card-close" onClick={() => setDetailCard(null)} aria-label="关闭">
                ×
              </button>
            </div>
            <div className="wp-event-card-detail-body">
              ...
              <div className="wp-event-card-detail-row">
                ...
                <span className="wp-event-card-detail-value">{new Date(detailCard.timestamp).toLocaleString('zh-CN')}</span>
              </div>
              ...
              {detailCard.actionPayload && (
                <div className="wp-event-card-detail-payload">
                  ...
                  <pre className="wp-event-card-detail-json">{JSON.stringify(detailCard.actionPayload, null, 2)}</pre>
                </div>
              )}
            </div>
          </div>
      ...
      {displayCards.map((card) => (
        <EventCardItem key={card.id} card={card} onDismiss={handleDismiss} onAction={handleAction} />
      ))}
    </div>
  )
}
