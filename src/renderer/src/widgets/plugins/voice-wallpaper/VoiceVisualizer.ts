...
export class VoiceVisualizer {
  ...
  private renderBackground(ctx: CanvasRenderingContext2D, state: VoiceVisualizationState): void {
    ...
    const emotionGlow = ctx.createRadialGradient(
      ...
      this.width * 0.3, this.height * 0.4, this.width * 0.5,
    )
  ...
  private renderRipple(ctx: CanvasRenderingContext2D, state: VoiceVisualizationState, config: VoiceWallpaperConfig): void {
    ...
    glow.addColorStop(0, `${accent}${Math.round(60 * energy).toString(16).padStart(2, '0')}`)
    glow.addColorStop(0.5, `${accent}${Math.round(20 * energy).toString(16).padStart(2, '0')}`)
  ...
  private renderParticleWave(ctx: CanvasRenderingContext2D, state: VoiceVisualizationState, config: VoiceWallpaperConfig): void {
    ...
    for (let i = 0; i <= pointCount; i++) {
      ...
      const wave = Math.sin(this.time * 3 + i * 0.3) * 5 * energy +
        freqVal / 255 * 60 * energy * config.sensitivity
      const y = baseY - wave
    ...
    grad.addColorStop(0, `${accent}${Math.round(40 * energy).toString(16).padStart(2, '0')}`)
    grad.addColorStop(0.5, `${accent}${Math.round(15 * energy).toString(16).padStart(2, '0')}`)
  ...
  private updateColors(colorScheme: {
    primaryColor: string
