      ...
      if (data.hasActiveSessions) {
        ...
        displaySessions.forEach((session, sIdx) => {
          ...
          for (let i = 0; i < 6; i++) {
            ...
            ctx.strokeStyle = isActive ? `oklch(0.576 0.245 166 / ${baseAlpha * 0.5})` : `oklch(1 0 0 / ${baseAlpha * 0.12})`
            ctx.lineWidth = 2
            ctx.stroke()
          }
        ...
        >
          ...
          >
            ...
            {detailSession.skippedStages > 0 && (
              <div style={{ fontSize: 11, color: '#a78bfa', marginBottom: 8 }}>已跳过 {detailSession.skippedStages} 个阶段</div>
            )}
