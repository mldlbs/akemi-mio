...
export function DesktopToolbar() {
  const { tasks, feedback, visible, expanded, addTask, updateTask, showFeedback, toggleVisible, toggleExpanded } =
    useDesktopToolbarStore()
...
function QueueItem({ task }: { task: DesktopTask }) {
  ...
  const className =
    task.status === 'error' ? 'dt-queue-item error' : task.status === 'success' ? 'dt-queue-item success' : 'dt-queue-item'
