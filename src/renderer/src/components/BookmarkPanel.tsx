  ...
  return (
    <div className="bookmark-panel">
      ...
      <div className="bookmark-panel-header">
        <h3 className="bookmark-panel-title">
          <i className="ri-bookmark-3-line" />
          <span>语音书签</span>
        </h3>
        <button className="bookmark-panel-refresh" onClick={fetchBookmarks} title="刷新">
              ...
              <div className="bookmark-item-bottom">
                {bookmark.audioPath && (
                  <span className="bookmark-item-audio-badge">
                    ...
                    {bookmark.ttsDurationMs > 0
                      ? `${(bookmark.ttsDurationMs / 1000).toFixed(1)}s`
                      : '有声'}
                  </span>
                )}
                {bookmark.tags.length > 0 && (
                  <span className="bookmark-item-tags">
                    {bookmark.tags.slice(0, 3).map((tag) => (
                      <span key={tag} className="bookmark-item-tag">#{tag}</span>
                    ))}
                  </span>
                )}
              ...
              {bookmark.conversationContext.length > 0 && (
                <div className="bookmark-item-context">
                  <div className="bookmark-context-entry">
                    <span className="bookmark-context-role">
                      {bookmark.conversationContext[0].role === 'user' ? '你' : 'AI'}
                    </span>
                  ...
                  {bookmark.conversationContext.length > 1 && (
                    <span className="bookmark-context-more">
                      +{bookmark.conversationContext.length - 1} 条上下文
                    </span>
                  )}
                </div>
              )}
