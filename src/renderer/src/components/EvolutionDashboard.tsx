...
function ProgressBar({ progress, stage }: { progress: number; stage: string }) {
  ...
  return (
    <div className="evo-dash-progress-track">
      ...
      />
      {!isIndeterminate && (
        <span className="evo-dash-progress-label">{clamped}%</span>
      )}
    </div>
  )
}
    ...
    <div className="evolution-dashboard" data-stage={stage} style={{ opacity, transition: 'opacity 0.8s ease' }}>
      ...
      {collapsed ? (
        ...
        <div className="evo-dash-card">
          ...
          {/* 摘要 */}
          {data.summary && (
            <p className="evo-dash-summary">{data.summary}</p>
          )}
          ...
          <div className="evo-dash-footer">
            <span className="evo-dash-time">
              {formatTimeAgo(data.lastRunAt)}
            </span>
            <span className="evo-dash-mode">
              {data.safetyMode === 'review' ? '审查模式' : '自动模式'}
            </span>
          </div>
        </div>
      )}
