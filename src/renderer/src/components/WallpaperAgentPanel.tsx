...
const EXAMPLES = ['帮我整理桌面文件', '今天有什么待办事项？', '打开计算器', '截屏并翻译']
...
function MessageBubble({ msg }: { msg: WallpaperChatMessage }) {
  ...
  return (
    <div className={`wp-agent-msg ${isUser ? 'wp-agent-msg-user' : 'wp-agent-msg-assistant'}`}>
      <div className="wp-agent-msg-avatar">{isUser ? '👤' : '🤖'}</div>
...
export function WallpaperAgentPanel() {
  const { interactive, messages, status, addMessage, clearMessages, setStatus, setInteractive } = useWallpaperInteractionStore()
    ...
    <div className="wp-agent-panel">
      ...
      <div className="wp-agent-header">
        <div className="wp-agent-header-left">
          ...
          <span className="wp-agent-status-label">{status === 'processing' ? '处理中…' : '就绪'}</span>
        </div>
        <div className="wp-agent-header-right">
          {hasMessages && (
            <button className="wp-agent-header-btn" onClick={clearMessages} title="清空对话">
              🗑️
            </button>
          )}
          <button className="wp-agent-header-btn" onClick={() => setInteractive(false)} title="关闭 (Esc)">
            ✕
          </button>
        </div>
      </div>
          ...
          <div className="wp-agent-empty">
            ...
            {showExamples && (
              <div className="wp-agent-examples">
                {EXAMPLES.map((ex) => (
                  <button key={ex} className="wp-agent-example-btn" onClick={() => handleExampleClick(ex)}>
                    {ex}
                  </button>
                ))}
              </div>
