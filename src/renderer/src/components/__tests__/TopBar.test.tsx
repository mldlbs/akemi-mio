...
describe('TopBar', () => {
  it('renders logo', () => {
    renderWithProviders(<TopBar conversationActive={false} />)
  ...
  it('toggles sidebar on menu button click', () => {
    renderWithProviders(<TopBar conversationActive={false} />)
    // Sidebar starts open by default in SlotContext
    const menuBtn = screen.getByTitle('收起侧栏')
    fireEvent.click(menuBtn)
    // After toggle, the title should change to '展开侧栏'
  ...
  it('renders settings button that calls onOpenSettings', () => {
    ...
    renderWithProviders(<TopBar conversationActive={false} onOpenSettings={onOpenSettings} />)
  ...
  it('renders StatusBar with provided props', () => {
    renderWithProviders(
      <TopBar conversationActive={true} ttsPlaying={true} error="test error" sessionHealth="80:DEGRADED:memory" personaLevel="writer" />,
    )
    expect(screen.getByText('回复中')).toBeTruthy()
    expect(screen.getByText('80 DEGRADED')).toBeTruthy()
  ...
  it('shows sidebar as closed after toggle', () => {
    renderWithProviders(<TopBar conversationActive={false} />)
    // Initial: menu-fold (sidebar open by default)
