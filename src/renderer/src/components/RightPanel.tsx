...
const TIER_COLORS: Record<string, string> = { permanent: '#f59e0b', semi: '#60a5fa', ephemeral: '#888' }
    ...
    <aside className="right-panel">
      <div className="right-panel-header">
        <div className="right-panel-tabs">
          <button className={`right-panel-tab${tab === 'evolution' ? ' active' : ''}`} onClick={() => setTab('evolution')} title="进化">
            <i className="ri-robot-2-line" />
          </button>
          <button className={`right-panel-tab${tab === 'monitor' ? ' active' : ''}`} onClick={() => setTab('monitor')} title="监控">
            <i className="ri-dashboard-3-line" />
          </button>
          <button className={`right-panel-tab${tab === 'tools' ? ' active' : ''}`} onClick={() => setTab('tools')} title="工具">
        ...
        {tab === 'evolution' && (
          <div className="right-panel-section">
            ...
            <div className="rp-evo-stage">
              <span className="rp-evo-badge" style={{ background: stageColor }}>
                {stageLabel}
              </span>
            ...
            <div className="rp-evo-progress">
              <div className="rp-evo-track">
                <div
                  ...
                  style={{
                    width: stage === 'analyzing' || stage === 'fixing' ? '40%' : `${Math.max(0, Math.min(100, evoData?.progress ?? 0))}%`,
