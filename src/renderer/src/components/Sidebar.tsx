...
export function Sidebar() {
  ...
  const activeCategory = useMemo(() => {
    ...
    return sessions.find((s) => s.id === activeSessionId)?.category ?? null
    ...
    return (
      <aside className={`sidebar${collapsed ? ' collapsed' : ''}`}>
        <div className="sidebar-content">
          {collapsed ? (
            <button className="sidebar-collapsed-icon" onClick={toggleSidebar} title="展开侧栏" aria-label="展开侧栏">
    ...
    return (
      <aside className="sidebar collapsed">
        <div className="sidebar-content">
          ...
          <div className="sidebar-collapsed-sep" />
          <button
          ...
          {Array.from(categorySet).map((cat) => (
            <button
              key={cat}
              className={`sidebar-collapsed-icon${cat === activeCategory ? ' active' : ''}`}
    ...
    <aside className="sidebar">
      <div className="sidebar-content">
          ...
          return (
            <div key={cat} className="sidebar-section">
              ...
              {items.map((s) => (
                ...
                >
                  <i className={`${COLLAPSED_ICONS[s.category] || 'ri-chat-1-line'} sidebar-item-icon`} aria-hidden="true" />
        ...
        <div className="sidebar-tools">
          ...
          <button className={`sidebar-item${uiState.activeSlot === 'blog' ? ' active' : ''}`} onClick={openBlogEditor}>
