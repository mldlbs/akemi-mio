...
export function BlogEditor({ initialContent = '', onChange }: BlogEditorProps) {
  ...
  async function loadRecordings() {
    try {
      ...
      // 后端 BlogAudioEntry.transcribedText 可选（纯音频记录没有转写文本），
      // 而本地视图模型要求必填，故在边界处归一化，避免类型不匹配与 undefined 渗入渲染。
      setRecordings(
        list.map((entry) => ({
          id: entry.id,
    ...
    try {
      ...
      if (transcribedText) {
        ...
        try {
          ...
          // 契约是 { success, entry? }：原先读 saved.id，永远是 undefined，
          // 导致录音回链 ID 恒为空串、只能用 local_<ts> 占位。
          if (saved?.entry?.id) audioId = saved.entry.id
        } catch {
          // 保存失败不阻塞主流程
        }
    ...
    <div className="blog-editor">
      ...
      <div className="blog-editor-toolbar">
        ...
        <div className="blog-editor-toolbar-group">
          ...
          <button className="blog-editor-tb-btn" onClick={() => setShowRecordings(!showRecordings)} title="历史录音">
      ...
      <div className="blog-editor-body">
        {preview ? (
          <div className="blog-editor-preview markdown-body">{renderMarkdown(content)}</div>
        ) : (
          <textarea
            ref={textareaRef}
...
function renderInline(text: string): React.ReactNode[] {
  ...
  while (remaining.length > 0) {
    ...
    const candidates = [
      ...
      italicMatch ? { type: 'italic' as const, index: italicMatch.index!, length: italicMatch[0].length, content: [italicMatch[1]] } : null,
      linkMatch
        ? { type: 'link' as const, index: linkMatch.index!, length: linkMatch[0].length, content: [linkMatch[1], linkMatch[2]] }
        : null,
    ]
    ...
    switch (earliest.type) {
      case 'code':
        parts.push(
          <code key={`ic-${idx}`} className="md-inline-code">
            {earliest.content[0]}
          </code>,
        )
        break
      case 'bold':
        parts.push(
          <strong key={`b-${idx}`} className="md-bold">
            {renderInline(earliest.content[0])}
          </strong>,
        )
        break
      case 'italic':
        parts.push(
          <em key={`i-${idx}`} className="md-italic">
            {earliest.content[0]}
          </em>,
        )
        break
