  ...
  return (
    <div className={`qt-card qt-level-${task.recommendationLevel}`}>
      ...
      <div className="qt-card-header" onClick={() => setExpanded(!expanded)}>
        ...
        <span className="qt-card-confidence">
      ...
      <div className="qt-card-actions">
        <button
          className="qt-btn qt-btn-execute"
        ...
        <button
          className="qt-btn qt-btn-dismiss"
...
export function QuickTaskPanel({
  ...
  return (
    <div className="qt-panel">
      ...
      {/* ── 反馈提示 ── */}
      {actionFeedback && (
        <div className="qt-feedback">{actionFeedback}</div>
      )}
      ...
      {panelExpanded && (
        <div className="qt-panel-body">
          {loading && tasks.length === 0 && (
            <div className="qt-empty">分析行为模式中...</div>
          )}

          {!loading && tasks.length === 0 && (
            <div className="qt-empty">
              <p>暂无快捷任务推荐</p>
              <p className="qt-empty-hint">
                持续使用后，Agent 会自动学习你的操作习惯
              </p>
            </div>
          )}
          ...
          {activeRecommendations.length > maxDisplay && (
            <div className="qt-more-hint">
              还有 {activeRecommendations.length - maxDisplay} 个推荐...
            </div>
          )}
        </div>
      )}
