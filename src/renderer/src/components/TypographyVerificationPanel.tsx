...
function DiffSegmentView({ seg }: { seg: DiffSegment }) {
  ...
  const display = seg.type === 'insertion' ? `[${seg.recognized}]` : seg.type === 'deletion' ? `[${seg.original}]` : seg.original
  ...
  return (
    <div className={`tv-sentence ${statusClass}`}>
      <div className="tv-sentence-header" onClick={() => setExpanded(!expanded)}>
        ...
        <span className="tv-sentence-meta">{diffRatePct}% 差异</span>
      </div>
        ...
        <div className="tv-sentence-detail">
          ...
          <div className="tv-sentence-line">
            ...
            <span className="tv-diff-text">
              {sentence.diffSegments.map((seg, i) => (seg.type !== 'insertion' ? <DiffSegmentView key={i} seg={seg} /> : null))}
            </span>
          </div>
          ...
          <div className="tv-sentence-line">
            ...
            <span className="tv-diff-text">
              {sentence.diffSegments.map((seg, i) => (seg.type !== 'deletion' ? <DiffSegmentView key={i} seg={seg} /> : null))}
            </span>
          </div>
          ...
          <div className="tv-sentence-stats">
            ...
            {sentence.suspicious && <span className="tv-warning">⚠️ 可能存在吞词或断句错误</span>}
          </div>
          ...
          {sentence.originalSentence && (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation()
                onPlaySentence(sentence.originalSentence)
              }}
              title="朗读此句"
            >
  ...
  if (!panelVisible && !verifying && !error) {
    return (
      <div className="tv-trigger-bar">
        ...
        {text && text.length >= 10 && (
          <button className="tv-btn-read" onClick={() => playSentence(text.slice(0, 200))} title="朗读全文预览">
      ...
      {error && !isVerifying && (
        <div className="tv-error">
          ...
          <div className="tv-error-actions">
            <button className="tv-btn-secondary" onClick={() => setError(null)}>
              关闭
            </button>
            <button className="tv-btn-primary" onClick={runVerification}>
              重试
            </button>
          </div>
        </div>
      ...
      {report && !isVerifying && (
        <div className="tv-report">
          ...
          <div className={`tv-summary-card ${report.summary.hasDiscrepancies ? 'tv-summary-warn' : 'tv-summary-pass'}`}>
            <div className="tv-summary-icon">{report.summary.hasDiscrepancies ? '⚠️' : '✅'}</div>
            <div className="tv-summary-stats">
              ...
              <div className="tv-summary-stat">
                <span className="tv-summary-value">
                  {report.summary.verificationMs > 1000
                    ? (report.summary.verificationMs / 1000).toFixed(1) + 's'
                    : report.summary.verificationMs + 'ms'}
                </span>
          ...
          <div className="tv-actions-bar">
            <span className="tv-action-hint">
              {report.summary.hasDiscrepancies ? '发现可能的问题，请审查下方差异' : '语音校验通过，未发现明显问题'}
            </span>
          ...
          <div className="tv-sentences">
            ...
            {report.sentences.length === 0 && <div className="tv-sentences-empty">文本过短，无法逐句对比</div>}
            {report.sentences.map((sentence, idx) => (
              <SentenceRow key={idx} sentence={sentence} index={idx} onPlaySentence={playSentence} />
            ))}
          </div>
