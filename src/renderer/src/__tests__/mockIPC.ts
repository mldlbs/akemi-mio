...
export function createMockIPC(overrides?: Partial<MockedElectronAPI>): MockedElectronAPI {
  const mock: Partial<MockedElectronAPI> = {
    ...
    // ASR / AI / TTS
    transcribe: vi
      .fn<(...args: [ArrayBuffer]) => Promise<{ text: string; request_id?: string; error?: string }>>()
    ...
    executeVoiceChain: vi
      .fn<
        (...args: [string, Record<string, string>]) => Promise<{
          success: boolean
    ...
    getBehaviorTopActions: vi.fn<ElectronAPI['getBehaviorTopActions']>().mockResolvedValue({ success: true, actions: [], timestamp: 0 }),
    onBehaviorTopActions: vi.fn<ElectronAPI['onBehaviorTopActions']>().mockReturnValue(vi.fn()),
    invokeDesktopTool: vi
      .fn<(...args: [string, Record<string, unknown>]) => Promise<{ success: boolean }>>()
    ...
    getToolParamDefaults: vi.fn<ElectronAPI['getToolParamDefaults']>().mockResolvedValue({ success: true, combinations: [] }),
    recordToolParamFeedback: vi.fn<ElectronAPI['recordToolParamFeedback']>().mockResolvedValue({ success: true }),
    ...
    onToolStatus: vi
      .fn<(...args: [(status: { type: string; tool: string; message: string }) => void]) => () => void>()
      .mockReturnValue(vi.fn()),
    onMessageNew: vi
      .fn<
        (
    ...
    getMessageHistory: vi
      .fn<
        (
          ...
          { id: string; source: string; role: string; content: string; category: string; sessionId?: string; createdAt: number }[]
        >
      >()
      .mockResolvedValue([]),
    getSessions: vi
    ...
    // Auto-update
    checkUpdate: vi
      .fn<(...args: []) => Promise<{ available: boolean; version?: string; error?: string }>>()
    ...
    onToolInvoked: vi
      .fn<(...args: [(data: { tool: string; args: Record<string, any>; id: string }) => void]) => () => void>()
      .mockReturnValue(vi.fn()),
    onToolCompleted: vi
    ...
    onPlanStep: vi
      .fn<(...args: [(data: { planId: string; stepIndex: number; status: string }) => void]) => () => void>()
      .mockReturnValue(vi.fn()),
    ...
    onAgentObserve: vi
      .fn<
        (
          ...args: [(data: { requestId: string; step: number; proceduresFound: number; patternsFound: number; durationMs: number }) => void]
        ) => () => void
      >()
      .mockReturnValue(vi.fn()),
    onAgentThink: vi
      .fn<
        (...args: [(data: { requestId: string; step: number; toolCallCount: number; strategyPrompted: boolean }) => void]) => () => void
      >()
      .mockReturnValue(vi.fn()),
    onAgentReflect: vi
      .fn<
    ...
    onBudgetExhausted: vi
      .fn<(...args: [(data: { resource: string; utilization: number }) => void]) => () => void>()
      .mockReturnValue(vi.fn()),
    onBudgetRestored: vi
      .fn<(...args: [(data: { resource: string; utilization: number }) => void]) => () => void>()
      .mockReturnValue(vi.fn()),
    ...
    // Workflow events
    onWorkflowRunCreated: vi
      .fn<(...args: [(data: { runId: string; workflowDefId: string }) => void]) => () => void>()
      .mockReturnValue(vi.fn()),
    ...
    enableWorkflowDefinition: vi
      .fn<(...args: [string]) => Promise<{ success: boolean; error?: string }>>()
      .mockResolvedValue({ success: true }),
    disableWorkflowDefinition: vi
      .fn<(...args: [string]) => Promise<{ success: boolean; error?: string }>>()
      .mockResolvedValue({ success: true }),
    stopWorkflowRun: vi.fn<(...args: [string]) => Promise<{ success: boolean; error?: string }>>().mockResolvedValue({ success: true }),
    duplicateWorkflowDefinition: vi
      .fn<(...args: [string]) => Promise<{ success: boolean; error?: string }>>()
      .mockResolvedValue({ success: true }),
    ...
    quickTaskRecommendNow: vi
      ...
      .mockResolvedValue({ success: true, tasks: [] }),
    quickTaskExecute: vi
      .fn<(...args: [string]) => Promise<{ success: boolean; task?: any; error?: string }>>()
      .mockResolvedValue({ success: true }),
    quickTaskDismiss: vi
      .fn<(...args: [string]) => Promise<{ success: boolean; task?: any; error?: string }>>()
      .mockResolvedValue({ success: true }),
    ...
    quickTaskEdit: vi
      .fn<
        (
          ...args: [string, Array<{ tool: string; label: string; args?: Record<string, unknown> }>]
    ...
    quickTaskSetSensitivity: vi
      .fn<(...args: [number]) => Promise<{ success: boolean; error?: string }>>()
      .mockResolvedValue({ success: true }),
    quickTaskAnalyze: vi
      .fn<(...args: []) => Promise<{ success: boolean; tasks: any[]; error?: string }>>()
      .mockResolvedValue({ success: true, tasks: [] }),
    ...
    onQuickTaskUpdate: vi
      .fn<(...args: [(data: { task: any; action: string; timestamp: number }) => void]) => () => void>()
      .mockReturnValue(vi.fn()),
