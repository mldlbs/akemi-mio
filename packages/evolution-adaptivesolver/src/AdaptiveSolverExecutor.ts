...
export class AdaptiveSolverExecutor implements FixExecutor {
  ...
  readonly supportedSources = ['synthetic'] as const
  ...
  async execute(problem: AssignedProblem): Promise<FixResult> {
    ...
    try {
      ...
      // 9. Git 提交
      try {
        await this.gitOps.autoGitCommit(`[adaptive-solver] upgrade ${solverName} to adaptive step-size RK45`)
      } catch {
...
状态: ✅ tsc 编译验证通过，已提交 Git`,
   ...
   */
  private buildAdaptiveSolverPrompt(solverName: string, stepValue: number, sourceCode: string): string {
...
只返回完整的 TypeScript 源码，用 \`\`\`typescript ... \`\`\` 包裹。不要添加额外说明。`
  ...
  private containsAdaptiveFeatures(code: string): boolean {
    const requiredFeatures = ['AdaptiveStepConfig', 'isFixedStep = false', 'tolerance', 'minStepSize', 'maxStepSize', 'solve(']
  ...
  private async runBenchmarkValidation(
    ...
    try {
      ...
      const cmd = `npx ts-node -e "
        const { EulerSolver, RK4Solver, AdaptiveRK45Solver } = require('@akemi-mio/evolution-adaptivesolver/solvers');
        const { lorenzSystem, lorenzFixedConfig, lorenzAdaptiveConfig, stiffSystem, stiffFixedConfig, stiffAdaptiveConfig } = require('@akemi-mio/evolution-adaptivesolver/benchmark');
        const { runBenchmark } = require('@akemi-mio/evolution-adaptivesolver/benchmark');
