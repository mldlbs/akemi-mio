...
} from '@akemi-mio/intelligence-startup-radar/types'
 ...
 */
function computeCompositeScore(scores: SignalDimensionScores, source: SignalSource, category: StartupSignalCategory): number {
  const baseScore =
 ...
 */
function evaluateRelevance(title: string, summary: string, category: StartupSignalCategory, focusArea?: string): number {
...
function evaluateImpact(title: string, summary: string): number {
  ...
  // 中影响信号
  const mediumImpact = ['launch', '推出', '发布', 'release', 'partnership', '合作', 'expansion', '扩张', 'growth', '增长', 'hiring', '招聘']
...
function evaluateConfidence(source: SignalSource, hasUrl: boolean, textLength: number): number {
  ...
  let score = sourceTrust[source] ?? 0.5
  ...
  else if (textLength < 30) score -= 0.1
...
export class StartupRadarAdapter implements IStartupRadarProvider {
  ...
  formatForTelegram(signal: StartupSignal, options?: TelegramFormatOptions): string {
    const { detailed = false, showScores = true, showSource = true, showUrl = true } = options ?? {}
    ...
    const lines: string[] = [`${urgencyEmoji[signal.urgency]} [${categoryLabel[signal.category]}] ${signal.title}`]
  ...
  formatBatchForTelegram(signals: StartupSignal[], options?: TelegramFormatOptions): string {
    ...
    const lines: string[] = ['📡 **创业雷达 — 信号报告**', `━━━ ${signals.length} 条新信号 ━━━`, '']
   ...
   */
  private async collectRawData(sources: SignalSource[], keywords?: string[], limit?: number): Promise<RawDataItem[]> {
  ...
  private async fetchRemoteSignals(): Promise<StartupSignal[]> {
    try {
      const res = await fetch('https://radar.crlkcloud.cyou/radar/pipeline', {
