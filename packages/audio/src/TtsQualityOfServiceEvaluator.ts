...
export class TtsQualityOfServiceEvaluator {
  ...
  evaluate(): QosEvaluation {
    ...
    // ── 计算各维度分数（0-1，越高越好） ──
    // 网络分数：延迟越低分越高
    const networkScore = networkAvailable ? Math.max(0, 1 - (Math.max(0, networkLatencyMs) / this.config.highLatencyThresholdMs) * 1.5) : 0
    ...
    const qosScore = networkAvailable ? networkScore * 0.5 + cpuScore * 0.5 : cpuScore * 0.8 + 0.1 // 网络不可用时降低基准
