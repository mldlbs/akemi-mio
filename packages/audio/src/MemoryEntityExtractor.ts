...
/** 中文人名候选：姓氏+1-2字 */
const CN_NAME_PATTERN =
  /([王李张刘陈杨赵黄周吴徐孙胡朱高林何郭马罗梁宋郑谢韩唐冯于董萧程曹袁邓许傅沈曾彭吕苏卢蒋蔡贾丁魏薛叶阎余潘杜戴夏钟汪田任姜范方石姚谭廖邹熊金陆郝孔白崔康毛邱秦江史顾侯邵孟龙万段漕钱汤尹黎易常武乔贺赖龚文])([一-鿿]{1,2}(?:生|总|工|哥|姐|叔|姨|先生|女士|小姐)?)/g
...
export class MemoryEntityExtractor {
   ...
   */
  feedEntries(entries: MemoryEntry[], interactions?: InteractionRecord[]): void {
    const now = Date.now()
  ...
  private pruneExcess(): void {
    ...
    const sorted = Array.from(this.entityMap.entries()).sort((a, b) => a[1].frequency - b[1].frequency)
