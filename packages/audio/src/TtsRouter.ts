...
import type { TtsEngine, TtsUserPreference, TtsRoutingDecision, TtsRouterConfig, PiperPerformanceInfo } from './types'
...
export class TtsRouter {
  ...
  decideSync(options?: {
    ...
    if (latency > this.config.maxLatencyMs && latencyWeight > qualityWeight) {
      ...
      return this.makeDecision(
        'cloud',
        `high_latency_${latency}ms_piper_unhealthy`,
        latency,
        true,
        qualityWeight,
        latencyWeight,
    ...
    return this.makeDecision(
      'cloud',
      `sync_cloud_${latency}ms_piper_bias_${piperCloudBias.toFixed(2)}`,
      latency,
      true,
      qualityWeight,
      latencyWeight,
