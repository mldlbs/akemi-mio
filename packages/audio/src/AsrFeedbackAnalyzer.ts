...
export class AsrFeedbackAnalyzer {
  ...
  analyzeAndOptimize(): BatchOptimizationResult {
    ...
    if (now - this.lastAnalyzeAt < MIN_INTERVAL_MS) {
      const remainingMin = Math.round((MIN_INTERVAL_MS - (now - this.lastAnalyzeAt)) / 60000)
    ...
    result.topCorrected = analyzed.slice(0, 10).map((a) => `${a.corrected}(${a.frequency}, w=${a.weight.toFixed(2)})`)
  ...
  private analyzePattern(pattern: AsrErrorPattern, maxFreq: number): AnalyzedPattern {
    ...
    return {
      ...
      suggestHotword: true,
      suggestDomainBoost: pattern.category === 'domain_term' || pattern.category === 'new_word' || pattern.frequency >= 5,
  ...
  private buildPatches(patterns: AnalyzedPattern[]): AsrConfigPatch[] {
    ...
    for (const p of patterns) {
      ...
      const alreadyExists = existingVocab.some((v) => v.word.toLowerCase() === p.corrected.toLowerCase())
