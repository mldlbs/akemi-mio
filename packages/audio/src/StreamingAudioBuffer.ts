...
export class StreamingAudioBuffer {
    ...
    try {
      if (useLocal) {
        ...
        await new Promise<void>((resolve, reject) => {
          const proc = execFile(
            'edge-tts',
            ['--voice', voice, '--text', clean, '--write-media', outputFile, '--rate', rate, '--pitch', pitchCfg],
   ...
   * - local: 强制 Piper
   */
  private async synthesizeSentence(sentence: string, cfg: StreamingConfig): Promise<SentenceSynthesisResult> {
    const t0 = Date.now()
  ...
  private async synthesizeWithEdge(
    ...
    await new Promise<void>((resolve, reject) => {
      const proc = execFile(
        'edge-tts',
        ['--voice', voice, '--text', sentence, '--write-media', outputFile, '--rate', rate, '--pitch', pitch],
        { timeout: 30000, windowsHide: true },
        (err) => {
          if (err) reject(err)
          else resolve()
