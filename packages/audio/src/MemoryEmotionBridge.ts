...
export class MemoryEmotionBridge {
  ...
  recordUserMessageEmotion(userText: string, memoryService: MemoryService): void {
    ...
    try {
      ...
      // 3. 构造情感标签（含效价/唤醒度维度）
      const valence = this.computeValence(result.polarity, result.score, result.matchedWords)
      const arousal = this.computeArousal(userText, result.score, result.matchedWords)
      ...
      const recentEntry = entries.filter((e) => e.type === 'user_fact' || e.type === 'interaction').slice(-1)[0]
  ...
  getRecentEmotionContext(memoryService: MemoryService): AggregatedEmotionContext {
    ...
    const dominantPolarity = (Object.entries(polarityCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || 'neutral') as
   ...
   */
  private mapPolarityToLabel(polarity: 'positive' | 'negative' | 'neutral', contentType: string, matchedWords: string[]): string {
  ...
  private buildContextReason(
    ...
    const parts: string[] = [`记忆情感：${labelCN}`, `${trendCN[trend] || '平稳'}`, `基于 ${totalCount} 条历史记录`]
