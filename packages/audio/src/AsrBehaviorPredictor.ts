...
export class AsrBehaviorPredictor {
   ...
   */
  recordTranscription(text: string, confidence: number, voiceEmotion?: VoiceEmotion): void {
  ...
  getPrediction(forceRefresh = false): AsrBehaviorPrediction {
    ...
    // 缓存命中
    if (!forceRefresh && this.cachedPrediction && now - this.lastPredictionTime < this.predictionTtlMs) {
  ...
  private detectFillers(text: string): { words: string[]; ratio: number } {
    ...
    // 从长到短匹配（避免短词先匹配阻断长词）
    const sortedFillers = Array.from(FILLER_WORDS).sort((a, b) => b.length - a.length)
      ...
      for (const [original, { frequency, wasCorrected }] of patternMap) {
        ...
        if (wasCorrected) {
          try {
            ...
            const match = corrections.find((c) => c.original === original || c.original.includes(original))
   ...
   * - 领域热词 → 注入相关领域词汇
   */
  private buildPromptBoosts(hesitation: HesitationProfile, boostedTerms: string[]): string[] {
    const boosts: string[] = []
   ...
   */
  private calcGlobalConfidenceAdjust(hesitation: HesitationProfile): number {
    let adjust = 0
    ...
    if (this.records.length >= MIN_SAMPLES) {
      const recordsWithFeatures = this.records.filter((r) => r.voiceFeatures)
      if (recordsWithFeatures.length >= 2) {
        const avgSilenceRatio =
          recordsWithFeatures.reduce((sum, r) => sum + (r.voiceFeatures?.silenceRatio ?? 0), 0) / recordsWithFeatures.length
        ...
        // 语速分析：平均语速
        const avgSpeechRate =
          recordsWithFeatures.reduce((sum, r) => sum + (r.voiceFeatures?.speechRate ?? 0), 0) / recordsWithFeatures.length
