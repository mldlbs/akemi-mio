...
export class TopicToneMatcher {
  ...
  recordToneForTopic(topic: string, params: EmotionTtsParams, confidence = 0.6): void {
    ...
    try {
      ...
      const newConfidence = existing ? Math.min(MAX_CONFIDENCE, existing.confidence + MATCH_BOOST) : confidence
   ...
   */
  getToneForTopics(currentTopics: string[]): { params: EmotionTtsParams; confidence: number; matchedTopic: string } | null {
