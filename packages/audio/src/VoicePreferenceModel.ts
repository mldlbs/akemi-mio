...
import type { EmotionTtsParams, TtsOutputRecord, ImplicitFeedbackAction, PreferenceRecommendation, ImplicitFeedbackConfig } from './types'
import { DEFAULT_IMPLICIT_FEEDBACK_CONFIG, IMPLICIT_FEEDBACK_WEIGHTS } from './types'
...
export class VoicePreferenceModel {
  ...
  updateModel(): void {
    ...
    // 收集所有有反馈记录的输出
    const scored = this.history.filter((r) => r.actions.length > 0 || r.cumulativeScore !== 0)
    ...
    // 选择平均分最高的 rate 范围
    const rateCategory = this.pickBestCategory(rateScores, classifyRate(parseRate(DEFAULT_PARAMS.rate)))
    ...
    // 选择平均分最高的 pitch 范围
    const pitchCategory = this.pickBestCategory(pitchScores, classifyPitch(parsePitch(DEFAULT_PARAMS.pitch)))
    ...
    const reason = this.buildRecommendationReason(scored.length, bestVoice, rateCategory, pitchCategory, hasSufficientData)
   ...
   */
  private pickBestCategory(scores: Record<string, { total: number; count: number }>, defaultCategory: string): string {
   ...
   * 但单步变化不超过 maxDelta。
   */
  private lerpWithLimit(current: number, target: number, blendRatio: number, maxDelta: number): number {
