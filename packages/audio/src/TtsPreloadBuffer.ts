...
export class TtsPreloadBuffer {
  ...
  async getBuffered(text: string, emotionParams: EmotionTtsParams): Promise<string | null> {
    ...
    // 查找匹配的条目（相同文本 + 相同引擎）
    const idx = this.buffer.findIndex((entry) => entry.text === text && entry.engine === targetEngine && entry.verified)
  ...
  tryPreload(text: string, engine: 'cloud' | 'local', emotionParams: EmotionTtsParams): void {
    ...
    // 检查是否已有相同文本的预加载
    const existing = this.buffer.some((entry) => entry.text === text && entry.engine === targetEngine)
    if (existing) return
   ...
   * 用目标引擎异步合成文本并加入缓冲。
   */
  private async synthesizeWithTarget(text: string, engine: 'cloud' | 'local', emotionParams: EmotionTtsParams): Promise<void> {
    if (!this.synthesizeFn) return
