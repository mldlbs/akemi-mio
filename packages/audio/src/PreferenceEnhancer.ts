...
export class PreferenceEnhancer {
  ...
  enhanceParams(baseParams: EmotionTtsParams, topics?: string[]): EmotionTtsParams {
    ...
    try {
      ...
      const blendedRate = Math.round(currentRate * (1 - TOPIC_TONE_BLEND_WEIGHT) + matchedRate * TOPIC_TONE_BLEND_WEIGHT)
      const blendedPitch = Math.round(currentPitch * (1 - TOPIC_TONE_BLEND_WEIGHT) + matchedPitch * TOPIC_TONE_BLEND_WEIGHT)
  ...
  enhance(text: string, baseParams: EmotionTtsParams, topics?: string[]): EnhancementResult {
    ...
    const topicToneMatched = enhancedParams.label.startsWith(TOPIC_TONE_LABEL_PREFIX) || enhancedParams.label !== baseParams.label
