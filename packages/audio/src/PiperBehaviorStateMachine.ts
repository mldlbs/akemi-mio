...
import type { BehaviorMode } from '@akemi-mio/evolution/behavior'
...
class SensorMonitor {
  ...
  getSnapshot(): SensorSnapshot {
    ...
    return {
      hour: new Date().getHours(),
      screenBrightness: now - this._brightnessTime < this.config.sensorCacheTtlMs ? this._brightness : null,
      systemMuted: now - this._mutedTime < this.config.sensorCacheTtlMs ? this._muted : null,
      screenOff: this._screenOffTimestamp > 0 && now - this._screenOffTimestamp < this.config.sensorCacheTtlMs ? true : null,
      timestamp: now,
    }
  }
...
export class PiperBehaviorStateMachine {
   ...
   */
  evaluateRules(sensor: SensorSnapshot, behavior: BehaviorContext): StateMachineOutput {
    ...
    if (isNight && (isLowBrightness || brightness === null)) {
      ...
      return {
        ...
        reason: brightness !== null ? `夜间时段+低亮度(${brightness}%)` : `夜间时段(亮度未知，安全降级)`,
        sensorSnapshot: sensor,
      }
    }
