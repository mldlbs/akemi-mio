...
import { findFfmpeg } from '@akemi-mio/core/utils/ffmpeg'
...
export class AudioConcatenator {
   ...
   */
  private generateSilence(outputPath: string, durationMs: number, ffmpeg: string, sampleRate: number): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      execFile(
        ffmpeg,
        ['-f', 'lavfi', '-i', `anullsrc=r=${sampleRate}:cl=mono`, '-t', String(roundedSec), '-acodec', 'pcm_s16le', '-y', outputPath],
   ...
   */
  private runFfmpegConcat(ffmpeg: string, concatFile: string, config: ConcatConfig): Promise<void> {
    return new Promise((resolve, reject) => {
      // 构建滤镜链
      // 1. concat 拼接所有输入
      ...
      execFile(
        ffmpeg,
        ['-f', 'concat', '-safe', '0', '-i', concatFile, '-c', 'copy', '-y', tempOutput],
   ...
   * 对音频文件添加淡入淡出并编码为 MP3。
   */
  private addFadeAndEncode(ffmpeg: string, inputPath: string, config: ConcatConfig): Promise<void> {
    return new Promise((resolve, reject) => {
      // 获取音频时长以计算淡出起始点
      this.getAudioDuration(inputPath, ffmpeg)
        .then((durationSec) => {
          if (durationSec <= 0) {
