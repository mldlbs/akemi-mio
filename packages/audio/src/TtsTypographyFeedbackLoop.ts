...
import { TypographyMemoryManager, type TypographyParameters } from '@akemi-mio/creativity/TypographyMemoryManager'
...
export class TtsTypographyFeedbackLoop {
  ...
  start(): void {
    eventBus.track('tts.playback.started', () => this.onTtsPlaybackStarted(), this.subs, 'tts_typography_loop:playback_start')
    eventBus.track('tts.playback.finished', () => this.onTtsPlaybackFinished(), this.subs, 'tts_typography_loop:playback_finish')
  ...
  private onTtsPlaybackFinished(): void {
    ...
    if (now - this.lastEvaluationTime >= this.config.evaluationIntervalMs) {
      ...
      this.evaluateAndAdjust().catch((err) => log('WARN', 'tts_typography_evaluation_error', { error: String(err) }))
    }
  }
   ...
   */
  recordFeedback(action: ImplicitFeedbackAction, storyId?: string, platformTag?: string): void {
    const now = Date.now()
    ...
    const currentParams: Partial<TypographyParameters> = records.length > 0 ? { ...records[0].parameters } : {}
   ...
   * 更新 EMA 状态，纳入一次新样本。
   */
  private updateEMA(signalWeight: number, currentParams: Partial<TypographyParameters>, timestamp: number): void {
  ...
  private buildAdjustedParams(): TypographyParameters {
    // 获取当前排版参数作为基线
    const records = this.typographyManager.queryRecords(this.config.storyId, this.config.platformTag, { limit: 1 })
    ...
    const base: TypographyParameters = records.length > 0 ? { ...baseFallback, ...records[0].parameters } : { ...baseFallback }
    ...
    for (const paramName of Object.keys(PARAM_OPTIONS) as AdjustableTypographyParam[]) {
      ...
      if (
        count &&
        count >= this.config.minSamples &&
        rate !== undefined &&
        rate < this.config.negativeThreshold &&
        direction !== undefined
      ) {
        ...
        if (currentValue && typeof currentValue === 'string') {
          ...
          if (adjusted !== currentValue) {
            ;(result as Record<string, string>)[paramName] = adjusted
          }
        }
      }
    }
   ...
   */
  private async applyTypographyAdjustment(adjustedParams: TypographyParameters, convergence: string): Promise<void> {
    // 检查是否有实际变更
    const changedParams = Object.entries(adjustedParams).filter(([key, value]) => {
      const records = this.typographyManager.queryRecords(this.config.storyId, this.config.platformTag, { limit: 1 })
