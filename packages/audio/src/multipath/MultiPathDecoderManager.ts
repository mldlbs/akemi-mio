...
import { WhisperGpuDecoderInstance, WhisperCpuDecoderInstance, BaiduDecoderInstance } from './DecoderInstance'
...
export class MultiPathDecoderManager {
   ...
   * 注册 Whisper GPU 解码器。
   */
  registerWhisperGpu(engine: WhisperGpuEngine, config?: Partial<DecoderConfig>): string {
   ...
   * 注册 Whisper CPU 解码器。
   */
  registerWhisperCpu(engine: WhisperEngine, config?: Partial<DecoderConfig>): string {
    const decoderConfig: DecoderConfig = {
      ...DEFAULT_DECODER_CONFIG,
   ...
   * 注册 Baidu 解码器。
   */
  registerBaidu(engine: BaiduEngine, apiKey?: string, secretKey?: string, config?: Partial<DecoderConfig>): string {
    const decoderConfig: DecoderConfig = {
      ...DEFAULT_DECODER_CONFIG,
   ...
   * @returns          融合后的转录结果
   */
  async transcribe(audio: Float32Array, requestId?: string): Promise<MultiPathFusionResult> {
    ...
    const decoderPromises = activeDecoders.map((decoder) =>
      decoder.transcribe(audio, { requestId }).catch((err): DecoderResult => ({
        name: decoder.name,
        text: '',
        confidence: 0,
        latencyMs: 0,
    ...
    if (!fusionResult.text && decoderResults.some((r) => r.success && r.text)) {
      ...
      const bestSuccess = decoderResults.filter((r) => r.success && r.text).sort((a, b) => b.confidence - a.confidence)[0]
