...
import type { DecoderConfig, DecoderHealth, DecoderResult, FusionMethod, MultiPathFusionConfig, MultiPathFusionResult } from './types'
...
export class FusionEngine {
   ...
   * @returns               融合后的最优输出
   */
  fuse(results: DecoderResult[], decoderHealths: DecoderHealth[]): MultiPathFusionResult {
    const t0 = Date.now()
   ...
   * 5. 最终置信度 = 该类总分 / 该类总权重
   */
  private fuseByWeightedVote(results: DecoderResult[]): Pick<MultiPathFusionResult, 'text' | 'confidence' | 'primaryEngine'> {
    ...
    const finalConfidence =
      winningCluster.totalWeight > 0 ? Math.min(1, winningCluster.weightedScore / winningCluster.totalWeight) : bestResult.confidence
   ...
   */
  private fuseByBestConfidence(results: DecoderResult[]): Pick<MultiPathFusionResult, 'text' | 'confidence' | 'primaryEngine'> {
   ...
   * 单解码器策略：直接返回唯一结果。
   */
  private fuseSingle(result: DecoderResult): Pick<MultiPathFusionResult, 'text' | 'confidence' | 'primaryEngine'> {
    return {
      text: result.text,
