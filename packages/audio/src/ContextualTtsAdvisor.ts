...
import type { InteractionCadence, DayPeriod, InteractionIntervalStats, InteractionContext, EmotionTtsParams } from './types'
...
export class ContextualTtsAdvisor {
  ...
  private computeIntervalStats(): InteractionIntervalStats {
    ...
    if (timestamps.length < 2) {
      const lastGap = timestamps.length === 1 ? (now - timestamps[0]) / 1000 : 0
      return {
        meanIntervalSec: 0,
        medianIntervalSec: 0,
   ...
   * 生成人类可读的情境描述。
   */
  private buildDescription(cadence: InteractionCadence, dayPeriod: DayPeriod, stats: InteractionIntervalStats): string {
