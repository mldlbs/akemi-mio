...
import type { ToneFeatures, UserToneProfile, MessageToneAnalysis, ToneLabel } from './types'
...
export class ToneProfileAnalyzer {
  ...
  updateProfile(text: string): UserToneProfile {
    ...
    log('DEBUG', 'tone_profile_update', {
      ...
      features: Object.fromEntries(Object.entries(this.runningFeatures).map(([k, v]) => [k, v.toFixed(3)])),
    })
  ...
  getProfile(): UserToneProfile {
    ...
    const variance =
      Object.values(this.runningFeatures).reduce((sum, v) => sum + Math.abs(v - 0.5), 0) / Object.keys(this.runningFeatures).length
