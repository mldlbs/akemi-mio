...
import type { AsrPlugin, TtsPlugin, SpeechPluginManifest } from './types'
import type { IPluginRegistry } from '@akemi-mio/intelligence-plugin/registry/types'
...
export class SpeechPluginRegistry {
  ...
  getAsrPlugins(): AsrPlugin[] {
    return Array.from(this.asrPlugins.values()).sort((a, b) => (b.manifest.priority ?? 0) - (a.manifest.priority ?? 0))
  }
  ...
  getTtsPlugins(): TtsPlugin[] {
    return Array.from(this.ttsPlugins.values()).sort((a, b) => (b.manifest.priority ?? 0) - (a.manifest.priority ?? 0))
  }
