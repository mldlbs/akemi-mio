...
import { PiperTtsPlugin, EdgeTtsPlugin } from './adapters'
...
export class TtsService {
  ...
  recordImplicitFeedback(
    action: 'REPLAY' | 'SKIP' | 'INTERRUPT_SPEECH' | 'CONTINUE_CONVERSATION' | 'MODIFY_REQUEST' | 'COMPLETED_NATURALLY',
  ): void {
    if (action === 'REPLAY') {
   ...
   */
  private async _synthesizeWithStyle(text: string, speakingStyle: string, styleDegree: number, isCloudEngine: boolean): Promise<void> {
    const clean = cleanTTS(text)
    if (!clean || clean.length < 15) return
    ...
    try {
      if (isCloudEngine) {
        ...
        const args = [
          ...
          speakingStyle,
          '--style-degree',
          String(Math.max(0, Math.min(2, styleDegree))),
        ]
