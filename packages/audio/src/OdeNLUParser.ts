...
const EQUATION_FULL_RE =
  /(?:dy\/dx|y'|y′|y＇)\s*=\s*(.+?)(?=\s+(?:初值|y\(|初始|边界|起始|从|在|区间|步长|step|from|with|using|use|method|,|$))/
...
export const MISSING_FIELD_PROMPTS: Record<OdeRequiredField, string[]> = {
  equation: ['请告诉我微分方程是什么？例如 "dy/dx = x + y"', '方程未识别，请说清楚右侧表达式，如 "dy/dx 等于 x 平方加 y"'],
  initialCondition: ['还需要初始条件，例如 "y(0) = 1"', '请提供初值，例如 "y0 等于 1"'],
  interval: ['请指定求解区间，例如 "从 0 到 2"', '还需要 x 的范围，例如 "从 0 到 2"'],
}
    ...
    if (fullEqMatch) {
      ...
      if (eqParts.length >= 2) {
        ...
        if (lhs.includes('dy/dx') || lhs.includes("y'") || lhs.includes('导数')) {
          // 清理 RHS：去掉尾部的其他参数描述
          let cleaned = rhs
          const tailCut = cleaned.match(/^(.*?)(?:\s+(?:初值|y\(|初始|起始|从|在|区间|步长|step|from|with|using|use|method)|$)/)
