...
export class PiperTtsEnvironmentMonitor {
  ...
  private async collectAllSignals(): Promise<void> {
    try {
      const [brightness, muted] = await Promise.all([this.fetchScreenBrightness(), this.fetchSystemMuted()])
...
'@
  ...
  private runPowerShell(script: string): Promise<string> {
    ...
    const oneLiner = script.replace(/\r?\n/g, '; ').replace(/"/g, '\\"')

    return execAsync(`powershell -NoProfile -NonInteractive -Command "${oneLiner}"`, { timeout: PS_TIMEOUT_MS, windowsHide: true })
  ...
  private detectAndNotify(signals: EnvironmentSignals): void {
    ...
    const changed = prev.screenBrightness !== signals.screenBrightness || prev.systemMuted !== signals.systemMuted
