...
export class EmotionalNarrativeService {
   ...
   */
  private buildNarrativeSegments(narrative: LlmNarrativeResponse, context: AggregatedEmotionContext): NarrativeEmotionSegment[] {
    const segments: NarrativeEmotionSegment[] = []
    const total = narrative.segments.length
    const initialArousal = context.recentEmotions.length > 0 ? (context.recentEmotions[0].arousal ?? 0) : 0
   ...
   * 逐段播报叙事，每段发射事件。
   */
  private async speakNarrative(segments: NarrativeEmotionSegment[], context: AggregatedEmotionContext): Promise<void> {
  ...
  private async fallbackNeutralNarrative(
    ...
    if (customText) {
      ...
      const trendCN =
        context.stats.trend === 'rising' ? '情绪整体不错' : context.stats.trend === 'falling' ? '情绪有些起伏' : '情绪比较平稳'
