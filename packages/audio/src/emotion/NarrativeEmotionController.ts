...
export class NarrativeEmotionController {
  ...
  private buildAutoCurve(
    ...
    // 为每个段落分配情感值
    const emotionSegments = this.assignEmotionToSegments(rawSegments, latestVector, trendSummary, this.config.maxSegments)
   ...
   * 回退到平面单段风格（禁用/错误时）。
   */
  private fallbackToFlat(text: string, baseParams: EmotionTtsParams, isCloudEngine: boolean): SpeakingStyleResult {
    return speakingStyleGenerator.generateSingleStyle(text, { valence: 0, arousal: 0 }, baseParams, isCloudEngine)
  }
}
