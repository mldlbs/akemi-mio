...
import type { IEngineService, EngineStatus, EngineMetrics } from '@akemi-mio/engine'
...
export class PiperOrchestrator implements IEngineService {
  ...
  async warmupModel(modelName: string): Promise<void> {
    ...
    if (!existsSync(modelPath) || !existsSync(`${modelPath}.json`)) {
      log('WARN', 'piper_warmup_model_unavailable', {
        model: modelName,
        modelPath,
    ...
    // Use a short valid utterance so Piper writes a complete WAV header.
    const warmupText = '预热'
  ...
  getSynthesisStats(): PiperSynthesisStats {
    ...
    const total = { requests: 0, success: 0, failure: 0 }
  ...
  private synthesizeWithModel(text: string, model: string, speed: number, pitch: number): Promise<Result<string, string>> {
    ...
    return new Promise((resolve) => {
      try {
        ...
        if (!existsSync(modelPath) || !existsSync(`${modelPath}.json`)) {
          resolve(err(`Piper 模型文件不可用: ${modelPath}`))
          return
        }
        ...
        resolve(err(e instanceof Error ? e.message : String(e)))
      }
    })
  }

  /** Resolve a catalog model name to its ONNX path next to the configured default model. */
  private getModelPath(model: string): string {
    const ext = extname(PIPER_MODEL)
    const dir = dirname(PIPER_MODEL)
    return join(dir, model + ext)
  }
}
