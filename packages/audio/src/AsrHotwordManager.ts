...
import { ASR_HOTWORD_WINDOW_SIZE, ASR_HOTWORD_FREQ_THRESHOLD, WORKSPACE } from '@akemi-mio/core/config'
...
/** 领域标签枚举 */
export const DOMAIN_LABELS = ['编程开发', '系统运维', '音乐娱乐', '图像创作', '社交沟通', '生活日常', '专业术语', '其他'] as const
...
export class AsrHotwordManager {
  ...
  constructor(windowSize = DEFAULT_WINDOW_SIZE, freqThreshold = DEFAULT_FREQ_THRESHOLD, vocabFilePath?: string) {
  ...
  loadPersistedVocabulary(): void {
    try {
      ...
      const data: Array<{ word: string; count: number; domain: string; firstSeenAt: number; lastSeenAt: number }> = JSON.parse(raw)
      ...
      for (const item of data) {
        ...
        const domain = DOMAIN_LABELS.includes(item.domain as DomainLabel) ? (item.domain as DomainLabel) : '其他'
  ...
  deleteWord(word: string): boolean {
    ...
    this.entries = this.entries.filter((e) => e.word.toLowerCase() !== lowered)
   ...
   */
  seedVocabulary(words: Array<string | { word: string; domain?: DomainLabel }>, initialCount?: number): void {
    const count = initialCount ?? this.freqThreshold
    const now = Date.now()

    for (const entry of words) {
      const word = typeof entry === 'string' ? entry : entry.word
      const domain = typeof entry === 'object' && entry.domain ? entry.domain : classifyDomain(word)
