...
export class AudioFeatureExtractor {
  ...
  private computeAggregateFeatures(frames: FrameResult[], durationSec: number): AudioFeatures {
    ...
    // ── 能量特征 ──
    const energy = voiceFrameCount > 0 ? voiceFrames.reduce((s, f) => s + f.rms, 0) / voiceFrameCount : 0
    const energyVariance = voiceFrameCount > 0 ? this.variance(voiceFrames.map((f) => f.rms)) : 0
    ...
    const pitchHz = pitchValues.length > 0 ? pitchValues.reduce((s, v) => s + v, 0) / pitchValues.length : 0
    const pitchVariance = pitchValues.length > 1 ? this.variance(pitchValues) : 0
    const pitchRange = pitchValues.length > 1 ? Math.max(...pitchValues) - Math.min(...pitchValues) : 0

    // ── 过零率 ──
    const zcrValues = frames.map((f) => f.zcr)
    const avgZeroCrossingRate = zcrValues.length > 0 ? zcrValues.reduce((s, v) => s + v, 0) / zcrValues.length : 0
