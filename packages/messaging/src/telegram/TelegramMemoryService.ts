...
import { getEmbedding, cosineSimilarity } from '@akemi-mio/intelligence-memory/embedding'
 ...
 * 这些是系统级事件，用户不应错过。
 */
const CRITICAL_CATEGORIES = new Set<string>(['budget', 'recovery', 'system', 'stability'])
...
export class TelegramMemoryService {
   ...
   * @returns 是否推送及原因
   */
  async shouldPush(category: string, content: string): Promise<{ push: boolean; reason: string }> {
    // 1. 关键类别始终推送
   ...
   */
  private async isRelevantToRecentActivity(category: string, content: string): Promise<boolean> {
    try {
      // 获取该类别的语义话题标签
  ...
  private isWithinActiveHours(): boolean {
    try {
      ...
      const prefs = this.memoryService.getUserPreferences()
      const activeHoursPref = prefs.find((p) => p.key === TELEGRAM_PREFERENCE_KEYS.ACTIVE_HOURS)
  ...
  async getRecentUserContext(): Promise<{
    ...
    try {
      ...
      topics = profile ? Array.from(profile.topicWeights.keys()) : []
    } catch {
      // 静默
    }
  ...
  private async loadPushHistory(): Promise<void> {
    try {
      ...
      const historyEntries = entries.filter((e) => e.type === 'interaction' && e.content.startsWith('[telegram_push]'))
