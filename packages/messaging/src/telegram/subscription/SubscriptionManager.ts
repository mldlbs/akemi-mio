...
import type {
  ...
  SubscriptionFilter,
} from '@akemi-mio/messaging/telegram/subscription/types'
import { subscriptionMemoryKey, SUBSCRIPTION_MEMORY_KEY_PREFIX } from '@akemi-mio/messaging/telegram/subscription/types'
...
export interface SubscriptionMemoryRepository {
  ...
  saveUserProfile(key: string, value: string, structuredData: string, migratedFrom: string, confidence: number): void
}
...
export class TelegramSubscriptionManager {
  ...
  getSubscriptions(filter?: SubscriptionFilter): RadarSubscription[] {
    ...
    if (filter) {
      ...
      if (filter.sources && filter.sources.length > 0) {
        results = results.filter((s) => filter.sources!.some((src) => s.sources.includes(src)))
      }
      if (filter.frequency) {
   ...
   * 创建新订阅。
   */
  createSubscription(data: Omit<RadarSubscription, 'id' | 'createdAt' | 'updatedAt'>): SubscriptionOperationResult<RadarSubscription> {
  ...
  formatSubscriptionsForTelegram(): string {
    ...
    const lines: string[] = ['📡 **订阅列表**', `━━━ ${subs.length} 条订阅 ━━━`, '']
    ...
    try {
      ...
      for (const p of profiles) {
        ...
        try {
          ...
          if (parsed.key && parsed.key.startsWith(SUBSCRIPTION_MEMORY_KEY_PREFIX) && parsed.subscription) {
   ...
   * 持久化单条订阅到 Memory。
   */
  private persistSubscription(sub: RadarSubscription, source: 'manual' | 'default'): { success: boolean; error?: string } {
    if (!this.memoryRepo) {
