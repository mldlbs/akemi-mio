...
import type { RadarPushRule, PushRuleOperationResult, PushRuleListResult } from '@akemi-mio/messaging/telegram/radar/types'
import { CREATE_PUSH_RULES_TABLE_SQL, MAX_PUSH_RULES } from '@akemi-mio/messaging/telegram/radar/types'
...
export class RadarPushRuleStore {
  ...
  get(id: string): RadarPushRule | undefined {
    this.ensureInitialized()
    try {
      const rows = getRawDb().exec(`SELECT * FROM radar_push_rules WHERE id = ?`, [id])
  ...
  getAll(): PushRuleListResult {
    this.ensureInitialized()
    try {
      const rows = getRawDb().exec(`SELECT * FROM radar_push_rules ORDER BY created_at ASC`)
      if (!rows.length) return { rules: [], total: 0 }

      const rules = rows[0].values.map((row: any[]) => this.rowToRule(row, rows[0].columns))
  ...
  getEnabled(): RadarPushRule[] {
    this.ensureInitialized()
    try {
      const rows = getRawDb().exec(`SELECT * FROM radar_push_rules WHERE enabled = 1 ORDER BY created_at ASC`)
      if (!rows.length) return []
      return rows[0].values.map((row: any[]) => this.rowToRule(row, rows[0].columns))
    } catch {
      return []
    }
  }
  ...
  markPushed(id: string): void {
    try {
      const db = getRawDb()
      db.run(`UPDATE radar_push_rules SET last_pushed_at = ?, push_count = push_count + 1, updated_at = ? WHERE id = ?`, [
        Date.now(),
        Date.now(),
        id,
      ])
  ...
  formatRulesForTelegram(rules?: RadarPushRule[]): string {
    ...
    const lines: string[] = ['📡 **雷达推送规则**', `━━━ ${list.length}/${MAX_PUSH_RULES} 条规则 ━━━`, '']
