...
import { insertOutbox, type OutboxCategory } from '@akemi-mio/core/db/outbox'
import { TELEGRAM_SERVER_URL, TELEGRAM_ENABLED } from '@akemi-mio/core/config'
...
export class TelegramService {
  ...
  private telegramGateway: ExternalMessageGateway
  private baseUrl = ''
  ...
  constructor(agentService: AgentService) {
    ...
    const insertGatewayOutbox: OutboxInsert = (row) =>
      insertOutbox({
        ...row,
        category: row.category as OutboxCategory | undefined,
      })
    this.telegramGateway = new ExternalMessageGateway(agentService, this.telegramAdapter, insertGatewayOutbox)
  }
  ...
  private async handleMessage(msg: TelegramMessage): Promise<void> {
    ...
    if (msg.type === 'command') {
      await this.processExternalAgentMessage(msg)
      return
    }
    if (!msg.text && !msg.command) return

    const chatId = msg.chatId
    const userText = msg.text || (msg.command ? `/${msg.command}` : '')
    ...
    if (progressMsgId === null) {
      ...
      try {
        const result = await this.processExternalAgentMessage(msg)
        if (!result) return
        if (result.reply) {
  ...
  private subscribePushEvents(): void {
    ...
    const dispatchPushNotification = (
      ...
      title: string,
      body: string,
      urgency: NotificationEvent['urgency'] = 'low',
    ): void => {
    ...
    eventBus.on('evolution.cycle.started', (p: any) => {
      ...
      dispatchPushNotification('evolution_complete', 'Evolution cycle started', lines.join('\n'))
    ...
    eventBus.on('evolution.cycle.completed', (p: any) => {
      if (p.success && p.summary?.startsWith('预过滤跳过')) {
        ...
        dispatchPushNotification('evolution_complete', 'Evolution cycle completed', lines.join('\n'))
        return
      }
    ...
    eventBus.on('evolution.snapshot.created', (p: any) => {
      dispatchPushNotification('evolution_complete', 'Evolution snapshot', `🧬 快照: ${p.tag} (${p.branch})`)
    })
    eventBus.on('evolution.rollback.completed', (p: any) => {
      dispatchPushNotification('evolution_complete', 'Evolution rollback', `🧬 回滚${p.success ? '完成' : '失败'}: ${p.level}/${p.ref}`)
    })
    eventBus.on('evolution.proposal.validated', (p: any) => {
      ...
      dispatchPushNotification(
        ...
        `🧬 提案验证: ${p.proposalId} ${icon} 风险:${p.regressionRisk}`,
      )
    })
    eventBus.on('evolution.action.executed', (p: any) => {
      ...
      dispatchPushNotification('evolution_complete', 'Evolution action executed', lines.join('\n'))
    })
    ...
    eventBus.on('pipeline.started', (p: any) => {
      ...
      dispatchPushNotification('system_update', 'Pipeline started', lines.join('\n'))
    })
    ...
    eventBus.on('pipeline.completed', (p: any) => {
      ...
      dispatchPushNotification('system_update', 'Pipeline completed', lines.join('\n'))
    })
    ...
    eventBus.on('pipeline.errored', (p: any) => {
      ...
      dispatchPushNotification('system_update', 'Pipeline error', lines.join('\n'), 'medium')
    })

    // === 🔍 洞察 ===
    eventBus.on('insight.analysis.started', () => {
      dispatchPushNotification('memory_update', 'Insight analysis started', `🔍 洞察分析开始...`)
    })
    eventBus.on('insight.analysis.completed', (p: any) => {
      if (p.count > 0) {
        dispatchPushNotification(
          ...
          `🔍 洞察分析完成: ${p.count} 条发现${p.hasValue ? ' (含高价值)' : ''}`,
        )
      } else {
        dispatchPushNotification('memory_update', 'Insight analysis completed', `🔍 洞察分析完成: 无发现`)
      }
    })
    eventBus.on('insight.found', (p: any) => {
      ...
      dispatchPushNotification('memory_update', 'Insight found', lines.join('\n'))
    })

    // === 💡 创意 ===
    eventBus.on('creativity.cycle.started', () => {
      dispatchPushNotification('memory_update', 'Creativity cycle started', `💡 创意生成开始...`)
    })
    eventBus.on('creativity.cycle.completed', (p: any) => {
      if (p.count > 0 && p.hasValue) {
        dispatchPushNotification('memory_update', 'Creativity cycle completed', `💡 创意生成完成: ${p.count} 个新想法`)
      } else if (p.count > 0) {
        dispatchPushNotification('memory_update', 'Creativity cycle completed', `💡 创意生成完成: ${p.count} 个想法全部重复`)
      } else {
        dispatchPushNotification('memory_update', 'Creativity cycle completed', `💡 创意生成完成: 无新想法`)
      }
    })
    eventBus.on('creativity.dream.completed', (p: any) => {
      dispatchPushNotification('memory_update', 'Creativity dream completed', `💡 梦境完成: ${p.count} 个组合, 最高新颖度 ${p.topNovelty}%`)
    })
    eventBus.on('creativity.ideas.generated', (p: any) => {
      ...
      dispatchPushNotification('memory_update', 'Creativity ideas generated', lines.join('\n'))
    })

    // === 📋 计划 ===
    eventBus.on('agent.plan.created', (p: any) => {
      dispatchPushNotification('plan_update', 'Plan created', `Plan ${p.planId}: ${p.title}`)
    })
    eventBus.on('agent.plan.step', (p: any) => {
      dispatchPushNotification('plan_update', 'Plan step', `Plan ${p.planId} step #${p.stepIndex}: ${p.status}`)
    })
    eventBus.on('agent.plan.completed', (p: any) => {
      dispatchPushNotification('plan_update', 'Plan completed', `Plan ${p.planId}`)
    })
    ...
    eventBus.on('budget.exhausted', (p: any) => {
      this.dispatchNotification({
        ...
        requireAction: true,
        title: '预算耗尽',
        body: `${p.resource} (使用率 ${Math.round(p.utilization * 100)}%)`,
        context: { chatId: String(chatId) },
      })
    })
    ...
    eventBus.on('recovery.checkpoint.created', (p: any) => {
      dispatchPushNotification('recovery_update', 'Recovery checkpoint', `Run ${p.runId} via ${p.trigger}\n${p.path}`, 'medium')
    })
    eventBus.on('recovery.session.restored', (p: any) => {
      dispatchPushNotification(
        'recovery_update',
        'Recovery session restored',
        `${p.runId}${p.hasUnfinishedPlan ? ' (unfinished plan)' : ''}`,
        'medium',
      )
    })
    eventBus.on('recovery.error.classified', (p: any) => {
      dispatchPushNotification('recovery_update', 'Recovery error classified', `${p.category} -> ${p.strategy}`, 'medium')
    })
    eventBus.on('recovery.recovery.completed', (p: any) => {
      dispatchPushNotification('recovery_update', `Recovery ${p.success ? 'completed' : 'failed'}`, p.newRunId, 'medium')
    })

    // === 📊 稳定性（仅状态变更时推送，避免每60秒刷屏） ===
    eventBus.on('stability.status.changed', (p: any) => {
      dispatchPushNotification('stability_alert', 'Stability status changed', `${p.previous} -> ${p.current} (${p.score})`, 'medium')
    })
    ...
    eventBus.on('plugin.registered', (p: any) => {
      dispatchPushNotification('system_update', 'Plugin registered', `${p.name} v${p.version} (${p.toolCount} tools)`)
    })
    eventBus.on('plugin.unregistered', (p: any) => {
      dispatchPushNotification('system_update', 'Plugin unregistered', `${p.name}${p.reason ? ` (${p.reason})` : ''}`)
    })
    eventBus.on('plugin.error', (p: any) => {
      dispatchPushNotification('system_update', 'Plugin error', `${p.name} [${p.phase}]: ${p.error}`, 'medium')
    })
    eventBus.on('engine.registered', (p: any) => {
      dispatchPushNotification('system_update', 'Engine registered', `${p.name} (${p.type})`)
    })
    eventBus.on('engine.unregistered', (p: any) => {
      dispatchPushNotification('system_update', 'Engine unregistered', p.name)
    })
    eventBus.on('engine.activated', (p: any) => {
      dispatchPushNotification('system_update', 'Engine activated', `${p.name}${p.previous ? ` (from ${p.previous})` : ''}`)
    })
    eventBus.on('skill.enabled', (p: any) => {
      dispatchPushNotification('system_update', 'Skill enabled', p.name)
    })
    eventBus.on('skill.disabled', (p: any) => {
      dispatchPushNotification('system_update', 'Skill disabled', `${p.name}${p.reason ? ` (${p.reason})` : ''}`)
    })
  }
  ...
  private subscribeRadarEvents(): void {
    ...
    eventBus.on('radar.push.rule_fired', (p: any) => {
      ...
      this.dispatchNotification({
        ...
        urgency: 'medium',
        requireAction: false,
        title: 'Radar rule fired',
        body: p.message,
        context: { chatId: String(target.chatId) },
      })
    })
  }
  ...
  private dispatchNotification(event: NotificationEvent): number {
    const inserted = this.telegramGateway.dispatchNotification(event)
    this.tryReactivateOutbox()
    return inserted
  }

  private async processExternalAgentMessage(msg: TelegramMessage): Promise<ChatResult | undefined> {
    const inbound = this.telegramAdapter.normalizeInbound({
    ...
    return await this.telegramGateway.handleExternalMessage(inbound)
  }
}
