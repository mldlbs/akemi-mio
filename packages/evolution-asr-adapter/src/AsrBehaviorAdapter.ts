...
import type { PreProcessContext, PostProcessContext, PostProcessResult } from '@akemi-mio/evolution/user-behavior/types'
 ...
 */
const ADAPTED_LOW_CONF_KEYWORDS = ['嗯', '呃', '啊', '哦', '噢', '哎', '嗯哼', '呃呃', '啊啊']
 ...
 */
function mapEnvironmentToContext(environment: AsrEnvironmentInput): Pick<BehaviorContextHint, 'environmentLabel' | 'isQuiet' | 'isNoisy'> {
 ...
 * - 音频特征评估规则（能量、静音比）
 */
function mapConfidenceToQualitySignals(confidence: AsrConfidenceInput): AsrQualitySignal[] {
  ...
  adapt(input: {
    ...
    if (input.voiceEmotion && this.config.enableVoiceEmotionMapping) {
      ...
      if (emotion.confidence > 0.3) {
        ...
        if (!this.config.pocMode) {
          messages.push(`[ASR适配] 语音情感 "${emotion.label}" → 活跃度 ${emotionResult.activityScore.toFixed(2)}`)
        }
    ...
    if (input.environment && this.config.enableEnvironmentMapping) {
      ...
      if (env.confidence > 0.3) {
        ...
        if (!this.config.pocMode) {
          messages.push(`[ASR适配] 声学环境 "${env.environment}" → ${envResult.isQuiet ? '安静' : envResult.isNoisy ? '嘈杂' : '一般'}`)
        }
    ...
    if (input.confidence && this.config.enableConfidenceMapping) {
      ...
      if (!this.config.pocMode && confidenceSignals.length > 0) {
        messages.push(`[ASR适配] 置信度评分 ${(input.confidence.score * 100).toFixed(0)} → ${confidenceSignals.length} 个质量信号`)
      }
    }
    ...
    return async (ctx: PreProcessContext): Promise<PreProcessContext> => {
      ...
      try {
        ...
        if (result.contextHint) {
          ...
          ;(ctx as unknown as Record<string, unknown>).asrContext = result.contextHint
