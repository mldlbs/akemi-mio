...
import type { BlogAssessmentInput, BlogDimensionScores } from '@akemi-mio/intelligence/agent/blog/PlanBlogWritingAdapter'
...
export class AsrReasoningAdapter {
  ...
  async planForBlog(ctx: ReasoningContext): Promise<{
    ...
    if (this.config.contextBoost.enabled) {
      const patternScore = {
        cause_effect: 0.6,
        hypothesis_verification: 0.5,
        option_evaluation: 0.6,
        goal_constraint_tradeoff: 0.7,
        none: 0.3,
      }
  ...
  private buildContextBoost(): ContextBoostStrategy {
    const booster = new GenericContextBoost('blog-context-boost', this.config.contextBoost.maxBoost, this.config.contextBoost.decayFactor)
  ...
  private buildCompositeScoring(): CompositeScoringStrategy {
    ...
    scorer.addChannel({
      ...
      score: (input) => {
        ...
        let s = 0.5
        if (h2Count >= 2) s += 0.12
        if (paragraphs >= 5 && paragraphs <= 30) s += 0.1
    ...
    scorer.addChannel({
      ...
      score: (input) => {
        ...
        if (sentences.length >= 5) {
          ...
          if (Math.sqrt(variance) >= 15) s += 0.1
        }
  ...
  private rateContentDepth(content: string, codeBlockCount: number, headingCount: number): number {
    ...
    let s = 0.5
    const techTerms =
      /TypeScript|JavaScript|React|Vue|API|SDK|HTTP|async|await|interface|type|class|component|hook|算法|架构|性能|优化|设计模式|函数|组件|状态|异步/g
  ...
  private rateCodeQuality(codeBlocks: string[], content: string): number {
    ...
    if (inlineCodes.length >= codeBlocks.length) s += 0.1
    return Math.min(1, Math.max(0, s))
  }
  ...
  private ratePublishReadiness(content: string, _platform?: string): number {
    ...
    if (hasTitle && hasBody) s += 0.1
    else if (!hasTitle) s -= 0.1
    const placeholders = /TODO|FIXME|XXX|待定|待补充|TBD/i
    if (placeholders.test(content)) s -= 0.1
    return Math.min(1, Math.max(0, s))
  }
