...
export class PlanTaskEngine {
  ...
  constructor(planId: string, deps: PlanSchedulerDeps, toolExecutor: ToolExecutor, config?: Partial<PlanSchedulerConfig>) {
    this.planId = planId
    this.deps = deps
  ...
  createTask(
    ...
    const resolvedDepIds = dependsOnStepIndices.filter((idx) => indexToTaskId.has(idx)).map((idx) => indexToTaskId.get(idx)!)
  ...
  private async handleExecutionFailure(task: PlanTask, result: ToolResult): Promise<boolean> {
    ...
    if (this.config.enableDegradation && task.analysis) {
      const currentToolIdx = task.analysis.toolSuggestions.findIndex((s) => s.toolName === task.activeTool)
      // 尝试下一个备选工具
