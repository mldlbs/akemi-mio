...
import { eventBus, type SubscriptionTracker } from '@akemi-mio/core/core/EventBus'
...
export class SchedulerNotificationBridge {
  ...
  start(): void {
    ...
    for (const [eventName, template] of Object.entries(EVENT_TEMPLATES)) {
      ...
      const disposer = eventBus.on(eventKey, (payload: any) => this.handleEvent(eventName, payload, template), {
        priority: template.priority === 'high' ? 'high' : 'low',
        label: `sched_notif:${eventName}`,
      })
  ...
  startWithTracker(tracker: SubscriptionTracker): void {
    ...
    for (const [eventName, template] of Object.entries(EVENT_TEMPLATES)) {
      ...
      eventBus.track(eventKey, (payload: any) => this.handleEvent(eventName, payload, template), tracker, {
        priority: template.priority === 'high' ? 'high' : 'low',
        label: `sched_notif:${eventName}`,
      })
    }
