...
export class PlanSnapshotStore {
   ...
   */
  createSnapshot(planId: string, planTitle: string, tasks: PlanTask[], reason: SnapshotReason): string | null {
  ...
  private reinforceSnapshot(entry: MemoryEntry): void {
    ...
    this.memoryService.addEntry('plan_snapshot', entry.content, entry.confidence, {
      tier: entry.tier,
