...
import { log, initLogFile, getLogFilePath, sanitizeForLog } from '@akemi-mio/core/logger/Logger'
...
import { piperTtsEnvironmentMonitor } from '@akemi-mio/audio/PiperTtsEnvironmentMonitor'
import { engineProvider } from '@akemi-mio/engine'
import { ttsScheduler } from '@akemi-mio/audio/TtsScheduler'
...
import { sleepOrchestrator } from '@akemi-mio/intelligence/agent/SleepOrchestrator'
...
import { initEvolution, evolutionService, planManager, SelfEvolutionService } from '@akemi-mio/evolution'
import {
  ...
  ExecutionPolicy,
} from '@akemi-mio/evolution/automation'
import { initInsight, insightService, insightStore } from '@akemi-mio/intelligence-insight'
import { initCreativity, creativityService } from '@akemi-mio/creativity'
import { initInspiration } from '@akemi-mio/intelligence-inspiration'
import type { GitHubInspiration } from '@akemi-mio/intelligence-inspiration/GitHubInspiration'
import { setMemoryService } from '@akemi-mio/intelligence-mcp/LocalProvider'
import {
  setPlanManager,
...
import {
  ...
  setUserContextOverride,
  setOrganizerPause,
  setOrganizerResume,
  setOrganizerSkip,
  setSubtitleToggle,
  setVoiceRoleSchemeSwitch,
  setTaskPanelToggle,
  setVoiceNoteToggle,
  setVoiceNoteSave,
  setEnginePreferenceToggle,
  updateEnginePreference,
} from '@akemi-mio/core/core/TrayManager'
import { initUpdater, setUpdateWindow } from '@akemi-mio/updater/UpdaterService'
import {
...
import { TaskPanelService } from '@akemi-mio/platform/wallpaper/TaskPanelService'
import { WallpaperInteractiveService } from '@akemi-mio/platform/wallpaper/WallpaperInteractiveService'
import { VoiceNoteService } from '@akemi-mio/voicenote'
import { MonitoringService } from '@akemi-mio/monitoring'
import { WallpaperEventBridge } from '@akemi-mio/platform/wallpaper/WallpaperEventBridge'
import { initDatabase, closeDatabase, getRawDb, getEventRawDb } from '@akemi-mio/core/db/connection'
import { ConstitutionEngine } from '@akemi-mio/evolution-constitution'
import {
  CapabilityEngine,
  freezeDefaults,
  CapabilityCatalog,
  ...
  CapabilitySchemaAdapter,
  CapabilityFunctionSchemaAdapter,
} from '@akemi-mio/capabilities/capability'
import { SocialPublishScheduler } from '@akemi-mio/capabilities/social-publish/SocialPublishScheduler'
import { setCapabilityAdapter } from '@akemi-mio/intelligence/agent/context'
import { ToolSchemaProvider, ToolInvocationRouter } from '@akemi-mio/capabilities/tool'
import { CognitiveService } from '@akemi-mio/intelligence/cognitive'
...
import { Kernel } from '@akemi-mio/core/core/Kernel'
import { WorkerPool } from '@akemi-mio/core/core/WorkerPool'
import { ProcessManager } from '@akemi-mio/core/core/ProcessManager'
import { AgentModule, MemoryModule, EvolutionModule, McpModule } from '@akemi-mio/core/core/kernel-modules'
import { SessionGovernor } from '@akemi-mio/evolution-governance'
import { CheckpointV2 } from '@akemi-mio/evolution-governance'
import { systemBus } from '@akemi-mio/core/core/SystemBus'
import { SyscallBus, HealthChecker } from '@akemi-mio/core/core/lifecycle/index'
import type { IModule, SubsystemState } from '@akemi-mio/core/core/lifecycle/types'
import { TelegramService } from '@akemi-mio/messaging/telegram/TelegramService'
import { OutboxWorker } from '@akemi-mio/messaging/telegram/OutboxWorker'
import { UumitService } from '@akemi-mio/platform/uumit/index'
import type { TaskExecutionResult } from '@akemi-mio/core/core/tasks/unified/TaskTypes'
import { TaskRunner } from '@akemi-mio/core/core/tasks/unified/TaskRunner'
import { MetricsCollector } from '@akemi-mio/intelligence/observability/MetricsCollector'
import { SystemStabilityScore } from '@akemi-mio/intelligence/observability/SystemStabilityScore'
import { ComfyUIManager } from '@akemi-mio/image'
import { setComfyUIManager as setImageToolComfyUI } from '@akemi-mio/capabilities/tool/definitions/ImageGenerationTool'
import { ResourceBudget } from '@akemi-mio/core/core/ResourceBudget'
import { BudgetRebalancer } from '@akemi-mio/core/core/BudgetRebalancer'
...
import { EventArchiver } from '@akemi-mio/core/core/evaluation/EventArchiver'
import { RetentionScheduler } from '@akemi-mio/core/core/evaluation/RetentionScheduler'
import { UIBridge } from '@akemi-mio/intelligence/agent/UIBridge'
import { EventStore } from '@akemi-mio/core/core/event-sourcing/EventStore'
import { RuntimeHealthManager } from '@akemi-mio/monitoring/health/RuntimeHealthManager'
...
export class AppRuntime {
  ...
  private guardrailDecisionStore?: GuardrailDecisionStore
  ...
  private componentRegistry?: import('@akemi-mio/intelligence/runtime/ComponentRegistry').ComponentRegistryImpl
  private checkpointManager?: import('@akemi-mio/intelligence/runtime/SqliteCheckpointManager').SqliteCheckpointManager
  private runtimeRestoreService?: import('@akemi-mio/intelligence/runtime/RuntimeRestoreService').RuntimeRestoreServiceImpl
  private feedbackLoop?: import('@akemi-mio/evolution-feedback-loop').MCPFeedbackLoopService
  ...
  private behaviorMemoryAnalyzer?: import('@akemi-mio/evolution/behavior/BehaviorDrivenMemoryAnalyzer').BehaviorDrivenMemoryAnalyzer
  ...
  private metricsStore?: import('@akemi-mio/core/core/evaluation/GuardrailMetricsStore').GuardrailMetricsStore
  private metricsProjection?: import('@akemi-mio/core/core/evaluation/GuardrailMetricsProjection').GuardrailMetricsProjection
  private metricsQueryRef: ServiceRef<import('@akemi-mio/core/core/evaluation/GuardrailMetricsQueryService').GuardrailMetricsQueryService> =
    createServiceRef()
  ...
  private hjgo2claudeRestarts = 0
  private hjgo2claudeRestartTimer: ReturnType<typeof setTimeout> | null = null
  private hjgo2claudeStopping = false
  ...
  async start(): Promise<void> {
    ...
    mcpRegistry.register({
      ...
      capabilitySchemas: {
        'browser.automation': {
          type: 'object',
          properties: {
            action: { type: 'string', description: 'Browser action to perform: navigate, click, extract, screenshot' },
            url: { type: 'string', description: 'Target URL for navigate actions' },
            selector: { type: 'string', description: 'Target selector for click or extract actions' },
            text: { type: 'string', description: 'Optional text payload for browser interactions' },
          },
          required: ['action'],
        },
        'web.scraping': {
          type: 'object',
          properties: {
            url: { type: 'string', description: 'Target page URL to scrape' },
            selector: { type: 'string', description: 'Optional selector to narrow scraped content' },
          },
          required: ['url'],
        },
      },
      dependencies: [
        ...
        // M5.6.5 coverage remediation: Playwright evaluate tool
        { capability: 'browser.automation', tool: 'browser_evaluate' },
      ],
      permissions: ['browser'],
    })
    mcpRegistry.register({
    ...
    mcpRegistry.register({
      ...
      dependencies: [
        { capability: 'publishing', tool: 'fanqie_publish_novel' },
        // M5.6.5 coverage remediation: writing + blog session tools
        { capability: 'publishing', tool: 'writing_system' },
        { capability: 'publishing', tool: 'blog_start_session' },
        { capability: 'publishing', tool: 'blog_list_sessions' },
        { capability: 'publishing', tool: 'blog_memory_stats' },
      ],
      permissions: ['network.http'],
    })
    ...
    mcpRegistry.register({
      ...
      capabilitySchemas: {
        'search.retrieval': {
          type: 'object',
          properties: {
            operation: { type: 'string', description: 'Search operation: grep or files' },
      ...
      dependencies: [
        ...
        { capability: 'search.retrieval', tool: 'search_files' },
        // M5.6.5 coverage remediation: grep_search alias + memory / ASR queries
        { capability: 'search.retrieval', tool: 'grep_search' },
        { capability: 'search.retrieval', tool: 'search_memories' },
        { capability: 'search.retrieval', tool: 'retrieve_memory' },
        { capability: 'search.retrieval', tool: 'query_asr_vocabulary' },
    ...
    // 2026-08-12: 联网搜索工具，拆分为独立 capability（web.search / web.fetch）
    mcpRegistry.register({
      id: 'web-search',
      name: 'Web Search',
      version: '1.0.0',
    ...
    mcpRegistry.register({
      ...
      dependencies: [
        ...
        // M5.6.5 coverage remediation: MCP management / planning / task state
        { capability: 'system.execution', tool: 'connect_mcp_server' },
        { capability: 'system.execution', tool: 'list_mcp_servers' },
        { capability: 'system.execution', tool: 'create_dev_plan' },
        { capability: 'system.execution', tool: 'update_plan_progress' },
        { capability: 'system.execution', tool: 'list_plans' },
        { capability: 'system.execution', tool: 'get_user_preferences' },
        { capability: 'system.execution', tool: 'save_task_state' },
      ],
    ...
    import('@akemi-mio/capabilities/capability/adapters')
      .then(({ fanqiePublishAdapter, playwrightAdapter, fileSystemAdapter, searchAdapter, systemAdapter, socialPublishAdapter }) => {
        capabilityService.setAdapter('fanqie-publish', 'fanqie_publish_novel', fanqiePublishAdapter)
        capabilityService.setAdapter('social-publish', 'social_publish', socialPublishAdapter)
        ...
        capabilityService.setAdapter('search-engine', 'grep', searchAdapter)
        capabilityService.setAdapter('search-engine', 'search_files', searchAdapter)
        // 2026-08-12: web search / web fetch capabilities
    ...
    try {
      ...
      try {
        require('fs').unlinkSync(staleConfig)
      } catch {}
    ...
    try {
      ...
      const fanqieAuthorUrl = credentialsManager.get('fanqie_author_url') || process.env.FANQIE_AUTHOR_URL
      mcpManager
        .addServer({
          name: 'fanqie-publish',
          transport: 'stdio',
          command: 'node',
          args: [fanqieMcpPath],
          env: (fanqieAuthorUrl ? { ...process.env, FANQIE_AUTHOR_URL: fanqieAuthorUrl } : { ...process.env }) as Record<string, string>,
        })
    ...
    import('@akemi-mio/intelligence-learning/LearningAsrBridge')
    ...
    // ── 预检测网络状态（后台异步，不阻塞启动） ──
    import('@akemi-mio/audio/NetworkMonitor').then(({ networkMonitor }) => {
    ...
    const { setSubAgentPool } = await import('@akemi-mio/capabilities/tool/definitions/SkillAgentTools')
    ...
    const { ComponentRegistryImpl } = await import('@akemi-mio/intelligence/runtime/ComponentRegistry')
    const { WorkflowRuntimeCheckpointableComponent } = await import('@akemi-mio/intelligence/runtime/WorkflowRuntimeCheckpointableComponent')
    ...
    // 社交平台旧版明文凭据迁移（设计 4.4）：.creds.json → CredentialsManager，成功后删除明文文件
    try {
      const socialCredsPath = join(app.getPath('userData'), 'evolution_workspace', 'social', 'config', '.creds.json')
      credentialsManager.migrateFromSocialCreds(socialCredsPath)
    } catch (err) {
      log('WARN', 'social_creds_migration_failed', { error: String(err) })
    }
    if (app.isPackaged) {
      llmService.refreshFromCredentials((key) => credentialsManager.get(key))
      log('INFO', 'llm_config_loaded_from_credentials')
      if (!credentialsManager.get('llm_key')) {
        log('WARN', 'missing_api_key')
      }
    }
    ...
    const { SqliteCheckpointManager } = await import('@akemi-mio/intelligence/runtime/SqliteCheckpointManager')
    this.checkpointManager = new SqliteCheckpointManager()

    const { RuntimeRestoreServiceImpl } = await import('@akemi-mio/intelligence/runtime/RuntimeRestoreService')
    const { RuntimeRecoveryActivator } = await import('@akemi-mio/intelligence/runtime/RuntimeRecoveryActivator')
    this.runtimeRestoreService = new RuntimeRestoreServiceImpl(this.checkpointManager, this.componentRegistry!)
    ...
    this.guardrailDecisionStore = guardrailDecisionStore
    ...
    const { GuardrailMetricsProjection } = await import('@akemi-mio/core/core/evaluation/GuardrailMetricsProjection')
    ...
    const { GuardrailMetricsQueryService } = await import('@akemi-mio/core/core/evaluation/GuardrailMetricsQueryService')
    ...
    {
      const { behaviorPredictiveMemoryPrewarmer } = await import('@akemi-mio/evolution/behavior/BehaviorPredictiveMemoryPrewarmer')
      behaviorPredictiveMemoryPrewarmer.setDependencies({
        ...
        getBehaviorWeightedEntries: (tier?: string, limit?: number) => memoryService.getBehaviorWeightedEntries(tier as any, limit),
    ...
    {
      const { behaviorPredictionMemoryEngine } = await import('@akemi-mio/evolution/behavior/BehaviorPredictionMemoryEngine')
      ...
      this.subs.add(
        eventBus.on('behavior.state.updated' as any, (state: any) => {
    ...
    const { PlanSchedulerCoordinator, createAndStartSchedulerNotificationBridge } = await import('@akemi-mio/evolution-plan-scheduler')
    const { setPlanSchedulerCoordinator } = await import('@akemi-mio/capabilities/tool/deps')
    const planSchedulerCoordinator = new PlanSchedulerCoordinator(
      ...
      agentService['toolScheduler'],
      {
        ...
        log: (level, msg, meta) => log(level as any, msg, meta),
      },
      {
    ...
    {
      const { ToolChainOrchestrator, ToolChainDecomposer, OrchestrationBridge } = await import('@akemi-mio/intelligence/orchestrator')
      const { setToolChainOrchestrator } = await import('@akemi-mio/capabilities/tool/deps')
      const mcpManager = (llmService as any).mcpManager
      const decomposer = new ToolChainDecomposer(llmService, () => {
        try {
          return (
            mcpManager?.getAllSchemas()?.map((s: any) => ({
    ...
    voiceBookmarkSvc.setSynthesizeFn(async (text: string, outputPath: string): Promise<boolean> => {
      try {
        ...
        const { ttsPiperBridge } = await import('@akemi-mio/audio/TtsPiperBridge')
    ...
    try {
      const { IndustrialOdeLayer } = await import('@akemi-mio/blog-publish/gongye-songge/IndustrialOdeLayer')
      const { parseFeaturesFromEnv } = await import('@akemi-mio/blog-publish/gongye-songge/types')
    ...
    try {
      const { initBlogMemoryRecorder, initBlogMemoryRetriever } = await import('@akemi-mio/intelligence-memory/plan-memory-blog')
      const { setBlogMemoryRecorder, setBlogMemoryRetriever } = await import('@akemi-mio/capabilities/tool/deps')
    ...
    try {
      const { initExperienceMemoryService } = await import('@akemi-mio/intelligence-memory/plan-memory-blog')
    ...
    try {
      const { blogModeService } = await import('@akemi-mio/intelligence/agent/blog/BlogModeService')
      const { setBlogModeService } = await import('@akemi-mio/capabilities/tool/deps')
    ...
    try {
      const { WorkspaceCleanupLayer } = await import('@akemi-mio/evolution-workspace-cleanup/WorkspaceCleanupLayer')
      const { parseFeaturesFromEnv } = await import('@akemi-mio/evolution-workspace-cleanup/types')
    ...
    const kernelModule: IModule = {
      name: 'syscall',
      prefix: 'packages/core/src/core/lifecycle/',
      hotReloadable: false,
      exports: ['SyscallBus'],
    ...
    // M5.5: 注入 CapabilityResolver，用于 postCall 的 capability identity 解析
    mcpManager.setCapabilityResolver(capabilityResolver)
    ...
    const { behaviorDrivenMemoryAnalyzer: bdma } = await import('@akemi-mio/evolution/behavior/BehaviorDrivenMemoryAnalyzer')
    ...
    const { LLMKnowledgeExtractor } = await import('@akemi-mio/intelligence-memory/extractors/LLMKnowledgeExtractor')
    ...
    // 社交平台自动发布（每分钟检查 content_calendar.yaml，走 publishing 能力）
    const socialPublishScheduler = new SocialPublishScheduler(capabilityService)
    this.taskRunner.register('social.tick', () => socialPublishScheduler.tick(), 60000, { cooldownMs: 30000 })

    // 休眠编排检测（每 10 分钟检查用户是否 60 分钟无交互）
    this.taskRunner.register('sleep.orchestration', () => sleepOrchestrator.tick(), 10 * 60 * 1000, { cooldownMs: 60 * 1000 })
    ...
    // 注册到清理函数防止泄漏
    const { addGlobalDisposer } = await import('@akemi-mio/core/core/Lifecycle')
  ...
  private startHjgo2claude(): void {
    ...
    if (this.hjgo2claudeRestartTimer) {
      clearTimeout(this.hjgo2claudeRestartTimer)
      this.hjgo2claudeRestartTimer = null
    }

    try {
      // Windows 下 hjgo2claude 以 .cmd shim 安装，直接 spawn 裸命令会 ENOENT，需经 cmd.exe 启动
      const isWindows = process.platform === 'win32'
      const child = isWindows
        ? spawn('cmd', ['/c', 'hjgo2claude', 'serve', '--port', '1841'], {
            env: { ...process.env, OPENCODE_API_KEY: opencodeKey },
            stdio: ['ignore', 'pipe', 'pipe'],
            windowsHide: true,
          })
        : spawn('hjgo2claude', ['serve', '--port', '1841'], {
            env: { ...process.env, OPENCODE_API_KEY: opencodeKey },
            stdio: ['ignore', 'pipe', 'pipe'],
            windowsHide: true,
          })
      ...
      child.on('exit', (code, signal) => {
        ...
        this.hjgo2claudeProcess = null
        // 代理意外退出后自动重启（上限 5 次），避免创意执行器长期不可用
        if (!this.hjgo2claudeStopping) {
          if (this.hjgo2claudeRestarts < 5) {
            this.hjgo2claudeRestarts++
            this.hjgo2claudeRestartTimer = setTimeout(() => this.startHjgo2claude(), 30_000)
            log('WARN', 'hjgo2claude_restart_scheduled', { restarts: this.hjgo2claudeRestarts })
          } else {
            log('WARN', 'hjgo2claude_restart_limit', { restarts: this.hjgo2claudeRestarts })
          }
        }
      })
      ...
      // 进程稳定存活 5 分钟后重置重启计数
      this.hjgo2claudeRestartTimer = setTimeout(
        () => {
          this.hjgo2claudeRestarts = 0
        },
        5 * 60 * 1000,
      )
  ...
  private stopHjgo2claude(): void {
    this.hjgo2claudeStopping = true
    if (this.hjgo2claudeRestartTimer) {
      clearTimeout(this.hjgo2claudeRestartTimer)
      this.hjgo2claudeRestartTimer = null
    }
    const proc = this.hjgo2claudeProcess
    if (!proc) return
    try {
      const pid = proc.pid
      if (pid) {
        if (process.platform === 'win32') {
          // cmd.exe 包装进程：taskkill /T 连子进程一起终止，避免 hjgo2claude 孤儿进程占用 1841 端口
          spawn('taskkill', ['/pid', String(pid), '/T', '/F'], { windowsHide: true, stdio: 'ignore' })
        } else {
          proc.kill('SIGTERM')
        }
        log('INFO', 'hjgo2claude_stopped', { pid })
      }
    } catch (err: any) {
        ...
        setTimeout(async () => {
          try {
            ...
            if (result.archive.length > 0 || result.retention.some((r) => r.deletedCount > 0)) {
              const { markDirty } = require('@akemi-mio/core/db/connection')
              markDirty()
            }
  ...
  private registerLazyServices(
    ...
    this.lazyInit!.add({
      ...
      fn: async () => {
        if (process.env.EVOLUTION_SERVICE_DISABLED === 'true') {
          ...
          // 确保被其他模块引用的子目录仍存在
          const { ensureEssentialDirs } = await import('@akemi-mio/evolution-core')
          ensureEssentialDirs()
          return
        }
        ...
        const { PluginServiceLoader, WallpaperPlugin, PluginCollectorAdapter, PluginExecutorAdapter } = await import('@akemi-mio/evolution-plugin-system')
        ...
        const { PiperEvolutionPlugin } = await import('@akemi-mio/evolution-piper-evolution')
        ...
        const { MemoryOptimizationCollector, MemoryOptimizationExecutor } = await import('@akemi-mio/evolution/automation')
        ...
        const { BehaviorCollector, BehaviorOptimizationExecutor } = await import('@akemi-mio/evolution/automation')
        ...
        import('@akemi-mio/audio/typing/TypographyAsrBridge')
        ...
        // 注册行为记录钩子（增强时序数据采集）
        const { registerBehaviorRecordHook } = await import('@akemi-mio/evolution/user-behavior/BehaviorFeatureExtractor')
        ...
        // ★ 自进化计划优化器初始化
        {
          const { planOptimizerEngine, planOptimizerCollector, planOptimizerExecutor } = await import('@akemi-mio/evolution-plan-organizer')
          // 注入 LLM 分析能力
          planOptimizerEngine.setAnalyzeFn((prompt: string, opts?: any) => llmService.chatJson(prompt, opts))
        ...
        const { PolicyDecisionObserver } = await import('@akemi-mio/core/core/evaluation/observers/PolicyDecisionObserver')
        if (this.evaluationEmitter) {
          new PolicyDecisionObserver(this.evaluationEmitter).start()
        }

        // ★ CI/CD Orchestrator 初始化：加载 MCP CI/CD 工具集
        const { CicdOrchestrator } = await import('@akemi-mio/evolution-cicd')
        ...
        const { UserBehaviorLayer } = await import('@akemi-mio/evolution/user-behavior/UserBehaviorLayer')
        const { parseFeaturesFromEnv } = await import('@akemi-mio/evolution/user-behavior/types')
        ...
        if (features.includes('mcp_feedback_loop')) {
          const { createMCPFeedbackLoop } = await import('@akemi-mio/evolution-feedback-loop')
    ...
    this.lazyInit!.add({
      ...
      fn: async () => {
        ...
        try {
          const { planManager } = await import('@akemi-mio/evolution')
        ...
        if (taskPanelRef) taskPanelRef.current = taskPanel
        // 托盘切换回调
        setTaskPanelToggle(() => {
    ...
    this.lazyInit!.add({
      ...
      fn: async () => {
        ...
        const { planTypeScriptExecutor } = await import('@akemi-mio/intelligence-learning/PlanTypeScriptExecutor')
    ...
    this.lazyInit!.add({
      ...
      fn: async () => {
        ...
        service.start()
        if (wallpaperInteractiveRef) wallpaperInteractiveRef.current = service
    ...
    this.lazyInit!.add({
      ...
      delayMs: 2000,
      fn: async () => {
        const { UserBehaviorService } = await import('@akemi-mio/evolution/behavior/UserBehaviorService')
        ...
        const { WallpaperPluginRegistry, UserBehaviorPluginAdapter } = await import('@akemi-mio/platform/wallpaper/plugin')
    ...
    this.lazyInit!.add({
      ...
      delayMs: 5000,
      fn: async () => {
        const { behaviorPeriodicPreloadService } = await import('@akemi-mio/evolution/behavior/BehaviorPeriodicPreloadService')
    ...
    this.lazyInit!.add({
      ...
      delayMs: 3000,
      fn: async () => {
        const { behaviorActionCounter } = await import('@akemi-mio/evolution/behavior/BehaviorActionCounter')
  ...
  private buildBehaviorSources(memoryService: MemoryService | null): any[] {
    ...
    const recent = interactions
      ...
      .slice(0, 3)
    const content =
      recent.length > 0 ? `最近交互 ${interactionCount} 次，最近记录：${recent.join(' | ')}` : `最近交互 ${interactionCount} 次`
