...
import { WAKE_WORDS } from '@akemi-mio/core/config'
...
export function registerCredentialsHandlers({ agentService, ttsService }: HandlerContext): void {
  ...
  ipcMain.handle('credentials:set', async (_event, name: string, value: string) => {
    ...
    if (name.startsWith('llm_') && app.isPackaged) {
      try {
        agentService.getLlmService().refreshFromCredentials((k) => credentialsManager.get(k))
