...
import { audioFeatureExtractor } from '@akemi-mio/audio/AudioFeatureExtractor'
import { atmosphereMapper } from '@akemi-mio/audio/AtmosphereMapper'
import { inspirationService } from '@akemi-mio/creativity/writing/InspirationService'
import { voiceContinuationService } from '@akemi-mio/creativity/writing/VoiceContinuationService'
...
export function registerWritingHandlers(_ctx: HandlerContext): void {
  ...
  ipcMain.handle('writing:inspiration:process', async (_event, text: string) => {
    try {
      return await inspirationService.process(text)
    } catch (err) {
  ...
  ipcMain.handle(
    ...
    async (_event, params: { storyName: string; chapterNum: number; userVoiceText?: string; atmosphere?: any }) => {
      try {
  ...
  ipcMain.handle('writing:continuation:init', async (_event, params: { storyName: string; chapterNum: number }) => {
    try {
      return await voiceContinuationService.initializeContext(params.storyName, params.chapterNum)
    } catch (err) {
