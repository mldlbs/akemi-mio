...
import { ANALYSIS_PROMPT, detectPlanMode, pickBestPlan, buildPlanInjection } from '@akemi-mio/evolution-core'
...
import { formatExecutionGoalContext } from '@akemi-mio/evolution-goals'
...
export class EvolutionAnalyzer implements ISubsystem {
  ...
  async analyze(input: AnalysisInput): Promise<AnalysisResult> {
    const prompt =
      ...
      input.strategyCtx +
      (input.creativityCtx ? '\n' + input.creativityCtx : '') +
      this.buildExecutionGoalContext()
  ...
  async hasPendingSocialTask(): Promise<boolean> {
    try {
      ...
      const calendarPath = join(socialDir, 'config', 'content_calendar.yaml')
  ...
  private buildExecutionGoalContext(): string {
    try {
      const stats = executionGoalStore.getStats()
      const byMethodology = executionGoalStore.getMethodologyStats()
      return formatExecutionGoalContext(stats, byMethodology)
    } catch {
      return ''
    }
  }
