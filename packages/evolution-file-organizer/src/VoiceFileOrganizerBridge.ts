...
export class VoiceFileOrganizerBridge {
  ...
  private async listenForCommand(): Promise<string> {
    ...
    try {
      ...
      // 2. 超时后返回空字符串
      const text = await Promise.race([this.waitForVoiceCommand(), this.timeout(this.config.listenTimeoutMs, '语音监听超时')])
  ...
  private async confirmHighRiskOperation(intent: VoiceFileIntent): Promise<boolean> {
    ...
    // 确认超时保护
    try {
      const response = await Promise.race([this.waitForVoiceConfirmation(), this.timeout(this.config.confirmTimeoutMs, '确认超时')])
   ...
   * 将解析出的意图映射到具体的文件操作。
   */
  private async executeVoiceIntent(intent: VoiceFileIntent, files: FileFeatures[]): Promise<VoiceFileOrganizerResult> {
    ...
    for (const file of matchedFiles) {
      try {
        ...
        const ruleTarget = bestRule ? genePool.expandTargetTemplate(bestRule.action.target, file) : null
    ...
    const summary =
      failCount > 0 ? `已整理 ${successCount} 个文件，${failCount} 个失败` : `已成功整理 ${successCount} 个文件到 ${targetDir}`
  ...
  private matchFiles(intent: VoiceFileIntent, files: FileFeatures[]): FileFeatures[] {
    ...
    if (inferred) {
      ...
      try {
        const globMatch = files.filter((f) => {
          try {
            return minimatch(f.name, inferred, { dot: true })
          } catch {
            return false
          }
        })
    ...
    if (descWords.length > 0) {
      const fuzzy = files.filter((f) => {
        ...
        return descWords.some((w) => lowerName.includes(w) || lowerStem.includes(w) || f.extension.includes(w))
      })
