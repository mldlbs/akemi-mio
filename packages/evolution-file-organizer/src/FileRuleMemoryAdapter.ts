...
export class FileRuleMemoryAdapter {
   ...
   */
  saveRule(filePattern: string, targetPath: string, description: string, sourceText?: string): string | null {
    ...
    const saved = ms.getEntries().find((e) => e.type === 'user_fact' && e.content === content)
    if (saved) {
  ...
  getAllRules(): FileRuleEntry[] {
    ...
    return ms
      .getEntries()
      .filter((e) => e.type === 'user_fact' && e.content.startsWith(RULE_PREFIX) && e.structuredData)
      .map((e) => {
        try {
