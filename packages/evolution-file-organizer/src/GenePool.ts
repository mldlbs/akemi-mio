...
import type { OrganizerRule, FileFeatures, GenePoolData, EvolutionConfig, RuleCondition, RuleAction, ConditionOperator } from './types'
...
export class GenePool {
  ...
  findMatchingRules(features: FileFeatures): OrganizerRule[] {
    return this.rules.filter((rule) => this.matchesAllConditions(rule.conditions, features)).sort((a, b) => b.fitness - a.fitness)
  }
  ...
  private crossover(parentA: OrganizerRule, parentB: OrganizerRule): OrganizerRule {
    ...
    const conditions: RuleCondition[] = [...parentA.conditions.slice(0, splitPoint), ...parentB.conditions.slice(splitPoint)]
    // 去重
    const uniqueConditions = this.dedupeConditions(conditions)

    const action: RuleAction = Math.random() < 0.5 ? { ...parentA.action } : { ...parentB.action }
