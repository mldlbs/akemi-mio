...
import type { Insight, DetectionContext, InsightTriggerPolicy, ReturnReport, InsightStoreLike } from './types'
import { DEFAULT_TRIGGER_POLICY, MIN_REPORT_SCORE, MIN_REPORT_CONFIDENCE } from './types'

export interface TaskRunner {
  register(type: string, handler: () => Promise<{ success: boolean }>, intervalMs: number): void
  stopType(type: string): void
}
