...
export interface DetectionContext {
  memoryEntries: { type: string; content: string; createdAt: number }[]
  summaries: string[]
  interactionCount: number
  plans: { title: string; status: string; updatedAt: number; steps: { status: string }[]; createdAt: number }[]
  eventCount: number
}
...
export const MIN_REPORT_SCORE = 50
export const MIN_REPORT_CONFIDENCE = 0.7
