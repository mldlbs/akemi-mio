...
import type { FixExecutor, AssignedProblem, FixResult, ProblemSource } from '@akemi-mio/evolution/automation/types'
...
export class PlanOptimizerExecutor implements FixExecutor {
    ...
    try {
      ...
      if (!canAutoExecute) {
        ...
        return {
          ...
          success: true,
          summary:
            `优化建议 "${suggestion.title}" 需人工审批（置信度 ${(suggestion.confidence * 100).toFixed(0)}%，` +
  ...
  private async applyOptimization(
    ...
    const snapshot = planOptimizerSnapshotManager.createSnapshot(allPlans, `${suggestion.suggestedAction}: ${suggestion.title}`)
   ...
   * 合并任务：将多个任务合并为一个。
   */
  private applyMerge(suggestion: OptimizationSuggestion, snapshotId: string): { success: boolean; summary: string; detail?: string } {
    if (!this.planManager) {
   ...
   */
  private applyDelete(suggestion: OptimizationSuggestion, snapshotId: string): { success: boolean; summary: string; detail?: string } {
    if (!this.planManager) {
      return { success: false, summary: 'PlanManager 不可用' }
    }
   ...
   */
  private applyReorder(suggestion: OptimizationSuggestion, snapshotId: string): { success: boolean; summary: string; detail?: string } {
    return {
      success: false,
