...
import type { UserBehaviorFeature } from '@akemi-mio/evolution/user-behavior/types'
...
export class PlanExperiment42Plugin {
    ...
    try {
      ...
      if (this.config.debug) {
        log('DEBUG', `${LOG_TAG}_pre_ctx_detail`, {
          ...
          planStepProgress: activePlan ? `${activePlan.steps.filter((s) => s.status === 'done').length}/${activePlan.steps.length}` : null,
        })
      }
    } catch (err: any) {
  ...
  generateReport(): ExperimentReport {
    ...
    const collectionWindow: ExperimentReport['collectionWindow'] =
      allTs.length > 0 ? { from: Math.min(...allTs), to: Math.max(...allTs) } : { from: this.startTime, to: Date.now() }

    const planOverrideRate = this.totalDecisionCount > 0 ? this.planOverrideCount / this.totalDecisionCount : 0
    const conflictRate = this.totalConcurrencyChecks > 0 ? this.conflictCount / this.totalConcurrencyChecks : 0
