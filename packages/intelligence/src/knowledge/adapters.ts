...
import type { ReflectLoop } from '@akemi-mio/intelligence/agent/ReflectLoop'
import type { FailureAnalyzer } from '@akemi-mio/intelligence/agent/FailureAnalyzer'
import type { IMemoryPlugin, MemoryRetrievalResult } from '@akemi-mio/intelligence-memory/IMemoryPlugin'
...
export class MemoryPluginAdapter implements IKnowledgeSource {
  ...
  async query(query: string, options?: QueryOptions): Promise<KnowledgeItem[]> {
    ...
    if (options?.minScore !== undefined) {
      return results.filter((r) => r.score >= options.minScore!).map((r) => toKnowledgeItem(r, this.name))
    }
  ...
  async query(query: string, options?: QueryOptions): Promise<KnowledgeItem[]> {
    ...
    const results: KnowledgeItem[] = hot
      .filter((p) => {
        ...
        return q.length === 0 || allNames.some((n) => n.toLowerCase().includes(q)) || p.errors.some((e) => e.toLowerCase().includes(q))
      })
      .map((p, i) => ({
