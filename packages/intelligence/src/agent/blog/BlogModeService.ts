...
import { DEFAULT_HABIT_PROFILE } from './types'
...
export class BlogModeService {
  ...
  async checkAndAutoSwitch(
    ...
    // 构建简化的 profile
    const profile: WritingHabitProfile = {
    ...
    // 让 monitor 评估是否需要切换
    const { shouldSwitch, recommendation } = blogModeMonitor.shouldSwitch(currentMode, sessionState.topic, userInput, profile)
  ...
  getModeReport(sessionId: string): {
    ...
    return {
      ...
      recentSwitches: this.auditLog.filter((entry) => entry.sessionId === sessionId).slice(-10),
    }
  }
