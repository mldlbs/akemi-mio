 ...
 */
function computeCompositeScore(scores: BlogDimensionScores, input: BlogAssessmentInput): number {
  const baseScore =
 ...
 */
function computeStageBonus(scores: BlogDimensionScores, currentStage?: BlogStage): number {
 ...
 */
function evaluateContentDepth(content: string, topic?: string, audienceLevel?: string): number {
...
function evaluateCodeQuality(content: string): number {
  ...
  if (reasonableRatio >= 0.7) score += 0.1

  // 3. 有行内代码引用（在正文中对代码进行解释）
  const inlineCodes = content.match(/`[^`\n]+`/g) || []
  if (inlineCodes.length >= codeBlocks.length) score += 0.1
  ...
  if (explainedBlocks >= codeBlocks.length * 0.6) score += 0.1
...
function evaluateReadability(content: string): number {
  ...
  // 2. 空洞副词检测（减分项）
  const emptyAdverbs = [/非常(地|之)?/, /极其/, /十分/, /很(大|小|多|少|好|坏)/g, /超级/, /特别(地)?/]
 ...
 * 检查关键词覆盖、标题优化、元描述完整性。
 */
function evaluateSEORelevance(content: string, topic?: string, platform?: string): number {
  if (!content) return 0.5
  ...
  if (topic) {
    ...
    if (topicWords.length > 0) {
      ...
      const parts = [content.slice(0, third), content.slice(third, 2 * third), content.slice(2 * third)]
 ...
 */
function evaluatePublishReadiness(content: string, platform?: string): number {
  if (!content) return 0.4
  ...
  if (hasTitle && hasBody) score += 0.1
  else if (!hasTitle) score -= 0.1
  ...
  if (platform) {
    ...
    if (platform === '公众号' || platform === '微信公众号') {
      const hasLead = content.split('\n').filter((l) => l.trim()).length > 10 && content.length > 800
  ...
  if (placeholders.test(content)) score -= 0.1
 ...
 */
function detectIssues(content: string, scores: BlogDimensionScores): BlogWritingIssue[] {
