...
import type { BlogExecutionMode, InputFeatureProfile, ModeRecommendation } from './BlogExecutionMode'
import { evaluateInputFeatures, MCP_OPTIMAL_CONDITIONS, PLAN_CHAIN_OPTIMAL_CONDITIONS } from './BlogExecutionMode'
...
export class BlogModeMonitor {
   ...
   */
  recommendMode(topic: string, userInput: string, profile?: WritingHabitProfile): ModeRecommendation {
  ...
  shouldSwitch(
    ...
    const shouldSwitch = recommendation.recommendedMode !== currentMode && recommendation.confidence >= SWITCH_CONFIDENCE_THRESHOLD
