...
export class BlogAnalyticsTracker {
  ...
  recordPostPerformance(data: Omit<PostPerformance, 'engagementRate'>): void {
    const engagementRate = data.views > 0 ? (data.likes + data.comments + data.shares) / data.views : 0

    const existingIdx = this.posts.findIndex((p) => p.postId === data.postId && p.platform === data.platform)
  ...
  generateReport(): AnalyticsReport {
    ...
    const bestPlatform =
      platformPerformance.length > 0
        ? platformPerformance.reduce((a, b) => (a.avgEngagementRate > b.avgEngagementRate ? a : b)).platform
        : '未知'

    // 表现最好的类别
    const topCategory =
      categorySummary.length > 0 ? categorySummary.reduce((a, b) => (a.performanceScore > b.performanceScore ? a : b)).category : '未知'

    // 生成洞察
    const insights = this.generateInsights(posts, categorySummary, platformPerformance, bestTimeSlots)
  ...
  adjustStrategy(plannedTopics?: string[]): StrategyOutput {
    ...
    if (!report.hasSufficientData) {
      const output: StrategyOutput = {
        timestamp: Date.now(),
        dataDriven: false,
        recommendedTopics: plannedTopics?.slice(0, 3) ?? ['技术教程与指南', '项目经验分享', '工具推荐与评测'],
        adjustments: [],
        avoidTopics: [],
    ...
    if (plannedTopics) {
      for (const topic of plannedTopics) {
        if (!recommendedTopics.includes(topic) && !avoidTopics.includes(topic)) {
          const matchingCat = report.categorySummary.find((c) => topic.toLowerCase().includes(c.category.toLowerCase()))
    ...
    const output: StrategyOutput = {
      ...
      adjustments,
      avoidTopics,
      summary: this.buildStrategySummary(recommendedTopics, avoidTopics, adjustments, report),
    }
  ...
  private buildCategorySummary(posts: PostPerformance[]): CategoryPerformanceSummary[] {
    ...
    for (const [category, categoryPosts] of categoryMap) {
      ...
      const totalEngagements = categoryPosts.reduce((sum, p) => sum + p.likes + p.comments + p.shares, 0)
      ...
      const performanceScore =
        normalizedViews * VIEWS_WEIGHT + avgEngagementRate * ENGAGEMENT_WEIGHT + Math.min(1, categoryPosts.length / 10) * RECENCY_WEIGHT
  ...
  private buildTimeSlotAnalysis(posts: PostPerformance[]): TimeSlotPerformance[] {
    ...
    for (const [hour, hourPosts] of hourMap) {
      ...
      const normalizedViews = Math.min(1, avgViews / 1000)
      const score =
        normalizedViews * VIEWS_WEIGHT + avgEngagementRate * ENGAGEMENT_WEIGHT + Math.min(1, hourPosts.length / 10) * RECENCY_WEIGHT
  ...
  private generateInsights(
    ...
    const overallEngagement = posts.length > 0 ? totalEngagements / posts.length : 0

    insights.push(`累计发布 ${posts.length} 篇文章，总阅读 ${totalViews}，平均每篇文章互动 ${overallEngagement.toFixed(1)} 次`)

    // 2. 最佳类别和平台
    if (categorySummary.length > 0) {
      const top = categorySummary[0]
      insights.push(`表现最好的主题类别是「${top.category}」（${top.postCount} 篇，平均阅读 ${top.avgViews}）`)
    }

    if (platformPerformance.length >= MIN_PLATFORMS_FOR_COMPARISON) {
      const best = platformPerformance[0]
      insights.push(`${best.platform} 的互动率最高（${(best.avgEngagementRate * 100).toFixed(1)}%），建议作为内容分发重点`)
    }

    // 3. 时段建议
    if (bestTimeSlots.length > 0) {
      const best = bestTimeSlots[0]
      insights.push(`最佳发布时段为 ${best.hour}:00（平均互动率 ${(best.avgEngagementRate * 100).toFixed(1)}%），建议优先选择该时段发布`)
    }

    // 4. 改进建议
    const lowEngagementPosts = posts.filter((p) => p.engagementRate < 0.01)
    if (lowEngagementPosts.length > posts.length * 0.3) {
      insights.push(`${lowEngagementPosts.length} 篇文章互动率偏低（<1%），建议优化标题吸引力和内容开头段落`)
    }
  ...
  private extractCategory(title: string): string {
    ...
    for (const { category, patterns: regexps } of patterns) {
  ...
  private buildStrategySummary(
    ...
    const lines: string[] = ['📊 选题策略调整报告', `基于 ${report.totalPosts} 篇文章的效果数据`, '']
