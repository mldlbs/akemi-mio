...
import { GoalGuardrail } from '@akemi-mio/evolution-governance'
import { ToolScheduler, type ToolResult } from './ToolScheduler'
import type { TokenAccount } from '../cognitive/TokenEconomy'
import { SkillManager } from '@akemi-mio/intelligence-skill'
...
import {
  ...
  type StoredMessage,
} from '@akemi-mio/core/db/messages'
import { RunState, RunContext, type GovernorRecord } from './runstate'
...
import type { UnifiedKnowledgeQuery } from '@akemi-mio/intelligence/knowledge/UnifiedKnowledgeQuery'
...
import { setPersonaStateManager } from '@akemi-mio/capabilities/tool/deps'
...
import { buildRouteRuntimePrompt } from './routing/prompts'
import { resolveRouteExecutionPolicy, type RouteExecutionPolicy, type ToolChoiceMode } from './routing/routePolicy'
import { GoalPipeline } from '@akemi-mio/evolution-goals'
import { executionGoalStore } from '@akemi-mio/evolution-goals'
import { deriveSuccessCriteria } from '@akemi-mio/evolution-goals'
import { selectMethodology, buildMethodologyHint, getMethodologyById } from '../superpowers'
import type { IntentRouteDecision, RouteInput } from './routing/types'
import { behaviorStateMachine } from '@akemi-mio/evolution/behavior/BehaviorStateMachine'
import type { BehaviorMode } from '@akemi-mio/evolution/behavior/BehaviorStateMachine'
import { correctionPatternLearner } from '@akemi-mio/evolution/behavior/CorrectionPatternLearner'
import { taskOrchestrationModeManager } from '@akemi-mio/evolution/behavior/TaskOrchestrationModeManager'
import { behaviorPreferenceStore } from '@akemi-mio/evolution/behavior/BehaviorPreferenceStore'
import { toolDefaultAdjuster } from '@akemi-mio/evolution/behavior/ToolDefaultAdjuster'
import { buildTtsNeed } from '@akemi-mio/evolution/behavior/UserBehaviorTtsContract'
import { asrHotwordManager } from '@akemi-mio/audio/AsrHotwordManager'
import { sentimentAnalyzer } from '@akemi-mio/audio/SentimentAnalyzer'
...
import type { EmotionTtsParams, UserToneProfile, VoiceStyle, ReplyCategory } from '@akemi-mio/audio/types'
import { toneProfileAnalyzer } from '@akemi-mio/audio/ToneProfileAnalyzer'
import { toneToVoiceMapper } from '@akemi-mio/audio/ToneToVoiceMapper'
import { toneProfileCache } from '@akemi-mio/audio/UserToneProfileCache'
import { voiceStyleMap } from '@akemi-mio/audio/VoiceStyleMap'
import { behaviorEmotionDetector } from '@akemi-mio/audio/BehaviorEmotionDetector'
import type { AgentIngressContext } from './types'
import { userInputEmotionAnalyzer, type UserInputEmotionResult } from '@akemi-mio/audio/UserInputEmotionAnalyzer'
import { contextualTtsAdvisor } from '@akemi-mio/audio/ContextualTtsAdvisor'
import { memoryEmotionBridge } from '@akemi-mio/audio/MemoryEmotionBridge'
import { memoryTtsBridge } from '@akemi-mio/audio/MemoryTtsBridge'
import { userContextClassifier } from '@akemi-mio/audio/UserContextClassifier'
import { implicitFeedbackTracker } from '@akemi-mio/audio/ImplicitFeedbackTracker'
...
import type {
  ...
  VoiceEmotionLabel,
} from '@akemi-mio/audio/types'
import { VOICE_EMOTION_TTS_MAP } from '@akemi-mio/audio/types'
import type { VoiceEmotion } from '@akemi-mio/audio/types'
import type { McpAgentHybridPipeline } from '../hybrid/McpAgentHybridPipeline'
import { narrativeEmotionController, type StyledTtsSegment } from '@akemi-mio/audio/emotion'
import { TaskStepRecorder } from '@akemi-mio/intelligence-memory/TaskStepRecorder'
import { SessionMemory, sessionMemory as defaultSessionMemory } from '@akemi-mio/intelligence-memory/SessionMemory'
...
export class ChatExecutor {
  ...
  private runtimeManager: import('@akemi-mio/intelligence/runtime/RuntimeManagerImpl').RuntimeManagerImpl | null
  ...
  /** 当前会话近期工具名，用于语义路由 */
  private currentSessionRecentToolNames: string[] = []
  ...
  /** ADR-008: 工具策略规划器 */
  private toolPolicyPlanner = new ToolPolicyPlanner(() => userBehaviorAnalyzer.getRecentToolCallCount())
  /** ADR-008: 工具策略→Prompt 转换器 */
  private toolPromptAssembler = new ToolPromptAssembler()
  private intentRouter: { route(input: RouteInput, requestId?: string): Promise<IntentRouteDecision> } | null = null
  /** M6.1 current execution goal id (exposed to AgentService recovery snapshot) */
  private goalPipeline = new GoalPipeline(executionGoalStore)
  /** M6.1 当前轮执行目标 id（由路由激活，绑定到 RunContext） */
  private currentGoalId: string | null = null
  /** M6.2 current methodology id, re-injected into toolLoop every round */
  private currentMethodologyId: string | null = null
  ...
  constructor(
    ...
    runtimeManager: import('@akemi-mio/intelligence/runtime/RuntimeManagerImpl').RuntimeManagerImpl | null,
    ...
    this.intentRouter = new IntentRouter(llmService as any)
  }
  ...
  /** M6.1: 将当前活跃执行目标标记为 abandoned（用户停止等外部终止）。 */
  abandonActiveGoal(reason: string): void {
    const goalId = this.currentGoalId
    if (!goalId) return
    try {
      this.goalPipeline.abandonGoal(goalId, reason)
    } catch (err) {
      log('WARN', 'execution_goal_abandon_failed', { error: String(err), id: goalId })
    } finally {
      this.currentGoalId = null
      this.currentMethodologyId = null
    }
  }

  /** 上一轮未终结的执行目标被新请求取代（仅限同一 session，避免跨会话误伤）。 */
  private abandonSupersededGoal(sessionId: string): void {
    const goalId = this.currentGoalId
    if (!goalId) return
    try {
      const goal = executionGoalStore.get(goalId)
      if (!goal) return
      if (goal.sessionId !== null && goal.sessionId !== sessionId) return
      this.goalPipeline.abandonGoal(goalId, 'superseded_by_new_request')
    } catch (err) {
      log('WARN', 'execution_goal_supersede_failed', { error: String(err), id: goalId })
    }
  }
  ...
  private refreshMemory(sessionMemCtx?: InjectionContext): void {
    ...
    if (sessionMemCtx?.text) {
      memCtx = memCtx ? `${memCtx}\n\n${sessionMemCtx.text}` : sessionMemCtx.text
    }
  ...
  /** 缓存的记忆情感上下文（供 IPC 展示） */
  private lastMemoryEmotionContext: import('@akemi-mio/audio/MemoryEmotionBridge').AggregatedEmotionContext | null = null
  ...
  async run(
    ...
    const t0 = Date.now()
    // Route decisions must see the active session, not stale state from the previous one.
    ...
    if (effectiveSessionId !== this.currentSessionId) {
      ...
      this.workingMemory = new WorkingMemory('chat')
      this.currentSessionRecentToolNames = []
      userBehaviorAnalyzer.reset()
      try {
        this.refreshMemory()
      } catch (err) {
        log('WARN', 'refresh_memory_skipped', { error: String(err) })
      }
      const history = getMessagesBySession(effectiveSessionId)
      userBehaviorAnalyzer.loadFromStoredMessages(history)
    ...
    const routeInput: RouteInput = {
      ...
      hasProjectContext:
        this.currentDomainLabel === 'code' ||
        this.currentDomainLabel === 'task' ||
        this.currentScene === 'code_debugging' ||
        this.currentScene === 'task_execution' ||
        this.currentScene === 'system_evolution',
      recentToolNames: this.currentSessionRecentToolNames.slice(-5),
    }
    try {
      this.currentRouteDecision = (await this.intentRouter?.route(routeInput, rid)) ?? {
        route: 'observe_first',
        confidence: 0,
        reason: 'router unavailable; uncertain intent defaults to observation',
      }
    } catch (err) {
      log('WARN', 'intent_route_failed', { request_id: rid, error: String(err) })
      this.currentRouteDecision = {
        route: 'observe_first',
        confidence: 0,
        reason: 'router failed; uncertain intent defaults to observation',
      }
    }
    // M6.1: 目标型路由 → 创建并激活执行目标（失败仅降级，不影响对话）
    // 新请求取代上一轮未终结的执行目标（同一 session 才处理，避免跨会话误伤）
    this.abandonSupersededGoal(effectiveSessionId)
    this.currentGoalId = null
    this.currentMethodologyId = null
    if (
      this.currentRouteDecision &&
      (this.currentRouteDecision.route === 'tool_first' || this.currentRouteDecision.route === 'tool_required')
    ) {
      try {
        const activePlan = this.planManager?.getActivePlan?.()
        const llmCriteria = this.currentRouteDecision?.successCriteria
        const criteria = llmCriteria && llmCriteria.length > 0 ? llmCriteria : deriveSuccessCriteria(text)
        const methodology = selectMethodology({ objective: text, successCriteria: criteria, route: this.currentRouteDecision.route })
        const goal = this.goalPipeline.createGoal({
          sessionId: effectiveSessionId,
          objective: text,
          // M6.2: LLM 抽取的 successCriteria 优先，失败/缺失时回退到确定性推导
          successCriteria: criteria,
          planId: activePlan?.id ?? null,
          methodology: methodology.id,
        })
        this.currentGoalId = goal.id
        this.currentMethodologyId = methodology.id
        this.goalPipeline.markExecuting(goal.id)
        log('INFO', 'execution_goal_activated', {
          id: goal.id,
          route: this.currentRouteDecision.route,
          criteria: goal.successCriteria,
          sessionId: effectiveSessionId,
        })
      } catch (err) {
        log('WARN', 'execution_goal_create_failed', { error: String(err) })
        this.currentGoalId = null
        this.currentMethodologyId = null
      }
    }
    ...
    this.emitUserMessage(rid, effectiveSessionId, text)
    this.emitChatTaskStarted(rid, effectiveSessionId, text)
    ...
    try {
      this.refreshMemory(injectionCtx)
    } catch (err) {
      log('WARN', 'refresh_memory_skipped', { error: String(err) })
    }
    ...
    const userMsg: StoredMessage = {
      ...
      sessionId: effectiveSessionId,
      telegramChatId: extra?.telegramChatId ?? toNullableNumber(extra?.chatId),
      telegramUserId: extra?.telegramUserId ?? toNullableNumber(extra?.userId),
      telegramFrom: extra?.telegramFrom ?? null,
      telegramMessageId: extra?.telegramMessageId ?? extra?.messageId ?? null,
      createdAt: Date.now(),
    }
    ...
    try {
      ...
      ctx.activeGoalId = this.currentGoalId
      this.runContext = ctx
      ...
      try {
        this.sessionMemory.checkCompaction(effectiveSessionId, source)
      } catch {
        /* noop */
      }
      this.emitAgentResponse(rid, effectiveSessionId, reply, Date.now() - t0)
      this.emitChatTaskCompleted(rid, effectiveSessionId, 'completed', Date.now() - t0)
      return { reply }
    } catch (err) {
      ...
      // M6.1: 内部错误终止本轮 → 未终结的执行目标标记 abandoned
      this.abandonActiveGoal('chat_handler_error')
      this.emitChatTaskCompleted(rid, effectiveSessionId, 'failed', Date.now() - t0, String(err))
  ...
  private resolveToolRoutingForModelCall(routePolicy: RouteExecutionPolicy | null): {
    allowedToolNames: string[] | undefined
    toolChoiceMode: ToolChoiceMode
  } {
    if (!routePolicy) {
      return { allowedToolNames: this.sceneAllowedTools, toolChoiceMode: 'auto' }
    }

    const routeAllowedTools = routePolicy.allowedToolNames
    const sceneAllowedTools = this.sceneAllowedTools
    const allowedToolNames =
      sceneAllowedTools === undefined
        ? routeAllowedTools
        : routeAllowedTools === undefined
          ? sceneAllowedTools
          : routeAllowedTools.filter((toolName) => sceneAllowedTools.includes(toolName))
    const toolChoiceMode =
      routePolicy.toolChoiceMode === 'required' && Array.isArray(allowedToolNames) && allowedToolNames.length === 0
        ? 'auto'
        : routePolicy.toolChoiceMode

    return { allowedToolNames, toolChoiceMode }
  }

  /** M6.2: ??? ? ??????????toolLoop ???????????? */
  private injectActiveGoalMethodologyHint(ctx: RunContext): void {
    if (!ctx.activeGoalId) return
    try {
      const methodologyId = this.currentMethodologyId ?? executionGoalStore.get(ctx.activeGoalId)?.methodology ?? null
      if (!methodologyId) return
      const methodology = getMethodologyById(methodologyId)
      if (methodology) this.workingMemory.scratchpad.add('system_hint', buildMethodologyHint(methodology))
    } catch (err) {
      log('WARN', 'execution_goal_methodology_hint_failed', { error: String(err), id: ctx.activeGoalId })
    }
  }
  ...
  private async toolLoop(messages: Message[], ctx: RunContext, requestId: string, source: string): Promise<string> {
    ...
    const MAX_ROUTE_FORCED_RETRIES = 2
    let routeForcedRetries = 0
    let routeDowngradedToAuto = false
    try {
      for (let i = 0; i < MAX_TURNS; i++) {
        ...
        this.injectActiveGoalMethodologyHint(ctx)
        ...
        // 单一决策源：先解析路由执行策略，行为适应系统不得覆盖强制工具路由
        const routePolicy =
          (!hasCalledLlm || routeForcedRetries > 0) && this.currentRouteDecision
            ? resolveRouteExecutionPolicy(this.currentRouteDecision)
            : null
        const routeRequiresTools = routePolicy?.toolChoiceMode === 'required'
        const modeRecs = taskOrchestrationModeManager.getModeRecommendations(routeRequiresTools)

        // 简化/引导模式：进一步限制工具链长度（强制工具路由下不缩减工具链）
        ...
        // 注入模式相关的 system prompt 片段（通过 scratchpad）（强制工具路由下不注入简化/引导提示）
        ...
        // 引导模式：连续求助 → 插入探测性问题确认用户意图（强制工具路由下不打断执行）
        ...
        const toolRouting = this.resolveToolRoutingForModelCall(routePolicy)
        let result = await this.llmService.chatWithTools(
          messages,
          requestId,
          120000,
          onToken,
          undefined, // externalSignal
          toolRouting.allowedToolNames, // allowedToolNames — 场景工具过滤
          { toolChoiceMode: toolRouting.toolChoiceMode },
        )
        hasCalledLlm = true
        if (result.error === 'NO_TOOLS_AVAILABLE' && toolRouting.toolChoiceMode === 'required') {
          routeDowngradedToAuto = true
          log('WARN', 'route_required_tools_unavailable', { step: i, requestId })
          result = await this.llmService.chatWithTools(messages, requestId, 120000, onToken, undefined, toolRouting.allowedToolNames, {
            toolChoiceMode: 'auto',
          })
        }
        // 路由强制：required 路由下模型首轮未调用任何工具时，注入纠正提示并重试（有可用工具时才重试）
        if (
          routeRequiresTools &&
          !routeDowngradedToAuto &&
          routeForcedRetries < MAX_ROUTE_FORCED_RETRIES &&
          !result.error &&
          (!result.toolCalls || result.toolCalls.length === 0) &&
          (!Array.isArray(toolRouting.allowedToolNames) || toolRouting.allowedToolNames.length > 0)
        ) {
          routeForcedRetries++
          log('INFO', 'route_forced_retry', {
            step: i,
            requestId,
            route: this.currentRouteDecision?.route,
            retry: routeForcedRetries,
          })
          this.workingMemory.scratchpad.add(
            'system_hint',
            this.currentRouteDecision?.route === 'observe_first'
              ? '当前请求需要先观察工作区再回答：请先调用一个只读工具（list_files/read_file/grep/analyze_codebase）获取证据，然后再回答。'
              : '当前请求必须调用工具才能完成：请先调用相关工具获取结果，再基于结果回答，不要直接文字作答。',
          )
          continue
        }
        ...
        if (result.toolCalls && result.toolCalls.length > 0) {
          ...
          // ── [GOAL] M6.1 Evidence Pipeline：收集证据 → 持久化 → 判定 → 注入收敛提示 ──
          if (ctx.activeGoalId) {
            try {
              const goalRound = this.goalPipeline.recordRound(ctx.activeGoalId, result.toolCalls, toolResults, i)
              if (goalRound) {
                ctx.evidence = [...ctx.evidence, ...goalRound.evidence]
                if (goalRound.evaluation.verdict === 'completed') {
                  log('INFO', 'execution_goal_round_completed', {
                    id: ctx.activeGoalId,
                    matched: goalRound.evaluation.matchedCriteria,
                  })
                  this.workingMemory.scratchpad.add(
                    'system_hint',
                    `【目标完成】已根据证据判定目标达成（${goalRound.evaluation.matchedCriteria.join('、')}）。请停止工具调用，向用户汇报完成结果。`,
                  )
                  const completedGoalId = ctx.activeGoalId
                  void this.goalPipeline
                    .verifyCompletion(completedGoalId, this.llmService, requestId)
                    .catch((err) => log('WARN', 'execution_goal_verify_error', { error: String(err), id: completedGoalId }))
                } else if (goalRound.evaluation.verdict === 'blocked') {
                  log('WARN', 'execution_goal_round_blocked', {
                    id: ctx.activeGoalId,
                    failed: goalRound.evaluation.failedCriteria,
                  })
                  this.workingMemory.scratchpad.add(
                    'system_hint',
                    '【目标受阻】当前证据显示目标无法按标准完成。请向用户说明阻塞原因与建议，不要继续无意义重试。',
                  )
                }
              }
            } catch (err) {
              log('WARN', 'execution_goal_pipeline_error', { error: String(err), id: ctx.activeGoalId })
            }
          }
          ...
          this.currentSessionRecentToolNames = [...this.currentSessionRecentToolNames, ...result.toolCalls.map((tc) => tc.name)].slice(-5)
        ...
        if (!finalReply) {
          ...
          if (executedToolNames.size > 0) {
            const toolList = [...executedToolNames].map((n) => {
  ...
  private checkMilestone(step: number, tr: ToolResult[], ctx: RunContext, rid: string): void {
    ...
    this.recoveryManager
      .createCheckpoint({
        ...
        runId: rid,
        executionGoalId: ctx.activeGoalId,
        planState: {
  ...
  private applySentimentToTts(llmReply: string, toolResultTexts?: string[]): void {
    ...
    try {
      ...
      if (
        this.currentUserInputEmotion &&
        this.currentUserInputEmotion.confidence > 0.3 &&
        this.currentUserInputEmotion.emotion !== 'neutral'
      ) {
  ...
  getMemoryEmotionState(): {
    enabled: boolean
    context: import('@akemi-mio/audio/MemoryEmotionBridge').AggregatedEmotionContext | null
  } {
    return {
