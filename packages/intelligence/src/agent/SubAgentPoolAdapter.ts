...
import { RuntimeTaskImpl } from '@akemi-mio/intelligence/runtime/RuntimeTaskImpl'
import type { SubAgentResult, SpawnTaskOptions, SubAgentStatus } from './SubAgentPool'
...
export class SubAgentPoolAdapter {
  ...
  constructor(mcpManager: ServerManager, chatKey: string, codeKey: string, runtimeManager?: RuntimeManagerImpl) {
    this.runtimeManager = runtimeManager ?? new RuntimeManagerImpl(() => new SupervisedAgentSupervisorImpl(mcpManager, chatKey, codeKey))
  }
  ...
  /** 确保有默认 Task */
  private ensureTask(): RuntimeTaskImpl {
    if (!this.defaultTask) {
