...
import { ConversationContext, type Message, buildSystemPrompt } from './context'
...
export class Scratchpad {
  ...
  flushNew(lastCount: number): ScratchEntry[] {
    if (this.entries.length <= lastCount) return []
    const newEntries = this.entries.splice(lastCount)
    return newEntries
  }

  /** 将 scratchpad 条目渲染为 messages 注入对话 */
  injectInto(messages: Message[], asType: 'system' | 'user' = 'user'): number {
...
export class AttentionSet {
  private entities: Map<string, AttentionEntity> = new Map()
  private decayRate = 0.95

  /** 关注一个实体 */
  add(name: string, type: AttentionEntity['type'], relevance = 1.0): void {
  ...
  extractFrom(text: string): void {
    ...
    if (conceptMatches) {
      for (const c of conceptMatches) {
        ...
        if (name.length > 1) this.add(name, 'concept', 0.4)
      }
    }
  }

  /** 获取当前注意力实体列表（按相关度排序） */
  getActive(threshold = 0.3): AttentionEntity[] {
    const now = Date.now()
  ...
  getFormattedContext(limit = 5): string {
...
export class WorkingMemory {
  ...
  /** 添加助手回复 */
  addAssistant(text: string, toolCalls?: Array<{ name: string; arguments?: Record<string, unknown> }>): void {
    this.context.addAssistant(text, toolCalls as any)
    if (toolCalls) {
