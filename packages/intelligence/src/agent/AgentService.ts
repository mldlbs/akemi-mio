...
import { type IntentResult, type IntentHandler } from './intent/types'
...
import { planManager as defaultPlanManager } from '@akemi-mio/evolution-core'
...
import type { GuardrailPipeline } from '@akemi-mio/core/core/evaluation/GuardrailPipeline'
...
import { setProceduralMemory, setTtsService, setAsrService } from '@akemi-mio/capabilities/tool/deps'
import { UnifiedKnowledgeQuery, MemoryPluginAdapter, adaptProceduralMemory, adaptReflectLoop, adaptFailureAnalyzer } from '@akemi-mio/intelligence/knowledge'
import { agentPluginRegistry, ObserveStagePluginAdapter, ThinkStagePluginAdapter, ReflectStagePluginAdapter } from './plugin'
import type { IndustrialOdeLayer } from '@akemi-mio/blog-publish/gongye-songge'
import type { WorkspaceCleanupLayer } from '@akemi-mio/evolution-workspace-cleanup'
import type { IEngineService, EngineStatus, EngineMetrics } from '@akemi-mio/engine/types'
...
export class AgentService implements IEngineService {
  ...
  private runtimeManager: import('@akemi-mio/intelligence/runtime/RuntimeManagerImpl').RuntimeManagerImpl | null = null
  ...
  constructor(
    ...
    this.subAgentPool = new SubAgentPoolAdapter(
      ...
      this.runtimeManager ?? undefined, // 共享 RuntimeManagerImpl
    )
  ...
  async processExternalMessage(message: ExternalMessage, requestId?: string, sessionId?: string, noTts?: boolean): Promise<ChatResult> {
    return this.processTextInput(
      ...
      'telegram',
      {
        ...
        telegramChatId: toOptionalNumber(message.context.chatId),
        telegramUserId: toOptionalNumber(message.userId),
      ...
      if (this.workspaceCleanupLayer && this.workspaceCleanupLayer.getActiveFeatures().length > 0 && finalReply.reply) {
        ...
        if (postCtx.enhanced && postCtx.text !== finalReply.reply) {
          ...
          log('INFO', 'workspace_cleanup_post_enhanced', {
            request_id: rid,
            original_len: finalReply.reply?.length ?? 0,
            formatted_len: postCtx.text.length,
            description: postCtx.description,
          })
        }
  ...
  async stopConversation(): Promise<void> {
    ...
    // M6.1: 用户停止 → 未终结的执行目标标记 abandoned，避免长期 executing 残留
    this.chatExecutor?.abandonActiveGoal?.('user_stopped')
  ...
  getMetrics(): EngineMetrics {
    ...
    return {
      ...
      reliability: totalCalls > 0 ? Math.max(0, 1 - openBreakers.reduce((s, b) => s + b.failures, 0) / totalCalls) : 1,
    }
  }
  ...
  async saveRecoverySnapshot(trigger: 'milestone' | 'error' | 'interrupt' | 'shutdown', error?: string): Promise<void> {
    ...
    try {
      ...
      await this.recoveryManager.createCheckpoint({
        ...
        executionGoalId: this.chatExecutor?.getCurrentGoalId() ?? this.runContext?.activeGoalId ?? null,
        planState: {
  ...
  private tryRestoreSession(): boolean {
    ...
    // M6.1 restore execution goal id into the recovery prompt only; do not re-execute or replan
    let hasUnfinishedGoal = false
    if (checkpoint.executionGoalId) {
      try {
        const goal = executionGoalStore.get(checkpoint.executionGoalId)
        if (goal && goal.status !== 'completed' && goal.status !== 'abandoned') {
          hasUnfinishedGoal = true
          recoveryMsg += `\n\n之前的执行目标: ${goal.objective} (${goal.id})`
          if (goal.successCriteria.length > 0) {
            recoveryMsg += `\n完成标准: ${goal.successCriteria.join('；')}`
          }
          recoveryMsg += `\n状态: ${goal.status}，已收集证据: ${goal.evidence.length} 条`
          recoveryMsg += '\n请基于已保存的执行目标继续推进，不要重新规划。'
          log('INFO', 'session_execution_goal_restored', { goalId: goal.id, status: goal.status, evidence: goal.evidence.length })
        }
      } catch (err: any) {
        log('WARN', 'session_execution_goal_restore_failed', { goalId: checkpoint.executionGoalId, error: String(err) })
      }
    }
    if (checkpoint.planState.activePlanId) {
      recoveryMsg += `\n\n之前的活跃计划: ${checkpoint.planState.activePlanTitle} (${checkpoint.planState.activePlanId})`
      recoveryMsg += '\n请继续完成该计划，不要重新开始。'
    } else if (hasUnfinishedGoal) {
      recoveryMsg += '\n请继续执行上述目标，不要重新规划。'
    } else {
      recoveryMsg += '\n请从断点继续工作。'
    }
    ...
    this.eventBus.emit('recovery.session.restored' as any, {
      ...
      hasUnfinishedGoal,
      executionGoalId: checkpoint.executionGoalId ?? null,
    })
