...
const PATTERNS: Array<{ category: MessageCategory; patterns: RegExp[] }> = [
  {
    category: 'writing',
    patterns: [
      /写.*故事|写.*小说|创作.*故事/,
...
/** 检测用户消息的内容类型，默认 chat */
export function classifyContent(text: string): MessageCategory {
  if (!text) return 'chat'
...
/** 所有分类的中文标签和图标 */
export const CATEGORY_META: Record<MessageCategory, { label: string; icon: string }> = {
