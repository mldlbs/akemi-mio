...
export class UserBehaviorAnalyzer {
  ...
  /** 获取最近工具名列表（供语义路由判定使用） */
  getRecentToolNames(limit = 5): string[] {
    return this.recentToolCalls.slice(-limit).map((call) => call.name)
  }
   ...
   */
  /** 获取交互详情队列（用于构建行为分析片段） */
  getInteractionDetails(): readonly InteractionDetail[] {
    return this.interactionDetails
  }

  /** 获取最近用户消息 */
  getRecentUserMessages(): readonly { content: string; timestamp: number }[] {
    return this.recentUserMessages
  }
