...
export class AgentMonitor {
  ...
  getTrendAnalysis(): TrendAnalysis {
    ...
    const previous =
      previousSlice.length >= this.config.minRoundsForAnalysis
    ...
    if (previous && successRateDelta !== null && successRateDelta < -this.config.degradationSuccessRateThreshold) {
      signals.push({
        ...
        description:
          `对话成功率下降 ${(Math.abs(successRateDelta) * 100).toFixed(1)}%` +
    ...
    if (previous && sentimentDelta !== null && sentimentDelta < -this.config.degradationSentimentThreshold) {
      signals.push({
        ...
        description:
          `用户情感分数下降 ${(Math.abs(sentimentDelta) * 100).toFixed(1)}%` +
    ...
    if (previous && current.avgDurationMs > previous.avgDurationMs * 1.5 && current.avgDurationMs > 20000) {
      signals.push({
        ...
        description: `平均响应延迟显著增加` + `（${previous.avgDurationMs}ms → ${current.avgDurationMs}ms）`,
    ...
    if (previous && current.interruptionRate > previous.interruptionRate + 0.1 && current.interruptionRate > 0.2) {
      signals.push({
        ...
        severity: 'warning',
        description:
          `用户中断率上升` + `（${(previous.interruptionRate * 100).toFixed(0)}% → ${(current.interruptionRate * 100).toFixed(0)}%）`,
