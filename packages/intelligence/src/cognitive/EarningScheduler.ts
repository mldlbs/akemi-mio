...
import { TokenAccount } from '@akemi-mio/intelligence/cognitive/TokenEconomy'
...
export class EarningScheduler {
  ...
  async runOnce(): Promise<EarningResult | null> {
    return this.runEarningCycle()
  }

  private async runEarningCycle(): Promise<EarningResult | null> {
    ...
    try {
      ...
      const result: EarningResult = JSON.parse(raw)
      ...
      if (totalEstimated > 0) {
        this.tokenAccount.earn(
          Math.floor(totalEstimated * 0.1),
          'earning_effort',
          `每日执行 ${result.results.length} 个平台，预估 ¥${totalEstimated}`,
        )
      }
  ...
  async getLastResult(): Promise<EarningResult | null> {
    try {
      await fs.promises.access(RESULT_FILE)
      const raw = await fs.promises.readFile(RESULT_FILE, 'utf-8')
