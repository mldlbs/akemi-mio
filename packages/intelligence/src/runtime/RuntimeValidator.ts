...
export class RuntimeValidator {
  ...
  start(rm: RuntimeManager | null): void {
    ...
    this.disposers.push(eventBus.on(RUNTIME_EVENT as any, (evt: RuntimeEvent) => this.onRuntimeEvent(evt), 'RuntimeValidator'))

    // 订阅 Validator 专用通道（Task 生命周期事件）
    this.disposers.push(eventBus.on(RUNTIME_VALIDATOR_EVENT as any, (evt: any) => this.onValidatorEvent(evt), 'RuntimeValidator'))
