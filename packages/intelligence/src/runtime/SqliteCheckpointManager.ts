...
import type { Checkpoint, CheckpointId, ValidationResult, SafePoint } from './CheckpointTypes'
  ...
  async create(context: CheckpointContext): Promise<Checkpoint> {
    ...
    return {
      ...
      executionState: {
        ...
        lastSafePoint: (context.executionState?.lastSafePoint ?? 'before_llm') as SafePoint,
