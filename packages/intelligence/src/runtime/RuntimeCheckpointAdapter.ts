...
import type { Checkpoint, SerializedContext, ExecutionState, TaskState } from './CheckpointTypes'
import { SafePoint } from './CheckpointTypes'
import type { CheckpointContext } from './CheckpointManager'
...
export function snapshotRuntimeTask(task: RuntimeTaskImpl): RuntimeSnapshot {
  ...
  for (const w of status.workers) {
    ...
    workers.push({
      ...
      pendingDecision:
        w.state === RuntimeState.WAITING_SUPERVISOR
...
export function toCheckpointContext(
  ...
  return {
    ...
    componentSnapshots: extra ? [{ name: 'conversation', snapshot: () => extra.conversationContext }] : undefined,
  }
}
...
const STATE_TO_SAFEPOINT: Record<string, SafePoint> = {
  [RuntimeState.RUNNING]: SafePoint.BeforeLLM,
  [RuntimeState.WAITING_TOOL]: SafePoint.AfterTool,
  [RuntimeState.WAITING_SUPERVISOR]: SafePoint.AfterLLM,
  [RuntimeState.PAUSED]: SafePoint.BeforeLLM,
  [RuntimeState.INTERRUPTED]: SafePoint.BeforeLLM,
}
