...
export const SCENARIO_SINGLE_TASK: ReplayScenario = {
  ...
  steps: [{ spawn: [{ goal: 'single task test' }], wait: true }, { expect: { leakedWorkers: 0, illegalTransitions: 0 } }],
}
...
export class ReplayRunner {
  ...
  async runUntilPhase(targetPhase: string, batchSize = 10, maxBatches = 200): Promise<RcReport> {
    for (let b = 0; b < maxBatches; b++) {
      ...
      if ((b + 1) % 5 === 0) {
        ...
        console.log(
          `[ReplayRunner] batch ${b + 1}/${maxBatches} — tasks: ${m.tasksCreated}, workers: ${m.workersCreated}, phase: ${report.phase}`,
        )
      }
    }
  ...
  private checkExpect(expect: Record<string, unknown>, report: RcReport): string | null {
    const m = report.metrics as unknown as Record<string, unknown>
