...
import { DEFAULT_HYBRID_CONFIG } from '@akemi-mio/intelligence/hybrid/types'
import type {
  HybridPathOutput,
  ConvergenceResult,
...
export class McpAgentArbitrator {
   ...
   * @returns 仲裁结果
   */
  arbitrate<T>(point: ConvergencePointName, agent: HybridPathOutput<T>, mcp: HybridPathOutput<T>, mcpLatencyMs = 0): ConvergenceResult<T> {
    this.mcpCallCount++
  ...
  private computeDivergence<T>(point: ConvergencePointName, a: T, b: T): number {
    switch (point) {
      case 'tool_selection':
        return this.divergenceForToolSelection(a as unknown as ToolSelectionVerdict, b as unknown as ToolSelectionVerdict)
      case 'result_validation':
        return this.divergenceForResultValidation(a as unknown as ResultValidationOutput, b as unknown as ResultValidationOutput)
      case 'reply_quality':
        return this.divergenceForReplyQuality(a as unknown as ReplyQualityOutput, b as unknown as ReplyQualityOutput)
      default:
   ...
   */
  private divergenceForToolSelection(agent: ToolSelectionVerdict, mcp: ToolSelectionVerdict): number {
    // 评估结果不一致 → 高分歧
   ...
   */
  private divergenceForResultValidation(agent: ResultValidationOutput, mcp: ResultValidationOutput): number {
    // 判定不一致 → 高分歧
   ...
   */
  private divergenceForReplyQuality(agent: ReplyQualityOutput, mcp: ReplyQualityOutput): number {
    // 判定不一致 → 高分歧
    if (agent.verdict !== mcp.verdict) {
  ...
  private resolveToolSelectionConflict(
    ...
    // suggested 取并集
    const suggestedSet = new Set([...agent.output.suggestedAdditionalTools, ...mcp.output.suggestedAdditionalTools])
    // 排除被拒绝的工具
