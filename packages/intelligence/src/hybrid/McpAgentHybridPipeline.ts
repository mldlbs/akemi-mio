...
import { McpAgentArbitrator } from '@akemi-mio/intelligence/hybrid/arbitrator'
...
export class McpAgentHybridPipeline {
  ...
  async validateToolSelection(
    ...
    const result = this.arbitrator.arbitrate<ToolSelectionVerdict>('tool_selection', agentOutput, mcpOutput, mcpResult.latencyMs)
  ...
  private async runMcpToolSelection(
    ...
    try {
      ...
      const toolList = toolCalls.map((t) => `- ${t.name}(${JSON.stringify(t.arguments)})`).join('\n')
  ...
  private buildAgentResultValidation(toolResults: ToolResult[]): ResultValidationOutput {
    ...
    return {
      ...
      rationale: issues.length > 0 ? `Agent 路径检测到 ${issues.length} 个问题` : 'Agent 路径: 所有工具执行成功',
    }
  }
  ...
  async validateReplyQuality(
    ...
    const result = this.arbitrator.arbitrate<ReplyQualityOutput>('reply_quality', agentPathOutput, mcpPathOutput, mcpResult.latencyMs)
