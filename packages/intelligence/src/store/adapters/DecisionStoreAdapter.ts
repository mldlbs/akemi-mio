 ...
 */
import type { DecisionStore, DecisionCategory } from '@akemi-mio/intelligence-memory/DecisionStore'
...
export class DecisionStoreAdapter implements WritableStore {
  ...
  constructor(private decisionStore: DecisionStore) {}
  ...
  async search(query: string, options?: SearchOptions): Promise<SearchResult[]> {
    ...
    const records = this.decisionStore.query({
  ...
  async getContext(_keywords?: string[]): Promise<string> {
    return this.decisionStore.getFormattedContext()
  }

  async store(input: WriteInput): Promise<WriteResult> {
    const metadata = input.metadata || {}
    const id = this.decisionStore.record({
  ...
  getStats(): Record<string, unknown> {
    const recent = this.decisionStore.query({ limit: 100 })
    ...
    return {
      ...
      crossSession: this.decisionStore.getCrossSessionSummary(),
    }
  }
}
