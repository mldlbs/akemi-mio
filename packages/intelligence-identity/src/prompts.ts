import type { CoreIdentity, EvolvedTrait, GrowthMetrics } from '@akemi-mio/intelligence-identity/IdentitySchema'

/**
 * 构建可注入 system prompt 的自我认知段落。
 * 格式与原先 PROMPT_IDENTITY 兼容，但增加了动态 trait 和 metrics 信息。
 */
export function buildIdentityPrompt(core: CoreIdentity, metrics: GrowthMetrics, traits?: EvolvedTrait[]): string {
...
function buildTraitSummary(traits: EvolvedTrait[]): string {
...
function buildMetricsSummary(m: GrowthMetrics): string {
  const parts: string[] = []
  if (m.sessionsCompleted > 0) parts.push(`已完成 ${m.sessionsCompleted} 次会话`)
  if (m.toolsUsed > 0) parts.push(`调用工具 ${m.toolsUsed} 次`)
  if (m.goalsCompleted > 0) parts.push(`完成 ${m.goalsCompleted} 个目标`)
