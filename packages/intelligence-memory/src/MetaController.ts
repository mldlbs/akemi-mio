...
import type { MemoryService, UserProfileData } from './MemoryService'

/** LLM 摘要生成的轻量接口（兼容 LlmService.chatJson 返回类型） */
export interface SummaryLLM {
  chatJson(
    userText: string,
    opts?: { system?: string; temperature?: number; timeoutMs?: number; requestId?: string },
...
export class MetaController {
   ...
   */
  private createLLMSummary(context: { userMessage: string; assistantReply: string; planActive: boolean }, turnEnd: number): void {
...
助手：${context.assistantReply.slice(0, 500)}`
      ...
      .then((response: { data?: any; error?: string }) => {
        ...
        if (keyEntities.length > 0 && this.memory) {
          for (const entity of keyEntities) {
            this.memory.knowledgeGraph.ingest(`对话实体: ${entity} (上下文: ${summary.slice(0, 60)})`, 0.55)
          }
        }
  ...
  private fallbackSummary(context: { userMessage: string; assistantReply: string; planActive: boolean }, turnEnd: number): void {
    const summaryText = context.assistantReply
      ? `用户: ${context.userMessage.slice(0, 60)} → ${context.assistantReply.slice(0, 60)}`
      : context.userMessage.slice(0, 80)
    this.summary?.addSummary(summaryText, this.stats.totalInteractions, turnEnd, {
