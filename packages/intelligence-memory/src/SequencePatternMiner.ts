...
export class SequencePatternMiner {
  ...
  private matchTemplates:
    ((query: string, topK: number) => Array<{ name: string; description: string; steps: string[]; score: number }>) | null = null
   ...
   */
  setTemplateMatcher(
    matcher: (query: string, topK: number) => Array<{ name: string; description: string; steps: string[]; score: number }>,
  ): void {
  ...
  matchPatterns(query: string, options: MiningOptions = {}): PatternMatch[] {
    ...
    const scored = this.patterns
      .map((p) => {
        // 1) 语义相似度
        const similarity = p.embedding && p.embedding.length > 0 ? cosineSimilarity(queryEmb, p.embedding) : this.textSimilarity(query, p)
        ...
        // 5) 新鲜度因子（基于 recency）
        const daysSinceUse = p.lastUsedAt > 0 ? (now - p.lastUsedAt) / (1000 * 60 * 60 * 24) : 365
      ...
      // 过滤：置信度 + 相似度双阈值
      .filter((s) => s.pattern.confidence >= minConfidence && s.similarity >= minSimilarity)
      ...
      return procedures.map((p) => {
        ...
        const embedding =
          p.embedding && p.embedding.length > 0
            ? p.embedding
            : this.computePatternEmbedding({
                name: p.name,
                description: p.description,
                steps: p.steps,
                triggerKeywords: p.triggerKeywords,
              } as unknown as SequencePattern)
  ...
  private computePatternEmbedding(pattern: { name: string; description: string; steps: string[]; triggerKeywords: string[] }): number[] {
    const text = [pattern.name, pattern.description, ...pattern.steps, ...pattern.triggerKeywords].join(' ')
  ...
  private mergeSimilarPatterns(patterns: SequencePattern[]): SequencePattern[] {
    ...
    for (let i = 0; i < patterns.length; i++) {
      ...
      for (let j = i + 1; j < patterns.length; j++) {
        ...
        const overlap = base.triggerKeywords.filter((t) => patterns[j].triggerKeywords.includes(t)).length
        if (overlap > 0) {
