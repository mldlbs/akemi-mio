...
import type { ToolResult } from '@akemi-mio/intelligence-shared/types'
...
export class TaskStepRecorder {
   ...
   */
  private updateTaskStateSummary(taskId: string, stepIndex: number, toolResults: ToolResult[], sessionId: string): void {
    const ms = this.memoryService
    ...
    const newStep = {
      ...
      result: toolResults
        .map((r) => `${r.name}: ${r.success ? 'OK' : `FAIL: ${r.error}`}`)
        .join('; ')
        .slice(0, 200),
    ...
    if (existing) {
      ...
      ms.saveTaskState({
        ...
        updatedAt: Date.now(),
        sessionIds: existing.sessionIds.includes(sessionId) ? existing.sessionIds : [...existing.sessionIds, sessionId],
      })
    } else {
      // 新任务
      ms.saveTaskState({
      ...
      try {
        ...
        if (relatedSteps.length > 0) {
          ...
          for (const rs of relatedSteps) {
            ...
            parts.push(
              `- ${taskLabel}步骤 ${rec.stepIndex} → ${rec.toolName}: ${rec.toolSuccess ? '成功' : `失败: ${(rec.toolError || '').slice(0, 80)}`}`,
            )
          }
          parts.push('---')
        }
