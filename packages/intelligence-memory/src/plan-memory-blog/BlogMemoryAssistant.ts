...
import { BLOG_MEMORY_ENABLED, BLOG_MEMORY_DEFAULT_CONFIDENCE, BLOG_MEMORY_DEFAULT_TIER } from '@akemi-mio/core/config'
...
export class BlogMemoryAssistant {
  ...
  constructor(config?: Partial<BlogMemoryAssistantConfig>, llmGenerator?: SuggestionGenerator | null) {
   ...
   */
  private retrieveWorkflowExperiences(query: string): Array<{ content: string; category: string; similarity: number; title?: string }> {
  ...
  private buildRuleBasedSuggestions(
    ...
    return {
      titleOptimization: {
        ...
        reasoning: references.length > 0 ? `基于 ${references.length} 条相似历史记忆的主题倾向生成` : '基于当前主题生成的通用标题建议',
      },
      publishingTime: {
        suggestedHour,
        suggestedDay,
        confidence: references.length > 0 ? 0.5 : 0.2,
        reasoning: references.length > 0 ? '基于历史写作活跃度的通用推荐' : '历史数据不足，使用默认推荐时段',
      },
      contentImprovements,
      relatedTopics: references.length > 0 ? [...uniqueCategories].slice(0, 3).map((c) => this.categoryToTopic(c)) : undefined,
      references: references.map((r) => ({
        content: r.content.slice(0, 200),
        category: r.category,
        similarity: Math.round(r.similarity * 100) / 100,
 ...
 */
export async function suggestForNewBlog(topic: string, platform?: string): Promise<{ context: NewBlogContext; promptSegment: string }> {
