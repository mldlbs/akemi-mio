...
import type { BlogMemoryStructuredData, BlogMemoryCategory, BlogMemoryQueryOptions, BlogMemoryRenderOptions } from './types'
...
export class BlogMemoryRetriever {
  ...
  renderForBlog(blogId: string, options?: BlogMemoryRenderOptions): string {
    const entries = this.getForBlog(blogId, options as BlogMemoryQueryOptions)
    ...
    if (options?.intro) {
      ...
      lines.push(
        'This article documents key decisions and discoveries made during implementation, automatically recorded from development plan execution.',
      )
      lines.push('')
    }
  ...
  private renderChronological(
    ...
    if (showTimestamps) {
      lines.push(
        `| ${locale === 'zh' ? '时间' : 'Time'} | ${locale === 'zh' ? '分类' : 'Category'} | ${locale === 'zh' ? '内容摘要' : 'Summary'} |`,
      )
      lines.push('|------|--------|----------|')
    }
