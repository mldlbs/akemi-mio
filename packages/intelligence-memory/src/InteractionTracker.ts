...
export class InteractionTracker {
  ...
  load(): void {
    try {
      const db = getRawDb()
      const result = db.exec(`SELECT * FROM interaction_log ORDER BY timestamp DESC LIMIT ${RING_SIZE}`)
  ...
  prune(maxRecords = 100): void {
    try {
      const db = getRawDb()
      const count = Number(db.exec('SELECT COUNT(*) AS c FROM interaction_log')[0]?.values[0]?.[0] || 0)
      if (count > maxRecords) {
        db.run(`DELETE FROM interaction_log WHERE id IN (SELECT id FROM interaction_log ORDER BY timestamp ASC LIMIT ?)`, [
          count - maxRecords,
        ])
        markDirty()
      }
