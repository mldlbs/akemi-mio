...
import { getMessagesBySession, getSessions } from '@akemi-mio/core/db/messages'
import type { StoredMessage } from '@akemi-mio/core/db/messages'
import type { EvaluationEmitter } from '@akemi-mio/core/core/evaluation/EvaluationEmitter'
import type { AttentionEntity } from '@akemi-mio/intelligence-shared/types'
...
export class SessionMemory {
  ...
  private extractUnresolved(messages: StoredMessage[]): string[] {
    ...
    const questionPatterns = [/怎[么样]解决/, /有[什么]办法/, /待解决/, /还没[做搞好处理]/, /下一步/, /TODO/, /todo/, /未完成/]
  ...
  private emitScoringEvent(
    ...
    scored: Array<{ score: number; matchedFactors: any; compaction: SessionCompaction }>,
    ...
    if (attentionAvailable && scoredCount > 0 && this.currentAttention && this.currentAttention.length > 0) {
      ...
      const fallbackFactors = this.computeFactors(top.compaction, { ...spec, attentionEntities: [] })
  ...
  getCompactionsBySession(sessionId: string): SessionCompaction[] {
    try {
      const db = getRawDb()
      const result = db.exec(`SELECT * FROM session_compactions WHERE session_id = ? ORDER BY created_at DESC`, [sessionId])
  ...
  getAllCompactions(): SessionCompaction[] {
    try {
      const db = getRawDb()
      const result = db.exec('SELECT * FROM session_compactions ORDER BY session_end_at DESC')
      return this.parseCompactionRows(result)
    } catch (err) {
  ...
  private parseCompactionRows(result: Array<{ columns: string[]; values: any[][] }>): SessionCompaction[] {
