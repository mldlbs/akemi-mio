...
export class MemoryCleaner {
  ...
  runCleanup(entries: MemoryEntry[], removedIds: Set<string>): CleanupReport {
    ...
    // 确保清理后最少保留数
    const nonPermanent = entries.filter((e) => e.tier !== 'permanent' && !e.isPinned && e.type === 'user_fact')
    ...
    const report: CleanupReport = {
      ...
      pendingConfirmationIds: this.requireConfirmation ? allCandidates.map((c) => c.entry.id) : [],
   ...
   * @returns 实际删除的数量
   */
  confirmCleanup(entries: MemoryEntry[], removedIds: Set<string>, confirmIds?: string[]): number {
