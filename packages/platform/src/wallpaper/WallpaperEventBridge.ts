...
import { MonitoringService } from '@akemi-mio/monitoring/MonitoringService'
 ...
 */
export function checkSchemaVersion(actualVersion: number, expectedVersion: number, eventName: string): 'exact' | 'ahead' | 'behind' {
...
export class WallpaperEventBridge {
  ...
  private subscribeBehaviorEvents(): void {
    ...
    eventBus.track(
      ...
      (raw: any) => {
        ...
        const context: 'coding' | 'browsing' | 'resting' = mode === 'focus' ? 'coding' : 'browsing'
  ...
  private onPlanStepCompleted(payload: any): void {
    const versionCheck = checkSchemaVersion(payload.version ?? 0, PLAN_SCHEMA_VERSION, 'plan.ts.step.completed')
  ...
  private onPlanProgressUpdated(payload: any): void {
    const versionCheck = checkSchemaVersion(payload.version ?? 0, PLAN_SCHEMA_VERSION, 'plan.ts.progress.updated')
   ...
   */
  private inferActivityContext(activityState: string, appCategory: string, mode: string): 'coding' | 'browsing' | 'resting' {
