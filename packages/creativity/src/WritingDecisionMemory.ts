...
export class WritingDecisionService {
  ...
  recordDecision(input: {
    ...
    const structuredData = JSON.stringify({
      ...
      sceneTitle: input.sceneTitle ?? undefined,
      chapterNumber: input.chapterNumber ?? undefined,
  ...
  recordDecisionFromDiff(input: {
    ...
    for (const entity of entities) {
      ...
      const entityText = entityLines
        .map((s) => s.text)
        .join('\n')
        .slice(0, 500)
      const oldEntityText = changeBlocks
        .filter(
          (s) => (s.type === 'delete' || s.type === 'modify') && (s.text.includes(entity) || (s.oldText && s.oldText.includes(entity))),
        )
        .map((s) => s.oldText || s.text)
        .join('\n')
        .slice(0, 500)
  ...
  getFormattedContext(storyName: string, entityFilter?: string[]): string {
    const decisions = entityFilter ? this.queryByEntities(storyName, entityFilter) : this.queryByStory(storyName)
  ...
  private computeDiff(oldContent: string, newContent: string): DiffSegment[] {
    ...
    while (oi < oldLines.length || ni < newLines.length) {
      if (oi < oldLines.length && ni < newLines.length && oldLines[oi].trim() === newLines[ni].trim()) {
        ...
        ni < newLines.length &&
        (oi >= oldLines.length || !oldSet.has(newLines[ni].trim()) || oldSet.get(newLines[ni].trim())!.every((idx) => idx < oi))
      ) {
