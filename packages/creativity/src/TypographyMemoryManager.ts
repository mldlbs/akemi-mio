...
export class TypographyMemoryManager {
  ...
  private pruneOldRecords(storyId: string, platformTag: string): void {
    ...
    for (const oldRecord of toRemove) {
      ...
      const content = `${MEMORY_KEY_PREFIX}${serialized}`
      const entry = entries.find((e) => e.type === 'user_fact' && e.content === content)
      if (entry) {
   ...
   * 返回按时间降序排列（最新在前）。
   */
  queryRecords(storyId: string, platformTag: string, options?: { limit?: number }): TypographyRecord[] {
    const ms = getMemoryService()
    if (!ms) return []
   ...
   * - 趋势摘要（如果有多章记录）
   */
  buildTypographyContext(storyId: string, platformTag: string, chapterNumber?: number): TypographyContextHint {
    const records = this.queryRecords(storyId, platformTag, {
    ...
    // 2. 用户最近的修正记录
    const recentCorrections = records.filter((r) => r.source === 'user_correction' && r.userCorrections).slice(0, 2)
    ...
    lines.push('')
    lines.push('请参考以上排版偏好调整当前章节的排版设置，保持与历史章节一致的排版风格。')
    lines.push(`---`)
   ...
   */
  analyzeTrends(storyId: string, platformTag: string): TypographyTrendReport | null {
    ...
    for (const key of allKeys) {
      ...
      const direction: TypographyTrendReport['parameterTrends'][string]['direction'] =
        changeCount === 0 ? 'stable' : changeCount <= 2 ? 'changed' : changeCount > values.length / 3 ? 'increasing' : 'decreasing'
