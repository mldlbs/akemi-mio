...
export class WorldTrendProvider {
  ...
  resetConsumed(): void {
    ...
    this.rng = resolveRandom(this.seed)
  }
   ...
   */
  getTrendSignals(days = 3, limit = 20): { keyword: string; snippets: string[]; score: number }[] {
  ...
  private loadSignals(days: number): TrendSignal[] {
  ...
  private buildObsMap(days: number): Map<string, Observation> {
