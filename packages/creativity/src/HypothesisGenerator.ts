...
export class HypothesisGenerator {
  ...
  async generate(
    ...
    if (llmResults.length > 0) {
      return llmResults.map((r) => ({
        ...
        sourceDetails: this.buildSourceDetails(r.sourceLabels, sources),
  ...
  > {
    ...
    const comboInfo = topCombos.map((c) => ({
      sources: c.sources as [string, string],
    ...
    try {
      ...
      return ideas.filter(
        (i: any) =>
          ...
          typeof i.idea === 'string' &&
          Array.isArray(i.sourceLabels) &&
          typeof i.novelty === 'number' &&
          this.hasRelevanceJustification(i),
      )
    } catch (err: any) {
  ...
  private buildSourceDetails(labels: string[], sources: CreativitySource[]): Array<{ name: string; depth: 'shallow' | 'medium' | 'full' }> {
    const byName = new Map(sources.map((s) => [s.name, s.sourceDepth ?? 'shallow']))
    return labels.map((name) => ({ name, depth: byName.get(name) ?? 'shallow' }))
  }

  /**
   * 模板 fallback — 从旧实现保留
   */
  private templateFallback(combo: ConceptCombo): Hypothesis {
    ...
    return {
      ...
      risk: t.risk,
      sourceLabels: [a, b],
      sourceDetails: [a, b].map((name) => ({ name, depth: 'shallow' as const })),
