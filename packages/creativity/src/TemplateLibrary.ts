...
/**
 * TemplateLibrary — 30+ 组合模式的本地假设生成引擎
 *
 * 当 LLM 不可用或超时时，TemplateLibrary 提供高质量的本地方案。
 * 相比旧版 5 个硬编码模板，这里按类型匹配合适的模式，
 * 并利用 source 名称和类型内容做填充，产出更有意义的假设。
 *
 * 模式分类：
 *   fusion       融合 — 两个概念合并为统一系统
 *   transplant   移植 — 把 A 的能力移植到 B 的上下文
 *   contrast     对比 — 利用差异构造互补系统
 *   feedback     反馈 — A 驱动 B 改进 B，B 反哺 A
 *   abstract     抽象 — 提取共同模式，泛化复用
 *   metaphor     隐喻 — 用 A 的领域隐喻重新设计 B
 *   inversion    反转 — 做 A/B 当前做法的相反假设
 *   evolution    演化 — 在 A 之上渐进叠加 B 的能力
 *   symbiosis    共生 — A 和 B 保持边界但事件耦合
 *   orchestrate  编排 — A 编排 B 产生涌现行为
 */

type PatternCategory =
  'fusion' | 'transplant' | 'contrast' | 'feedback' | 'abstract' | 'metaphor' | 'inversion' | 'evolution' | 'symbiosis' | 'orchestrate'
...
const TEMPLATES: TemplateDef[] = [
  // ========== FUSION ==========
  {
    ...
    ideaTemplate:
      '将「{a}」和「{b}」的核心能力通过统一接口层合并，使两者共享数据和上下文。{b} 的输出作为 {a} 的新输入维度，{a} 的状态变化反向驱动 {b} 的行为调整，形成双向往来的融合架构。',
  ...
  },
  {
    ...
    ideaTemplate:
      '将「{a}」的处理流程嵌入到「{b}」的管线中，在关键节点插入 {a} 的判断逻辑。两条路径并行执行并在汇合点进行交叉验证，不一致时触发仲裁机制取最优或加权融合。',
  ...
  },
  {
    ...
    ideaTemplate:
      '把「{a}」领域成熟的数据模型和判断规则改造成「{b}」能理解的输入格式，让 {b} 在不重构自身架构的前提下利用 {a} 的知识积累。通过适配器模式渐进集成，先做 POC 验证再全量上线。',
  ...
  // ========== TRANSPLANT ==========
  {
    ...
    ideaTemplate:
      '识别「{a}」中可独立封装的核心算法或策略，将其抽离为通用模块后注入到「{b}」的执行链路中。移植后 {b} 获得 {a} 的核心能力而不需要继承 {a} 的全部复杂度。',
  ...
  },
  {
    ...
    ideaTemplate:
      '分析「{a}」架构中成功的设计模式（事件驱动、分层、插件化等），识别出该模式在 {a} 中解决的具体问题，然后在「{b}」中寻找相同性质的问题域，用适配的方式重构实现。',
  ...
  },
  {
    ...
    ideaTemplate:
      '提取「{a}」核心算法的输入输出接口，为「{b}」的上下文实现一个兼容适配层。复用不追求 1:1 精确移植，而是保留算法核心逻辑并用 {b} 的数据格式做输入输出转换。',
  ...
  // ========== CONTRAST ==========
  {
    ...
    ideaTemplate:
      '分析「{a}」和「{b}」各自的准确率和失败模式，找出 {a} 擅长但 {b} 薄弱、以及 {b} 擅长但 {a} 薄弱的区域。设计一个路由层根据输入特征自动分派到更合适的模块，覆盖双方的盲区。',
  ...
  },
  {
    ...
    ideaTemplate:
      '定义「{a}」和「{b}」各自的最佳工作条件（输入特征、负载范围、响应时间要求）。设计一个监控 + 切换器：条件满足时用 {a}，条件变化时切换到 {b}，切换时做状态保存和恢复保证无缝过渡。',
  ...
  },
  {
    ...
    ideaTemplate:
      '列举「{b}」当前的已知短板或失败案例，逐一检查「{a}」中是否有可弥补的能力。为每个短板设计一条 {a}→{b} 的修复链路，优先级按短板影响面排序，逐步缩小 {b} 的能力缺口。',
  ...
  // ========== FEEDBACK LOOP ==========
  {
    ...
    ideaTemplate:
      '设计一个闭环：{a} 的每次执行输出被「{b}」作为反馈信号消费，{b} 分析结果后调整自身的参数或策略，进而影响下一次 {a} 的执行质量。初始阶段人工监控闭环稳定性，收敛后转为自动运行。',
  ...
  },
  {
    ...
    ideaTemplate:
      '收集「{a}」的每一次执行结果（成功/失败/耗时/质量分），将聚合指标作为「{b}」的超参数输入。{b} 根据历史趋势自动调整阈值、权重或策略选择，形成一个数据驱动的优化闭环。',
  ...
  },
  {
    ...
    ideaTemplate:
      '将「{a}」的执行结果标注为训练信号，定期用新积累的数据微调或更新「{b}」的策略。建立反馈数据集自动去重和采样的机制，避免数据分布偏移导致 {b} 退化。',
  ...
  // ========== ABSTRACT ==========
  {
    ...
    ideaTemplate:
      '分析「{a}」和「{b}」的对外接口，找出语义相似的操作抽象为共用接口。先提取只读接口（查询类），再扩展到写接口。两套实现共存于统一接口之后，可以用策略模式在运行时选择具体实现。',
  ...
  },
  {
    ...
    ideaTemplate:
      '找出「{a}」和「{b}」在数据处理流程、状态管理、错误处理等方面的共性逻辑，将其提取为核心库。核心库只包含无偏见的通用逻辑，具体的领域差异通过插件或策略注入。',
  ...
  },
  {
    ...
    ideaTemplate:
      '对比「{a}」的实现路径和「{b}」的实现路径，提取出它们共享的解决模式（如：先过滤再聚合、分级缓存、渐进式加载）。将该模式文档化为设计模板，应用到其他模块的改造中。',
  ...
  // ========== METAPHOR ==========
  {
    ...
    titleTemplate: '{a} 式 {b}',
    ideaTemplate:
      '借用「{a}」领域的概念模型（如：免疫系统、城市规划、进化论）重新思考「{b}」的设计。将 {a} 中的实体和关系一一映射到 {b} 的领域：哪些是细胞/哪些是信号/哪些是防御机制。',
  ...
  },
  {
    ...
    ideaTemplate:
      '从「{a}」的运行原理中提取一条核心启发式规则（如：最少惊讶原则、帕累托改进、最速下降），然后检视「{b}」的当前行为是否违背了该规则，针对违背点设计改进方案。',
  ...
  },
  {
    ...
    ideaTemplate:
      '想象如果「{a}」是一个决策主体，它会如何设计「{b}」？分析 {a} 的核心原则（容错优先？效率优先？可解释优先？），将这些原则翻译为 {b} 的功能需求和非功能需求，设计符合 {a} 风格的新方案。',
  ...
  // ========== INVERSION ==========
  {
    ...
    ideaTemplate:
      '列举「{a}」和「{b}」当前关系的假设前提（a是主、b是从；a先执行、b后执行；a决策、b执行），逐个反转这些前提。针对每个反转版本评估可行性，挑出有意义的反转方向做原型验证。',
  ...
  },
  {
    ...
    ideaTemplate:
      '不再问「{a} 如何改进 {b}」，而是问「{b} 需要 {a} 以什么形态存在」。站在 {b} 的消费者视角定义 {a} 的输出格式、响应速度和容错要求，反向重构 {a} 的需求规格。',
  ...
  },
  {
    ...
    ideaTemplate:
      '不再试图避免「{a}」的失败，而是设计「{b}」使其在 {a} 失败时反而能获得有用信息。例如：{a} 的报错模式 → {b} 建立错误模式库并自动调整阈值；{a} 超时 → {b} 启动降级路径并记录边界值。',
  ...
  // ========== EVOLUTION ==========
  {
    ...
    ideaTemplate:
      '不一次性改造 {a}，而是在 {a} 的现有架构上以插件形式注入 {b} 的能力。第一阶段：旁路输出不做决策；第二阶段：作为建议源影响部分决策；第三阶段：在验证可靠后替换 {a} 的核心模块。',
  ...
  },
  {
    ...
    ideaTemplate:
      '保持 {a} 不动，在其上新增一层 {b} 的能力层。{b} 层拦截 {a} 的输入输出，进行预处理或后处理增强。上层能力通过 feature flag 控制，逐步开放给不同用户群体验证效果。',
  ...
  },
  {
    ...
    ideaTemplate:
      '将 {a} 的功能按独立程度拆分为子模块，标记每个子模块是否适合被 {b} 的能力替代。优先替换边界清晰、影响面小的子模块，每替换一个就运行一个月观察稳定性，再决定下一步。',
  ...
  // ========== SYMBIOSIS ==========
  {
    ...
    ideaTemplate:
      '通过事件总线连接 {a} 和 {b}：{a} 产生的事件经总线广播，{b} 选择性订阅感兴趣的事件来触发自身行为。总线定义标准化的事件 Schema，{a} 和 {b} 通过 Schema 演进保持兼容。',
  ...
  },
  {
    ...
    ideaTemplate:
      '将 {a} 的能力接口定义为一组插件契约，{b} 作为插件实现接入。{a} 在运行时通过 ServiceLoader 发现并加载 {b} 的插件，{b} 专注于实现契约而不需关心 {a} 的内部调度。',
  ...
  },
  {
    ...
    ideaTemplate:
      '为 {a} 附加一个 {b} 边车进程，{a} 的每次请求先经过边车处理（监控、缓存、过滤、转换）后再到达主逻辑。边车无状态可独立扩缩容，{a} 不需要引入 {b} 的依赖库。',
  ...
  // ========== ORCHESTRATE ==========
  {
    ...
    ideaTemplate:
      '将 {b} 的现有能力封装为标准步骤（step），{a} 作为编配器定义这些步骤的执行顺序、条件分支和异常处理。{a} 维护一个 DAG 定义工作流拓扑，{b} 只关心单个步骤的实现。',
  ...
  },
  {
    ...
    ideaTemplate:
      '将 {a} 和 {b} 组合为一条数据处理流水线：{a} 的输出是 {b} 的输入。每个环节定义明确的输入/输出 Schema，中间结果可缓存可重放。流水线用 JSON 定义，运行时动态加载。',
  ...
  },
  {
    ...
    ideaTemplate:
      '{a} 不直接输出结果，而是生成一条推理路径（中间步骤链），{b} 沿着这条路径逐步验证和执行。{a} 负责任务分解和优先级排序，{b} 负责每一步的细节执行，最终结果由 {b} 汇总返回。',
...
export class TemplateLibrary {
  ...
  private scoreNoise(): number {
    return Math.round(this.rng() * 12)
  }
}

/** 填充模板中的 {a} / {b} 占位符 */
function fill(template: string, a: CreativitySource, b: CreativitySource): string {
  return template
    .replace(/\{a\}/g, a.name)
    .replace(/\{b\}/g, b.name)
