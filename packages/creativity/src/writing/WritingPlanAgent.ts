...
import type {
  ...
  ProgressRecord,
  UserConstraint,
} from '@akemi-mio/creativity/writing/types'
...
export class WritingPlanAgent {
  ...
  async compareChapters(
    ...
    try {
      ...
      const chaptersText = chapters.map((c) => `=== 第${c.number}章 ===\n${c.content.slice(0, 3000)}`).join('\n\n')
  ...
  private normalizeDesignAnalysis(rawData: any, docPath: string): DesignDocAnalysis {
    ...
    const chapterGuidelines: ChapterGuideline[] = (rawData.chapterGuidelines || rawData.chapters || rawData.chapterGuide || []).map(
      (g: any) => ({
    ...
    if (tasks.length === 0) {
      ...
      if (criticalChapters.length > 0) {
        ...
        tasks.push({
          ...
          chapterNumbers: criticalChapters,
          dependencies: [],
          status: 'pending',
          relatedGuidelines: [],
        })
  ...
  private normalizeQualityReport(rawData: any, chapterNumber: number, threshold: number): QualityReport {
    ...
    const scores: Array<{ dimension: string; score: number; issues: string[]; suggestions: string[] }> = rawScores.map((s: any) => ({
