...
export class ModuleFeedbackManager {
  ...
  recordRejection(moduleName: string, signalType: RejectionSignalType, strength: number, description: string): void {
    ...
    state.modificationPriority = Math.max(MIN_MOD_PRIORITY, Math.min(MAX_MOD_PRIORITY, state.modificationPriority + effectiveDeltaMod))
    ...
    // 发出事件
    eventBus.emit(
      'evolution.feedback.signal' as any,
      {
        moduleName,
        signalType,
        strength,
    ...
    if (Math.abs(state.modificationPriority - oldModPriority) > 0.05) {
      eventBus.emit(
        'evolution.feedback.weight_adjusted' as any,
        {
          moduleName,
          oldModPriority,
  ...
  getStoreSnapshot(): FeedbackStore {
    ...
    return {
      ...this.store,
      modules: Object.fromEntries(Object.entries(this.store.modules).map(([key, val]) => [key, { ...val }])),
