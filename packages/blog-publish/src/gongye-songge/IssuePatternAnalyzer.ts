...
import type { CorrectionResult, ChapterCorrectionSnapshot, CorrectionSeverity, CorrectionCategory } from '@akemi-mio/blog-publish/gongye-songge/PlanSonggeCorrectionAdapter'
...
export class IssuePatternAnalyzer {
  ...
  analyze(result: CorrectionResult): IssuePatternAnalysis {
    ...
    // 生成可读摘要
    const summary = this.buildSummary(clusters, severityTrend, density, categoryFrequencies, totalChapters, totalIssues)
  ...
  private aggregateCategoryFrequencies(snapshots: ChapterCorrectionSnapshot[]): CategoryFrequency[] {
    const stats = new Map<CorrectionCategory, { count: number; chapters: Set<number>; severitySum: number }>()
  ...
  private buildSummary(
    ...
    // 趋势
    const trendIcon =
      trend.direction === 'improving' ? '📈' : trend.direction === 'declining' ? '📉' : trend.direction === 'volatile' ? '📊' : '➡️'
    ...
    if (density.range) {
      lines.push(`📌 问题最密集区间：第${density.range.from}-${density.range.to}章（${density.range.totalIssues} 个问题）`)
    }
