...
const CONTINUITY_BONUS_WEIGHT = 0.1
...
function evaluateStyleConsistency(chapterText: string): number {
  ...
  const styleIndicators = [
    ...
    { pattern: /•/, weight: 0.1 }, // 列表符号
    { pattern: /┃/, weight: 0.15 }, // 引用标记
    { pattern: /〖.*?〗/, weight: 0.1 }, // 强调标记
    { pattern: /\n\n/, weight: 0.1 }, // 段落间距
    { pattern: /^\s*╔═/, weight: 0.15, multiline: true }, // 标题框
    { pattern: /✅|📱|📝/, weight: 0.1 }, // emoji 标记
    { pattern: /---\s*\n/, weight: 0.05 }, // 分隔线
    { pattern: /《.*?》/, weight: 0.1 }, // 书名号引用
  ]
...
function evaluateStructuralAccuracy(chapterText: string): number {
  ...
  if (avgParaLen >= 50 && avgParaLen <= 300) score += 0.1
  else if (avgParaLen >= 20 && avgParaLen <= 500) score += 0.05

  // 行数合理
  if (lines.length >= 20 && lines.length <= 200) score += 0.1
 ...
 */
function evaluateContinuityScore(chapterText: string, chapterIndex: number, allChapters: Record<number, string>): number {
  let score = 0.7 // 默认中高

  // 如果不是首章(19)，检查是否有与上一章衔接的线索
  if (chapterIndex > 19 && allChapters[chapterIndex - 1]) {
    const prevChapter = allChapters[chapterIndex - 1]
    const prevLastLines = prevChapter
      .split('\n')
      .filter((l) => l.trim())
      .slice(-5)
      .join(' ')
    const currentFirstLines = chapterText
      .split('\n')
      .filter((l) => l.trim())
      .slice(0, 5)
      .join(' ')
 ...
 */
function detectIssues(chapterText: string, chapterIndex: number, scores: DimensionScores): CorrectionIssue[] {
...
export class SonggeCorrectionAdapter implements ISonggeCorrectionProvider {
   ...
   */
  private checkFormatting(chapterText: string, options?: Partial<WechatFormatOptions>): FormatResult | null {
    try {
