...
export class BlogPublishService {
  ...
  listActiveTasks(): PublishTask[] {
    return Array.from(this.tasks.values()).filter((t) => !['completed', 'failed', 'rolled_back'].includes(t.status))
  }
    ...
    try {
      ...
      if (imageResult.uploaded > 0 || imageResult.totalImages === 0) {
        this.emitProgress(
          ...
          undefined,
          'success',
          `图片预处理完成: ${imageResult.uploaded} 上传, ${imageResult.failed} 失败`,
          100,
        )
      } else {
        this.emitProgress(taskId, 'prepare', undefined, 'failed', `图片预处理失败: ${imageResult.failed} 张图片上传失败`, 100)
      }
  ...
  private async executePublish(task: PublishTask): Promise<void> {
    ...
    for (let i = 0; i < platforms.length; i++) {
      ...
      this.emitProgress(task.taskId, 'publish', platform, 'started', `正在发布到 ${platform}...`, progress)
      ...
      try {
        ...
        this.emitProgress(
          ...
          platform,
          'success',
          `✓ ${platform} 发布成功${result.postUrl ? `: ${result.postUrl}` : ''}`,
          progress,
        )
      } catch (err: any) {
        ...
        this.emitProgress(task.taskId, 'publish', platform, 'failed', `✗ ${platform} 发布失败: ${err.message}`, progress)
    ...
    if (failCount === 0) {
      ...
      this.emitProgress(task.taskId, 'complete', undefined, 'success', `✅ 全平台发布完成（${successCount}/${platforms.length} 成功）`, 100)
    } else if (successCount === 0) {
      task.status = 'failed'
      this.emitProgress(task.taskId, 'complete', undefined, 'failed', `❌ 全平台发布失败（${platforms.length} 个平台均失败）`, 100)
    } else {
      task.status = 'partial'
      this.emitProgress(task.taskId, 'complete', undefined, 'success', `⚠️ 部分完成: ${successCount} 成功, ${failCount} 失败`, 100)
    }
  ...
  private async doRollback(task: PublishTask, platforms: PublishPlatform[]): Promise<PublishResult> {
    ...
    for (const platform of platforms) {
      ...
      this.emitProgress(task.taskId, 'rollback', platform, 'started', `回滚 ${platform}...`, 50)
      ...
      try {
        ...
        this.emitProgress(task.taskId, 'rollback', platform, 'success', `✓ ${platform} 已回滚`, 80)
      } catch (err: any) {
        status.status = 'failed'
        failedRollback.push(platform)
        results.push({ platform, status: 'failed', postId: status.postId, error: err.message })

        this.emitProgress(task.taskId, 'rollback', platform, 'failed', `✗ ${platform} 回滚失败: ${err.message}`, 80)
      }
    }
    ...
    this.emitProgress(
      ...
      task.status === 'rolled_back' ? '✅ 回滚完成' : '⚠️ 部分回滚失败',
      100,
    )
  ...
  getStatusText(taskId: string): string {
    ...
    const lines: string[] = [
      ...
      task.approvalStatus
        ? `审批: ${task.approvalStatus === 'approved' ? '✅ 已批准' : task.approvalStatus === 'rejected' ? '❌ 已驳回' : '⏳ 待审批'}`
        : '',
          ...
          : ps.status === 'failed'
            ...
            : ps.status === 'publishing'
              ...
              : ps.status === 'rolled_back'
                ? '↩️'
                : ps.status === 'rolling_back'
                  ? '🔄'
                  : '⬜'
      lines.push(
        `  ${icon} ${ps.platform}: ${ps.status}${ps.postUrl ? ` → ${ps.postUrl}` : ''}${ps.error ? ` (${ps.error.slice(0, 100)})` : ''}`,
      )
    }
