...
import { scheduler, Scheduler } from '@akemi-mio/core/core/Scheduler'
...
import { EVOLUTION_SAFETY_MODE, WORKSPACE } from '@akemi-mio/core/config'
...
import type { UserBehaviorLayer } from '@akemi-mio/evolution/user-behavior/UserBehaviorLayer'
import type { PreProcessContext, PostProcessContext, PostProcessResult, DegradationSignal } from '@akemi-mio/evolution/user-behavior/types'
import { createMessageId } from '@akemi-mio/core/db/messages'
import { getMainWindow } from '@akemi-mio/core/core/Lifecycle'
import { evolutionCheckpointManager } from '@akemi-mio/evolution-core'
import type { MemoryEvolutionBridge, MemoryChangeEvent } from '@akemi-mio/intelligence-memory/MemoryEvolutionBridge'
...
import { moduleFeedbackManager } from '@akemi-mio/evolution-feedback'
  ...
  private onMemoryChangeEvent(event: MemoryChangeEvent): void {
    switch (event.type) {
      ...
      case 'user_preference_changed':
        ...
        if (this.schedulerState === EvolutionSchedulerState.IDLE && this.recoveryCooldownUntil === 0) {
  ...
  async stop(): Promise<void> {
    ...
    if (currentId) {
      try {
        ...
        const db = getRawDb()
        if (db) {
          db.run('UPDATE evolution_checkpoints SET status = ?, completed_at = ? WHERE id = ?', ['interrupted', Date.now(), currentId])
