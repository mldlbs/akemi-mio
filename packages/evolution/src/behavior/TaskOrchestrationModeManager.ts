...
export class TaskOrchestrationModeManager {
   ...
   */
  recordToolBatchResult(toolResults: Array<{ name: string; success: boolean; error?: string }>): void {
   ...
   */
  getModeRecommendations(forceExecute = false): {
    /** 是否应减少工具调用链长度 */
    ...
    // 强制工具路由（route_required）下行为适应系统不得干预执行
    if (forceExecute) {
      return {
