...
export class AsrKeywordActionTracker {
  ...
  getStats(): AsrKeywordTrackerStats {
    ...
    const allSlots = this.cacheValid && this.cachedSlots ? this.cachedSlots : this.getAllSlots()
    return {
      totalRecords: this.records.length,
  ...
  private extractActions(text: string): AsrActionCategory[] {
    ...
    const sortedKeywords = Object.keys(KEYWORD_ACTION_MAP).sort((a, b) => b.length - a.length)
   ...
   * 获取或构建单个时段的预测。
   */
  private getOrBuildSlot(slotKey: string, dayOfWeek: number, hour: number, bucket: number): TimeSlotActionPrediction | null {
    // 如果完整缓存可用且有该时段，直接返回
    ...
    const relevant = this.records.filter((rec) => {
      ...
      return d.getDay() === dayOfWeek && d.getHours() === hour && Math.floor(d.getMinutes() / MINUTES_PER_BUCKET) === bucket
    })
  ...
  private buildSlotFromRecords(
    ...
    for (const [category, data] of actionCounts) {
      ...
      predictions.push({
        ...
        periodicityStrength,
        suggestHint: confidence >= HINT_CONFIDENCE_THRESHOLD && periodicityStrength >= PERIODICITY_STRENGTH_THRESHOLD,
        actionLabel,
        hintMessage,
