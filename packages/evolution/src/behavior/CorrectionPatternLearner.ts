...
export class CorrectionPatternLearner {
   ...
   * 查找两个参数对象之间值发生变化的键。
   */
  private findChangedParams(original: Record<string, unknown>, corrected: Record<string, unknown>): string[] {
  ...
  private processCorrectionEvent(event: CorrectionEvent): void {
    ...
    for (const param of changedParams) {
      ...
      if (existing !== undefined && existing !== String(newValue)) {
        // 值变化：记录新的偏好值
        behaviorPreferenceStore.set(prefKey, newValue as PreferenceValue, 'tool_default', `corrected_from_${JSON.stringify(existing)}`)
