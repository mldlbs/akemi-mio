...
import { behaviorPredictor, type SessionStartPredictionResult } from '@akemi-mio/intelligence-mcp/BehaviorPredictor'
import { periodicPatternAnalyzer } from '@akemi-mio/intelligence-mcp/PeriodicPatternAnalyzer'
...
export interface PreloadStats {
  /** 预测结果 */
  prediction: SessionStartPredictionResult | null
  /** 成功预加载的工具数 */
...
export class BehaviorPreloaderService {
  ...
  async preloadSessionStart(): Promise<PreloadStats> {
    ...
    const results = await Promise.allSettled(toPreload.slice(0, MAX_CONCURRENT).map((item) => this.executeSinglePreload(item.toolName, {})))
  ...
  private async executeSinglePreload(toolName: string, args: Record<string, any>): Promise<void> {
    ...
    await behaviorPredictor.preloadTool(cacheKey, toolName, toolName, args, this.executor)
  }
