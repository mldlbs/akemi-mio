...
export class BehaviorPeriodicPreloadService {
  ...
  start(intervalMs: number = CHECK_INTERVAL_MS): void {
    ...
    // 首次执行延迟 30 秒
    setTimeout(() => {
      this.checkAndPreload().catch((err) => log('WARN', 'periodic_preload_first_check_failed', { error: String(err) }))
    }, 30_000)

    this.timer = setInterval(() => {
      this.checkAndPreload().catch((err) => log('WARN', 'periodic_preload_check_failed', { error: String(err) }))
    }, intervalMs)
  }
        ...
        if (this.preloadExecutor && this.activePreloads.size < MAX_CONCURRENT_PRELOADS) {
          ...
          if (!this.activePreloads.has(cacheKey)) {
            ...
            try {
              ...
              const result = await Promise.race([this.preloadExecutor(prediction), timeoutPromise])
