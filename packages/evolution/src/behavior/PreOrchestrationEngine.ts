...
import type { BehaviorPattern, PatternStep, PatternMatchResult, PreOrchestrationPlan, ResolvedStep, ExecutionEntry } from './types'
...
export class PreOrchestrationEngine {
   ...
   */
  async confirm(planId: string, modifiedSteps?: ResolvedStep[]): Promise<boolean> {
  ...
  private matchContext(
    ...
    if (pattern.associatedCategories.length > 0 && context.category) {
      if (pattern.associatedCategories.includes(context.category)) {
        ...
        // 部分匹配：检查是否包含相似类别
        const partialMatch = pattern.associatedCategories.some((cat) => cat.includes(context.category) || context.category.includes(cat))
        if (partialMatch) {
          score += 0.15
          reasons.push('类别部分匹配')
        }
   ...
   */
  private resolveArgs(template: Record<string, string>, vars: Record<string, string>, context: ToolCallContext): Record<string, any> {
  ...
  private async callTool(toolName: string, args: Record<string, any>): Promise<string> {
    ...
    try {
      ...
      const durationMs = Date.now() - startTime
      toolCallLogStore.record(toolName, { ...args, _source: 'pre_orchestration' }, null, err.message, durationMs, false)
      throw err
    }
  }
