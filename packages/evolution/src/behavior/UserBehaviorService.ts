...
export class UserBehaviorService {
  ...
  getEnrichedState(): EnrichedBehaviorState {
    ...
    const snapshot = behaviorStateMachine.computeMode(base.activityState, base.appCategory, base.idleTimeMs, base.focused)
   ...
   */
  private detectActivityContext(activityState: ActivityState, appCategory: AppCategory, mode: BehaviorMode): ActivityContext {
    // 休息 / 空闲 / 离开 → resting
    ...
    this.eventSubscriptions.push(
      eventBus.on('agent.plan.step', (p: EventPayload['agent.plan.step']) => {
        ...
        if (p.status === 'done' || p.status === 'failed') {
          planTypeScriptExecutor.onPlanStepCompleted(`步骤 ${p.stepIndex}`, p.status === 'done')
          // 每 3 步检查一次自适应调整
