...
export class TtsCleanupDualModeController {
  ...
  getCurrentModeLabel(): string {
    return this.currentMode === 'tts' ? 'TTS 语音交互模式' : '工作区清理计划模式'
  }
  ...
  evaluateConditions(
    ...
    const interactionRate = behavior.idleTimeMs > 0 ? Math.round(60000 / Math.max(behavior.idleTimeMs, 1000)) : 30 // 没有空闲说明一直在交互
    ...
    if (this.currentMode === 'tts') {
      ...
      if (shouldSwitch) {
        return this.buildDecision('tts', 'plan-cleanup', 'user_idle_long', shouldSwitch.confidence, metrics, shouldSwitch.debounceMs)
      }
    }
  ...
  private evaluateCleanupModeConditions(
    ...
    if (metrics.rootLevelFileCount >= ROOT_FILE_CLUTTER_THRESHOLD) {
      ...
      // 超出越多，权重越高
      confidence += Math.min(0.3, 0.1 + excess * 0.02)
      activeFactors.push(true)
    ...
    if (metrics.emptyDirCount >= EMPTY_DIR_THRESHOLD) {
      confidence += Math.min(0.2, 0.05 + (metrics.emptyDirCount - EMPTY_DIR_THRESHOLD) * 0.02)
      activeFactors.push(true)
    }
  ...
  getDiagnostics(): Record<string, unknown> {
    return {
      ...
      lastTtsSnapshotAge: this.lastTtsSnapshot ? Date.now() - this.lastTtsSnapshot.savedAt : null,
      lastCleanupSnapshotAge: this.lastCleanupSnapshot ? Date.now() - this.lastCleanupSnapshot.savedAt : null,
    }
  }
}
