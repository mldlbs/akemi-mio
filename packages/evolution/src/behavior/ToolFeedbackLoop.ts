...
export class ToolFeedbackLoop {
  ...
  constructor(config?: ToolFeedbackLoopConfig) {
    this.config = {
      ...
      unconfirmThreshold: config?.unconfirmThreshold ?? UNCONFIRM_THRESHOLD,
  ...
  updateConfig(patch: Partial<ToolFeedbackLoopConfig>): void {
    ...
    if (patch.unconfirmThreshold !== undefined) this.config.unconfirmThreshold = patch.unconfirmThreshold
  ...
  private recordSample(toolName: string, success: boolean, error?: string): void {
    ...
    if (!state) {
      ...
      const delta = newEma - prevEma
      const direction = delta > CONVERGENCE_RATE_THRESHOLD ? 1 : delta < -CONVERGENCE_RATE_THRESHOLD ? -1 : 0
    ...
    if (now - this.lastEvaluationTime >= this.config.evaluationIntervalMs) {
      ...
      this.evaluateAll().catch((err) => log('WARN', 'tool_feedback_evaluation_error', { error: String(err) }))
    }
  }
