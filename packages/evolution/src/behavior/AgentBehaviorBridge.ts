...
import { eventBus, type EventName, type EventPayload, type Listener } from '@akemi-mio/core/core/EventBus'
import {
...
export type BehaviorProducedEvent = Extract<AgentBehaviorEventName, 'behavior.state.updated' | 'behavior.mode.switch'>
...
export class AgentBehaviorBridge {
   ...
   */
  emitAgentEvent<E extends AgentProducedEvent & EventName>(event: E, payload: EventPayload[E]): void {
   ...
   * @param payload 事件载荷（不含 _schemaVersion，由桥接自动注入）
   */
  emitBehaviorEvent<E extends BehaviorProducedEvent & EventName>(event: E, payload: EventPayload[E]): void {
   ...
   */
  private emitWithSchema<E extends BridgeManagedEvent & EventName>(event: E, payload: EventPayload[E], expectedSource: EventSource): void {
   ...
   * @returns disposer 函数
   */
  onAgentEvent<E extends AgentProducedEvent & EventName>(event: E, handler: Listener<E>, label?: string): () => void {
   ...
   * @returns disposer 函数
   */
  onBehaviorEvent<E extends BehaviorProducedEvent & EventName>(event: E, handler: Listener<E>, label?: string): () => void {
    return this.subscribeWithSchema(event, handler, label)
  }
   ...
   */
  private subscribeWithSchema<E extends BridgeManagedEvent & EventName>(event: E, handler: Listener<E>, label?: string): () => void {
