...
import { type VoiceIntentDef, registerDynamicIntent, unregisterDynamicIntent, clearDynamicIntents } from '@akemi-mio/capabilities/tool/voice-intent-map'
...
export class BehaviorSequenceLearner {
   ...
   */
  private extractSequences(calls: Array<{ name: string; timestamp: number }>): Map<string, { count: number; latestGapMs: number }> {
  ...
  private registerVoiceCommands(): void {
    ...
    // 获取活跃且高频的模式
    const activePatterns = this.patterns.filter((p) => p.isActive && p.frequency >= this.minFrequency).slice(0, this.maxVoiceIntents)
    ...
    for (const pattern of activePatterns) {
      ...
      const hasFileOps = pattern.tools.some((t) => t.includes('read_file') || t.includes('write_file') || t.includes('edit_file'))
      if (hasFileOps) {
