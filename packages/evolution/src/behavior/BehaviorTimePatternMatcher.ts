...
import type { InteractionTracker } from '@akemi-mio/intelligence-memory/InteractionTracker'
...
export class BehaviorTimePatternMatcher {
  ...
  start(intervalMs: number = CHECK_INTERVAL_MS): void {
    ...
    // 首次执行延迟 10 秒
    setTimeout(() => {
      this.runAnalysis().catch((err) => log('WARN', 'time_pattern_matcher_first_run_failed', { error: String(err) }))
    }, 10_000)

    this.timer = setInterval(() => {
      this.runAnalysis().catch((err) => log('WARN', 'time_pattern_matcher_run_failed', { error: String(err) }))
    }, intervalMs)
  }
  ...
  async runAnalysis(): Promise<PreloadRunResult> {
    ...
    try {
      ...
      const { getRecentInteractions, getSuggestedTopicsForHour, getBehaviorWeightedEntries, getUserBehaviorState, getAllEntries } =
        this.deps

      // ── 1. 构建行为指纹 ──
      const fingerprint = this.buildFingerprint(getRecentInteractions(RECENT_INTERACTION_WINDOW), getUserBehaviorState)
  ...
  getPreloadCacheContext(): string {
    ...
    const parts: string[] = ['---', '【行为预测记忆】根据你当前的行为模式（时间·应用·话题），以下信息可能正被需要：']
  ...
  private buildCandidatePool(
    ...
    if (suggestedTopics.length > 0) {
      ...
      for (const entry of all) {
        ...
        const matchesTopic = suggestedTopics.some((topic) => entryTopics.includes(topic) || lowerContent.includes(topic.toLowerCase()))
   ...
   */
  private scoreAllEntries(entries: MemoryEntry[], fingerprint: BehaviorFingerprint): MatchedMemory[] {
    const now = Date.now()
    ...
    return entries
      .map((entry) => {
        ...
        // ── timeScore: 时段话题匹配度 ──
        const timeScore = this.computeTimeScore(entryTopics, entry.content, fingerprint.currentHour)

        // ── topicScore: 近期话题 Jaccard 相似度 ──
        const topicScore = this.computeTopicScore(entryTopics, entry.content, fingerprint.recentTopics)
        ...
        // ── 综合评分 ──
        const compositeScore = Math.round((0.35 * timeScore + 0.3 * topicScore + 0.2 * appScore + 0.15 * recencyScore) * 100) / 100
   ...
   */
  private computeTimeScore(entryTopics: string[], entryContent: string, currentHour: number): number {
   ...
   */
  private computeTopicScore(entryTopics: string[], entryContent: string, recentTopics: string[]): number {
  ...
  private makeResult(scored: MatchedMemory[], fingerprint: BehaviorFingerprint, skipReason: string, startTime: number): PreloadRunResult {
    const aboveThreshold = scored.filter((m) => m.compositeScore >= MATCH_THRESHOLD)
