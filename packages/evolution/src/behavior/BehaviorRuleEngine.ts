...
export class BehaviorRuleEngine {
  ...
  private _buildResult(decisions: RuleDecision[], byType: Record<string, RuleDecision[]>): RuleEngineResult {
    ...
    const autoToolTrigger = toolDecisions.length > 0 ? toolDecisions.sort((a, b) => b.priority - a.priority)[0].value === true : true // 默认启用
   ...
   * 评估单条规则的条件组是否满足。
   */
  private _evaluateConditions(conditions: ConditionGroup, profile: BehaviorProfile): boolean {
    // 评估 all 组（与条件）
   ...
   */
  private _evaluateAtomic(cond: AtomicCondition, profile: BehaviorProfile): boolean {
   ...
   */
  private _getProfileField(field: string, profile: BehaviorProfile): any {
    switch (field) {
  ...
  /** 执行比较操作 */
  private _compare(actual: any, op: ConditionOp, expected: any): boolean {
    switch (op) {
      case 'gt':
   ...
   * 在此添加需要特殊逻辑的判断。
   */
  private _evaluateCustom(customName: string, profile: BehaviorProfile): boolean {
    switch (customName) {
      case 'topToolDominance':
