 ...
 * 过时版本视为兼容（自动迁移）。
 */
export function isSchemaVersionCompatible(event: string, payloadVersion: number | undefined): boolean {
 ...
 */
export function migratePayload<E extends AgentBehaviorEventName>(event: E, payload: Record<string, unknown>): Record<string, unknown> {
 ...
 */
export function logUnexpectedFields(event: string, payload: Record<string, unknown>, expectedFields: string[]): void {
  const actualFields = Object.keys(payload)
  const unexpected = actualFields.filter((f) => !expectedFields.includes(f) && f !== '_schemaVersion')
...
export const BEHAVIOR_PRODUCED_EVENTS: ReadonlySet<string> = new Set(['behavior.state.updated', 'behavior.mode.switch'])
