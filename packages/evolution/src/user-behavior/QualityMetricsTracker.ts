...
import type { QualityMetricsSnapshot, MetricSnapshot, MetricTrend, DegradationSignal, QualityMetricsConfig } from './types'
...
export class QualityMetricsTracker {
  ...
  recordSnapshot(): QualityMetricsSnapshot {
    ...
    const degradationSignals = isDegraded ? this.detectDegradation(metricsWithTrend, prev!) : []
   ...
   */
  private computeMetrics(features: ReturnType<typeof behaviorFeatureExtractor.extract>, heatmap: ModuleHeatmap): MetricSnapshot[] {
    const metrics: MetricSnapshot[] = []

    // ── 1. 整体错误率 ──
    if (features.frequentTools.length > 0) {
      const totalCalls = features.frequentTools.reduce((s, t) => s + t.callCount, 0)
      const failedCalls = features.frequentTools.reduce((s, t) => s + Math.round(t.callCount * (1 - t.successRate)), 0)
    ...
    if (heatmap.hasSufficientData && heatmap.entries.length > 0) {
      const avgModuleErrorRate = heatmap.entries.reduce((s, e) => s + e.errorRate, 0) / heatmap.entries.length
      const highErrorModules = heatmap.entries.filter((e) => e.errorRate > 0.2).map((e) => e.module)
      metrics.push({
    ...
    // ── 3. 平均工具成功率 ──
    if (features.frequentTools.length > 0) {
      const avgSuccessRate = features.frequentTools.reduce((s, t) => s + t.successRate, 0) / features.frequentTools.length
      metrics.push({
    ...
    if (features.pausePoints.length > 0) {
      ...
      const avgSeverity = features.pausePoints.reduce((s, p) => s + p.avgWaitMs / maxPause, 0) / features.pausePoints.length
      // 倒置：1 = 无停顿（好），0 = 严重停顿（差）
      metrics.push({
        name: 'pause_severity',
        value: 1 - avgSeverity,
    ...
    // ── 6. 高频失败工具占比 ──
    const highFailTools = features.frequentTools.filter((t) => t.successRate < 0.7 && t.callCount >= 3)
   ...
   * 计算指标趋势：与上一周期对比。
   */
  private computeTrends(current: MetricSnapshot[], previous: QualityMetricsSnapshot | null): MetricSnapshot[] {
   ...
   */
  private detectDegradation(current: MetricSnapshot[], previous: QualityMetricsSnapshot): DegradationSignal[] {
    ...
    for (const metric of current) {
      ...
      const relatedModules = metric.relatedModules ? metric.relatedModules.split(',').filter(Boolean) : []
   ...
   */
  private buildDegradationDescription(metric: MetricSnapshot, prevMetric: MetricSnapshot, severity: number): string {
