...
const FEATURE_RULES: FeatureRule[] = [
  // ── 基于雷达指标（RadarMetricsMonitor）─
  {
    ...
    evaluate: (a) => {
      ...
      const ratio =
        a.collectorHealth.healthyCollectors.length /
  ...
  },
  {
    ...
    reasonTemplate: (s) => `外部雷达扫描显示模块关注度峰值 ${Math.round(s * 100)}%，行为驱动优化可针对性改进`,
    source: 'radar_feedback',
  },
  ...
  // ── 基于外部趋势（radar_scan）─
  {
    ...
    reasonTemplate: (_s, a) => `雷达扫描到 ${a.trendingTopics.length} 个外部趋势热点，热力图可结合内外信号优化进化方向`,
    source: 'radar_trend',
  },
  {
    ...
    reasonTemplate: (_s, a) => `外部趋势丰富（${a.trendingTopics.length} 个），应集中进化资源在热点模块上`,
    source: 'radar_trend',
  },
  ...
  },
  {
    ...
    reasonTemplate: (_s, a) => `${a.collectorHealth.unhealthyCollectors.length} 个采集器异常，需阻尼防止参数震荡`,
    source: 'system_health',
  },
]
...
export class RadarLedFeatureController {
  ...
  constructor(deps?: RadarServiceDeps, envWhitelist?: UserBehaviorFeature[]) {
    this.deps = deps ?? {
      isAvailable: () => false,
    }
    // 读取环境变量作为白名单（两级控制的上限）
  ...
  async evaluateAndApply(): Promise<FeatureRecommendation> {
    ...
    const directives: RadarFeatureDirective[] = FEATURE_RULES.map((rule) => {
      ...
      return {
        ...
        strength,
        reason: strength >= this.THRESHOLD ? rule.reasonTemplate(strength, assessment) : `强度不足（${strength.toFixed(2)}），跳过`,
        source: rule.source,
      }
    })

    // ── 步骤 3：过滤出推荐开启的 feature ──
    const radarRecommended = directives.filter((d) => d.strength >= this.THRESHOLD).map((d) => d.feature)

    // ── 步骤 4：与环境变量白名单取交集 ──
    // 两级控制：env var 是允许范围，雷达推荐是动态子集
    // feature 必须在两者中都出现才生效
    const enabledFeatures: UserBehaviorFeature[] =
      this.envWhitelist.size > 0 ? radarRecommended.filter((f) => this.envWhitelist.has(f)) : radarRecommended

    // 如果环境变量有值但雷达无推荐，保留 env var 的原始行为（安全降级）
    const finalFeatures = enabledFeatures.length > 0 ? enabledFeatures : this.envWhitelist.size > 0 ? Array.from(this.envWhitelist) : []

    // ── 步骤 5：比较是否有变更 ──
    const previous = this.lastRecommendation
    const changed = !previous || !this.featureSetsEqual(new Set(previous.enabledFeatures), new Set(finalFeatures))
  ...
  private async collectAssessment(): Promise<RadarSystemAssessment> {
    ...
    try {
      ...
      const attSources = [...assessment.collectorHealth.unhealthyCollectors, ...assessment.trendingTopics.slice(0, 5)]
 ...
 */
export function createRadarLedController(layer?: UserBehaviorLayer): RadarLedFeatureController {
 ...
 * ```
 */
export async function radarLedPreProcess(controller: RadarLedFeatureController): Promise<FeatureRecommendation> {
 ...
 */
export function adaptRadarMetricsToDeps(getAssessment: () => RadarMetricsAssessment | null): Partial<RadarServiceDeps> {
  return {
