...
import { getLocalProviderAdapter } from '@akemi-mio/intelligence-mcp/LocalProvider'
...
import { fixTemplateRegistry } from '@akemi-mio/capabilities/tool/FixTemplateRegistry'
...
import { failurePatternAnalyzer } from '@akemi-mio/capabilities/tool/FailurePatternAnalyzer'
import type { FailurePattern } from '@akemi-mio/capabilities/tool/FailurePatternAnalyzer'
...
const CONFIG = {
  ...
  /** 工具定义目录 */
  DEFINITIONS_DIR: 'packages/capabilities/src/tool/definitions',
}
  ...
  for (const call of historicalCalls) {
    try {
      ...
      if (result.isError) {
        ...
        failures.push({ args: call.args, error: errorText.slice(0, 200) })
        continue
      }
      passedCount++
    } catch (err: any) {
...
export class ToolEvolutionExecutor implements FixExecutor {
  ...
  async execute(problem: AssignedProblem): Promise<FixResult> {
    ...
    const targetPattern = patterns.length > 0 ? patterns.find((p) => String(p.suggestedFix) === String(suggestion)) || patterns[0] : null
