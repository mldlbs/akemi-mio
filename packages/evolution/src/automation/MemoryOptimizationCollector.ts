...
import type { MemoryOptimizationStats } from '@akemi-mio/intelligence-memory/MemoryOptimizationConfig'
import { TUNABLE_RANGES } from '@akemi-mio/intelligence-memory/MemoryOptimizationConfig'
...
export class MemoryOptimizationCollector implements SignalCollector {
  ...
  private analyze(stats: MemoryOptimizationStats): AnalysisFinding[] {
    ...
    if (stats.accessStats.longTailRatio > LONG_TAIL_THRESHOLD) {
      ...
      const suggestedDecay = Math.min(currentDecay * 1.3, TUNABLE_RANGES.behaviorScoreDailyDecay.max)
      ...
      findings.push({
        ...
        suggestedParams: suggestedDecay !== currentDecay ? { behaviorScoreDailyDecay: suggestedDecay } : undefined,
    ...
    if (stats.utilityStats.lowUtilityRatio > LOW_UTILITY_THRESHOLD) {
      ...
      const suggestedThreshold = Math.max(TUNABLE_RANGES.utilityLowThreshold.min, currentLow - 0.03)
      ...
      findings.push({
        ...
        suggestedParams: suggestedThreshold !== currentLow ? { utilityLowThreshold: suggestedThreshold } : undefined,
    ...
    if (stats.accessStats.staleEntriesCount > 5 && stats.accessStats.staleEntriesCount > stats.totalEntries * STALE_THRESHOLD) {
      ...
      const suggestedDecay = Math.min(currentDecay * 1.5, TUNABLE_RANGES.utilityDailyDecay.max)
      ...
      findings.push({
        ...
        suggestedParams: suggestedDecay !== currentDecay ? { utilityDailyDecay: suggestedDecay } : undefined,
    ...
    if (semiUsage > SEMI_CAPACITY_USAGE_THRESHOLD) {
      const suggestedMax = Math.min(Math.ceil(stats.semiCount * 1.3), TUNABLE_RANGES.maxSemi.max)
      findings.push({
        ...
        suggestedParams: suggestedMax > stats.currentConfig.maxSemi ? { maxSemi: suggestedMax } : undefined,
