...
/** 转换规则名称列表 */
type TransformRule = 'inject_timeout' | 'inject_res_ok' | 'add_fallback' | 'inject_retry' | 'inject_metrics' | 'externalize_url'
...
function transformInjectTimeout(content: string): TransformResult {
  ...
  // 模式: fetch(url) 无第二个参数
  content = content.replace(/(fetch\s*\(\s*['"][^'"]+['"])\s*\)/g, (match, urlPart) => {
  ...
  content = content.replace(/(fetch\s*\(\s*['"][^'"]+['"]\s*,\s*\{)([^}]*?)(\}\s*\))/g, (match, open, opts, close) => {
...
function transformInjectResOk(content: string): TransformResult {
  ...
  for (let i = 0; i < modifiedLines.length; i++) {
    ...
    const fetchMatch = line.match(/(\w+)\s*=\s*await\s+fetch\s*\(/)
    ...
    const isInLoop = modifiedLines.slice(Math.max(0, i - 5), i).some((l) => l.includes('for') || l.includes('while'))

    const guardLine = isInLoop ? `${innerIndent}if (!${varName}.ok) continue` : `${innerIndent}if (!${varName}.ok) { continue }`
 ...
 *
 * 注入方式：在文件顶部 import { withRetry } from '@akemi-mio/core/utils/async'
...
function transformInjectRetry(content: string): TransformResult {
  ...
  // 检查是否已有重试相关 import
  const hasRetryImport = content.includes('withRetry') && content.includes('@akemi-mio/core/utils/async')
  // 检查是否已有重试逻辑
  ...
  if (!hasRetryImport) {
    ...
    for (let i = modifiedLines.length - 1; i >= 0; i--) {
      if (modifiedLines[i].startsWith('import ')) {
        ...
        modifiedLines.splice(endIdx, 0, "import { withRetry } from '@akemi-mio/core/utils/async'")
        changed = true
        break
      }
    }
  ...
  for (let i = 0; i < modifiedLines.length; i++) {
    ...
    const fetchMatch = line.match(/^(.*?)(const\s+\w+\s*=\s*await\s+)(fetch\s*\([^)]*\))\s*(.*)$/)
    if (fetchMatch) {
  ...
  // 重新合成 content
  const resultContent = modifiedLines.join('\n')
...
function transformInjectMetrics(content: string): TransformResult {
  ...
  // 检查 import 中是否已有 log
  const hasLogImport =
    content.includes("import { log } from '@akemi-mio/core/logger/Logger'") || content.includes("import { log } from '@akemi-mio/core/logger/Logger'")
    ...
    for (let i = 0; i < modifiedLines.length; i++) {
      for (let uIdx = 0; uIdx < uniqueUrls.length; uIdx++) {
        ...
        if (modifiedLines[i].includes(url) && modifiedLines[i].includes('fetch')) {
          modifiedLines[i] = modifiedLines[i].replace(new RegExp(`['"]${escapedUrl}['"]`), `this.API_URL_${uIdx}`)
        }
      }
    }
  }
...
状态: ✅ tsc 编译验证通过，${snapshotBranch ? '已提交 Git' : '已保存'}`,
  ...
  private inferRule(problem: AssignedProblem): TransformRule | null {
    ...
    if (searchText.includes('hardcoded_url') || searchText.includes('硬编码')) return 'externalize_url'
    if (searchText.includes('missing_timeout') || searchText.includes('超时')) return 'inject_timeout'
    if (searchText.includes('missing_res_ok') || searchText.includes('状态检查')) return 'inject_res_ok'
    if (searchText.includes('missing_retry') || searchText.includes('重试')) return 'inject_retry'
    if (searchText.includes('missing_metrics') || searchText.includes('指标')) return 'inject_metrics'
    if (searchText.includes('missing_fallback') || searchText.includes('备用')) return 'add_fallback'
    if (searchText.includes('hardcoded_secret') || searchText.includes('密钥')) return 'externalize_url'
