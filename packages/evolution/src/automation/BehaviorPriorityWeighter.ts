...
import type { ModuleHeatmap, ModuleHeatmapEntry } from '@akemi-mio/evolution/user-behavior/types'
...
export class BehaviorPriorityWeighter {
    ...
    try {
      ...
      for (const moduleName of moduleFeedbackManager.getTrackedModules()) {
        ...
        if (moduleName in adjusted) {
          ...
          adjusted[moduleName] = Math.max(MIN_FEEDBACK_WEIGHT, Math.min(adjusted[moduleName], adjusted[moduleName] * feedbackFactor))
        }
  ...
  private computeRawWeights(heatmap: ModuleHeatmap): BehaviorWeightMap {
    ...
    const errorSet = new Set(heatmap.errorModules.filter((e) => !hotSet.has(e.module)).map((e) => e.module))
    // 构建冷模块集合
  ...
  private smoothWeights(rawWeights: BehaviorWeightMap, alpha: number): BehaviorWeightMap {
    ...
    const allModules = new Set([...Object.keys(rawWeights), ...Object.keys(this.state.current)])
