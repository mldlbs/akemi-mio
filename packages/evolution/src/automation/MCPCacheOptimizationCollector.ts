...
/** 默认写入工具（不应该缓存） */
const WRITE_TOOL_PATTERNS = [/^set_/, /^write_/, /^create_/, /^update_/, /^delete_/, /^remove_/, /^send_/, /^post_/, /^upload_/, /^save_/]
...
export class MCPCacheOptimizationCollector implements SignalCollector {
      ...
      for (const report of summary.reports) {
        ...
        if (!isCached && !problem) {
          ...
          if (repeatCount >= CONFIG.HIGH_FREQ_REPEAT) {
            problem = this.buildCacheProblem(
              report,
              'add_cache_frequent_repeat',
              'warning',
   ...
   * 构建「建议开启缓存」的问题
   */
  private buildCacheProblem(report: any, suggestion: string, severity: 'critical' | 'warning' | 'info', customReason?: string): Problem {
    const isReadonly = this.isReadonlyTool(report.toolName)
   ...
   * 构建「建议调整 TTL」的问题
   */
  private buildTtlProblem(report: any, ttlSuggestion: { currentTTL: number; suggestedTTL: number; reason: string }): Problem {
    return {
   ...
   * 建议 TTL 延长的具体值
   */
  private suggestTtlExtension(toolName: string, latency: any): { currentTTL: number; suggestedTTL: number; reason: string } | null {
    // 获取当前缓存配置
