...
export class ProblemQueue {
  ...
  setBehaviorWeights(weights: Record<string, number> | null): void {
    ...
    log('INFO', 'problem_queue_behavior_weights_set', {
      ...
      topModule: weights ? Object.entries(weights).sort(([, a], [, b]) => b - a)[0]?.[0] : null,
    })
  }
  ...
  pop(): AssignedProblem | null {
    ...
    const idx = this.problems.findIndex(
      (p) => !this.completedIds.has(p.id) && !this.failedIds.has(p.id) && !this.skippedIds.has(p.id) && !this.blockedIds.has(p.id),
    )
