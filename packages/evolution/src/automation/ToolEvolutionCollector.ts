...
export class ToolEvolutionCollector implements SignalCollector {
  ...
  async collect(): Promise<Problem[]> {
    ...
    try {
      ...
      const shadowIdentityByTool = new Map(
        toolStatsTracker.getProblematicToolIdentitySummaries().map((summary) => [summary.problematicTool.toolName, summary]),
      )
      ...
      for (const pt of problematicTools) {
        const shadowIdentity = shadowIdentityByTool.get(pt.toolName)
        const completeIdentity = shadowIdentity?.identitySummary?.mixed ? undefined : shadowIdentity?.identitySummary
        ...
        // 构建增强的上下文描述
        let patternDetail = ''
        let suggestedFix: string = pt.suggestion
        let affectedParams = ''
          ...
          context: {
            ...
            metadata: {
              ...
              ...(completeIdentity
                ...
                : {}),
              ...(shadowIdentity?.unresolvedIdentityStats
                ? {
                    unresolvedIdentityStats: JSON.stringify(shadowIdentity.unresolvedIdentityStats),
                  }
                : {}),
            },
          },
