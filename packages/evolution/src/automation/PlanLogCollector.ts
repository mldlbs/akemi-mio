...
export class PlanLogCollector implements SignalCollector {
      ...
      for (const logFile of logFiles) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          for (const pattern of ERROR_PATTERNS) {
            ...
            // 提取文件路径和行号
            let fileInfo = pattern.extractFile ? pattern.extractFile(lines, i) : this.extractFileFromLine(line)
  ...
  private buildProblem(
    ...
    const id = `plan_log_${err.pattern.type}_${err.file}_${err.line}`.replace(/[^a-zA-Z0-9_\-.一-鿿]/g, '_').slice(0, 96)
