...
export class ChapterConsistencyCollector implements SignalCollector {
   ...
   */
  private async analyzeWithLLM(chapters: ChapterFile[], designDoc: string, earlySummary: string | null): Promise<AnalysisReport | null> {
...
输出严格 JSON 格式，不要包含任何额外文字。JSON 结构：
{
  ...
  "summary": {
    ...
    const chaptersText = chapters.map((ch) => `===== 第${ch.number}章 =====\n${ch.content.slice(0, 3000)}`).join('\n\n')
  ...
  private reportToProblems(report: AnalysisReport | null, chapters: ChapterFile[]): Problem[] {
    ...
    for (const [chapterNumber, issues] of byChapter) {
      ...
      const worstSeverity = issues.reduce(
        (worst, i) => {
