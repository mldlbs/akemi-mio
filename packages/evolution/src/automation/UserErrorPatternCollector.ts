...
import { toolFeedbackLoop } from '@akemi-mio/evolution/behavior/ToolFeedbackLoop'
...
export class UserErrorPatternCollector implements SignalCollector {
  ...
  async collect(): Promise<Problem[]> {
    try {
      ...
      // 2. 过滤出高/中错误率的工具
      const highErrorTools = reports.filter((r) => r.sampleCount >= MIN_SAMPLES_FOR_ANALYSIS && r.emaSuccessRate < HIGH_ERROR_THRESHOLD)
