...
export class EslintCollector implements SignalCollector {
  ...
  async collect(): Promise<Problem[]> {
    ...
    try {
      const stdout = await execAsync('npx eslint src/ --ext .ts,.tsx --format=compact 2>&1', {
      ...
      if (output.trim()) {
        const problems = this.parseEslintOutput(output)
        log('INFO', 'eslint_collector_done', { count: problems.length, via: 'non_zero_exit' })
        return problems
      }
