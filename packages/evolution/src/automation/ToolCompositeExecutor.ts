...
export class ToolCompositeExecutor implements FixExecutor {
  ...
  async execute(problem: AssignedProblem): Promise<FixResult> {
    ...
    try {
      ...
      const alreadyExists = existingProposals.some((p) => p.toolSequence.join('→') === fingerprint && p.status !== 'rejected')
      ...
      const isAutoRegistered = isProposal ? (result as any).status === 'auto_registered' : false
      ...
      return {
        ...
        success: true,
        summary:
          `复合工具 "${toolName}" 已生成${isAutoRegistered ? '并自动注册' : '（待审核）'}。` +
