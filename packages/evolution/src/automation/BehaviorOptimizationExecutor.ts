 ...
 */
function buildOptimizationPrompt(optimizationType: string, target: string, sourceCode: string, behaviorContext: string): string {
      ...
      const res = await fetch(code.apiUrl, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: '你是一个 TypeScript 代码优化专家。请根据优化需求生成改进后的完整 TypeScript 源码。只返回代码，不要添加额外说明。',
            },
...
export class BehaviorOptimizationExecutor implements FixExecutor {
  ...
  private async executeAutoPatch(
    ...
    try {
      ...
      // ── 步骤 9：Git 提交 ──
      try {
        await this.gitOps.autoGitCommit(`[behavior] ${optimizationType}: optimize ${target} based on usage patterns`)
      } catch {
...
状态: ✅ tsc 编译验证通过，已提交 Git`,
   ...
   * 构建优化计划对象。
   */
  private buildOptimizationPlan(problem: AssignedProblem, optimizationType: string, target: string): BehaviorOptimizationPlan {
  ...
  private generateImplementationSteps(optimizationType: string, target: string, problem: AssignedProblem): ImplementationStep[] {
  ...
  private resolveTargetFile(target: string): string {
    const knownModules: Record<string, string> = {
      grep: 'packages/intelligence/src/agent/tools/grep.ts',
      read_file: 'packages/intelligence/src/agent/tools/readFile.ts',
      write_file: 'packages/intelligence/src/agent/tools/writeFile.ts',
      edit_file: 'packages/intelligence/src/agent/tools/editFile.ts',
      analyze_codebase: 'packages/intelligence/src/agent/tools/analyzeCodebase.ts',
      remember_fact: 'packages/intelligence/src/agent/tools/rememberFact.ts',
      create_dev_plan: 'packages/intelligence/src/agent/tools/createDevPlan.ts',
      userBehaviorAnalyzer: 'packages/intelligence/src/agent/UserBehaviorAnalyzer.ts',
      SelfEvolutionService: 'packages/evolution/src/SelfEvolutionService.ts',
      ChatExecutor: 'packages/intelligence/src/agent/ChatExecutor.ts',
    }
