...
export interface EngineEventPayload {
  'constitution.violation': { path: string; pattern: string; reason: string; layer: string; mode: string }
  'constitution.mode_changed': { mode: EnforcementMode; previous: EnforcementMode }
}
...
export class ConstitutionEngine {
  private paths = new ProtectedPaths()
  private mode: EnforcementMode = 'warn'
  private initialized = false
  private listeners = new Map<ConstitutionEvent, Set<EngineListener<any>>>()
  ...
  /** 初始化：从指定目录加载宪法文档 */
  async initialize(constitutionDir?: string): Promise<void> {
  ...
  private emit<E extends ConstitutionEvent>(event: E, payload: EngineEventPayload[E]): void {
