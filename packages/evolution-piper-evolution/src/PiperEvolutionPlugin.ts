...
import { piperReasoningChainExecutor } from '@akemi-mio/audio/reasoning'
import type { EvolutionPlugin, EvolutionPluginManifest, PluginProblem, PluginFixResult } from '@akemi-mio/evolution-plugin-system/types'

import type { ProblemSource } from '@akemi-mio/evolution/automation/types'
import type { AssignedProblem } from '@akemi-mio/evolution/automation/types'
import type { FixResult } from '@akemi-mio/evolution/automation/types'
import type { PiperFailureCategory, PiperFailureSeverity } from '@akemi-mio/audio/reasoning/types'
...
threshold: ${desc.threshold}`,
  ...
  private fixHighLatency(problem: PluginProblem, t0: number): PluginFixResult {
    ...
    return {
      problemId: problem.id,
      success: result.success,
      summary: result.success ? '✅ Piper 已切换为轻量模型（zh_CN-huayan-medium）以降低延迟' : `⚠️ 切换轻量模型失败: ${result.message}`,
      durationMs: Date.now() - t0,
      output: result.message,
    }
  }
