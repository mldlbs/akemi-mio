...
import type { ProblemSource } from '@akemi-mio/evolution/automation/types'
...
export class EvolutionConsumerBridge implements IEvolutionConsumerBridge {
  ...
  refresh(): void {
    ...
    log('INFO', 'evolution_consumer_bridge_refreshed', {
      consumers: this.registrations.size,
      pipeline: fullContext.pipeline ? `collected=${fullContext.pipeline.totalCollected} fixed=${fullContext.pipeline.totalFixed}` : 'none',
  ...
  private buildFullContext(): EvolutionConsumerContext {
    ...
    const pipeline = this.lastPipelineMetrics ? this.buildPipelineSummary(this.lastPipelineMetrics) : null
  ...
  private buildPipelineSummary(metrics: PipelineMetrics): PipelineSummary {
    const healthStatus: PipelineSummary['healthStatus'] =
      metrics.totalFailed > metrics.totalFixed ? 'degraded' : metrics.totalCollected - metrics.totalFixed > 100 ? 'stalled' : 'good'
   ...
   */
  private trimToRequirements(context: EvolutionConsumerContext, requirements: ConsumerRequirements): EvolutionConsumerContext {
    ...
    // 判断是否需要 pipeline 字段
    const needsPipeline =
      allowedFields.has('pipeline.healthStatus') || allowedFields.has('pipeline.totalCollected') || allowedFields.has('pipeline.totalFixed')

    // 构建裁剪后的上下文
    const trimmed: EvolutionConsumerContext = {
      pipeline: needsPipeline ? context.pipeline : null,
      knownIssues:
        allowedFields.has('knownIssues') || allowedFields.has('knownIssues[].severity') || allowedFields.has('knownIssues[].source')
          ? context.knownIssues
          : [],
      scheduler: allowedFields.has('scheduler.state') ? context.scheduler : EMPTY_CONTEXT.scheduler,
