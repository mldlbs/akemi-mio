...
export class ToolCallChainStore {
   ...
   */
  recordLink(toolName: string, args: Record<string, any>, result: string, success: boolean, durationMs: number): void {
    // 如果没有活跃链，不记录（静默跳过）
  ...
  searchSimilar(query: string, limit?: number): CompletedChain[] {
    ...
    if (queryTokens.length === 0) {
      return [...this.completedChains].sort((a, b) => b.completedAt - a.completedAt).slice(0, maxResults)
    }
  ...
  getRecommendations(intent: string): ToolChainRecommendation[] {
    ...
    for (const chain of similar) {
      const score = this.computeSimilarity(intent.toLowerCase(), queryTokens, chain)
      ...
      recommendations.push({
        ...
        similarity: score,
        recommendedTools: chain.toolSequence.slice(0, this.config.MAX_RECOMMENDED_TOOLS),
   ...
   */
  private storeLinkToMemory(toolName: string, args: Record<string, any>, result: string, success: boolean): void {
    try {
      ...
      ms.addEntry('interaction', content, success ? this.config.MEMORY_CONFIDENCE : this.config.MEMORY_CONFIDENCE * 0.6, {
        tier: 'ephemeral',
        structuredData,
      })
   ...
   */
  private computeSimilarity(queryLower: string, queryTokens: string[], chain: CompletedChain): number {
    ...
    const keywordScore = union.size > 0 ? intersection.length / union.size : 0

    // 2. 分类匹配加分
    const categoryBonus = chain.category === this._currentCategory && chain.category !== 'general' ? 0.15 : 0

    // 3. 直接子串匹配加分
    const substringBonus = intentLower.includes(queryLower) || queryLower.includes(intentLower) ? 0.2 : 0

    // 4. 完整包含某关键词精确匹配
    const exactBonus = intersection.some((t) => intentLower.includes(t) && queryLower.includes(t)) ? 0.1 : 0
  ...
  private buildParamPresets(): Record<string, Record<string, string>> {
    const toolParamValues = new Map<string, Map<string, Map<string, number>>>()
