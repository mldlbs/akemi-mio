...
import { summarizeToolIdentity, type ToolIdentitySummary } from './ToolIdentitySummary'
...
export interface ToolAnalyticsReport {
  ...
  /** 最近调用时间 */
  lastCallAt: number
  /** Optional, non-authoritative identity context derived from persisted telemetry. */
  identitySummary?: ToolIdentitySummary
}
...
export class ToolAnalytics {
  ...
  analyzeTool(toolName: string): ToolAnalyticsReport {
    ...
    // ── 8. 优先级推荐 ──
    const { delta: priorityDelta, reason: priorityReason } = this.computePriorityDelta(toolName, errorRate, latency)
    const identitySummary = summarizeToolIdentity(this.getRawCalls(toolName))

    const report: ToolAnalyticsReport = {
      toolName,
      totalCalls,
      successCount,
      failureCount,
    ...
    if (identitySummary) {
      report.identitySummary = identitySummary
    }
  ...
  analyzeAll(): ToolAnalyticsSummary {
    ...
    const problematicCount = reports.filter((r) => r.errorRate >= this.config.ERROR_RATE_THRESHOLD && r.totalCalls >= 5).length
  ...
  getEvolutionContext(): {
    ...
    // 紧急问题：错误率 >= 阈值 且 样本数 >= 5
    const urgentProblems = summary.reports.filter((r) => r.errorRate >= this.config.ERROR_RATE_THRESHOLD && r.totalCalls >= 5)

    // 警告：趋势恶化 或 延迟异常高
    const warnings = summary.reports.filter(
      (r) => !urgentProblems.includes(r) && (r.trend.direction === 'declining' || (r.latency && r.latency.avg > 10_000)),
    )
  ...
  private computeSuccessRateTrend(toolName: string): SuccessRateTrend {
    ...
    const currentRate = currentWindow.filter((c) => c.success).length / currentSamples
    const previousRate = previousWindow.filter((c) => c.success).length / previousSamples
   ...
   */
  private computePriorityDelta(toolName: string, errorRate: number, latency: LatencyPercentiles | null): { delta: number; reason: string } {
    let delta = 0
    const reasons: string[] = []

    // 成功率因子
    if (errorRate <= 0.05 && (toolStatsTracker.getAllToolSummaries().find((s) => s.toolName === toolName)?.totalCalls ?? 0) >= 5) {
