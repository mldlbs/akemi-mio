...
export const VOICE_INTENT_MAP: VoiceIntentDef[] = [
  // ── 文件操作 ──
  {
    intent: 'read_file',
    description: '读取文件内容',
    patterns: ['读', '读取', '打开', '查看', '看看', 'read', 'open', 'show', 'view', 'cat', '读文件', '读取文件', '打开文件', '查看文件'],
    slotExtractors: {
...

  {
    intent: 'write_file',
    description: '写入内容到文件',
    patterns: ['写', '写入', '保存', '创建文件', '新建文件', 'write', 'save', 'create file', 'new file'],
    slotExtractors: {
      filename: /(?:到|为|文件\s*)\s*[""'']?([^\s""'']+(?:\.[a-zA-Z]+)?)[""'']?/,
      content: /(?:内容|写入|写)\s*[：:]\s*(.+)/,
    },
    tools: [{ tool: 'write_file', args: { path: '{{slot.filename}}', content: '{{slot.content}}' } }],
...

  {
    ...
    patterns: ['编辑', '修改', '替换', '改成', '改为', 'edit', 'modify', 'replace', 'change'],
    slotExtractors: {
      filename: /(?:文件)\s*[""'']?([^\s""'']+(?:\.[a-zA-Z]+)?)[""'']?/,
      oldText: /(?:把|将)\s*[""'']?(.+?)[""'']?\s*(?:替换|改成|改为|修改)/,
      newText: /(?:替换|改成|改为|修改)\s*[为成]?\s*[""'']?(.+?)[""'']?(?:\s*$)/,
    },
    tools: [{ tool: 'edit_file', args: { path: '{{slot.filename}}', old_string: '{{slot.oldText}}', new_string: '{{slot.newText}}' } }],
...

  {
    intent: 'search_code',
    description: '搜索代码/文本',
    patterns: ['搜索', '查找', '找', 'grep', '搜', 'search', 'find', 'look for'],
    slotExtractors: {
      pattern: /(?:搜索|查找|找|grep|搜)\s*[""'']?(.+?)[""'']?(?:\s|$)/,
      path: /(?:在|路径|目录)\s*[""'']?([^\s""'']+)[""'']?/,
    },
    tools: [{ tool: 'grep', args: { pattern: '{{slot.pattern}}', path: '{{slot.path}}' } }],
...

  {
    intent: 'list_files',
    description: '列出目录文件',
    patterns: ['列出', '列表', '有哪些', '显示文件', 'ls', 'list', 'ls', 'dir', 'what files'],
    slotExtractors: {
      path: /(?:目录|路径|文件夹)\s*[""'']?([^\s""'']+)[""'']?/,
    },
    tools: [{ tool: 'list_files', args: { path: '{{slot.path}}' } }],
  ...
  // ── 系统状态 / 查询 ──
  {
    ...
    description: '查询系统状态',
    patterns: ['状态', '怎么样', '如何', '运行情况', '健康', 'status', 'health', 'how is', 'diagnostics', '系统状态', '运行状态'],
    tools: [{ tool: 'get_system_health', args: {} }],
...

  {
    intent: 'list_plans',
    description: '查看开发计划',
    patterns: ['计划', '规划', 'plan', '任务', '有哪些计划', 'plans', 'tasks', 'todo'],
    tools: [{ tool: 'list_plans', args: {} }],
...

  {
    ...
    tools: [
      {
        tool: 'voice_update_plan_step',
        args: { stepDescription: '{{slot.stepDescription}}', stepIndex: '{{slot.stepIndex}}', status: 'done' },
      },
    ],
...

  {
    ...
    tools: [
      {
        tool: 'voice_update_plan_step',
        args: { stepDescription: '{{slot.stepDescription}}', stepIndex: '{{slot.stepIndex}}', status: 'in_progress' },
      },
    ],
...

  {
    ...
    patterns: ['完成计划', '计划完成', '结束计划', 'complete plan', 'finish plan', '这个计划完成了', '计划结束', '计划做完', '开发完成'],
    tools: [{ tool: 'complete_plan', args: { plan_id: '' } }],
...

  {
    ...
    tools: [{ tool: 'voice_switch_plan_focus', args: { planTitle: '{{slot.planName}}' } }],
  ...
  // ── 工作流操作 ──
  {
    ...
    description: '列出工作流',
    patterns: ['工作流', 'workflow', '流水线', '有哪些工作流', 'pipelines', '自动化'],
    tools: [{ tool: 'list_workflows', args: {} }],
  ...
  // ── 创造力 / 内容生成 ──
  {
    ...
    description: '生成图片',
    patterns: ['生成图', '画', '绘制', '生成图片', '图片生成', 'generate image', 'draw', 'create image', 'make picture'],
    slotExtractors: {
      prompt: /(?:生成图|画|绘制|生成图片|图片生成)\s*[：:]*\s*(.+)/,
    },
    tools: [{ tool: 'generate_image', args: { prompt: '{{slot.prompt}}' } }],
  ...
  // ── 链式操作：搜索 + 读取 ──
  {
    ...
    patterns: ['找.*读', '搜.*打开', '查.*看', 'find.*read', 'search.*open'],
    slotExtractors: {
  ...
  // ── 学习查询 ──
  {
    ...
    tools: [{ tool: 'learning_query', args: { query: '{{slot.concept}}' } }],
  ...
  // ── 口述代码 ──
  {
    ...
    tools: [{ tool: 'oral_code_generate', args: { description: '{{slot.description}}' } }],
  ...
  // ── 文件整理操作 ──
  {
    ...
    tools: [{ tool: 'organize_file_with_voice', args: { category: '{{slot.targetCategory}}', fileDescription: '{{slot.targetFile}}' } }],
...

  {
    ...
    patterns: ['移到', '移动到', '搬到', '搬去', '挪到', 'move', 'move file', 'mv', '把这个.*到', '把这个.*移到', '拖到', '拖入'],
    slotExtractors: {
      targetPath: /(?:到|去|入)\s*["']?([^\s"']{2,})["']?/,
      fileName: /(?:把|将)\s*["']?([^\s"']+\.[a-zA-Z]+)["']?/,
    },
    tools: [{ tool: 'move_file', args: { source: '{{slot.fileName}}', target: '{{slot.targetPath}}' } }],
...

  {
    ...
    tools: [{ tool: 'query_file_categories', args: {} }],
...

  {
    ...
    tools: [{ tool: 'confirm_voice_operation', args: {} }],
...

  {
    ...
    tools: [{ tool: 'cancel_voice_operation', args: {} }],
    confirmMessage: '取消操作',
    requireConfirmation: false,
  },
  ...
  // ── ODE 求解 ──
  {
    ...
    slotExtractors: {
      equation:
        /(?:求解|计算|solve|calculate)\s*(?:微分方程|diff eq|ode|equation|ODE|)?\s*(?:dy\/dx|y'|y′)?\s*=?\s*(.+?)(?=\s+(?:初值|y\(|y0|初始|起始|从|在|区间|步长|step|from|with|using|use|method|,|$))/,
 ...
 * 从用户文本中提取槽位值
 */
export function extractSlots(text: string, extractors: Record<string, RegExp>): Record<string, string> {
