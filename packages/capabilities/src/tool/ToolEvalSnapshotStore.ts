...
export class ToolEvalSnapshotStore {
   ...
   */
  createEvalSnapshot(appliedChanges: string[], description?: string): ToolEvalSnapshot {
    this.ensureLoaded()
  ...
  evaluateSnapshot(snapshotId: string): EvalVerdict {
    ...
    // 只考虑快照创建后有新调用记录的工具
    const relevantTools = snapshot.beforeMetrics.byTool.filter((bt) => {
    ...
    details.overallRateDelta =
      currentStats.totalRecords > snapshot.beforeMetrics.overall.totalCalls
        ? (currentStats.byTool
            ? Object.values(currentStats.byTool).reduce((s, t) => s + t.success, 0) /
              Math.max(
                Object.values(currentStats.byTool).reduce((s, t) => s + t.total, 0),
                1,
              )
  ...
  getPendingSnapshots(): ToolEvalSnapshot[] {
    this.ensureLoaded()
    return this.snapshots.filter((s) => s.status === 'pending').sort((a, b) => a.timestamp - b.timestamp)
  }
