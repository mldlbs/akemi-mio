...
import {
  summarizeToolIdentity,
  summarizeUnresolvedToolIdentity,
  type ToolIdentitySummary,
  type UnresolvedToolIdentityStats,
} from './ToolIdentitySummary'
...
export interface ProblematicToolIdentitySummary {
  problematicTool: ProblematicTool
  identitySummary?: ToolIdentitySummary
  unresolvedIdentityStats?: UnresolvedToolIdentityStats
}
...
export class ToolStatsTracker {
  ...
  getProblematicTools(): ProblematicTool[] {
    ...
    for (const s of summaries) {
      ...
      if (s.errorRate >= this.config.ERROR_RATE_THRESHOLD) {
        const suggestion: ToolImprovementSuggestion = 'general'
  ...
  /**
   * Returns shadow-only identity context for the existing problematic-tool
   * results. It does not participate in detection, ranking, or cooldowns.
   */
  getProblematicToolIdentitySummaries(): ProblematicToolIdentitySummary[] {
    const recentCalls = behaviorPredictor.getRecentCalls()

    return this.getProblematicTools().map((problematicTool) => {
      const calls = recentCalls.filter((call) => call.toolName === problematicTool.toolName)
      const identitySummary = summarizeToolIdentity(calls)
      const unresolvedIdentityStats = summarizeUnresolvedToolIdentity(calls)

      return {
        problematicTool,
        ...(identitySummary ? { identitySummary } : {}),
        ...(unresolvedIdentityStats ? { unresolvedIdentityStats } : {}),
      }
    })
  }
...
function buildSuggestionReason(summary: ToolCallSummary, suggestion: ToolImprovementSuggestion): string {
  ...
  const parts: string[] = [`工具 "${summary.toolName}" 错误率 ${errorRatePct}%（${summary.failureCount}/${summary.totalCalls}），`]
