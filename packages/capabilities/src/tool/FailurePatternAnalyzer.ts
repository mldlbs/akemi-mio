...
export type FailurePatternType =
  'argument_error' | 'timeout_error' | 'transient_error' | 'tool_missing' | 'permission_error' | 'high_frequency_error' | 'cache_error'
...
export class FailurePatternAnalyzer {
  ...
  analyzeTool(toolName: string, maxSamples: number = 50): FailurePattern[] {
    ...
    // 2. TIMEOUT_ERROR 模式
    // =========================================================
    const timeoutCalls = failedCalls.filter((r) => r.errorType === ToolErrorType.TRANSIENT && r.error?.toLowerCase().includes('timeout'))
    ...
    // =========================================================
    const transientCalls = failedCalls.filter((r) => r.errorType === ToolErrorType.TRANSIENT && !r.error?.toLowerCase().includes('timeout'))
    ...
    // =========================================================
    const remainingFailures = failedCalls.length - patterns.reduce((sum, p) => sum + p.frequency, 0)
   ...
   */
  private analyzeArgPatterns(failedCalls: ToolCallRecord[], allCalls: ToolCallRecord[], toolName: string): ArgPatternMatch[] {
   ...
   */
  private analyzeCachePattern(failedCalls: ToolCallRecord[], allCalls: ToolCallRecord[], toolName: string): FailurePattern | null {
    // 按参数签名分组
