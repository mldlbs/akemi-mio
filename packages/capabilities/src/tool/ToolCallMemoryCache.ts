...
class ToolCallMemoryCache {
   ...
   */
  set(toolName: string, args: Record<string, any>, result: string, durationMs: number, success: boolean, ttlMs?: number): void {
   ...
   * 多次同类失败会触发参数调整逻辑。
   */
  recordError(toolName: string, args: Record<string, any>, error: string, errorType: ToolErrorType | null): void {
    const key = this.buildKey(toolName, args)
    const now = Date.now()
    ...
    if (this.errorRecords.size > ERROR_TRACK_MAX) {
      const entries = [...this.errorRecords.entries()].sort((a, b) => a[1].lastErrorAt - b[1].lastErrorAt)
  ...
  getAdjustedArgs(toolName: string, args: Record<string, any>): ToolAdjustment {
    ...
    if (errorRecord.errorType === ToolErrorType.TRANSIENT && errorRecord.lastError.toLowerCase().includes('timeout')) {
