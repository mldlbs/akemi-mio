...
import type { ToolSpec, GeneratedTool, GenerateToolRequest, GenerateToolResult, GeneratorProgress, LlmToolSpecResponse } from './types'
...
- 从 '@akemi-mio/evolution/types' 导入 { buildTool, formatToolResult, formatToolError }
...
- 通过 await import('@akemi-mio/evolution/types') 获取工具函数
- 不要使用 require()
- 这是一个 async 函数体
...
- '{ execAsync }' from '@akemi-mio/core/utils/async'
- '{ log }' from '@akemi-mio/core/logger/Logger'`
...
function fallbackJsHandler(spec: ToolSpec): string {
  ...
  if (handlerMatch) {
    ...
    return `const { formatToolResult, formatToolError } = await import('@akemi-mio/evolution/types');
${body}`
  }
...
export class ToolGeneratorService {
   ...
   * 完整流程：生成 → 验证 → 注册新工具
   */
  async generateTool(request: GenerateToolRequest, onProgress?: (progress: GeneratorProgress) => void): Promise<GenerateToolResult> {
...
`
  ...
  private registerTool(spec: ToolSpec): string {
    ...
    const handlerFn = new Function('args', 'formatToolResult', 'formatToolError', 'return (async () => { ' + handlerBody + ' })();')

    const wrappedHandler = async (args: Record<string, any>): Promise<MCPToolResult> => {
      try {
        const { formatToolResult: fmtResult, formatToolError: fmtError } = await import('@akemi-mio/capabilities/tool/types')
        const result = await handlerFn(args, fmtResult, fmtError)
        return result as MCPToolResult
      } catch (err: any) {
        const { formatToolError: fmtError } = await import('@akemi-mio/capabilities/tool/types')
        return fmtError(`工具 "${spec.name}" 执行失败: ${err.message}`)
      }
    }

    // 通过 ToolProviderRegistry 注册，避免 capabilities 反向依赖 MCP LocalProvider。
    toolProviderRegistry.register(adaptToToolProvider(providerName, `动态生成工具：${spec.description}`, [
      {
        name: spec.name,
        description: spec.description,
      ...
      try {
        const res = await fetch(code.apiUrl, {
          ...
          headers: {
            ...
            ...(code.apiKey ? { Authorization: `Bearer ${code.apiKey}` } : {}),
          },
          body: JSON.stringify({
            model: code.model,
            messages: [
