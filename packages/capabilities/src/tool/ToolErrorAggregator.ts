...
export class ToolErrorAggregator {
  ...
  // 不可恢复的错误类型（触发降级链终结）
  private static readonly UNRECOVERABLE_TYPES = new Set([ToolErrorType.TOOL_MISSING, ToolErrorType.MCP_ERROR, ToolErrorType.PERMISSION])

  // 可恢复的错误类型（可触发回退）
  private static readonly RECOVERABLE_TYPES = new Set([ToolErrorType.TRANSIENT, ToolErrorType.ENVIRONMENT, ToolErrorType.UNKNOWN])
   ...
   */
  aggregateBatch(toolResults: MinimalToolResult[], batchId?: string): AggregatedBatchResult {
    const bid = batchId ?? this.nextBatchId()

    // 1. 将所有结果转为事件并推入队列
    const events: ToolExecutionEvent[] = toolResults.map((tr) => {
      const errorType = tr.success ? ToolErrorType.UNKNOWN : classifyToolError(tr.error || '')
    ...
    // 4. 生成批量级别建议
    const batchAdvice = this.generateBatchAdvice(recoverable, unrecoverable, successCount, totalCalls)
   ...
   * 记录将执行回退的事件
   */
  recordFallbackExecution(originalToolName: string, fallbackToolName: string, batchId: string): void {
    const event: ToolExecutionEvent = {
