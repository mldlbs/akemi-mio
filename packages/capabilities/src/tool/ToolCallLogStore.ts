...
export class ToolCallLogStore {
  ...
  /**
   * Backfill capability identity onto historical records (data quality fix).
   * Only records without a capability tag are updated; tagged records are kept as-is.
   * resolve is supplied by the caller (authoritative mapping source); undefined means unresolvable.
   */
  backfillCapabilityIdentity(resolve: (record: ToolCallRecord) => ToolCallIdentity | undefined): {
    scanned: number
    updated: number
    skipped: number
  } {
    ...
    for (const record of this.records) {
      ...
      if (!identity || typeof identity.capability !== 'string' || identity.capability.length === 0) {
        skipped++
        continue
      }

      record.capability = identity.capability
      if (identity.operation) record.operation = identity.operation
      if (identity.provider) record.provider = identity.provider
      updated++
    }

    this.persist()
    log('INFO', 'tool_call_log_capability_backfill', {
      scanned: this.records.length,
      updated,
      skipped,
    })

    return { scanned: this.records.length, updated, skipped }
  }
   ...
   * 用于理解错误模式
   */
  getHistoricalFailedCalls(toolName: string, maxSamples: number = 20, since: number = Date.now() - 24 * 60 * 60 * 1000): ToolCallRecord[] {
    return this.query({
      toolName,
      success: false,
      since,
