...
import { PolishingMemoryManager } from '@akemi-mio/creativity/PolishingMemoryManager'
import type { PolishingDecision } from '@akemi-mio/creativity/PolishingMemoryManager'
...
3. 每完成一个章节 → 调用 extract_rules 提炼规则`,
  inputJSONSchema: {
    ...
    properties: {
      ...
      styleRules: {
        type: 'string',
        description: '该段落应用的总体风格规则，JSON 字符串数组。store_decision 可选。如 ["show_dont_tell", "avoid_empty_adverbs"]',
      },
      originalExcerpt: {
        type: 'string',
  ...
  isReadOnly: false,
  handler: async (args) => {
    const { action, storyId, chapterTitle, paragraphIndex, modifications, styleRules, originalExcerpt, polishedExcerpt } = args as {
      action: string
      storyId?: string
      chapterTitle?: string
      ...
      switch (action) {
        ...
        case 'query_history': {
          ...
          if (decisions.length === 0) {
            return formatToolResult(`暂无关于《${storyId}》${chapterTitle ? `（${chapterTitle}）` : ''}的润色决策记录。`)
          }

          const formatted = decisions.map((d, i) => {
            const modSummary = d.modifications.map((m) => `[${m.type}] ${m.description.slice(0, 50)}`).join(' | ')
            return (
          ...
          return formatToolResult(`《${storyId}》的润色历史（最近 ${decisions.length} 条）：\n\n` + formatted.join('\n\n'))
        }
        ...
        case 'get_context': {
          ...
          if (context.isEmpty) {
            return formatToolResult('暂无润色历史记录。可直接按当前风格进行润色，润色完成后使用 store_decision 记录决策。')
          }
        ...
        case 'extract_rules': {
          ...
          if (rules.length === 0) {
            return formatToolResult('暂未提炼出新的风格规则。需要至少有 3 条润色决策记录才能进行规则提炼。')
          }

          const formatted = rules.map((r, i) => `#${i + 1} [${r.category}] ${r.rule} (置信度: ${(r.confidence * 100).toFixed(0)}%)`)

          return formatToolResult(`成功提炼 ${rules.length} 条风格规则：\n\n${formatted.join('\n')}`)
        }
        ...
        case 'get_rules': {
          ...
          return formatToolResult(`当前持久规则库共 ${rules.length} 条规则：\n\n${formatted.join('\n\n')}`)
        }

        default:
          return formatToolError(`未知操作: ${action}。支持的操作：store_decision, query_history, get_context, extract_rules, get_rules。`)
      }
    } catch (e: any) {
