...
import { getObserverService, getPlanManager } from '@akemi-mio/capabilities/tool/deps'
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
          try {
            ...
            const evolutionBlock =
              evolutionCtx?.knownIssues && evolutionCtx.knownIssues.length > 0
                ? '\n[Evolution 系统状态]\n' +
                  evolutionCtx.knownIssues.map((i) => `  [${i.severity}] ${i.title} (来源: ${i.source})`).join('\n') +
                  (evolutionCtx.pipeline
                    ? '\n  管道: 已采集 ' +
                      evolutionCtx.pipeline.totalCollected +
                      ' 个问题, 已修复 ' +
                      evolutionCtx.pipeline.totalFixed +
                      ', 队列 ' +
                      evolutionCtx.pipeline.queueSize +
                      ', 状态: ' +
                      evolutionCtx.pipeline.healthStatus
                    : '') +
                  '\n'
                : ''
