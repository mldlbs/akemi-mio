...
import { sshExec, sshReadFile, sshWriteFile, sshGrep, sshSearchFiles } from '@akemi-mio/capabilities/tool/utils/sshClient'

export const centosExecTool = buildTool({
  name: 'centos_exec',
  description:
    '通过 SSH 在远程 CentOS 服务器上执行 shell 命令。需要先在凭据中配置 centos_host、centos_user、centos_password 或 centos_ssh_key。',
  inputJSONSchema: {
    type: 'object',
    properties: {
