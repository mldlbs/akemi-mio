...
export const voiceUpdatePlanStepTool = buildTool({
  ...
  handler: async (args: any) => {
    try {
      ...
      if (args.planTitle) {
        const allPlans = pm.listPlans()
        plan = allPlans.find((p: any) => p.title.toLowerCase().includes(args.planTitle.toLowerCase()))
        if (!plan) plan = allPlans.find((p: any) => args.planTitle.toLowerCase().includes(p.title.toLowerCase()))
      } else {
      ...
      if (args.stepIndex !== undefined && args.stepIndex !== '') {
        ...
        // 精确匹配优先
        stepIdx = plan.steps.findIndex((s: any) => s.description.toLowerCase().includes(desc))
        // 二次模糊：每个词单独匹配
        if (stepIdx === -1) {
          const words = desc.split(/[\s,，、]+/).filter(Boolean)
          stepIdx = plan.steps.findIndex((s: any) => words.every((w: string) => s.description.toLowerCase().includes(w)))
        }
      } else {
        // 默认找第一个未完成的步骤
        stepIdx = plan.steps.findIndex((s: any) => s.status !== 'done' && s.status !== 'failed')
      }
      ...
      return formatToolResult(`✅ 已标记计划「${plan.title}」步骤「${stepLabel}」为 ${statusLabel(newStatus)}`)
    } catch (err: any) {
      return formatToolError(err.message)
    }
  },
...
export const voiceSwitchPlanFocusTool = buildTool({
  ...
  handler: async (args: any) => {
    try {
      ...
      // 模糊匹配：输入是计划标题的子串或反
      const plan =
        allPlans.find((p: any) => p.title.toLowerCase().includes(args.planTitle.toLowerCase())) ||
        allPlans.find((p: any) => args.planTitle.toLowerCase().includes(p.title.toLowerCase()))
