...
import { fileRuleMemoryAdapter } from '@akemi-mio/evolution-file-organizer'
...
export const rememberFileRuleTool = buildTool({
  ...
  inputJSONSchema: {
    ...
    properties: {
      filePattern: {
        type: 'string',
        description: 'glob 文件匹配模式，如 "*.pdf"、"reports/**/*.ts"、"**/*.{jpg,png}"。"*" 匹配单层，"**" 匹配任意层',
      },
      targetPath: {
        type: 'string',
      ...
      description: {
        type: 'string',
        description: '人类可读的规则描述，如 "将所有 PDF 文件整理到 reports 目录"。会自动添加前缀',
      },
      sourceText: {
        type: 'string',
  ...
  handler: async (args: { filePattern: string; targetPath: string; description: string; sourceText?: string }) => {
    try {
      const memoryId = fileRuleMemoryAdapter.saveRule(args.filePattern, args.targetPath, args.description, args.sourceText)
...
export const listFileRulesTool = buildTool({
  ...
  handler: async () => {
    try {
      ...
      const lines: string[] = [`📁 文件归档规则（共 ${rules.length} 条）`, `活跃: ${active.length} | 已禁用: ${disabled.length}`, '']
      ...
      if (active.length > 0) {
        ...
        for (const entry of active) {
          ...
          const lastRun = r.lastAppliedAt ? new Date(r.lastAppliedAt).toLocaleString('zh-CN') : '尚未执行'
          lines.push(
