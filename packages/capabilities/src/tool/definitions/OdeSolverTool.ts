...
export const odeSolverTool = buildTool({
  name: 'solve_ode',
...
- 用户说 "步长改为 0.05 重新算"
  ...
  handler: async (args: {
    ...
    try {
      ...
      let stdout: string
      try {
        stdout = execSync(`${pythonCmd} "${scriptPath}"`, {
          input: inputJson,
          timeout: 60_000,
        ...
        if (msg.includes('No module named') || msg.includes('ModuleNotFoundError')) {
          const mod =
            msg.match(/No module named ['"]?(\S+)['"]?/)?.[1] || msg.match(/ModuleNotFoundError.*named\s+'?(\S+)'?/)?.[1] || '未知模块'
          return formatToolError(`Python 模块 "${mod}" 未安装。请运行: pip install ${mod}`)
        }
...
/** @deprecated ????????? */
export const solveOdeTool = odeSolverTool
