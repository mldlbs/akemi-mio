...
import { blogPublishService } from '@akemi-mio/blog-publish/BlogPublishService'
import { workflowStore } from '@akemi-mio/capabilities/workflow/WorkflowStoreV2'
import { getWorkflowScheduler } from '@akemi-mio/capabilities/workflow/WorkflowScheduler'
import { validateWorkflow } from '@akemi-mio/capabilities/workflow/WorkflowValidator'
import type { WorkflowDef, WorkflowStepDef } from '@akemi-mio/capabilities/workflow/types'
import type { PublishConfig, PublishPlatform } from '@akemi-mio/blog-publish/types'
import { PLATFORM_LABELS } from '@akemi-mio/blog-publish/types'
...
export const blogApprovePublishTool = buildTool({
  ...
  description: '【BlogPublish】批准一个等待审批的发布任务。调用后立即执行多平台发布。发布进度通过 blog_publish_status 查看',
  inputJSONSchema: {
    type: 'object',
    properties: {
      taskId: {
...
export const blogRejectPublishTool = buildTool({
  ...
  description: '【BlogPublish】驳回一个等待审批的发布任务。任务将被标记为失败，不会执行发布',
  inputJSONSchema: {
    type: 'object',
    properties: {
      taskId: {
...
export const blogPublishStatusTool = buildTool({
  ...
  description: '【BlogPublish】查看发布任务的状态和详细进度。不传 taskId 时列出所有活跃的发布任务',
  inputJSONSchema: {
    type: 'object',
    properties: {
      taskId: {
  ...
  handler: async (args: { taskId?: string; all?: boolean }) => {
    try {
      ...
      const tasks = args.all ? blogPublishService.getAllTasks(20) : blogPublishService.listActiveTasks()
...
export const blogPublishCredentialCheckTool = buildTool({
  ...
  description:
    '【BlogPublish】检查各发布平台的 API 凭证配置状态。列出已配置和缺失的凭证，' + '帮助用户了解哪些平台可以立即发布，哪些需要先配置凭证',
  inputJSONSchema: {
    type: 'object',
    properties: {},
    required: [],
