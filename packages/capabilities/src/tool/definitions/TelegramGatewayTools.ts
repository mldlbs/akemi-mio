...
import { messageGateway } from '@akemi-mio/messaging/telegram/MessageGateway'
import { OUTPUT_FORMAT_LABELS, dataSourceResultToMCP } from '@akemi-mio/messaging/telegram/DataSourceTool'
import type { OutputFormat } from '@akemi-mio/messaging/telegram/DataSourceTool'
...
export const listDataSourcesTool = buildTool({
  ...
  handler: async () => {
    try {
      ...
      const lines: string[] = [`📡 消息网关 — 已注册 ${tools.length} 个数据源工具`, ``]
...
export const telegramGatewayTools = [executeDataSourceTool, listDataSourcesTool, messageGatewayStatusTool] as const
