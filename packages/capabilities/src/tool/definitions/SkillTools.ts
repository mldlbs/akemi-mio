...
export const listSkillsTool = buildTool({
  ...
  handler: async () => {
    try {
      ...
      if (skills.length === 0) return formatToolResult('尚未安装任何技能。将技能目录放到 AppData 下即可自动加载')
      const lines = skills.map(
        (s: any) => `${s.enabled ? '[启用]' : '[禁用]'} ${s.manifest.name} — ${s.manifest.description}${s.toolsLoaded ? ' (含工具)' : ''}`,
      )
