...
import { piperOrchestrator, type PiperTaskTag, PIPER_MODEL_CATALOG } from '@akemi-mio/audio/PiperOrchestrator'
import { piperBehaviorSidecar } from '@akemi-mio/audio/PiperBehaviorSidecar'
...
export const speakWithPiperTool = buildTool({
  ...
  inputJSONSchema: {
    ...
    properties: {
      ...
      task_tag: {
        type: 'string',
        description:
          '任务标签，自动选择最佳模型。可选: chat（日常对话/快速回复）, story（讲故事/朗读/高表现力）, alert（通知/提醒/警告）。与 role_id 二选一。',
        enum: ['chat', 'story', 'alert'],
      },
      role_id: {
        type: 'string',
        description:
          '语音角色 ID。通过角色方案映射到对应的 Piper 模型、语速和音调。与 task_tag 二选一。可用: gentle_female（温柔女声）, calm_male（沉稳男声）, lively_child（活泼童声）, warm_female（温暖女声）, professional_male（专业男声）。使用 list_voice_roles 查看详情。',
      },
      model: {
        type: 'string',
        description:
          '显式指定 Piper 语音模型。可选: zh_CN-huayan-medium（花颜·女声/默认）, zh_CN-ling_ling-medium（玲玲·温柔女声）, zh_CN-tx_mati-medium（马提·沉稳男声）。优先级高于 task_tag 和 role_id。',
      },
      speed: {
        type: 'number',
  ...
  handler: async (args: { text: string; task_tag?: string; role_id?: string; model?: string; speed?: number; pitch?: number }) => {
    const t0 = Date.now()
  ...
  inputJSONSchema: {
    ...
    properties: {
      model: {
        type: 'string',
        description:
          '要切换到的模型名。可选: zh_CN-huayan-medium（花颜·女声/通用）, zh_CN-ling_ling-medium（玲玲·温柔女声/讲故事）, zh_CN-tx_mati-medium（马提·沉稳男声/通知）。',
...
export const listVoiceRolesTool = buildTool({
  ...
  description:
    '列出所有可用的语音角色及其 TTS 参数和 Piper 模型映射。语音角色是角色化多任务语音引擎的核心概念，每个角色绑定了一套完整的 TTS 参数（Edge-TTS + Piper）。',
  inputJSONSchema: {
    type: 'object',
    properties: {},
    required: [],
  ...
  handler: async () => {
    ...
    return formatToolResult([`可用语音角色 (${roles.length} 个):`, ...roleLines, ...schemeLines].join('\n'))
  },
  isReadOnly: true,
})
...
${mappingLines.join('\n')}`
    ...
    return formatToolResult(`可用角色方案 (${schemes.length} 个):\n\n${lines.join('\n\n')}\n\n使用 set_voice_scheme 切换方案。`)
  },
  isReadOnly: true,
})
...
export const setVoiceSchemeTool = buildTool({
  ...
  description:
    '切换当前角色方案。方案切换后，Agent 在不同任务场景下的语音角色将自动更新。切换时会预加载新方案涉及的 Piper 模型以减少延迟。',
  inputJSONSchema: {
    type: 'object',
    properties: {
      scheme_id: {
        type: 'string',
        description:
          '要切换到的方案 ID。可选: default（默认方案）, geek（极客方案）, gentle（柔和方案）。使用 list_voice_schemes 查看所有方案详情。',
      },
    },
    required: ['scheme_id'],
  },
