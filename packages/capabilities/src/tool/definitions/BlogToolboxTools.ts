...
const juejinFormatter: PlatformFormatter = {
  ...
  formatContent: (md) => {
    // 掘金支持标准 Markdown
    // 掘金不支持 [TOC]
    const content = md.replace(/\[TOC\]/gi, '')
    return content
  },
  formatCode: (lang, code) => `\`\`\`${lang}\n${code}\n\`\`\``,
  formatImage: (alt, url) => `![${alt}](${url})`,
...
const wechatFormatter: PlatformFormatter = {
  ...
  formatContent: (md) => {
    ...
    const styled = html
      ...
      .replace(
        /<blockquote>/g,
        '<blockquote style="background:#f5f5f5;padding:10px 15px;border-left:4px solid #07c160;margin:10px 0;color:#666;">',
      )
...
function formatForPlatform(md: string, platform: string): { formatted: string; warnings: string[]; platform: string } {
  ...
  const content = formatter.formatContent(md)
...
export const blogPlatformFormatTool = buildTool({
  ...
  description: `【BlogToolbox】将博客内容格式化为目标平台的适配格式。支持平台: ${PLATFORM_KEYS.join('、')}。CSDN 自动添加 TOC 标记；微信公众号返回带样式的 HTML；博客园自动添加摘要分割线；知乎/掘金做代码块适配。可一次输出多个平台`,
  inputJSONSchema: {
    type: 'object',
    properties: {
      markdown: {
  ...
  handler: async (args: { markdown: string; platforms?: string[]; title?: string }) => {
    try {
      ...
      for (const platform of targets) {
        ...
        const lines: string[] = [`---`, `🎯 ${platformName}`, `---`, '']
      ...
      const header = ['📝 多平台格式化结果', `源内容: ${md.length} 字符`, `目标平台: ${targets.length} 个`, '']
...
export const blogToolboxPipelineTool = buildTool({
  ...
  inputJSONSchema: {
    ...
    properties: {
      ...
      stages: {
        ...
        items: { type: 'string' },
        description:
          '流水线阶段列表，顺序执行。可选值: seo_analyze, platform_format, convert_html。默认: ["seo_analyze", "platform_format", "convert_html"]',
      },
      platforms: {
        type: 'array',
        items: { type: 'string' },
  ...
  handler: async (args: { markdown: string; title?: string; stages?: string[]; platforms?: string[]; keywords?: string[] }) => {
    try {
      const md = String(args.markdown)
      if (!md.trim()) return formatToolError('Markdown 内容不能为空')
        ...
        try {
          switch (stage) {
            ...
            case 'platform_format': {
              ...
              const lastPlatform = (
                args.platforms && args.platforms.length > 0 ? args.platforms[args.platforms.length - 1] : 'generic'
              ).toLowerCase()
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (r.status === 'ok' && r.output) {
          // 截取输出摘要
          const summary = r.output.length > 500 ? r.output.substring(0, 500) + `\n... (共 ${r.output.length} 字符)` : r.output
          report.push(summary)
        }
...
export const blogToolboxInfoTool = buildTool({
  ...
  description: '【BlogToolbox】显示博客工具箱的完整工具列表、使用说明和版本信息。提供自文档化界面，帮助用户了解每个工具的功能和用法',
  inputJSONSchema: {
    type: 'object',
    properties: {
      toolName: {
  ...
  handler: async (args: { toolName?: string }) => {
    try {
      ...
      if (args.toolName) {
        ...
        if (!info) {
          return formatToolError(`未知工具 "${args.toolName}"。可用工具: ${Object.keys(TOOLS_INFO).join('、')}`)
        }
      ...
      const lines: string[] = ['🧰 MCP 标准化博客工具箱 (BlogToolbox)', '='.repeat(45), '', '📋 可用工具:', '']
