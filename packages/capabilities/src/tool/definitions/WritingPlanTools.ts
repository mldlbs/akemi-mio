...
import { getMemoryService, getWritingPlanAgent } from '@akemi-mio/capabilities/tool/deps'
...
export const writingPlanAnalyzeDesignTool = buildTool({
  ...
  inputJSONSchema: {
    ...
    properties: {
      designDocPath: {
        type: 'string',
        description: '设计文档路径（相对于项目根目录），如 "docs/工业颂歌_完整设计体系.md"。支持 docs/ 目录或项目根目录下的文件。',
      },
    },
  ...
  handler: async (args: { designDocPath: string }) => {
    try {
      ...
      if (result.error || !result.data) return formatToolError(result.error || '??????????')
  ...
  handler: async (args: {
    ...
    try {
      ...
      if (designResult.error || !designResult.data)
        return formatToolError(designResult.error ? `设计文档分析失败: ${designResult.error}` : '设计文档分析结果为空')
      ...
      if (compareResult.error || !compareResult.data)
        return formatToolError(compareResult.error ? `章节对比失败: ${compareResult.error}` : '章节对比结果为空')

      // Step 3: 分解重写任务
      const decomposeResult = await agent.decomposeRewrite(compareResult.data, designAnalysis)
      if (decomposeResult.error || !decomposeResult.data)
        return formatToolError(decomposeResult.error ? `任务分解失败: ${decomposeResult.error}` : '任务分解结果为空')
...
export const writingPlanStatusTool = buildTool({
  ...
  description: '查询写作计划的当前状态、各任务进度、当前执行阶段。' + '如果未提供 planId 则列出所有已保存的计划。',
  inputJSONSchema: {
    type: 'object',
    properties: {
      planId: {
...
export const writingPlanUpdateTaskTool = buildTool({
  ...
  handler: async (args: {
    ...
    try {
      ...
      const plan = agent.updateTaskStatus(args.planId, args.taskId, args.status as 'pending' | 'in_progress' | 'completed' | 'failed')
...
export const writingPlanQualityCheckTool = buildTool({
  ...
  handler: async (args: { designAnalysisJson: string; chapterPath: string; chapterNumber: number; threshold?: number }) => {
    try {
      const agent = getWritingPlanAgent()
      if (!agent) return formatToolError('WritingPlanAgent 尚未初始化')
