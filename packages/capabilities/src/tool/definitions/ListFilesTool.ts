...
import { resolveWorkspace, stripWorkspaceLabelPrefix } from '@akemi-mio/capabilities/tool/utils/workspace'
...
export const listFilesTool = buildTool({
  ...
  handler: async (args: { path: string; workspace?: string }) => {
    try {
      if (typeof args.path !== 'string') throw new Error('path 参数必须为字符串')
      ...
      if (!fullPath.startsWith(base)) throw new Error(`路径 ${args.path} 超出工作区目录`)
      ...
      if (!existsSync(fullPath)) throw new Error(`目录不存在: ${args.path}`)
