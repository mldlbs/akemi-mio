...
import { TypographyMemoryManager } from '@akemi-mio/creativity/TypographyMemoryManager'
import type { TypographyRecord } from '@akemi-mio/creativity/TypographyMemoryManager'
...
3. 每完成一个故事/系列 → 调用 analyze_trends 查看趋势`,
  ...
  isReadOnly: false,
  handler: async (args) => {
    const { action, storyId, platformTag, chapterNumber, chapterTitle, parameters, userCorrections, confirmedExcerpt, source } = args as {
      action: string
      storyId?: string
      platformTag?: string
      ...
      switch (action) {
        ...
        case 'analyze_trends': {
          if (storyId && platformTag) {
            ...
            if (!report) {
              return formatToolResult(`《${storyId}》在「${platformTag}」的记录不足 2 条，无法进行趋势分析。` + `建议先排版几章后再分析。`)
            }
          ...
          const lines: string[] = [`排版趋势分析报告（共 ${allRecords.length} 条记录，${groups.size} 个故事/平台组合）：`, '']
        ...
        default:
          return formatToolError(`未知操作: ${action}。支持的操作：load_preferences, save_preferences, analyze_trends。`)
      }
    } catch (e: any) {
...
function formatTrendReport(report: any): string {
  ...
  for (const [param, trend] of Object.entries(report.parameterTrends) as Array<[string, any]>) {
    ...
    const currentVal =
      (PARAM_VALUE_LABELS as Record<string, Record<string, string>>)[param]?.[String(trend.current)] || String(trend.current)
    const directionIcon =
      trend.direction === 'stable' ? '✅' : trend.direction === 'changed' ? '🔄' : trend.direction === 'increasing' ? '📈' : '📉'
