...
async function llmAnalyze<T>(systemPrompt: string, userPrompt: string, timeoutMs = DEFAULT_TIMEOUT): Promise<{ data?: T; error?: string }> {
  const getCredential = (key: string) => getCredentialsManager()?.get(key) ?? null
  const { text } = getRuntimeLlmConfig({ getCredential })
...
const AI_MARKER_PATTERNS: Array<{ pattern: RegExp; type: string; label: string; severity: 1 | 2 | 3 | 4 | 5 }> = [
  // 过度使用的过渡词
  {
    pattern: /(值得注意的是|值得一提的是|不可否认|毋庸置疑|毫无疑问|显而易见|众所周知)/g,
  ...
  // 过度修饰
  {
    pattern: /(深深地|默默地|静静地|轻轻地|缓缓地|微微地|悄悄地|渐渐地|默默)/g,
    type: 'overused_adverb',
    label: '过度使用副词',
    severity: 2,
  },
  // AI 常用套话
  {
    pattern: /(在这个充满|在这个喧嚣|在这个快节奏|在这个瞬息万变|在这个数字时代|在当今社会)/g,
    type: 'cliched_opening',
    label: '套话开头',
    severity: 4,
  },
  // 万能情感词
  {
    pattern: /(心中涌起一股|一股暖流|一种说不出的|一种莫名的|一种难以言喻的)/g,
    type: 'generic_emotion',
    label: '万能情感描写',
    severity: 3,
  },
  // 冗余强调
...
function ruleBasedDialogueEval(text: string): DialogueEvalResult {
  ...
  for (const line of dialogueLines) {
    // 检测过于书面化的对话
    const writtenMarkers = [/(罢了|而已|岂能|岂敢|何以|并非|绝非|未曾|尚未|未曾想|殊不知)/, /(在此|于此|于|之|其|所|与否)/]
...
function ruleBasedAiDetect(text: string): AiDetectResult {
  ...
  const markerScore = Math.min(
    100,
    markers.reduce((sum, m) => sum + m.severity * 4, 0),
  )
  const score = Math.min(100, markerScore)
...
function ruleBasedRhythmAnalyze(text: string): RhythmResult {
  ...
  // 标准差
  const variance = lengths.length > 0 ? lengths.reduce((sum, l) => sum + (l - avgLen) ** 2, 0) / lengths.length : 0
  const stdDev = Math.sqrt(variance)
