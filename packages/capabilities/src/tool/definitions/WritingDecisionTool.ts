...
import { WritingDecisionService, type WritingDecisionType } from '@akemi-mio/creativity/WritingDecisionMemory'
      ...
      switch (action) {
        ...
        case 'record_decision': {
          ...
          if (!VALID_DECISION_TYPES.includes(decisionType as WritingDecisionType)) {
            return formatToolError(`无效的决策类型: "${decisionType}"。有效值：${VALID_DECISION_TYPES.join(', ')}`)
          }
        ...
        case 'record_from_diff': {
          ...
          const summary = records
            .map((r, i) => `${i + 1}. [${r.decisionType}] ${r.entities.join(', ') || '无实体'} — ${r.contextSummary.slice(0, 60)}`)
            .join('\n')
        ...
        case 'query_by_story': {
          ...
          if (entries.length === 0) {
            return formatToolResult(`暂无关于《${storyName}》的写作决策记录。修改章节后使用 record_from_diff 或 record_decision 记录。`)
          }
          ...
          return formatToolResult(`《${storyName}》共 ${entries.length} 条决策记录：\n\n${formatted.join('\n\n')}`)
        }
        ...
        case 'query_by_entities': {
          ...
          if (matched.length === 0) {
            return formatToolResult(`未找到与实体「${parsedEntities.join('、')}」相关的决策记录。`)
          }
        ...
        case 'get_context': {
          ...
          if (!context) {
            return formatToolResult('暂无写作决策回溯信息。章节修改后可调用 record_from_diff 或 record_decision 来积累决策记忆。')
          }
        ...
        case 'check_consistency': {
          ...
          if (issues.length === 0) {
            return formatToolResult('✓ 一致性检查通过：新内容与所有历史决策记录无冲突。')
          }
          ...
          return formatToolResult(
            `⚠️ 一致性检查发现 ${issues.length} 个问题` + `（${conflictCount} 个冲突，${warningCount} 个提示）：\n${formatted}`,
          )
        }
