...
import { getEffectiveProjectRoot } from '@akemi-mio/core/workspace/project-root'

const MAX_OUTPUT = 3000
const MAX_FILES = 100

function grepCode(pattern: string, projectRoot: string, include?: string): string {
...
export const grepTool = buildTool({
  ...
  handler: async (args: { pattern: string; include?: string }) => {
    try {
      const result = grepCode(args.pattern, getEffectiveProjectRoot(), args.include)
