...
import { buildTool, formatToolResult } from '@akemi-mio/capabilities/tool/types'
    ...
    try {
      ...
      return formatToolResult(
        JSON.stringify({
          ...
          summary: passed ? 'TypeScript 编译检查通过，无类型错误' : `TypeScript 编译检查发现 ${errorLines.length} 个错误`,
...
export const cicdLintTool = buildTool({
  ...
  handler: async (args: { path?: string; fix?: boolean }) => {
    try {
      ...
      const stdout = await execAsync(`npx eslint ${lintPath} --ext .ts,.tsx --format=compact ${fixFlag} 2>&1`, {
      ...
      return formatToolResult(
        JSON.stringify({
          ...
          passed: errorCount === 0,
          errorCount,
          summary: errorCount === 0 ? 'ESLint 检查通过，无代码规范问题' : `ESLint 检查发现 ${errorCount} 个问题`,
    ...
    try {
      ...
      return formatToolResult(
        JSON.stringify({
          ...
          failedCount,
          totalCount,
          summary: passed ? `测试通过: ${passedCount}/${totalCount} 通过` : `测试失败: ${failedCount} 个失败`,
          raw: truncateOutput(stdout, 3000),
        }),
      )
    } catch (err: any) {
      ...
      return formatToolResult(
        JSON.stringify({
          ...
          passedCount,
          failedCount,
          summary: failedCount > 0 ? `测试失败: ${failedCount} 个失败` : `测试执行异常: ${err.message}`,
    ...
    try {
      ...
      return formatToolResult(
        JSON.stringify({
          ...
          script,
          errorCount,
          summary: passed ? `构建成功 (npm run ${script})` : `构建失败: ${errorCount} 个错误`,
          raw: truncateOutput(stdout, 3000),
        }),
      )
    } catch (err: any) {
    ...
    try {
      ...
      return formatToolResult(
        JSON.stringify({
          ...
          outputDir,
          hasOutput,
          summary: passed ? `文档构建成功 (npm run ${script})` : `文档构建失败`,
          raw: truncateOutput(stdout, 3000),
        }),
      )
    } catch (err: any) {
...
export const cicdQualityGateTool = buildTool({
  ...
  handler: async (args: { skipLint?: boolean; skipTest?: boolean; skipBuild?: boolean; skipDocs?: boolean; tsconfig?: string }) => {
    ...
    return formatToolResult(
      JSON.stringify({
        success: true,
        passed: allPassed,
        summary: allPassed ? `✅ 质量门禁通过（${durationMs}ms）` : `❌ 质量门禁未通过 (${durationMs}ms):\n${failures.join('\n')}`,
        checks: results,
        failures,
        durationMs,
        timestamp: Date.now(),
