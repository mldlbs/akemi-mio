...
import { CRED } from '@akemi-mio/capabilities/fanqie-publish/credential-keys'
import { fetchScene, fetchAllScenes } from '@akemi-mio/capabilities/fanqie-publish/FanqiePublisher'

const DEFAULT_AUTHOR_URL = 'https://author.fanqienovel.com'

function getAuthorUrl(): string {
  const configured = getCredentialsManager()?.get(CRED.AUTHOR_URL)?.trim()
  if (!configured) return DEFAULT_AUTHOR_URL
    ...
    try {
      ...
      if (publishAllScenes) {
        ...
        const loginNote = loginEstablished
          ? '✅ 已登录（cookie 有效）'
          : `⚠️ 需要先登录：打开 browser_navigate 到 ${authorUrl}，在浏览器中扫码登录`
        ...
        return formatToolResult(
          '📚 **内容已就绪，请 MIO 按以下步骤发布**\n\n' +
            ...
            `${loginNote}\n\n` +
            `### 发布步骤\n\n` +
            `1. 如果未登录，先使用 \`browser_navigate\` 打开 ${authorUrl}\n` +
      ...
      const loginNote = data.loginEstablished
        ? '✅ Cookie 已保存，可直接操作'
        : `⚠️ 未检测到登录状态。请先使用 browser_navigate 打开 ${authorUrl} 完成登录`
      ...
      return formatToolResult(
        '📝 **内容已获取，请 MIO 发布到番茄小说**\n\n' +
          ...
          `${loginNote}\n\n` +
          `### 发布操作计划\n\n` +
          `1. 使用 \`browser_navigate\` 打开 ${authorUrl}\n` +
