  ...
  inputJSONSchema: {
    ...
    properties: {
      ...
      paths: {
        type: 'string',
        description:
          '要备份的文件路径列表（JSON 字符串数组，action=create 时与 chapterDir 二选一），如 ["docs/chapters/19.md", "docs/chapters/20.md"]',
      },
      label: {
        type: 'string',
    ...
    try {
      ...
      switch (action) {
        ...
        case 'restore': {
          ...
          const lines: string[] = [
            restoredCount > 0 ? `✅ 已恢复 ${restoredCount}/${meta.files.length} 个文件` : '❌ 未能恢复任何文件',
            `  快照: ${snapshotId}`,
            `  标签: ${meta.label}`,
          ]
        ...
        default:
          return formatToolError(`未知操作: "${action}"。支持的操作：create, list, restore, delete, info`)
      }
    } catch (err: any) {
