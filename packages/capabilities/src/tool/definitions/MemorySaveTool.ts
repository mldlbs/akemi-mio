...
export const memorySaveTool = buildTool({
  ...
  inputJSONSchema: {
    ...
    properties: {
      ...
      type: {
        ...
        description:
          '经验类型。learned_preference=用户偏好, lesson_learned=错误教训, observation=模式观察, self_reflection=自我反思, workflow_insight=工作流洞察',
      },
      confidence: {
        type: 'number',
  ...
  handler: async (args: { content: string; type: ExperienceType; confidence?: number; topics?: string[]; tier?: 'ephemeral' | 'semi' }) => {
    try {
      ...
      const confidence = typeof args.confidence === 'number' ? Math.max(0.1, Math.min(1.0, args.confidence)) : 0.55
      ...
      const extractedTopics = userTopics.length > 0 ? userTopics : extractTopicsFromContent(content)
      ...
      // 5. 存储到记忆系统
      ms.addEntry('user_fact', `${prefix} ${content}`, confidence, {
        tier,
        structuredData,
      })
