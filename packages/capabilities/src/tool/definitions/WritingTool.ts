...
export const writingSystemTool = buildTool({
  ...
  handler: async (args) => {
    const { action, data, storyId, characterId, sceneId } = args as {
      action: string
      data?: string
      storyId?: string
      characterId?: string
    ...
    try {
      switch (action) {
        ...
        case 'update_story': {
          const body = data ? JSON.parse(data) : {}
          if (!body.storyId) return formatToolError('需要 storyId')
        ...
        case 'update_character': {
          const body = data ? JSON.parse(data) : {}
          if (!body.characterId) return formatToolError('需要 characterId')
        ...
        case 'get_scene': {
          ...
          if (!sid) return formatToolError('需要 sceneId')
        ...
        case 'update_scene': {
          const body = data ? JSON.parse(data) : {}
          if (!body.sceneId) return formatToolError('需要 sceneId')
        ...
        case 'update_relationship': {
          const body = data ? JSON.parse(data) : {}
          if (!body.relationshipId) return formatToolError('需要 relationshipId')
        ...
        case 'search': {
          const body = data ? JSON.parse(data) : {}
          if (!body.query) return formatToolError('需要 query')
          const result = await writingFetch('GET', `/search?q=${encodeURIComponent(body.query)}`)
          return formatToolResult(JSON.stringify(result, null, 2))
        }
        case 'ai_write': {
          const body = data ? JSON.parse(data) : {}
          if (!body.prompt) return formatToolError('需要 prompt')
