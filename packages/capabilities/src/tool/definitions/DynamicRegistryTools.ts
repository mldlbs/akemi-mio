...
export const registerTool = buildTool({
  name: 'register_tool',
  description:
    '在对话中注册一个临时工具。Agent 发现现有工具无法满足需求时，可直接提供工具定义和 handler 代码进行注册。注册的工具仅在当前对话有效，对话结束后自动清理。支持分级权限控制：none(纯计算)、readonly(只读)、files(文件读写)、network(网络请求)。',
  inputJSONSchema: {
    type: 'object',
    properties: {
      name: {
      ...
      input_schema: {
        type: 'object',
        description:
          '输入参数 JSON Schema。格式：{"type":"object","properties":{"param1":{"type":"string","description":"..."}},"required":["param1"]}',
        properties: {
          type: { type: 'string' },
      ...
      permission: {
        ...
        description:
          '权限级别：none(纯计算)、readonly(只读，默认)、files(文件读写)、network(网络请求+fetch)。选择够用的最低级别以提高安全性。',
      },
      tags: {
        type: 'array',
        items: { type: 'string' },
  ...
  handler: async (args: {
    ...
    try {
      // 动态导入避免循环依赖
      const [{ dynamicToolLifecycle }, { dynamicToolPermission, parsePermissionLevel, PermissionLevel }, { checkHandlerSafety }] =
        await Promise.all([
...
export const listRegisteredTools = buildTool({
  ...
  handler: async () => {
    try {
      ...
      const lines = [`📦 已注册 ${tools.length} 个动态工具：`, ``]
