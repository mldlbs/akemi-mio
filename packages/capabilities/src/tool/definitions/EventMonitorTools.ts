...
import { eventMonitorService } from '@akemi-mio/monitoring/event-monitor/EventMonitorService'
import {
  ...
  inputJSONSchema: {
    ...
    properties: {
      ...
      data_path: {
        type: 'string',
        description:
          'JSON 路径，用于从响应中提取比较值。如 "data.temperature"、"wind.speed"、"price"。' + '留空表示使用完整响应文本前 200 字符。',
      },
      change_threshold: {
        type: 'number',
      ...
      mute_time_ranges: {
        type: 'string',
        description:
          '静音时间段，JSON 数组格式。如 [{"start": "23:00", "end": "07:00"}]。' +
...
export const getEventMonitorTool = buildTool({
  ...
  handler: async (args: { id: string }) => {
    ...
    const muteDesc = config.filters.muteTimeRanges.map((r) => `${r.start}-${r.end}`).join(', ')
...
export const updateEventMonitorTool = buildTool({
  ...
  handler: async (args: {
    ...
    return formatToolResult(`✅ 监测器 "${args.name || existing.name}" 已更新 (${changedFields.join(', ')})`)
  },
  isReadOnly: false,
})
...
export const deleteEventMonitorTool = buildTool({
  ...
  handler: async (args: { id: string; confirm?: boolean }) => {
    if (args.confirm !== true) {
      return formatToolResult(`⚠ 请设置 confirm=true 确认删除监测器 "${args.id}"。此操作不可恢复。`)
    }
...
export const pollEventMonitorNowTool = buildTool({
  ...
  handler: async (args: { id: string }) => {
    ...
    if (result.success) {
      const msg = result.message ? `✅ 轮询完成: ${result.message}` : `✅ 轮询完成（${monitor.url}），无变化。`
...
export const eventMonitorGlobalConfigTool = buildTool({
  ...
  description: '配置事件监测系统全局设置：全局启停、播报风格、默认轮询间隔等。' + '无需参数时返回当前全局配置。',
  inputJSONSchema: {
    type: 'object',
    properties: {
      enabled: {
  ...
  handler: async (args: { enabled?: boolean; broadcast_style?: string; default_poll_interval_ms?: number }) => {
