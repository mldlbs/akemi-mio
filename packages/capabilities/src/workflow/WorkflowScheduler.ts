...
import type { WorkflowResumeResult } from '@akemi-mio/intelligence/runtime/CheckpointTypes'
import type { SpawnTaskOptions } from '@akemi-mio/intelligence/agent/SubAgentPool'
import { behaviorBlogBridge } from '@akemi-mio/evolution/behavior/BehaviorBlogBridge'
...
export class WorkflowSchedulerV2 {
  ...
  public running = new Map<string, AbortController>()
  /** 同步取消标记 — 避免 executeLoop 在 stopRun 之后再次写入 cancelled */
  public cancelled = new Set<string>()
  /** 每个运行追踪其 spawn 出的 agent ID，用于取消时 kill */
  public runAgentIds = new Map<string, string[]>()
