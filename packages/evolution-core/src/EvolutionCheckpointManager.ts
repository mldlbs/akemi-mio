...
import type { EvolutionCheckpointPhase, EvolutionCheckpointStatus } from '@akemi-mio/core/db/schema/evolution_checkpoints'
...
export class EvolutionCheckpointManager {
  ...
  completeCheckpoint(checkpointId: string): void {
    ...
    try {
      db.run('UPDATE evolution_checkpoints SET status = ?, completed_at = ? WHERE id = ?', ['completed', Date.now(), checkpointId])
  ...
  async rollbackCheckpoint(checkpointId: string): Promise<boolean> {
    ...
    try {
      ...
      const db = tryDb()
      if (db) {
        db.run('UPDATE evolution_checkpoints SET status = ?, completed_at = ? WHERE id = ?', ['rolled_back', Date.now(), checkpointId])
      }
  ...
  async recoverOnStartup(activeCycleId?: string): Promise<string> {
    ...
    const lines: string[] = ['【自进化检查点恢复】', `检测到 ${unfinished.length} 个未完成的检查点，正在尝试恢复...`, '']
    ...
    const handler = (signal: string) => {
      ...
      if (this.currentCycleId) {
        try {
          const db = tryDb()
          if (db) {
            const pending = db.exec(`SELECT id FROM evolution_checkpoints WHERE cycle_id = ? AND status IN ('pending', 'in_progress')`, [
              this.currentCycleId,
            ])
            if (pending.length && pending[0].values.length) {
              ...
              for (const row of pending[0].values) {
                ...
                db.run('UPDATE evolution_checkpoints SET status = ?, completed_at = ? WHERE id = ?', ['interrupted', Date.now(), pid])
              }
  ...
  getCycleCheckpoints(cycleId: string): CheckpointContext[] {
    ...
    try {
      const rows = db.exec(`SELECT * FROM evolution_checkpoints WHERE cycle_id = ? ORDER BY created_at ASC`, [cycleId])
  ...
  getRecentCheckpoints(limit = 10): CheckpointContext[] {
    ...
    try {
      const rows = db.exec(`SELECT * FROM evolution_checkpoints ORDER BY created_at DESC LIMIT ?`, [limit])
      return this.rowsToContexts(rows)
    } catch {
      return []
    }
  ...
  private markInterrupted(checkpointId: string): void {
    ...
    if (!db) return
    try {
      db.run('UPDATE evolution_checkpoints SET status = ?, completed_at = ? WHERE id = ?', ['interrupted', Date.now(), checkpointId])
    } catch {
      // silent
    }
  }
