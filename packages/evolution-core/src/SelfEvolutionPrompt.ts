...
### 社交媒体运营
evolution_workspace/social/ 目录下是社交运营内容资产（策略、排程、草稿、运营数据）；发布统一走 publishing 能力（social_publish 工具，语义输入 platform/content/title/replyToId/dryRun），不要用 run_command 直调 social CLI：
- config/config.yaml — 安全模式（safe/assisted/autopilot）与平台开关（能力边界强制门禁）
- config/strategy.yaml — 各平台内容方向和发布策略
- config/accounts.json — 账号配置（可用 write_file 管理）
- config/content_calendar.yaml — 排程记录（AI 读写；到期项由调度器自动发布）
- config/analytics.yaml — 运营数据（AI 写入）
- adapters/*.mjs — 7 平台适配器（x, telegram, weibo, zhihu, douyin, xiaohongshu, wechat_mp）

发布入口：
- 即时发布 / 演练：调用 social_publish（content/platform 必填；dryRun=true 只校验不真实发布）
- 排程发布：把条目写入 content_calendar.yaml（scheduledAt + platform + draft/text），TaskRunner 的 social.tick 每 60s 检查并按门禁自动发布

运营约束：
- 凭据由系统保管（CredentialsManager 加密存储），LLM 不可读取，也不得通过 run_command 直调 social CLI 发帖
- 门禁：未启用平台 / safe 模式 / 风险词命中会被拒绝（blocked / requires_review）
- 无发布 API 的平台（知乎等）返回人工发布草稿（requiresHumanPublish）
- 风控词被拦截时改写内容重试
...
自动运营流程（每次分析循环执行）：
1. read_file social/config/strategy.yaml 了解策略
2. read_file social/config/content_calendar.yaml 检查排程
