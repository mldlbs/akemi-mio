...
import { asrLogStore } from '@akemi-mio/audio/AsrLogStore'
import { asrConfidenceScorer, DEFAULT_CONFIDENCE_THRESHOLD } from '@akemi-mio/audio/AsrConfidenceScorer'
import type { AsrErrorPattern, LowConfidenceSegment } from '@akemi-mio/audio/AsrLogStore'
...
export class AsrLogCollector implements SignalCollector {
  ...
  private buildProblem(pattern: AsrErrorPattern): Problem | null {
    ...
    const id = `asr_${pattern.original}_${pattern.corrected}`.replace(/[^a-zA-Z0-9_一-鿿]/g, '_').slice(0, 64)
  ...
  private collectLowConfidenceProblems(since: number): Problem[] {
    ...
    for (const seg of segments) {
      ...
      const alreadyKnown = recentPatterns.some((p) => p.original === text || p.corrected === text)
      if (alreadyKnown) continue

      seenTexts.add(text)

      const id = `asr_lowconf_${text}`.replace(/[^a-zA-Z0-9_一-鿿]/g, '_').slice(0, 64)
