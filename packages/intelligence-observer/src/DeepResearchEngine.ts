...
import type { TopicCandidate, ResearchResult, ResearchPhase, ResearchPhaseName } from './types'

const PHASE_ORDER: ResearchPhaseName[] = ['expansion', 'structural_modeling', 'conflict_analysis']
...
export class DeepResearchEngine {
  ...
  async research(topic: TopicCandidate, relatedObs: string[]): Promise<ResearchResult> {
  ...
  private async generateFallbackSummary(topic: string, context: string): Promise<string | null> {
...
请用一段话概括这个主题的核心内容。不需要分析，只需要列举已知的关键信息。`
  ...
  private async runPhase(phase: ResearchPhaseName, topic: string, context: string): Promise<string | null> {
  ...
  private buildPhasePrompt(phase: ResearchPhaseName, topic: string, context: string): string {
    switch (phase) {
      case 'expansion':
        return `主题：「${topic}」
...
只输出 JSON：
{
  ...
  private assemble(topicId: string, phases: ResearchPhase[]): ResearchResult {
    let facts: string[] = []
    let timeline: { time: string; event: string }[] = []
    let causalLinks: { cause: string; effect: string; confidence: number }[] = []
    let perspectives: { viewpoint: string; source: string }[] = []
    let conflicts: { partyA: string; partyB: string; nature: string; evidence: string }[] = []
