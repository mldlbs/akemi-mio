...
export class ObserverService {
  ...
  // Optional `llmConfig` mirrors packages/observer: a host that has already
  // resolved the model connection injects it here instead of relying on the
  // process environment. Omitted, the LLM service falls back to LLM_* /
  // OBSERVER_* env vars and then its Ollama default.
   ...
   */
  async collectBySource(sourceNames: string[], keywords?: string[], limit: number = 20): Promise<Record<string, Observation[]>> {
    ...
    for (const name of sourceNames) {
      const canonical = name === 'weibo_hot' ? 'weibo-hot' : name === 'github_trending' ? 'github-trending' : name
      ...
      try {
        ...
        if (keywords && keywords.length > 0) {
          ...
          obs = obs.filter((o) => kwLower.some((kw) => o.content.toLowerCase().includes(kw)))
        }
