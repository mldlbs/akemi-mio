...
export class BehaviorProfileGenerator {
   ...
   */
  static generateProfile(interactions: InteractionRecord[], source: string = 'cold_start'): ProfileResult {
    try {
      const userTexts = interactions.filter((r) => r.userText && r.userText.trim().length > 0).map((r) => r.userText)
   ...
   * 通过统计中文字符和英文字符的比例来推断。
   */
  private static analyzeLanguagePreference(texts: string[], source: string, totalCount: number): BehaviorProfileEntry | null {
   ...
   * - 闲聊/寒暄 → 聊天模式
   */
  private static analyzeReplyStyle(texts: string[], source: string, totalCount: number): BehaviorProfileEntry | null {
    if (texts.length === 0) return null
   ...
   * 每个突出的话题生成一条偏好条目。
   */
  private static analyzeTopicPreferences(texts: string[], source: string, totalCount: number): BehaviorProfileEntry[] {
    ...
    const sorted = [...topicCounts.entries()].sort((a, b) => b[1] - a[1]).filter(([, count]) => count >= 2 || count / texts.length >= 0.5)
   ...
   */
  private static analyzeDetailPreference(texts: string[], source: string, totalCount: number): BehaviorProfileEntry | null {
