...
export class BehaviorPredictionMemoryEngine {
  ...
  start(): void {
    ...
    // 首次执行延迟 30 秒
    setTimeout(() => {
      this.checkAndPreload().catch((err) => log('WARN', 'prediction_memory_engine_first_check_failed', { error: String(err) }))
    }, 30_000)

    this.timer = setInterval(() => {
      this.checkAndPreload().catch((err) => log('WARN', 'prediction_memory_engine_check_failed', { error: String(err) }))
    }, CHECK_INTERVAL_MS)
  }
  ...
  async checkAndPreload(): Promise<void> {
    ...
    try {
      ...
      const { getBehaviorContext, getRecentInteractionTopics, getMemoryEntries, getBehaviorWeightedEntries } = this.deps
   ...
   * 其中各子评分归一化到 [0, 1]。
   */
  private computeMatchScore(entry: MemoryEntry, ctx: BehaviorContext, recentTopics: string[]): MatchScoreDetail {
    ...
    const combined = Math.min(1, Math.round((timeScore * 0.35 + appScore * 0.3 + topicScore * 0.35) * 1000) / 1000)
  ...
  private computeTimeMatch(entry: MemoryEntry, ctx: BehaviorContext): number {
    const timestamps = [entry.createdAt, entry.updatedAt, entry.lastAccessedAt].filter((t) => t > 0)
   ...
   */
  private updatePreloadBuffer(newMatches: Array<{ entry: MemoryEntry; scoreDetail: MatchScoreDetail }>): void {
    const now = Date.now()
    ...
    // 按评分降序排列，取 Top-N
    const sorted = [...merged.values()].sort((a, b) => b.scoreDetail.combined - a.scoreDetail.combined).slice(0, PRELOAD_BUFFER_MAX)
