...
function sanitizePluginName(name: string): string {
  ...
  if (!sanitized) throw new Error('插件名称无效，仅允许字母、数字、下划线和连字符')
  return sanitized
}

function createPlugin(name: string, content: string): string {
...
function listPlugins(): string {
  ...
  if (registrations.length === 0) return '暂无已注册的工具'

  const byPlugin = new Map<string, string[]>()
  for (const reg of registrations) {
    if (!byPlugin.has(reg.pluginName)) byPlugin.set(reg.pluginName, [])
