...
export class AuditTrail {
  record(entry: Omit<AuditEntry, 'id' | 'timestamp'>): void {
    ...
    try {
      db.run(
        `INSERT INTO audit_trail (id, timestamp, source, action, target, plugin_name, tool_name, details, allowed, duration, reason)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          id,
          now,
  ...
  query(filter: { source?: string; action?: string; limit?: number } = {}): AuditEntry[] {
    const db = getRawDb()
  ...
  getBySource(source: string, limit: number = 20): AuditEntry[] {
    return this.query({ source, limit })
  }

  getStats(): { total: number; bySource: Record<string, number>; byAction: Record<string, number> } {
    const db = getRawDb()
    ...
    try {
      ...
      if (srcRows[0]) for (const row of srcRows[0].values) bySource[String(row[0])] = Number(row[1])
      const actRows = db.exec('SELECT action, COUNT(*) AS c FROM audit_trail GROUP BY action')
      if (actRows[0]) for (const row of actRows[0].values) byAction[String(row[0])] = Number(row[1])
    } catch {
      /* ignore */
    }
