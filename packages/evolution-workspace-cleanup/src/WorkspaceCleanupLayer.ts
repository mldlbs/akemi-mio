...
export class WorkspaceCleanupLayer {
  ...
  constructor(config?: WorkspaceCleanupLayerConfig) {
    ...
    this.config = applyDefaults({ debug: config?.debug ?? false } as Partial<WorkspaceCleanupLayerConfig>, DEFAULT_CONFIG) as Required<
      Pick<WorkspaceCleanupLayerConfig, 'debug'>
    >
  ...
  private formatStatsSummary(stats: WorkspaceStats): string {
    const lines: string[] = [`📁 工作区概览：${stats.totalFiles} 个文件，${stats.totalDirs} 个目录`]
