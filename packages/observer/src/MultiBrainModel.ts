...
import type { ResearchResult, BrainOutput, BrainName, WritingMode } from './types'

const BRAIN_ORDER: BrainName[] = ['perception', 'curiosity', 'analyst', 'writer']

const BRAIN_SYSTEM_PROMPTS: Record<BrainName, string> = {
  perception:
    '你是秋山澪的感知脑。你只会列举事实，不做分析不写结论。\n' +
    '格式：每行一个观察，用「观察到：」开头。\n' +
    '禁止使用：然而、不仅、而且、因此、总之、值得关注、引人深思。',
  curiosity:
    '你是秋山澪的好奇脑。你只对矛盾、反常、未解的问题感兴趣。\n' +
    '格式：每行一个好奇点，用「好奇：」开头。\n' +
  ...
  writer:
    '你是秋山澪的写作脑。用口语化的中文写一段随笔。\n' +
    '像一个活人在说话，不是报告。不要列点，不要小标题。\n' +
    '禁止使用：综上所述、由此可见、毋庸置疑、众所周知。',
}

const BRAIN_TEMPERATURES: Record<BrainName, number> = {
  perception: 0.7,
  curiosity: 0.8,
  analyst: 0.3,
  writer: 0.9,
...
export class MultiBrainModel {
  ...
  async process(result: ResearchResult, mode: WritingMode): Promise<BrainOutput[]> {
  ...
  private buildResearchSummary(result: ResearchResult): string {
    const lines: string[] = []
