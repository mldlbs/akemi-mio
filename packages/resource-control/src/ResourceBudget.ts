...
export class ResourceBudget {
  ...
  constructor(config?: Partial<BudgetConfig>, budgetFilePath?: string) {
    ...
    this.budgetFilePath = budgetFilePath || join(rt.workspaceDir(), 'evolution', 'resource_budget.json')
  ...
  startRequest(): void {
    ...
    this.state = this.freshState()
    if (wasExhausted) {
      rt.bus.emit('budget.restored', { resource: 'request', utilization: 0 })
    }
  }
  ...
  consumeLlmCall(context: 'chat' | 'evolution' | 'background' | 'research'): void {
    ...
    if (current >= max) {
      ...
      if (!this.state.exhaustedEmitted.has(resourceKey)) {
        ...
        rt.bus.emit('budget.exhausted', {
          resource: resourceKey,
          utilization: current / max,
        })
        rt.logger('WARN', 'budget_hard_exhausted', { resource: resourceKey, used: current, limit: max })
      }
    ...
    if (this.state.toolLoopTurnsThisCycle >= this.config.maxToolLoopTurns) {
      ...
      if (this.state.softOverrideEnteredAt < 0) {
        ...
        if (!this.state.exhaustedEmitted.has('toolLoop')) {
          ...
          rt.logger('WARN', 'budget_soft_override_started', {
            resource: 'toolLoop',
      ...
      if (extraTurns >= this.config.softOverrideTurns) {
        ...
        if (!this.state.exhaustedEmitted.has('toolLoop.hard')) {
          ...
          rt.logger('WARN', 'budget_hard_stop', {
            resource: 'toolLoop',
            used: this.state.toolLoopTurnsThisCycle,
  ...
  consumeCpuMs(durationMs: number): void {
    ...
    if (this.state.cpuMsThisRequest >= this.config.maxCpuMs) {
      ...
      if (!this.state.exhaustedEmitted.has('cpu')) {
        ...
        rt.logger('WARN', 'budget_cpu_exhausted', { used: this.state.cpuMsThisRequest, limit: this.config.maxCpuMs })
      }
  ...
  consumeMemoryMb(currentHeapMb: number): void {
    ...
    if (currentHeapMb >= this.config.maxMemoryMb) {
      ...
      if (!this.state.exhaustedEmitted.has('memory')) {
        ...
        rt.logger('WARN', 'budget_memory_exhausted', { used: currentHeapMb, limit: this.config.maxMemoryMb })
      }
  ...
  private emitProcessExhausted(processName: string, resource: string, used: number, limit: number): void {
    rt.logger('WARN', 'budget_process_exhausted', { process: processName, resource, used, limit })
    rt.bus.emit('budget.exhausted', { resource: `process.${processName}.${resource}`, utilization: used / limit })
  }
  ...
  autoTune(stabilityScore: number): void {
    if (stabilityScore >= 90) {
      ...
      rt.logger('WARN', 'budget_autotune_reduced', { score: stabilityScore })
      ...
      rt.logger('INFO', 'budget_autotune_degraded', { score: stabilityScore })
    }
  }
  ...
  resetBackground(): void {
    ...
    rt.bus.emit('budget.restored', { resource: 'llm.background', utilization: 0 })
  }
  ...
  resetEvolution(): void {
    ...
    rt.bus.emit('budget.restored', { resource: 'llm.evolution', utilization: 0 })
  }
  ...
  updateConfig(patch: Partial<BudgetConfig>): void {
    ...
    this.originalConfig = { ...this.config }
    rt.logger('INFO', 'budget_config_updated', patch)
  }
  ...
  saveState(): void {
    try {
      ...
      rt.logger('WARN', 'budget_state_save_failed')
    }
  }
  ...
  loadState(): void {
    try {
      ...
      rt.logger('WARN', 'budget_state_load_failed')
    }
  }
}
