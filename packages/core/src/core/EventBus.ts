...
export type EventName =
  ...
  | 'agent.tool.failed'
  // ── Capability events (ADR-015 P0/P1/P1.3a) ──
  | 'capability.invoked'
  ...
  | 'creativity.hypothesis.promoted'
  | 'creativity.fermentation.start'
  | 'creativity.fermentation.done'
  | 'creativity.executed'
  | 'plugin.registered'
  | 'plugin.unregistered'
  | 'plugin.error'
  ...
  | 'execution_goal.created'
  | 'execution_goal.completed'
  | 'execution_goal.blocked'
  | 'execution_goal.abandoned'
  | 'execution_goal.verified'
  | 'agent.observe'
  | 'agent.think'
  | 'agent.reflect'
  ...
  | 'blog:publish:progress'
  | 'blog:publish:completed'
  | 'blog:publish:failed'
  | 'blog:publish:rolled_back'
  | 'evolution.piper.state.synced'
  | 'hybrid.mcp_agent.initialized'
  | 'hybrid.tool_selection.completed'
  | 'hybrid.result_validation.completed'
  | 'hybrid.reply_quality.completed'
  | 'hybrid.pipeline.initialized'
  | 'hybrid.step_planning.completed'
  | 'hybrid.difficulty.recorded'
  | 'hybrid.strategy.adjusted'
  | 'hybrid.progress.evaluated'
  | 'orchestration:progress'
  | 'parameter.adjusted'
  | 'parameter.rollback'
  | 'pipeline.stage.completed'
  | 'plan_first_pipeline.completed'
  | 'plan_memory_scope_synced'
  | 'radar.push.scheduler_started'
  | 'radar.push.voice_alert'
  | 'wallpaper.sleep_maintenance.completed'
export interface EventPayload {
  'blog.mode.switched': { sessionId: string; fromMode: string; toMode: string; reason?: string; snapshotId: string; timestamp: number }
  'blog.session.completed': { sessionId: string }
  'blog:publish:progress': {
    taskId: string
    stage: string
    platform?: string
  ...
  'evolution.piper.state.synced': { state: unknown; version: number }
  'hybrid.mcp_agent.initialized': { config: unknown; timestamp: number }
  'hybrid.tool_selection.completed': {
    point: string
  ...
  'hybrid.pipeline.initialized': { config: unknown; timestamp: number }
  'hybrid.step_planning.completed': { point: string; divergenceScore: number; method: string; stepCount: number; timestamp: number }
  'hybrid.difficulty.recorded': {
    point: string
    conceptName: string
  ...
  'hybrid.strategy.adjusted': {
    ...
    divergenceScore: number
    method: string
    strategies: Array<{ type: string; confidence: number }>
    timestamp: number
  }
  'hybrid.progress.evaluated': {
    point: string
  ...
  'orchestration:progress': {
    ...
    error?: string
    timestamp: number
    stepStatuses: Array<{ id: string; name: string; status: string; toolName: string }>
  }
  'parameter.adjusted': {
    changes: Array<{ key: string; oldValue: unknown; newValue: unknown; reason: string }>
    confidence: number
    snapshotId: string
    timestamp: number
  }
  'parameter.rollback': { snapshotId: string; reason: string; affectedParams: number; timestamp: number }
  'pipeline.stage.completed': {
    version: number
    stageId: string
    success: boolean
  ...
  plan_memory_scope_synced: { version: number; activeDomains: string[]; directiveCount: number; timestamp: number }
  'radar.push.scheduler_started': { timestamp: number }
  'radar.push.voice_alert': {
    ruleId: string
    ruleName: string
    signalCount: number
  ...
  'wallpaper.sleep_maintenance.completed': { ok: boolean; total: number; durationMs: number; maintenanceCount: number }
  ...
  'agent.input.received': { text: string; requestId: string; source: 'electron' | 'telegram' | 'workflow' }
  ...
  'capability.selected': { capability: string; source: string; toolCallId: string; input?: unknown; matchedTool?: string }
  ...
  'creativity.hypothesis.promoted': { hypothesisId: string; timestamp: number }
  'creativity.fermentation.start': { candidates: number; signals: number; timestamp: number }
  'creativity.fermentation.done': { promoted: number; rejected: number; merged: number; kept: number; timestamp: number }
  'creativity.executed': {
    hypothesisId: string
    problemId: string
    applied: boolean
  ...
  'execution_goal.created': { id: string; sessionId: string | null; objective: string; criteriaCount: number; timestamp: number }
  'execution_goal.completed': { id: string; sessionId: string | null; matchedCriteria: string[]; evidenceCount: number; timestamp: number }
  'execution_goal.verified': { id: string; sessionId: string | null; verdict: 'confirmed' | 'contested'; reason: string; timestamp: number }
  'execution_goal.blocked': { id: string; sessionId: string | null; failedCriteria: string[]; reason: string; timestamp: number }
  'execution_goal.abandoned': { id: string; sessionId: string | null; reason: string; evidenceCount: number; timestamp: number }
