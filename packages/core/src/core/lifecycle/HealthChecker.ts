 ...
 * 维护滚动健康历史，状态变更时发射事件。
 */
export class HealthChecker implements ISubsystem {
  ...
  /** 获取子系统最近的健康状态摘要 */
  getHealthSummary(): Array<{ name: string; healthy: boolean; detail?: string; trend: string }> {
    const summary: Array<{ name: string; healthy: boolean; detail?: string; trend: string }> = []
  ...
  private recordHealth(name: string, result: HealthCheckResult): void {
    ...
    if (last && last.healthy !== result.healthy) {
      eventBus.emit('subsystem.health_changed' as any, {
        name,
  ...
  private computeTrend(history: HealthCheckResult[]): string {
    if (history.length < 2) return 'stable'
    const recent = history.slice(-3).filter((h) => !h.healthy).length
    if (recent === 0) return 'stable'
    if (recent <= 1) return 'degraded'
    return 'critical'
  }
