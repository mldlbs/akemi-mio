...
export type TransitionGuard<TState extends string> = (from: TState, to: TState) => boolean | Promise<boolean>

/** 转移钩子：转移前/后调用 */
export type TransitionHook<TState extends string> = (from: TState, to: TState) => void | Promise<void>
...
export class StateMachine<TState extends string> {
   ...
   * @param options 可选配置
   */
  constructor(transitions: Record<TState, readonly TState[]>, initialState: TState, options?: StateMachineOptions<TState>) {
  ...
  async transition(to: TState, guards?: TransitionGuard<TState>[]): Promise<boolean> {
    ...
    if (!this.canTransition(to)) {
      if (this.strict) {
        throw new Error(`状态机[${this.loggerName}] 非法转移: ${from} → ${to}`)
      }
