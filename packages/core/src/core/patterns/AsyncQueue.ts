...
export class AsyncQueue<TInput, TOutput> {
   ...
   * 执行单个任务，支持超时。
   */
  private async processWithTimeout(item: QueuedItem<TInput, TOutput>): Promise<Result<TOutput, string>> {
    try {
      const result = await (this.timeoutMs > 0 ? withTimeout(this.processor(item.input), this.timeoutMs) : this.processor(item.input))
      return ok(result)
    } catch (e) {
