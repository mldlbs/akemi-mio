 ...
 */
export async function retryWithBackoff<T>(fn: () => Promise<T>, options?: RetryOptions): Promise<Result<T, string>> {
 ...
 * 失败时直接抛出最后的错误。
 */
export async function retryWithBackoffOrThrow<T>(fn: () => Promise<T>, options?: RetryOptions): Promise<T> {
