...
interface Migration {
  version: number
  sql: string | ((db: MigrationDatabase) => void)
...
const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    sql: `
      CREATE TABLE IF NOT EXISTS task_goals (
        ...
        status TEXT NOT NULL DEFAULT 'planning' CHECK(status IN ('planning', 'executing', 'blocked', 'completed', 'abandoned')),
        plan_id TEXT,
      ...
      CREATE INDEX IF NOT EXISTS idx_task_goals_session_id ON task_goals(session_id);
      CREATE INDEX IF NOT EXISTS idx_task_goals_status ON task_goals(status);
    `,
    revert: 'DROP TABLE IF EXISTS task_goals;',
    category: 'schema',
  },
  {
    version: 46,
    sql: `
      ALTER TABLE task_goals ADD COLUMN methodology TEXT;
    `,
    revert: '',
    category: 'schema',
  },
  {
    version: 47,
    sql: (db) => {
      // 修复 v44 版本号撞车：部分实例的 hypotheses 表从未获得发酵字段。
      // 兼容两种状态：已加列（跳过）与未加列（补齐）。
      ...
      stmt.free()
      const adds: string[] = []
      if (!columns.has('ferment_count')) adds.push('ALTER TABLE hypotheses ADD COLUMN ferment_count INTEGER NOT NULL DEFAULT 0')
      if (!columns.has('last_fermented_at')) adds.push('ALTER TABLE hypotheses ADD COLUMN last_fermented_at INTEGER')
      if (!columns.has('ferment_log')) adds.push('ALTER TABLE hypotheses ADD COLUMN ferment_log TEXT')
      if (!columns.has('merged_into')) adds.push('ALTER TABLE hypotheses ADD COLUMN merged_into TEXT')
      for (const sql of adds) db.run(sql)
    },
    revert: '',
    irreversible: true,
    category: 'schema',
  },
  {
    ...
    sql: (db) => {
      ...
      const additions = [
        ['source_details', 'TEXT'],
        ['implemented_commit_sha', 'TEXT'],
        ['implemented_at', 'INTEGER'],
        ['implementation_note', 'TEXT'],
      ] as const
      for (const [name, type] of additions) {
        if (!columns.has(name)) db.run(`ALTER TABLE hypotheses ADD COLUMN ${name} ${type}`)
      }
    },
    revert: '',
    irreversible: true,
...
export function runMigrations(sqlite: MigrationDatabase): void {
  ...
  for (const m of MIGRATIONS) {
    if (!applied.has(m.version)) {
      ...
      if (typeof m.sql === 'function') m.sql(sqlite)
      else sqlite.run(m.sql)
 ...
 */
export function revertMigration(sqlite: MigrationDatabase, version: number): void {
