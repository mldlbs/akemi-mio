...
export class ParameterHotReloader {
  ...
  checkDegradation(): string {
    ...
    if (
      ...
      issues.push(`成功率: ${(this.lastBaseline.successRate * 100).toFixed(1)}% → ${(success.currentValue * 100).toFixed(1)}%`)
    }
    ...
    const suggestion = ['⚠️ 检测到参数调整后指标恶化：', ...issues.map((i) => `  - ${i}`), '', '建议回滚到最近一次快照。'].join('\n')
