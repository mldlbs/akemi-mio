...
export class FeedbackMetadataStore {
  ...
  /** 记录一个反馈数据点 */
  record(input: { metric: string; value: number; category: FeedbackMetricCategory; context?: string }): void {
  ...
  private analyzeMetricCategory(category: FeedbackMetricCategory): FeedbackMetricAnalysis | null {
    ...
    const previousMean = previousPoints.length > 0 ? this.mean(previousPoints.map((p) => p.value)) : currentMean // 无历史数据则视为稳定
    ...
    const suggestion = isConcerning ? this.suggestAction(category, currentMean, trend) : undefined
  ...
  /** 生成建议操作文本 */
  private suggestAction(category: FeedbackMetricCategory, currentValue: number, trend: FeedbackMetricAnalysis['trend']): string {
    switch (category) {
      case 'user_satisfaction':
