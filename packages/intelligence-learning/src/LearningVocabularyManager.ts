...
import { IdentifiableJsonStore } from '@akemi-mio/core/core/persistence/JsonStore'
import { SlidingWindow } from '@akemi-mio/core/core/patterns/SlidingWindow'
import type { LearningItem, LearningCategory, LearningProgress, ConceptDifficulty } from './types'
...
/** 学习词表持久化存储 */
const VOCABULARY_STORE = new IdentifiableJsonStore<LearningItem>(join(WORKSPACE.cache, 'learning-vocabulary.json'), {
...
const CATEGORY_RULES: Array<{
  ...
  {
    category: '条件类型',
    patterns: [/^(conditional|extends\?|infer|distributive)/i, /^条件类型$/i, /^extends\?/i, /^infer\b/i],
  },
  {
    category: '映射类型',
    patterns: [/^(mapped|keyof|in\s+\w+|remap|property.modifier|filtering)/i, /^映射类型$/i, /^keyof/i, /^P\s+in\s+K/i],
  },
  {
    category: '模板字面量类型',
    patterns: [
  ...
  {
    category: '泛型',
    patterns: [/^(generic|type\s+parameter|constraint|parameter\s+default)/i, /^泛型$/i, /<T>/i, /^<.*>$/i],
  },
  {
    category: '工具类型',
    patterns: [
  ...
  {
    category: '类型守卫',
    patterns: [/^(type\s+guard|assertion|type\s+predicate|guards?)/i, /^类型守卫$/i, /\bis\s+\w+/i, /asserts/i],
  },
  {
    category: '类型推断',
    patterns: [/^(contextual\s+typing|best\s+common|narrowing|widening|inferr?ence)/i, /^类型推断$/i, /type\s+narrow/i, /type\s+widen/i],
  },
  {
    category: '基础类型',
    patterns: [/^(union|intersection|alias|literal|nullable)/i, /^基础类型$/i, /^type\s+alias/i, /\bunion\b/i, /\bintersection\b/i],
  },
]
...
export class LearningVocabularyManager {
  ...
  initialize(): void {
    ...
    const loaded = VOCABULARY_STORE.mergeInto(this.longTermItems, (existing, persisted) => ({
      ...existing,
  ...
  getAllItems(): LearningItem[] {
    return Array.from(this.longTermItems.values()).sort((a, b) => b.mastery - a.mastery)
  }
  ...
  getProgress(): LearningProgress {
    ...
    const learningCount = allItems.filter((i) => i.mastery >= LEARNING_THRESHOLD && i.mastery < MASTERY_THRESHOLD).length
    const notStartedCount = allItems.filter((i) => i.mastery < LEARNING_THRESHOLD).length

    const overallMastery = totalItems > 0 ? allItems.reduce((s, i) => s + i.mastery, 0) / totalItems : 0
