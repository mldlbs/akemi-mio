...
import type { LearningItem, LearningCategory, LearningFocusContext } from './types'
// ── 常量 ──
 ...
 * @returns 关注上下文
 */
export function buildFocusContext(items: LearningItem[], currentStepDescription?: string): LearningFocusContext {
  const focusCategories = new Set<LearningCategory>()
  const recentConcepts: string[] = []

  // 从掌握度最低的知识点提取关注类别（最需要关注的分类）
  const unmastered = items.filter((i) => i.mastery < 0.8).sort((a, b) => a.mastery - b.mastery)
...
export function buildPriorityItems(
  ...
  if (focusContext) {
    const contextItems = allItems.filter(
      (item) =>
        item.mastery < 0.8 && (focusContext.focusCategories.includes(item.category) || focusContext.recentConcepts.includes(item.name)),
    )
  ...
  for (const coreName of CORE_LEARNING_ITEMS) {
    const coreItem = allItems.find((i) => i.name.toLowerCase() === coreName.toLowerCase())
  ...
  // 3. 其余未掌握项（按掌握度升序）
  const remaining = allItems.filter((item) => !seen.has(item.id) && item.mastery < 0.8).sort((a, b) => a.mastery - b.mastery)
...
export function buildFocusSummary(focusContext: LearningFocusContext): string {
  ...
  if (focusContext.focusCategories.length > 0) {
    parts.push(`关注分类: ${focusContext.focusCategories.join('、')}`)
  }
  ...
  if (focusContext.recentInteractionText) {
    parts.push(`最近交互: ${focusContext.recentInteractionText.slice(0, 100)}`)
  }
...
export function isFocusMeaningful(context: LearningFocusContext): boolean {
  return context.focusCategories.length > 0 || context.recentConcepts.length >= MIN_MEANINGFUL_ITEMS
}
...
export function formatFocusPrefix(items: LearningItem[], limit = 10): string {
  ...
  const itemsStr = display.map((i) => `${i.name}(${Math.round(i.mastery * 100)}%)`).join(', ')
