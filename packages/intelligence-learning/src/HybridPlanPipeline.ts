...
export class HybridPlanPipeline {
  ...
  constructor(executor: PlanTypeScriptExecutor, config?: Partial<HybridPipelineConfig>) {
  ...
  createLearningPlan(): StepSuggestion[] | null {
    ...
    const result = this.arbitrator.arbitrate<StepSuggestion[]>('step_planning', originalOutput, evolutionOutput)
  ...
  private runEvolutionStepPlanning(): StepSuggestion[] | null {
    ...
    const difficultyPatterns = learningProgressTracker.getDifficultyPatterns(Date.now() - EVOLUTION_DIFFICULTY_LOOKBACK_MS)
    ...
    const categoriesByDifficulty = Array.from(categoryDifficultyMap.entries()).sort(([, a], [, b]) => b.frequency - a.frequency)
  ...
  recordDifficulty(conceptName: string, description: string): void {
    ...
    learningProgressTracker.recordDifficulty(conceptName.toLowerCase(), conceptName, description, finalCategory as any)
  ...
  private runEvolutionDifficulty(conceptName: string, description: string): DifficultyAssessmentOutput {
    ...
    // 查找同一知识点的历史困难记录
    const relatedPatterns = patterns.filter((p) => p.conceptName.toLowerCase() === conceptName.toLowerCase())

    const baseFrequency = relatedPatterns.length > 0 ? relatedPatterns.reduce((s, p) => s + p.frequency, 0) : 1
    ...
    return {
      ...
      description: relatedPatterns.length > 0 ? `${description}（历史出现 ${baseFrequency} 次）` : description,
    }
  }
  ...
  autoAdjustStrategy(): void {
    ...
    const result = this.arbitrator.arbitrate<StrategySuggestion[]>('strategy', originalOutput, evolutionOutput)
  ...
  private runEvolutionStrategyAdjustment(): StrategySuggestion[] {
    ...
    const allMasteryAvg = items.length > 0 ? items.reduce((s, i) => s + i.mastery, 0) / items.length : 0
  ...
  evaluateProgress(snapshotId?: string): ProgressEvaluationOutput {
    ...
    // ── 汇合：交叉验证 ──
    const result = this.arbitrator.arbitrate<ProgressEvaluationOutput>('progress_evaluation', originalOutput, evolutionOutput)
  ...
  private runEvolutionProgressEvaluation(snapshotId?: string): ProgressEvaluationOutput {
    ...
    const patterns = learningProgressTracker.getDifficultyPatterns(Date.now() - EVOLUTION_DIFFICULTY_LOOKBACK_MS)
   ...
   */
  private evolutionClassifyDifficulty(description: string, conceptName: string, patterns: LearningDifficulty[]): string {
