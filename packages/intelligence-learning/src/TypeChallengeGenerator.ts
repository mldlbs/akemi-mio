...
const _emptyCheck: string | undefined = empty;`,
    ...
    explanation:
      '泛型函数 `firstElement<T>` 接受类型参数 T。由于数组可能为空，`arr[0]` 的实际返回类型是 `T | undefined`（TypeScript 在 strict 模式下会这么推断）。当然也可以写 `T` 如果允许 undefined 的隐式返回，但更精确的做法是返回 `T | undefined`。',
  },
...
// 正向验证：确保字符串和数组正确工作即可`,
    ...
    explanation:
      '泛型约束 `T extends HasLength` 确保 T 具有 length 属性。函数体返回 a 或 b（都是 T 类型），所以返回值类型是 T。这就是泛型约束的典型用法：限制了类型参数的范围，但保留了具体类型信息。',
  },
...
const _t4: false = null as unknown as Test4;`,
    ...
    explanation:
      '条件类型 `IsString<T>` 检查 T 是否 extends string。如果是，结果为 true（字面量类型），否则为 false。这是 TypeScript 类型层面的 if/else。注意此处利用了分布式条件类型的特性：当 T 是联合类型时，条件会分发到每个成员。',
  },
...
const _t4: Promise<string> = null as unknown as Test4;`,
    ...
    explanation:
      '`infer U` 在条件类型中声明了一个待推断的类型变量。当 T 匹配 `Promise<infer U>` 模式时，U 会被自动推断为 Promise 内部的类型。这是 TypeScript 内置 `Awaited<T>` 和 `ReturnType<T>` 的实现基础。',
  },
...
const _keysCheck: 'name' | 'age' = null as unknown as ReadonlyKeys;`,
    ...
    explanation:
      '映射类型 `MyReadonly<T>` 遍历 T 的所有键（`P in keyof T`），对每个属性 `P`，值的类型保持 `T[P]` 不变，但添加了 `readonly` 修饰符。结果类型与原类型有相同的键，但所有属性都是只读的。这正是 TypeScript 内置 `Readonly<T>` 的实现方式。',
  },
...
const _checkNoDesc: true = null as unknown as _AssertNoDesc;`,
    ...
    explanation:
      '`MyPick<T, K extends keyof T>` 使用泛型约束确保 K 是 T 的键的子集，然后通过映射类型遍历 K 中的每个键，选取对应的属性值类型。这是 TypeScript 内置工具类型中 Pick 的实现方式。',
  },
...
const _checkNoDesc: true = null as unknown as _AssertNoDesc;`,
    ...
    explanation:
      '`MyOmit` 使用了 TypeScript 4.1+ 的键重映射（key remapping）语法。`as` 子句对每个键 P 应用条件类型：如果 P 在 K 中，映射到 never（被排除），否则保持原键。这比 Pick + Exclude 的组合更直接。',
  },
...
const _t3: string[] = null as unknown as Arr3;`,
    ...
    explanation:
      '当条件类型的检查类型是一个裸类型参数（如 `T extends ...`）且实际传入的是联合类型时，TypeScript 会将联合类型的每个成员分别代入条件计算，这就是分布式条件类型。用方括号 `[T]` 包裹可以阻止分发行为，将联合类型作为一个整体处理。',
  },
...
const _t3: 'onMouseEnter' = null as unknown as MouseEvent;`,
    ...
    explanation:
      '模板字面量类型使用 JavaScript 模板字符串语法在类型层面拼接字符串。`Capitalize<T>` 是 TypeScript 内置的字符串操作类型，将字符串字面量的首字母大写。配合映射类型的键重映射，可以构建类型安全的事件处理系统。',
  },
...
const _t3: string[] = null as unknown as ArrResult;`,
    ...
    explanation:
      '`DeepPartial` 是一个递归类型，在自身的定义中引用了自己。当 T 是对象类型时（用 `Record<string, unknown>` 判断），遍历所有属性并设为可选，同时递归调用 `DeepPartial` 处理属性的值类型。基本类型则直接返回不做处理。这种模式在处理深层嵌套的配置对象时非常有用。',
  },
...
function process(value: string | number): string {
  if (isNumber(value)) {
    ...
    explanation:
      '用户自定义类型守卫使用类型谓词 `value is TypeName` 语法。当函数返回 true 时，TypeScript 会在当前作用域中将参数的类型收窄为 TypeName。这是 `typeof`、`instanceof` 等内置守卫之外，扩展类型收窄能力的重要方式。',
  },
...
assertDefined(num);
const _n: number = num;`,
    hint: '断言函数的类型谓词语法：`asserts value is T`。注意断言函数不返回布尔值，而是在检查失败时抛出异常。如果函数正常返回，TypeScript 就认为类型已收窄。',
    solution:
      "function assertDefined<T>(value: T): asserts value is NonNullable<T> {\n  if (value === null || value === undefined) {\n    throw new Error('Value must not be null or undefined');\n  }\n}",
    explanation:
      '断言函数使用 `asserts` 关键字声明类型收窄效果。与类型守卫不同，断言函数不返回布尔值——如果函数执行完毕没有抛出异常，TypeScript 就认为条件满足。这让断言函数非常适合前置条件检查（如参数校验）。',
  },
...
const _age: number = u.age;`,
    ...
    explanation:
      '映射类型支持在遍历属性时使用 +/- 符号添加或移除修饰符。`-readonly` 移除只读（+`readonly` 添加只读），`-?` 移除可选标记（`+?` 或 `?` 添加可选）。这在需要对类型进行严格化处理时非常有用。',
  },
...
const _t4: never = null as unknown as Test4;`,
    ...
    explanation:
      "`MyExclude` 利用分布式条件类型的特性实现。当 T 是一个联合类型时，条件类型会分发到每个成员：`MyExclude<'a' | 'b', 'a'>` 相当于 `('a' extends 'a' ? never : 'a') | ('b' extends 'a' ? never : 'b') = never | 'b' = 'b'`。这是 TypeScript 内置 Exclude 的实现方式。",
  },
...
const _t4: Promise<unknown> = null as unknown as R4;`,
    ...
    explanation:
      '`MyReturnType` 使用 infer 在条件类型中匹配函数签名模式。`T extends (...args: any[]) => infer R ? R : never`：如果 T 匹配函数签名，`infer R` 推断出返回值类型；否则返回 never。这是 TypeScript 内置 ReturnType 的精确实现。',
  },
...
// 仅保留正向测试（验证正确用法）`,
    ...
    explanation:
      '可辨识联合（Discriminated Union）模式使用一个字面量类型的公共字段（称为 tag/discriminant）来区分联合的各个成员。TypeScript 可以通过 switch 或 if 检查该字段的值，自动将联合类型收窄到具体的成员类型。这种模式在 Redux reducer、状态机等领域非常常见。',
  },
...
const _t6: {} = null as unknown as Empty;`,
    ...
    explanation:
      '这个类型结合了三种高级类型技术：映射类型（`P in keyof T`）、键重映射（`as` 子句）、和条件类型（`T[P] extends U ? P : never`）。只有当属性的值类型匹配 U 时，键才会被保留。这是按值类型过滤属性的通用模式。',
  },
]
...
export class TypeChallengeGenerator {
  ...
  generateChallenge(conceptName?: string): GeneratedChallenge | null {
    const template = conceptName ? this.findTemplate(conceptName) : this.getRandomTemplate()
  ...
  generateChallengeByConceptId(conceptId: string): GeneratedChallenge | null {
    const template = CHALLENGE_TEMPLATES.find((t) => t.conceptId === conceptId.toLowerCase())
    if (!template) return null
  ...
  private findTemplate(conceptName: string): TypeChallengeTemplate | null {
    ...
    // 模糊匹配 conceptName
    const byName = CHALLENGE_TEMPLATES.find((t) => t.conceptName.toLowerCase() === lower)
    ...
    return (
      CHALLENGE_TEMPLATES.find((t) => {
        ...
        return lower.includes(name) || name.includes(lower) || lower.includes(id) || id.includes(lower) || t.label.includes(conceptName)
      }) || null
    )
  }
