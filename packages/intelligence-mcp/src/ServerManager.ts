...
import { type MCPServerConfig, type MCPToolDefinition, type MCPToolResult } from './types'
...
import { CapabilityEngine } from '@akemi-mio/capabilities/capability/CapabilityEngine'
...
import { setMemoryResourceProvider, setAsrVocabularyResource } from '@akemi-mio/capabilities/tool/deps'
import { MEMORY_TOOL_PERSONALIZATION, BEHAVIOR_PREDICTOR_PRELOAD_CONFIDENCE } from '@akemi-mio/core/config'
import { behaviorPredictor } from './BehaviorPredictor'
import { toolCallLogStore } from '@akemi-mio/capabilities/tool/ToolCallLogStore'
import { toolCallMemoryCache } from '@akemi-mio/capabilities/tool/ToolCallMemoryCache'
import { classifyToolError } from '@akemi-mio/capabilities/tool/ToolErrorType'
import { toolAnalytics } from '@akemi-mio/capabilities/tool/ToolAnalytics'
import { inferCapabilityOperation, type ToolCallIdentity } from '@akemi-mio/capabilities/tool/ToolCallIdentity'
import type { ProcessManager } from '@akemi-mio/core/core/ProcessManager'
...
export class ServerManager {
  ...
  private capabilityResolver: CapabilityResolver | null = null
  ...
  setCapabilityResolver(cr: CapabilityResolver): void {
    this.capabilityResolver = cr
    this.memoryInterceptor.setCapabilityResolver(cr)
    this.reconcileLoadedLogIdentity(cr)
  }

  /**
   * M5.6.5: reconcile capability identity on loaded log records at startup using
   * the catalog reverse lookup. Idempotent ? only fills untagged records; a
   * failure never blocks startup.
   */
  private reconcileLoadedLogIdentity(cr: CapabilityResolver): void {
    try {
      const result = toolCallLogStore.backfillCapabilityIdentity((record) => {
        const identity = cr.resolveByTool(record.toolName)
        if (!identity) return undefined

        return {
          capability: identity.capabilityId,
          operation: inferCapabilityOperation(identity.capabilityId, record.args ?? {}, record.toolName),
          provider: record.provider,
        }
      })

      if (result.updated > 0) {
        log('INFO', 'tool_call_log_capability_reconcile', {
          scanned: result.scanned,
          updated: result.updated,
        })
      }
    } catch (error: any) {
      log('WARN', 'tool_call_log_capability_reconcile_failed', { error: error.message })
    }
  }
  ...
  private initServers(): void {
    ...
    for (const entry of entries) {
      const config: MCPServerConfig =
        entry.transport === 'http' || entry.transport === 'sse' || entry.url
          ? {
              name: entry.name,
  ...
  private persistServers(): void {
    ...
    for (const [name, client] of this.controlPlane.servers) {
      entries.push(
        client.transportType === 'stdio'
          ? {
              name,
              command: client.getLaunchCommand() || '',
              cwd: client.cwd ? relative(WORKSPACE_DIR, client.cwd) || undefined : undefined,
            }
  ...
  getAllSchemas(): Array<{
    ...
    if (priorities.length > 0) {
      ...
      log('INFO', 'memory_tool_prioritized', {
        ...
        topBoosted: priorities
          .slice(0, 5)
          .map((p) => `${p.toolName}(+${p.boost})`)
          .join(', '),
      })
    }
  ...
  async callTool(name: string, args: Record<string, any>): Promise<string> {
    ...
    if (memoryCacheHit !== null) {
      const identity = this.resolveTelemetryIdentity(name, args, meta.serverName)
      this.memoryInterceptor.postCall(name, args, memoryCacheHit.result, true)
      toolCallLogStore.record(name, args, memoryCacheHit.result, null, Date.now() - startedAt, true, identity)
    ...
    if (cachedResult !== null) {
      const identity = this.resolveTelemetryIdentity(name, args, meta.serverName)
      behaviorPredictor.recordCall(name, args, 0, true, identity)
      this.memoryInterceptor.postCall(name, args, cachedResult, true)
      toolCallLogStore.record(name, args, cachedResult, null, Date.now() - startedAt, true, identity)
    ...
    try {
      ...
      toolCallLogStore.record(name, args, result, null, Date.now() - startedAt, true, identity)
      ...
      toolCallLogStore.record(
        ...
        Date.now() - startedAt,
        false,
        this.resolveTelemetryIdentity(name, args, meta.serverName),
      )
      ...
      behaviorPredictor.recordCall(name, args, 0, false, this.resolveTelemetryIdentity(name, args, meta.serverName))
      throw err
    }
  }
    ...
    switch (name) {
      case 'list_mcp_servers':
        return this.listServers()
          .map((s) => `${s.name} [${s.initialized ? '在线' : '离线'}] ${s.tools} 个工具`)
          .join('\n')
  ...
  private toolNameToCapability(toolName: string): import('@akemi-mio/capabilities/capability/types').CapabilityAction {
  ...
  private recordAndPredict(currentTool: string, args: Record<string, any>, result: string, serverName: string): void {
    const identity = this.resolveTelemetryIdentity(currentTool, args, serverName)
    behaviorPredictor.recordCall(currentTool, args, 0, true, identity)
  ...
  private triggerPreload(toolName: string, serverName: string): void {
    ...
    if (serverName === this.local.name) {
      if (!hasRequiredParams && FILE_READ_TOOLS.has(toolName)) {
        ...
        behaviorPredictor
          .preloadTool(noArgCacheKey, serverName, toolName, {}, async (name, preloadArgs) => {
  ...
  private resolveTelemetryIdentity(toolName: string, args: Record<string, any>, provider: string): ToolCallIdentity | undefined {
    try {
      const identity = this.capabilityResolver?.resolveByTool(toolName)
      if (!identity) return undefined

      return {
        capability: identity.capabilityId,
        operation: inferCapabilityOperation(identity.capabilityId, args, toolName),
        provider,
      }
    } catch {
      return undefined
