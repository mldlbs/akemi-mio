...
export class PeriodicPatternAnalyzer {
  ...
  analyze(): PeriodicAnalysisResult {
    ...
    for (const [toolName, hourMap] of toolHourMap) {
      ...
      // 找出超过置信度阈值的 peak 桶
      const peakBuckets = hourlyBuckets.filter((hb) => hb.proportion >= this.config.periodicConfidenceThreshold)
    ...
    const daysCovered = timestamps.length > 1 ? Math.round((timestamps[timestamps.length - 1] - timestamps[0]) / (24 * 60 * 60 * 1000)) : 0
