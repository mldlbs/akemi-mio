import { type Transport, StdioTransport, HttpTransport } from './transport'
import { type MCPRequest, type MCPResponse, type MCPToolSchema, type MCPToolResult, type MCPInitializeResult, type MCPServerConfig, type MCPToolDefinition } from './types'
...
export class McpClient {
  ...
  constructor(config: MCPServerConfig) {
    ...
    if (config.transport === 'http' || config.transport === 'sse' || config.url) {
      ...
      this.url = ''
      this.transport = new StdioTransport(config.command!, config.args, config.env, config.cwd, config.existingProcess)
    }
