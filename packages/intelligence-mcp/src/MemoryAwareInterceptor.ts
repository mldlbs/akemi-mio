...
import { MEMORY_TOOL_PERSONALIZATION, MEMORY_TOOL_MIN_RECORDS, MEMORY_TOOL_BOOST_FACTOR, MEMORY_TOOL_STAT_WINDOW } from '@akemi-mio/core/config'
...
export class MemoryAwareInterceptor {
  ...
  /** M5.5: inject CapabilityResolver for tool->capability identity lookup */
  setCapabilityResolver(cr: CapabilityResolver | null): void {
  ...
  private getEffectiveBoostFactor(): number {
    switch (this.personalizationLevel) {
      case 'off':
        return 0
      case 'conservative':
        return Math.round(MEMORY_TOOL_BOOST_FACTOR * 0.5)
      case 'balanced':
        return MEMORY_TOOL_BOOST_FACTOR
      case 'aggressive':
        return MEMORY_TOOL_BOOST_FACTOR * 2
    }
  }
  ...
  private hasSufficientData(): boolean {
    const totalCalls = Array.from(this.toolStats.values()).reduce((sum, s) => sum + s.callCount, 0)
  ...
  getToolPriorities(): ToolPriorityInfo[] {
    ...
    for (const [toolName, stats] of this.toolStats) {
      const successRate = stats.callCount > 0 ? stats.successCount / stats.callCount : 0
      ...
      // 综合 boost
      const boost = Math.round(effectiveBoost * freqFactor + effectiveBoost * successFactor + feedbackPenalty)
  ...
  preCall(toolName: string, args: Record<string, any>): MemoryContext {
    ...
    try {
      // 构建语义查询：工具名 + 字符串参数值
      // M5.5 Step 3: capability-aware query -- capability tokens first, then args
      let capabilityIdentity: { capabilityId: string; tool: string } | undefined
      try {
        capabilityIdentity = this.capabilityResolver?.resolveByTool(toolName)
      } catch (_resolveErr) {
        // non-fatal: fall back to tool-only query
      }
      const queryParts: string[] = []
      if (capabilityIdentity) {
        queryParts.push(`capability:${capabilityIdentity.capabilityId}`)
        const _op = this.guessOperation(capabilityIdentity.capabilityId, args)
        if (_op) queryParts.push(`operation:${_op}`)
      }
      ...
      const keywordScored = userFacts
        .map((content) => {
          ...
          let score = 0
          // M5.5 Step 3: capability token match (highest weight)
          if (capabilityIdentity) {
            if (contentLower.includes(`capability:${capabilityIdentity.capabilityId}`)) score += 4
            const _op2 = this.guessOperation(capabilityIdentity.capabilityId, args)
            if (_op2 && contentLower.includes(`operation:${_op2}`)) score += 2
          }
          // tool name match (legacy format + tool: prefix)
          if (contentLower.includes(toolName.toLowerCase())) score += 3
          // arg keyword match (skip structured tokens)
          for (const part of queryParts) {
            if (!part.startsWith('capability:') && !part.startsWith('operation:') && !part.startsWith('tool:')) {
   ...
   */
  postCall(toolName: string, args: Record<string, any>, result: string, success: boolean): void {
    // 更新内部统计
    ...
    try {
      // M5.5: resolver failure is isolated; does not block memory write
      let identity: { capabilityId: string; tool: string } | undefined
      try {
        identity = this.capabilityResolver?.resolveByTool(toolName)
      } catch (resolveErr) {
        log('WARN', 'memory_interceptor_resolve_failed', { tool: toolName, error: String(resolveErr) })
      }
      const summaryText = this.buildSummary(toolName, args, result, success, identity)
      if (summaryText) {
        this.memoryService.addFact(summaryText, success ? 0.6 : 0.3, {
          identity: identity
            ? {
                capability: identity.capabilityId,
                operation: this.guessOperation(identity.capabilityId, args),
                tool: toolName,
              }
            : undefined,
        })
  ...
  /** M5.5: Infer operation type from capabilityId + call args. */
  private guessOperation(capabilityId: string, args: Record<string, any>): string | undefined {
    if (capabilityId === 'file.management') return args.operation || undefined
    if (capabilityId === 'search.retrieval') return 'query'
    if (capabilityId === 'system.execution') return args.command ? 'run' : undefined
    if (capabilityId === 'browser.automation') return 'navigate'
  ...
  private buildSummary(
    ...
    result: string,
    success: boolean,
    identity?: { capabilityId: string; tool: string },
  ): string {
    ...
    if (identity) {
      const operation = this.guessOperation(identity.capabilityId, args)
      const identityTokens = [`capability:${identity.capabilityId}`, operation ? `operation:${operation}` : undefined, `tool:${toolName}`]
        .filter(Boolean)
        .join(' ')

      if (argSummary) {
        return `[能力调用] ${identityTokens} (${argSummary}) → ${status}: ${resultPreview}`
      }
      return `[能力调用] ${identityTokens} → ${status}: ${resultPreview}`
    }
