...
import type { ToolCallIdentity } from '@akemi-mio/capabilities/tool/ToolCallIdentity'
...
/** 会话启动预测条目 */
export interface SessionStartPrediction {
  /** 预测的工具名 */
  toolName: string
  /** 置信度 (0-1) */
  confidence: number
...
/** 会话启动预测结果 */
export interface SessionStartPredictionResult {
  /** 预测列表（按置信度降序） */
  predictions: SessionStartPrediction[]
  /** 预测依据描述 */
  basis: string
}
...
export class BehaviorPredictor {
   ...
   */
  recordCall(toolName: string, args: Record<string, any>, durationMs: number, success: boolean, identity?: ToolCallIdentity): void {
  ...
  predict(toolName: string): PredictionResult[] {
    ...
    if (deduped.length > 0) {
      log('INFO', 'behavior_predictor_prediction', {
        ...
        top: deduped
          .slice(0, 3)
          .map((p) => `${p.toolName}(${(p.confidence * 100).toFixed(0)}%)`)
          .join(', '),
  ...
  private computeConfidence(pattern: PatternRecord): number {
    ...
    const normalized = Math.min(
      confidence / (this.config.CONFIDENCE_FREQ_SCALE + this.config.CONFIDENCE_RECENCY_SCALE + this.config.CONFIDENCE_LENGTH_SCALE),
      1,
    )
  ...
  private evictPreloadCache(): void {
    ...
    if (this.preloadCache.size > this.config.PRELOAD_CACHE_MAX) {
      const entries = [...this.preloadCache.entries()].sort((a, b) => a[1].createdAt - b[1].createdAt)
  ...
  private async executePreloadWithTimeout(
    ...
    const timeoutPromise = new Promise<null>((_, reject) => {
      setTimeout(() => reject(new Error(`Preload timeout after ${this.config.PRELOAD_TIMEOUT_MS}ms`)), this.config.PRELOAD_TIMEOUT_MS)
    })

    const result = await Promise.race([executor(toolName, args), timeoutPromise])
    return result
  }
  ...
  /**
   * 预测会话启动时最可能用到的工具。
   * 基于历史高频模式的目标工具聚合，按频率加权置信度降序返回。
   */
  predictSessionStart(): SessionStartPredictionResult {
    const freqMap = new Map<string, { count: number; totalConfidence: number }>()
    for (const pattern of this.patterns.values()) {
      const existing = freqMap.get(pattern.targetTool) ?? { count: 0, totalConfidence: 0 }
      existing.count += pattern.frequency
      existing.totalConfidence += this.computeConfidence(pattern)
      freqMap.set(pattern.targetTool, existing)
    }
    const predictions: SessionStartPrediction[] = [...freqMap.entries()]
      .map(([toolName, v]) => ({
        toolName,
        confidence: Math.min(1, (v.totalConfidence / Math.max(1, v.count)) * Math.min(1, v.count / this.config.MIN_PATTERN_FREQUENCY)),
      }))
      .sort((a, b) => b.confidence - a.confidence)
      .slice(0, 10)
    return {
      predictions,
      basis: predictions.length > 0 ? 'historical-frequency' : 'no-history',
    }
  }
