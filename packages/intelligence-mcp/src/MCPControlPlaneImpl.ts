...
import { type MCPControlPlane, type MCPHealthStatus, type MCPServerState } from './MCPControlPlane'
...
function pingWithTimeout(client: McpClient, timeoutMs: number): Promise<boolean> {
  return Promise.race([client.ping(), new Promise<boolean>((_, reject) => setTimeout(() => reject(new Error('ping timeout')), timeoutMs))])
}
