...
describe('TaskRunner — getTaskHealthSummary', () => {
  ...
  it('单个已注册任务 idle', () => {
    runner.register('creativity.cycle' as any, async () => ({ success: true }), 60000)
  ...
  it('critical 任务 tier 反映正确', () => {
    runner.register('stability.tick' as any, async () => ({ success: true }), 60000, { tier: 'critical' })
    expect(runner.getTaskHealthSummary()[0].tier).toBe('critical')
  })

  it('important 任务 tier 反映正确', () => {
    runner.register('memory.index' as any, async () => ({ success: true }), 60000, { tier: 'important' })
    expect(runner.getTaskHealthSummary()[0].tier).toBe('important')
  })

  it('best_effort 多次失败 → stopType 移除定时器', async () => {
    runner.register('creativity.cycle' as any, async () => ({ success: false, summary: 'fail' }), 0, {
      tier: 'best_effort',
      maxFailures: 2,
    })

    await runner.triggerNow('creativity.cycle' as any)
    await runner.triggerNow('creativity.cycle' as any)

    const summary = runner.getTaskHealthSummary()
    expect(summary[0].consecutiveFailures).toBeGreaterThanOrEqual(1)
    expect((runner as any).timers.has('creativity.cycle')).toBe(false)
  })

  it('critical 多次失败 进 cooldown', async () => {
    runner.register('stability.tick' as any, async () => ({ success: false, summary: 'fail' }), 0, { tier: 'critical', maxFailures: 3 })

    await runner.triggerNow('stability.tick' as any)
    await runner.triggerNow('stability.tick' as any)
    await runner.triggerNow('stability.tick' as any)
  ...
  it('多个任务混合', async () => {
    runner.register('evolution.analysis' as any, async () => ({ success: true }), 0, { tier: 'important' })
    runner.register('creativity.dream' as any, async () => ({ success: false, summary: 'fail' }), 0, {
      tier: 'best_effort',
      maxFailures: 2,
    })

    await runner.triggerNow('creativity.dream' as any)
    await runner.triggerNow('creativity.dream' as any)
