...
import type { ProgressAnalyzer, ProgressConsumer, ProgressSnapshot } from '@akemi-mio/core/core/evaluation/progress'
...
function makeAnalyzer(store: EvaluationRepository): ProgressAnalyzer & { computeMock: ReturnType<typeof vi.fn> } {
  const computeMock = vi.fn((traceId: string, events: EvaluationEvent[]): ProgressSnapshot => ({
    traceId,
