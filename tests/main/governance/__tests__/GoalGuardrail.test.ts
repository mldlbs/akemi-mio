...
describe('RejectionTracker', () => {
  ...
  it('should respect custom threshold and window', () => {
    ...
    for (let i = 0; i < 4; i++) custom.record('GOAL_DRIFT', 'list_files')
...
describe('GoalGuardrail', () => {
  ...
  let mockGoalEngine: GoalEngine
  let mockConstitutionEngine: ConstitutionEngine
  ...
  beforeEach(() => {
    ...
    } as unknown as ConstitutionEngine
  ...
  it('should approve when no goals exist', async () => {
    const calls: ToolCallInfo[] = [{ id: 'tc-1', name: 'read_file', arguments: { path: '/f.ts' } }]
  ...
  it('should block write tools when ConstitutionEngine rejects', async () => {
    ...
    const call: ToolCallInfo = { id: 'tc-2', name: 'write_file', arguments: { path: '/core/K.ts' } }
  ...
  it('should allow read tools regardless of ConstitutionEngine', async () => {
    ...
    const call: ToolCallInfo = { id: 'tc-3', name: 'read_file', arguments: { path: '/core/K.ts' } }
    const result = await guardrail.checkBatch([call], mockMessages, mockCtx)
    expect(result.status).toBe('approved')
  })
  ...
  it('should NOT inject messages on hard rejection', async () => {
    ...
    const call: ToolCallInfo = { id: 'tc-4', name: 'write_file', arguments: { path: '/core/K.ts' } }
    const result = await guardrail.checkBatch([call], mockMessages, mockCtx)
    expect(result.status).toBe('denied')
  ...
  it('should inject alignment message on soft rejection', async () => {
    ...
    const calls: ToolCallInfo[] = [{ id: 'tc-9', name: 'list_files', arguments: {} }]
    const result = await guardrail.checkBatch(calls, mockMessages, mockCtx)
    expect(result.status).toBe('denied')
  ...
  it('should apply tighter scoring on consecutive drift', async () => {
    vi.mocked(mockGoalEngine.getActiveGoals).mockReturnValue([createGoal({ category: 'long_term' })])
    const calls: ToolCallInfo[] = [{ id: 'tc-11', name: 'list_files', arguments: {} }]
  ...
  it('should restore normal scoring after tool success', async () => {
    vi.mocked(mockGoalEngine.getActiveGoals).mockReturnValue([createGoal({ category: 'long_term' })])
    const calls: ToolCallInfo[] = [{ id: 'tc-13', name: 'list_files', arguments: {} }]
  ...
  it('should trip when rejection threshold exceeded', async () => {
    ...
    const wc: ToolCallInfo = { id: 'tc-14', name: 'write_file', arguments: { path: '/core/K.ts' } }

    await g.checkBatch([wc], [], mockCtx)
    await g.checkBatch([wc], [], mockCtx)

    const rc: ToolCallInfo = { id: 'tc-15', name: 'read_file', arguments: {} }
  ...
  it('should reset rejection tracking', async () => {
    ...
    const wc: ToolCallInfo = { id: 'tc-16', name: 'write_file', arguments: { path: '/core/K.ts' } }
    await g.checkBatch([wc], [], mockCtx)

    g.resetRejectionTracking()

    const rc: ToolCallInfo = { id: 'tc-17', name: 'read_file', arguments: {} }
    const result = await g.checkBatch([rc], [], mockCtx)
    expect(result.status).toBe('approved')
  })
  ...
  it('should extract path from arguments', async () => {
    ...
    const call: ToolCallInfo = { id: 'tc-18', name: 'write_file', arguments: { path: '/core/K.ts' } }
    const result = await guardrail.checkBatch([call], mockMessages, mockCtx)
    expect(result.status).toBe('denied')
  })
  ...
  it('should extract file_path as fallback', async () => {
    ...
    const call: ToolCallInfo = { id: 'tc-19', name: 'edit_file', arguments: { file_path: '/core/E.ts' } }
    const result = await guardrail.checkBatch([call], mockMessages, mockCtx)
    expect(result.status).toBe('denied')
  })

  it('should not crash on null arguments', async () => {
    const calls: ToolCallInfo[] = [{ id: 'tc-20', name: 'write_file', arguments: null as unknown as Record<string, unknown> }]
    const result = await guardrail.checkBatch(calls, mockMessages, mockCtx)
    expect(result.status).toBe('approved')
  })
  ...
  it('should integrate credit recovery with rejection count', async () => {
    ...
    const wc: ToolCallInfo = { id: 'tc-21', name: 'write_file', arguments: { path: '/core/K.ts' } }
    await g.checkBatch([wc], [], mockCtx)
    await g.checkBatch([wc], [], mockCtx)
