...
import { ToolSchemaProvider } from '@akemi-mio/capabilities/tool/ToolSchemaProvider'
...
describe('P1.3a Capability Routing (Synthetic)', () => {
  ...
  it('exposes browser.automation canonical schema with required action', () => {
    const schemas = adapter.buildSchemas()
    const browser = schemas.find((s) => s.function.name === 'browser_automation')

    expect(browser).toBeDefined()
    expect(browser!.function.parameters.required).toContain('action')
    expect(browser!.function.parameters.properties.action).toBeDefined()
    expect(browser!.function.parameters.properties.url).toBeDefined()
  })
  ...
  it('resolves publishing to fanqie-publish provider', async () => {
    ...
    expect(binding!.tool).toBe('fanqie_publish_novel') // tool from manifest.dependencies
  })
