...
import { ToolInvocationRouter } from '@akemi-mio/capabilities/tool/ToolInvocationRouter'
import { ToolSchemaProvider, SchemaExposureMode } from '@akemi-mio/capabilities/tool/ToolSchemaProvider'
import { CapabilityFunctionSchemaAdapter } from '@akemi-mio/capabilities/capability/CapabilityFunctionSchemaAdapter'
...
import { fanqiePublishAdapter } from '@akemi-mio/capabilities/capability/adapters/fanqie-adapter'
import { playwrightAdapter } from '@akemi-mio/capabilities/capability/adapters/playwright-adapter'
import { fileSystemAdapter } from '@akemi-mio/capabilities/capability/adapters/file-system-adapter'
import { searchAdapter } from '@akemi-mio/capabilities/capability/adapters/search-adapter'
import { systemAdapter } from '@akemi-mio/capabilities/capability/adapters/system-adapter'
  ...
  capabilitySchemas: {
    'browser.automation': {
      ...
      properties: {
        ...
        selector: { type: 'string', description: 'Target selector for browser interactions' },
  ...
  capabilitySchemas: {
    'search.retrieval': {
      ...
      properties: {
        ...
        scope: { type: 'string', description: 'Optional include pattern or workspace hint' },
      },
...
describe('P1.3b Exit Gate — Provider Adapters', () => {
  ...
  it('[fanqie] maps canonical title → novelName', () => {
    const result = fanqiePublishAdapter({ title: '我的小说', content: '正文' }, 'fanqie_publish_novel') as Record<string, unknown>
  ...
  it('[fanqie] falls back to raw keys when canonical keys are absent', () => {
    const result = fanqiePublishAdapter({ novelName: '已有名', content: '已有正文', category: '玄幻' }, 'fanqie_publish_novel') as Record<
      string,
      unknown
    >
  ...
  it('[fanqie] maps platform → category', () => {
    const result = fanqiePublishAdapter({ title: 'T', content: 'C', platform: 'fantasy' }, 'fanqie_publish_novel') as Record<
      string,
      unknown
    >
  ...
  it('[fanqie] empty title falls back to empty string', () => {
    const result = fanqiePublishAdapter({ content: '正文' }, 'fanqie_publish_novel') as Record<string, unknown>
  ...
  it('[playwright] infers navigate action when only url is provided', () => {
    const result = playwrightAdapter({ url: 'https://example.com' }, 'browser_navigate') as Record<string, unknown>
    expect(result.action).toBe('navigate')
    expect(result.url).toBe('https://example.com')
  })

  it('[playwright] throws if action is missing', () => {
    expect(() => playwrightAdapter({ selector: '#submit' }, 'browser_navigate')).toThrow('action')
  })
  ...
  it('[search] projects canonical query to grep params', () => {
    const result = searchAdapter({ query: 'TODO', scope: '*.ts' }, 'grep') as Record<string, unknown>
    expect(result.pattern).toBe('TODO')
    expect(result.include).toBe('*.ts')
  })

  it('[search] projects canonical query to file search params', () => {
    const result = searchAdapter({ query: '*.test.ts', scope: 'project' }, 'search_files') as Record<string, unknown>
  ...
  it('[M5.4 smoke] file.management read → adapter → read_file params', () => {
    const result = fileSystemAdapter({ operation: 'read', path: '/tmp/a.txt' }, 'read_file') as Record<string, unknown>
    expect(result.path).toBe('/tmp/a.txt')
    expect(result.content).toBeUndefined()
  })

  it('[M5.4 smoke] search.retrieval grep → adapter → grep params', () => {
    const result = searchAdapter({ operation: 'grep', query: 'FIXME', scope: '*.ts' }, 'grep') as Record<string, unknown>
    expect(result.pattern).toBe('FIXME')
    expect(result.include).toBe('*.ts')
  })

  it('[M5.4 smoke] search.retrieval files → adapter → search_files params', () => {
    const result = searchAdapter({ operation: 'files', query: '*.md', scope: 'project' }, 'search_files') as Record<string, unknown>
    expect(result.pattern).toBe('*.md')
    expect(result.workspace).toBe('project')
  })

  it('[M5.4 smoke] system.execution command → adapter → run_command params', () => {
    const result = systemAdapter({ command: 'pwd', cwd: '/tmp' }, 'run_command') as Record<string, unknown>
  ...
  it('CapabilityService invoke passes input through when no adapter registered', async () => {
    ...
    expect(serverManager.callTool).toHaveBeenCalledWith(binding!.tool, { title: 'test', content: 'hello' })
  })
  ...
  it('CapabilityService invoke applies adapter when registered', async () => {
    ...
    expect(serverManager.callTool).toHaveBeenCalledWith(resolvedTool, { novelName: '我的小说', content: '正文', category: 'fantasy' })
  })
})
...
describe('P1.3b Exit Gate — Capability-First Dispatch', () => {
  ...
  it('[M5.4] search.retrieval resolves to a real registered tool name', async () => {
    const binding = await capabilityService.resolve('search.retrieval')
    expect(binding).toBeDefined()
    expect(binding!.tool).toBe('grep')
  })
...
describe('P1.3b Exit Gate — Adapter + Invoke Integration', () => {
  ...
  it('[Exit Gate 3] fanqie adapter transforms input in full dispatch chain', async () => {
    ...
    expect(serverManager.callTool).toHaveBeenCalledWith('fanqie_publish_novel', {
      novelName: '我的小说',
      content: '正文',
      category: 'fantasy',
    })
  ...
  it('[Exit Gate 3] playwright adapter validates action in full chain', async () => {
    ...
    expect(serverManager.callTool).toHaveBeenCalledWith('browser_navigate', { action: 'navigate', url: 'https://example.com' })
  })

  it('[Exit Gate 3] playwright adapter infers navigate in full chain when only url is provided', async () => {
    await router.dispatch('browser_automation', { url: 'https://example.com' })

    expect(serverManager.callTool).toHaveBeenCalledWith('browser_navigate', { action: 'navigate', url: 'https://example.com' })
  })

  it('[Exit Gate 3] playwright adapter still rejects non-inferable input', async () => {
    const result = await router.dispatch('browser_automation', { selector: '#submit' })
