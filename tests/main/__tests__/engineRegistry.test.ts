...
import { EngineRegistry, Engine } from '@akemi-mio/core/core/EngineRegistry'

describe('EngineRegistry', () => {
  let mockBus: { emit: ReturnType<typeof vi.fn> }
  let registry: EngineRegistry<Engine>

  const ttsEngine1: Engine = { name: 'edge-tts', type: 'tts', priority: 10, available: true }
  const ttsEngine2: Engine = { name: 'elevenlabs', type: 'tts', priority: 5, available: true }
  const ttsEngine3: Engine = { name: 'offline', type: 'tts', priority: 1, available: false }
  const asrEngine: Engine = { name: 'whisper', type: 'asr', priority: 8, available: true }
  ...
  describe('EventBus 事件广播完整性', () => {
    it('注册首个引擎应依次广播 registered 和 activated', () => {
      registry.register(ttsEngine1)
      expect(mockBus.emit.mock.calls.map((c) => c[0])).toEqual(['engine.activated', 'engine.registered'])
    })
  ...
  describe('无 EventBus 构造', () => {
    it('不传 eventBus 时应正常工作不抛出', () => {
      const r = new EngineRegistry<Engine>()
