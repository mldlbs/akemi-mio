...
import { UserBehaviorAnalyzer, userBehaviorAnalyzer } from '@akemi-mio/intelligence/agent/UserBehaviorAnalyzer'
...
describe('UserBehaviorAnalyzer', () => {
  ...
  describe('tool frequency analysis', () => {
    ...
    it('高频工具应该按调用次数降序排列', () => {
      ...
      for (let i = 0; i < 3; i++) analyzer.recordToolCall('grep')
      analyzer.recordToolCall('write_file')
      analyzer.recordToolCall('edit_file')
      analyzer.recordToolCall('list_files')
    ...
    it('应该只分析窗口内的数据', () => {
      // 先填满旧数据
      analyzer = new UserBehaviorAnalyzer(2)
  ...
  describe('topic pattern extraction', () => {
    ...
    it('应该从英文消息中提取话题', () => {
      ...
      const result = analyzer.analyze({ topicMinOccurrences: 1 })
