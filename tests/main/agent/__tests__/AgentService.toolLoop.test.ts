...
vi.mock('@akemi-mio/core/db/connection', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@akemi-mio/core/db/connection')>()
  return actual
})
...
function createMockPlanManager() {
  let activePlan: DevPlan | null = null
  ...
  return {
    ...
    _setPlan: (p: DevPlan | null) => {
      activePlan = p
    },
  }
}

function makePlanWithPending(title: string): DevPlan {
  return {
    id: `plan_${title}`,
    title,
    description: `测试计划: ${title}`,
    steps: [
      { id: 'step_0', description: '步骤1', status: 'done' as const, result: 'ok' },
      { id: 'step_1', description: '步骤2', status: 'pending' as const },
      { id: 'step_2', description: '步骤3', status: 'pending' as const },
    ],
...
describe('AgentService toolLoop 状态机', () => {
  ...
  let mockPlan: ReturnType<typeof createMockPlanManager>
  ...
  beforeEach(async () => {
    ...
    // 注入 mock planManager
    agent = new AgentService(llmService, asrService, ttsService, undefined as any, undefined as any, mockPlan as any)
  })
  ...
  describe('计划强制续行', () => {
    it('追踪的活跃计划有未完成步骤 → 应触发 force-continue（注入后循环重试）', async () => {
      ...
      const eventBus = (agent as any).eventBus
