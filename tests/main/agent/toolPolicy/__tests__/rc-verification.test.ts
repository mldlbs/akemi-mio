...
import type { SceneLabel } from '@akemi-mio/intelligence/agent/UserBehaviorAnalyzer'
...
describe('ADR-008 RC Validation — 集成验证报告', () => {
  ...
  afterAll(() => {
    ...
    for (const r of report) {
      const status = r.pass ? '✓' : '✗'
      console.log(
        `  ${status} ${String(r.id).padEnd(3)} ${r.desc.padEnd(26)} ${r.scene.padEnd(14)} ${r.toolPreference.padEnd(10)} ${r.toolReason.padEnd(19)} ${r.filter.padEnd(10)} ${r.promptInjected ? 'YES' : 'NO '}  ${BASELINE[r.id].preference}/${BASELINE[r.id].reason}`,
      )
    }
    console.log()
