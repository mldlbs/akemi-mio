...
import { ConceptMixer } from '@akemi-mio/creativity/ConceptMixer'
import type { CreativitySource, Strategy } from '@akemi-mio/creativity/types'
...
describe('ConceptMixer', () => {
  describe('mix — default explore strategy', () => {
    ...
    it('组合描述包含来源内容要点而非仅标题', () => {
      const mixer = new ConceptMixer(42)
      const sources = [
        makeSource({ name: 'WebApp', content: '这是一个网站正文内容，用于理解真实能力与协同机制描述', type: 'knowledge', weight: 0.8 }),
        makeSource({ name: 'Memory', content: '记忆系统能力描述正文内容', type: 'behavior', weight: 0.6 }),
      ]
      const results = mixer.mix(sources)
      expect(results[0].combo.description).toContain('网站正文内容')
      expect(results[0].combo.description).toContain('记忆系统能力')
    })
