...
import { jaccardSimilarity, evaluateNovelty } from '@akemi-mio/creativity/NoveltyScorer'
...
describe('evaluateNovelty', () => {
  it('不相似时原样保留 novelty', () => {
    const result = evaluateNovelty(
      { title: '全新方向', idea: '用图神经网络分析代码依赖', novelty: 85 },
      [{ title: '语音增强', idea: '用卷积网络处理音频信号', novelty: 70 }],
      [],
    )
  ...
  it('与近期假设高度相似时扣 15 分', () => {
    const result = evaluateNovelty(
      { title: '新方案', idea: '用强化学习优化对话策略', novelty: 80 },
      [{ title: '旧方案', idea: '用强化学习优化对话管理策略', novelty: 75 }],
      [],
    )
    expect(result.shouldReject).toBe(false)
  ...
  it('扣分不低于 0', () => {
    const result = evaluateNovelty(
      { title: '相同内容', idea: '重复的假设文本', novelty: 10 },
      [{ title: '旧内容', idea: '重复的假设文本内容', novelty: 70 }],
      [],
    )
  ...
  it('已拒绝优先于降分', () => {
    ...
    const result = evaluateNovelty(
      { title: '雷同方案', idea: similarIdea, novelty: 75 },
      [{ title: '近期方案', idea: '文件系统缓存策略分析', novelty: 70 }],
      [{ title: '已拒方案', idea: similarIdea }],
    )
    expect(result.shouldReject).toBe(true)
  })
