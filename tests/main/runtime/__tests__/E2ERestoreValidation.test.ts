...
import { WorkflowRuntimeCheckpointableComponent } from '@akemi-mio/intelligence/runtime/WorkflowRuntimeCheckpointableComponent'
import { ComponentRegistryImpl } from '@akemi-mio/intelligence/runtime/ComponentRegistry'
import { RuntimeRestoreServiceImpl } from '@akemi-mio/intelligence/runtime/RuntimeRestoreService'
...
import type { WorkflowSchedulerV2 } from '@akemi-mio/capabilities/workflow/WorkflowScheduler'
import type { Checkpoint, VersionedState, CheckpointableComponent } from '@akemi-mio/intelligence/runtime/CheckpointTypes'
...
describe('E2E — Scene 1: assembly verification', () => {
  ...
  it('should accept workflow-runtime descriptor in registry', () => {
    ...
    registry.register({
      ...
      version: '1.0',
      create: () =>
        new WorkflowRuntimeCheckpointableComponent(
  ...
  it('should reject duplicate registration', () => {
    const registry = new ComponentRegistryImpl()
    registry.register({ id: 'workflow-runtime', version: '1.0', create: () => ({}) as CheckpointableComponent })
    expect(() => registry.register({ id: 'workflow-runtime', version: '1.0', create: () => ({}) as CheckpointableComponent })).toThrow(
      /duplicate/i,
    )
  })
})
...
describe('E2E — Scene 2: checkpoint round trip', () => {
  ...
  it('should snapshot → save → load → validate → restore → plans', async () => {
    ...
    const runPaused = createMockRun({
      runId: 'run_p1',
      status: 'paused',
      pendingGate: { stepId: 's2', message: '?', preview: '', options: [] },
    })
...
describe('E2E — Scene 3: workflow recovery decision matrix', () => {
  ...
  function setupE2E(runs: any[]) {
    ...
    registry.register({
      ...
      version: '1.0',
      create: () =>
        new WorkflowRuntimeCheckpointableComponent({ getActiveRunIds: vi.fn(() => []) } as unknown as WorkflowSchedulerV2, store as any),
    })
  ...
  it('should resume running workflows', async () => {
    const { restoreService, scheduler, sourceComp, mgr } = setupE2E([createMockRun({ runId: 'run_active', status: 'running' })])
  ...
  it('should skip completed workflows', async () => {
    const { restoreService, scheduler, sourceComp, mgr } = setupE2E([createMockRun({ runId: 'run_done', status: 'done' })])
  ...
  it('should register-only running workflows with pending gate', async () => {
    const { restoreService, scheduler, sourceComp, mgr } = setupE2E([
      createMockRun({
        runId: 'run_gate',
        status: 'running',
        pendingGate: { stepId: 's2', message: 'Approve?', preview: '...', options: [] },
      }),
    ])
