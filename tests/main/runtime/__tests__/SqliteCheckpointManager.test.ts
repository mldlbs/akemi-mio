...
import type { Checkpoint } from '@akemi-mio/intelligence/runtime/CheckpointTypes'
import { SqliteCheckpointManager } from '@akemi-mio/intelligence/runtime/SqliteCheckpointManager'
...
describe('SqliteCheckpointManager — Contract', () => {
  ...
  describe('Scene 4: identity separation', () => {
    ...
    it('should allow multiple checkpoints for one task', async () => {
      ...
      const cp1 = await mgr.create({
        taskId,
        taskName: 'm1',
        taskCreatedAt: Date.now(),
        executionState: { workerId: 'w1', goal: 'g', step: 1, lastSafePoint: 'before_llm' },
      })
      const cp2 = await mgr.create({
        taskId,
        taskName: 'm1',
        taskCreatedAt: Date.now(),
        executionState: { workerId: 'w1', goal: 'g', step: 5, lastSafePoint: 'after_tool' },
      })
      await mgr.save(cp1)
      await mgr.save(cp2)
