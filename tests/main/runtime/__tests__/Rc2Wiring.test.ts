...
describe('RC-2 SubAgentPoolAdapter 接线验证', () => {
  it('spawn() → createTask() → taskCount+1 → Worker 注册', { timeout: 30000 }, async () => {
    ...
    const adapter = new (await import('@akemi-mio/intelligence/agent/SubAgentPoolAdapter')).SubAgentPoolAdapter(mcp, 'mock_key', 'mock_key', runtimeManager)
    ...
    await vi.waitFor(() => expect(adapter.collectCompleted().length).toBe(2), { timeout: 15000, interval: 100 })
  })
  ...
  it.skip('getStatus().completed 应在 worker 完成后同步', { timeout: 30000 }, async () => {
    ...
    await vi.waitFor(() => expect(runtimeManager.listTasks()[0].getStatus().completed).toBe(2), { timeout: 15000, interval: 100 })
  })

  it('spawnTask() 同样经过同一链路', async () => {
    const mcp = new ServerManager() as any
    const runtimeManager = new RuntimeManagerImpl(() => new SupervisedAgentSupervisorImpl(mcp, 'mock_key', 'mock_key', mockLlm as any))
    const adapter = new (await import('@akemi-mio/intelligence/agent/SubAgentPoolAdapter')).SubAgentPoolAdapter(mcp, 'mock_key', 'mock_key', runtimeManager)
  ...
  it('两个 Adapter 实例不互相干扰', async () => {
    ...
    const a1 = new (await import('@akemi-mio/intelligence/agent/SubAgentPoolAdapter')).SubAgentPoolAdapter(mcp, 'k', 'k', rm1)
    const a2 = new (await import('@akemi-mio/intelligence/agent/SubAgentPoolAdapter')).SubAgentPoolAdapter(mcp, 'k', 'k', rm2)
