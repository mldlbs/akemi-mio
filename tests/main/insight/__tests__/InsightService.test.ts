...
function createMockStore(): InsightStoreLike & { _insights: Insight[]; _reportedIds: string[] } {
  ...
  return {
    ...
    markReported: vi.fn((id: string) => {
      if (!reportedIds.includes(id)) reportedIds.push(id)
    }),
    markAllReported: vi.fn(() => {
      insights.forEach((i) => {
        if (!reportedIds.includes(i.id)) reportedIds.push(i.id)
      })
    }),
    ...
    it('高价值洞察触发 insight.found 事件', async () => {
      ...
      chatJson.mockResolvedValue({
        data: [
          {
            title: '高价值洞察标题',
            description: '这是一条高价值洞察的描述文本',
            detector: 'conflict',
            severity: 'high',
            evidence: [longEvidence, longEvidence, longEvidence, longEvidence, longEvidence, longEvidence, longEvidence, longEvidence],
            novelty: 90,
            impact: 90,
            actionability: 80,
          },
