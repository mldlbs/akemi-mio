...
import type { ReplayResult, ReplayFailure, CommitContext, ReasoningDirective } from '@akemi-mio/reasoning/golden/types'
...
describe('RegressionReport Contract', () => {
  ...
  describe('I-6: Status decision matrix', () => {
    ...
    it('inconclusive when passed+failed=0', () =>
      expect(gen.generate(mockResult({ total: 0, passed: 0, failed: 0, skipped: 0 })).summary.status).toBe('inconclusive'))
  })
  ...
  describe('I-8: Capability maps failures', () => {
    it('affectedCaseIds is subset of failure caseIds', () => {
      ...
      for (const cap of r.capability.regressed) for (const id of cap.affectedCaseIds) expect(allIds.has(id)).toBe(true)
    })
