...
describe('ProtectedPaths', () => {
  ...
  const IMMUTABLE_RULES = [
    {
      pattern: '**/packages/core/src/core/**',
      mutable: false,
      reason: 'Runtime Kernel',
      layer: 'kernel' as const,
    },
    {
      pattern: '**/packages/evolution/src/constitution/**',
      mutable: false,
      reason: 'Constitution',
      layer: 'kernel' as const,
    },
  ...
  it('should block writes to core/ directory in enforce mode', () => {
    ...
    const result = pp.isWriteAllowed('/project/packages/core/src/core/EventBus.ts')
  ...
  it('should block writes to constitution/ directory', () => {
    ...
    const result = pp.isWriteAllowed('/project/packages/evolution/src/constitution/types.ts')
    expect(result).toBe(false)
  })
  ...
  it('should allow writes to memory/ directory (mutable)', () => {
    ...
    const result = pp.isWriteAllowed('/project/packages/intelligence/src/memory/MemoryService.ts')
  ...
  it('should allow writes to evolution/ directory', () => {
    ...
    const result = pp.isWriteAllowed('/project/packages/evolution/src/SelfEvolutionService.ts')
    expect(result).toBe(true)
  })
  ...
  it('should handle Windows path separators', () => {
    ...
    const result = pp.isWriteAllowed('D:\\project\\packages\\core\\src\\core\\EventBus.ts')
    expect(result).toBe(false)
  })
  ...
  it('should return null for unprotected paths', () => {
    ...
    const match = pp.isProtected('/project/packages/intelligence/src/memory/MemoryService.ts')
  ...
  it('should return the matching ProtectedPath for protected paths', () => {
    ...
    const match = pp.isProtected('/project/packages/core/src/core/Scheduler.ts')
    expect(match).not.toBeNull()
    expect(match!.pattern).toBe('**/packages/core/src/core/**')
...
describe('ConstitutionEngine', () => {
  ...
  it('should allow writes in warn mode even when path is protected', async () => {
    ...
    const check = engine.checkWrite('/project/packages/core/src/core/EventBus.ts')
  ...
  it('should allow writes in enforce mode for mutable paths', async () => {
    ...
    const check = engine.checkWrite('/project/packages/intelligence/src/memory/MemoryService.ts')
    expect(check.allowed).toBe(true)
  ...
  it('should fire violation event on protected write', async () => {
    ...
    engine.checkWrite('/project/packages/core/src/core/Scheduler.ts')
...
const TEST_KERNEL_PREFIXES = [
  '/project/packages/core/src/core/',
  '/project/packages/evolution/src/constitution/',
  '/project/src/main/bootstrap/',
  'D:/project/packages/core/src/core/',
  'D:/project/packages/evolution/src/constitution/',
...
describe('isKernelPath', () => {
  ...
  it('should detect core/ paths', () => {
    expect(isKernelPath('/project/packages/core/src/core/EventBus.ts', TEST_KERNEL_PREFIXES)).toBe(true)
  })

  it('should detect constitution/ paths', () => {
    expect(isKernelPath('/project/packages/evolution/src/constitution/types.ts', TEST_KERNEL_PREFIXES)).toBe(true)
  })

  it('should reject non-kernel paths', () => {
    expect(isKernelPath('/project/packages/intelligence/src/memory/MemoryService.ts', TEST_KERNEL_PREFIXES)).toBe(false)
  })

  it('should handle Windows paths', () => {
    expect(isKernelPath('D:\\project\\packages\\core\\src\\core\\Scheduler.ts', TEST_KERNEL_PREFIXES)).toBe(true)
  })
