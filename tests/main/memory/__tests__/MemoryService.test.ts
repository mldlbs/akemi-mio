...
describe('MemoryService', () => {
  ...
  it('addFact 无 identity 时 structuredData 为 null', () => {
    ms.addFact('普通事实', 0.7)
    expect(ms.getEntries()[0].structuredData).toBeNull()
  })

  it('addFact identity 写入 structuredData', () => {
    ms.addFact('写入配置文件', 0.7, {
      identity: { capability: 'file.management', operation: 'write', tool: 'write_file' },
    })
    const entry = ms.getEntries()[0]
    const parsed = JSON.parse(entry.structuredData!)
    expect(parsed.identity.capability).toBe('file.management')
    expect(parsed.identity.operation).toBe('write')
    expect(parsed.identity.tool).toBe('write_file')
  })

  it('addFact identity 与 tier 并存', () => {
    ms.addFact('永久写入', 0.9, {
      tier: 'permanent',
      identity: { capability: 'file.management', operation: 'write' },
    })
    const entry = ms.getEntries()[0]
    expect(entry.tier).toBe('permanent')
    expect(JSON.parse(entry.structuredData!).identity.capability).toBe('file.management')
  })

  it('addFact 旧调用不变（无 options）', () => {
    ms.addFact('旧式调用')
    const entry = ms.getEntries()[0]
    expect(entry.confidence).toBe(0.6)
    expect(entry.structuredData).toBeNull()
  })
  ...
  it('getFormattedContext 空库返回虚构暖启动记忆，10 条真实记忆后消失', () => {
    expect(ms.getFormattedContext()).toContain('模糊印象')
    for (let i = 0; i < 10; i++) ms.addEntry('user_fact', `真实记忆 ${i}`, 0.6)
    expect(ms.getFormattedContext()).not.toContain('模糊印象')
  })
