...
describe('RuntimeHealthManager', () => {
  let manager: RuntimeHealthManager
  ...
  it('init 状态转换', async () => {
    expect((manager as any).state).toBe('created')
    await manager.init()
    expect((manager as any).state).toBe('ready')
  })
  ...
  it('start 启动 modelHealth 和 tick', async () => {
    await manager.init()
    await manager.start()
    expect((manager as any).state).toBe('running')
  })
  ...
  it('stop 停止所有', async () => {
    ...
    await manager.start()
    await manager.stop()
    expect((manager as any).state).toBe('stopped')
  })
  ...
  it('composite = weighted 4 dimensions', () => {
    const session = { getScore: () => 50, getLevel: () => 'RISKY' as const, getConsecutiveFailures: () => 5 }
  ...
  it('composite 全低 → < 30', () => {
    const session = { getScore: () => 0, getLevel: () => 'CORRUPTED' as const, getConsecutiveFailures: () => 100 }
    const cap = {
    ...
    for (let i = 0; i < 10; i++) eventBus.emit('agent.tool.failed' as any, { tool: 't', error: 'err' })
  ...
  it('composite clamped [0,100]', () => {
    const session = { getScore: () => 200 as any, getLevel: () => 'HEALTHY' as const, getConsecutiveFailures: () => 0 }
    const cap = { getCapabilitySummary: () => ({ totalCapabilityHealth: 200 as any, servers: [] }) }
    const task = { getTaskHealthSummary: () => [] }
    manager.setSessionHealthProvider(session)
    manager.setCapabilityHealthProvider(cap)
    manager.setTaskHealthProvider(task)
  ...
  it('healthCheck composite < 50 → unhealthy', async () => {
    const session = { getScore: () => 10, getLevel: () => 'CRITICAL' as const, getConsecutiveFailures: () => 50 }
    const cap = {
  ...
  it('session low → 触发恢复流程', () => {
    const session = { getScore: () => 30, getLevel: () => 'CRITICAL' as const, getConsecutiveFailures: () => 20 }
    manager.setSessionHealthProvider(session)
  ...
  it('evaluateAll fires runtime.health.updated event', async () => {
    ...
    await manager.start()
    const events: any[] = []
    const unsub = eventBus.on('runtime.health.updated' as any, (p: any) => events.push(p))
    ;(manager as any).evaluateAll()
  ...
  it('trend stable with < 10 history', () => {
    expect((manager as any).computeTrend()).toBe('stable')
  })

  it('trend improving', () => {
    const h = (manager as any).history
    for (let i = 0; i < 20; i++) h.push({ composite: 40 + i, timestamp: i * 1000 })
    expect((manager as any).computeTrend()).toBe('improving')
  })

  it('trend declining', () => {
    const h = (manager as any).history
    for (let i = 0; i < 20; i++) h.push({ composite: 60 - i, timestamp: i * 1000 })
    expect((manager as any).computeTrend()).toBe('declining')
  })
