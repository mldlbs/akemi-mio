...
describe('RuntimeHealthManager 压力测试', () => {
  it('1000 次 evaluateAll 后 compositeScore 始终在 [0,100]', () => {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const session = { getScore: () => s, getLevel: () => 'NORMAL' as const, getConsecutiveFailures: () => (s < 50 ? 10 : 0) }
      const cap = {
  ...
  it('10000 ModelHealthTracker 事件模拟收敛正确', () => {
    ...
    for (let i = 0; i < 10000; i++) {
      const mod = i % 10
      if (mod < 7) {
        ;(manager.modelHealth as any).successCount++
      } else {
        ;(manager.modelHealth as any).failureCount++
      }
      if (mod >= 8) {
        const cat = i % 2 === 0 ? 'RETRYABLE' : 'TIMEOUT'
        ;(manager.modelHealth as any).errorCounts[cat] = ((manager.modelHealth as any).errorCounts[cat] || 0) + 1
      }
    }
