...
import { EvidenceBridge, EVIDENCE_REPORT_READY } from '@akemi-mio/evolution/automation/EvidenceBridge'
import type { EvidenceReportReadyPayload } from '@akemi-mio/evolution/automation/EvidenceBridge'
...
describe('EvidenceBridge — 事件发出', () => {
  it('bridge(report) 通过 emitter.emit 发出 evidence.report.ready', () => {
    ...
    expect(emit).toHaveBeenCalledWith(EVIDENCE_REPORT_READY, expect.objectContaining({ reportId: expect.stringContaining('report_') }))
  })

  it('payload 包含 reportId, report, generatedAt', () => {
    const calls: unknown[] = []
    const bridge = new EvidenceBridge({
      emit: (event, payload) => {
        calls.push(payload)
      },
    })
