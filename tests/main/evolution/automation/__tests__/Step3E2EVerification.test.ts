...
import { SyntheticTscExecutor, SYNTHETIC_EXECUTOR_NAME } from '@akemi-mio/evolution/automation/SyntheticTscExecutor'
import { eventBus } from '@akemi-mio/core/core/EventBus'
import type { CollectorExecutionEvent, SignalCollector, InterventionReviewRecord, SyntheticExecutionEvent } from '@akemi-mio/evolution/automation/types'
...
describe('Step 3 E2E — synthetic evidence injection', () => {
  ...
  afterEach(() => {
    ...
    if (tmpDir && existsSync(tmpDir)) {
      try {
        rmSync(tmpDir, { recursive: true, maxRetries: 5 })
      } catch {}
    }
  })
