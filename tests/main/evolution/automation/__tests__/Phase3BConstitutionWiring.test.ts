...
function makeProposal(overrides?: Partial<Proposal>): Proposal {
  return {
    ...
    targetFiles: ['packages/intelligence/src/memory/MemoryService.ts'],
...
describe('Phase 3B: ProposalValidator 无 Constitution', () => {
  ...
  it('即使目标路径是核心文件，未注入时也不拦截', async () => {
    const result = await validator.validate(makeProposal({ targetFiles: ['packages/core/src/core/EventBus.ts'] }))
    expect(result.constitutional.passed).toBe(true)
...
describe('Phase 3B: ProposalValidator 带 Constitution (enforce)', () => {
  ...
  it('非受保护路径 → 通过 constitutional 检查', async () => {
    const result = await validator.validate(makeProposal({ targetFiles: ['packages/intelligence/src/memory/MemoryService.ts'] }))
    expect(result.constitutional.passed).toBe(true)
    expect(result.constitutional.violations).toEqual([])
  })
  ...
  it('多个路径中任一受保护 → 整体不通过', async () => {
    const result = await validator.validate(
      makeProposal({
        targetFiles: ['packages/intelligence/src/memory/MemoryService.ts', 'packages/core/src/core/EventBus.ts'],
      }),
    )
    expect(result.constitutional.passed).toBe(false)
  ...
  it('全部受保护路径 → 全部报告违反', async () => {
    const result = await validator.validate(
      makeProposal({
        targetFiles: ['packages/core/src/core/Kernel.ts', 'packages/evolution/src/constitution/types.ts'],
      }),
    )
    expect(result.constitutional.passed).toBe(false)
  ...
  it('结果中包含 violation 原因', async () => {
    const result = await validator.validate(makeProposal({ targetFiles: ['packages/core/src/core/Scheduler.ts'] }))
