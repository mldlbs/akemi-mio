...
describe('ProposalValidator', () => {
  const validProposal = {
    ...
    targetFiles: ['packages/intelligence/src/memory/RecallStrategy.ts'],
  ...
  it('should detect high regression risk for core files', async () => {
    ...
    const result = await validator.validate({
      ...validProposal,
      targetFiles: ['packages/core/src/core/EventBus.ts', 'packages/core/src/core/Scheduler.ts'],
    })
  ...
  it('should detect medium regression risk for single core file', async () => {
    ...
    const result = await validator.validate({
      ...validProposal,
      targetFiles: ['packages/core/src/core/EventBus.ts'],
    })
...
describe('RegressionDetector', () => {
  ...
  it('should detect regression when compile errors increase', async () => {
    ...
    const report = await detector.detectRegression(before, after, { newFiles: [], modifiedFiles: ['packages/core/src/core/test.ts'] })
