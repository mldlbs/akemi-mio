...
function walk(dir) {
  ...
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (e.name.endsWith('.json') && e.name !== 'golden-schema.json' && e.name !== 'manifest.json') files.push(path.join(dir, e.name))
  }
  return files
}
...
for (const f of files) {
  const d = JSON.parse(fs.readFileSync(f, 'utf8'))
  if (JSON.stringify(d.level1) !== before[f]) {
    console.log('CONTENT CHANGED: ' + f)
    process.exit(1)
  }
  if (d.changelog) {
    console.log('STILL HAS changelog: ' + f)
    process.exit(1)
  }
  if ('level2' in d) {
    console.log('STILL HAS level2: ' + f)
    process.exit(1)
  }
  ok++
}
