...
types = types.replace(/\/\*\* @future[\s\S]*?export interface GoldenResponseSnapshot \{[\s\S]*?\n\}\n\n/, '')
...
for (const f of files) {
  ...
  if (data.changelog) {
    data.revisionHistory = data.changelog
...
if (changed) {
  console.log('   ERROR: directive content changed!')
  process.exit(1)
}
