...
function main() {
  ...
  console.log('\n▶ 2. Retrieval Metrics')
  const retrievals = db
    .prepare(`SELECT type, COUNT(*) as cnt FROM evaluation_events WHERE type LIKE 'session.digest.%' GROUP BY type`)
    .all()
  ...
  if (scoredCount > 0) {
    ...
    if (allTopScores.length > 0) {
      console.log(
        `    Top scores overall:    ${allTopScores
          .sort((a, b) => b - a)
          .slice(0, 5)
          .map((s) => s.toFixed(4))
          .join(', ')}`,
    ...
    console.log(
      `    Avg token utilization: ${avgTokUtil.toFixed(0)} / ${avgTokBudget.toFixed(0)} (${((avgTokUtil / avgTokBudget) * 100).toFixed(1)}%)`,
    )
    ...
    const scoreVariance = allMaxs.length > 1 ? allMaxs.reduce((s, v) => s + (v - avg_fn(allMaxs)) ** 2, 0) / allMaxs.length : 0
  ...
  if (retrievedCount > 0) {
    ...
    for (const r of retrievedEvents) {
      try {
        ...
        if (f) {
          if (f.recency != null) {
            recencyVals.push(f.recency)
            factorsPresent.recency = true
          }
          if (f.frequency != null) {
            freqVals.push(f.frequency)
            factorsPresent.frequency = true
          }
          if (f.importance != null) {
            importanceVals.push(f.importance)
            factorsPresent.importance = true
          }
          if (f.attention != null) {
            attentionVals.push(f.attention)
            factorsPresent.attention = true
          }
        }
      } catch {
        /* skip parse errors */
    ...
    const printFactor = (name, vals) => {
      ...
      console.log(
        `    ${name.padEnd(15)} min=${Math.min(...vals).toFixed(4)}  p25=${p25.toFixed(4)}  p50=${p50.toFixed(4)}  p75=${p75.toFixed(4)}  max=${Math.max(...vals).toFixed(4)}  avg=${avgFn(vals).toFixed(4)}  n=${vals.length}`,
      )
    }
  ...
  if (gapEvents.length > 0) {
    ...
    if (sample.attentionFactors) {
      ...
      console.log(
        `    recency=${sample.attentionFactors.recency?.toFixed(4)} attention=${sample.attentionFactors.attention?.toFixed(4)} frequency=${sample.attentionFactors.frequency?.toFixed(4)} importance=${sample.attentionFactors.importance?.toFixed(4)}`,
      )
    }
    if (sample.fallbackFactors) {
      console.log(`  ── Sample fallback factors:`)
      console.log(
        `    recency=${sample.fallbackFactors.recency?.toFixed(4)} frequency=${sample.fallbackFactors.frequency?.toFixed(4)} importance=${sample.fallbackFactors.importance?.toFixed(4)}`,
      )
    }
  }
  ...
  const exitCriteria = [
    ...
    {
      name: 'Factor distribution ≥ 2/3',
      value: `${[hasRecency, hasFreq, hasImportance].filter(Boolean).length}/3`,
      pass: hasRecency && hasFreq && hasImportance,
    },
  ]
  ...
  console.log('')
  console.log(
    `  Phase 3 Context Injection: ${phase3Ready ? '🔓 READY for ADR decision' : '🔒 Blocked'} ${phase3Ready ? '' : '(missing criteria above)'}`,
  )
