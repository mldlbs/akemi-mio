...
async function main() {
  ...
  const checkForms = async () => {
    for (const kind of FORMS) {
      ...
      // 动画确实在 running（`自检动画 2/2`），但 GPU 差值恒 ≈0 —— pet 1.6/1.4、
      // chat -0.2/-0.1、wallpaper 0.4/0.5（下限 5），与主窗口的 147 差两个数量级。
      // 不是 headless 夹具假象，是真实行为；根因候选（GPU 进程全机共享被淹没 /
      // 隐藏窗口不合成）各有数据出入，见报告 §10.23 —— 但无论哪种，判红都会
      // 是永久抖动红（FM-8）。形态窗口的承重判据是上面的空闲动画数：
      // per-window、走 JS、不依赖 GPU 合成，CI 已确认不误红。
