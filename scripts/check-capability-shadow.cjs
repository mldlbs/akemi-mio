...
initSqlJs()
  .then((SQL) => {
    ...
    const suggestedOther = count(
      `SELECT COUNT(*) FROM evaluation_events WHERE type = 'capability.suggested' AND json_extract(payload, '$.contextSource') NOT IN ('llm_function_schema', 'llm_context') ${sinceNoPrefix}`,
    )

    // Schema injection per model invocation
    const schemaInjectionRate = modelInvoked > 0 ? (suggestedFnSchema / modelInvoked).toFixed(2) : 'N/A'
    ...
    try {
      ...
      if (rawS[0])
        perCapSelected = rawS[0].values.map(function (r) {
          return { cap: r[0], cnt: r[1] }
        })
      ...
      if (rawI[0])
        perCapInvoked = rawI[0].values.map(function (r) {
          return { cap: r[0], cnt: r[1] }
        })
      ...
      if (rawSuc[0])
        perCapSuccess = rawSuc[0].values.map(function (r) {
          return { cap: r[0], cnt: r[1] }
        })
    } catch {}
    ...
    try {
      ...
      if (raw[0])
        capSchemaCounts = raw[0].values.map(function (r) {
          return { cap: r[0], cnt: r[1] }
        })
    } catch {}
    ...
    console.log(
      `║  Window: ${obsSince > 0 ? new Date(obsSince).toISOString().split('T')[0] : 'all time'} → ${new Date(now).toISOString().split('T')[0]}         ║`,
    )
    console.log('╚══════════════════════════════════════════════╝')
    console.log('')
    ...
    console.log(
      `  Sel→Inv:  ${selectedMatchedInvoked}/${selected} = ${selToInvRate}%  ${selectedNoInvoked > 0 ? '⚠️ ' + selectedNoInvoked + ' unmatched' : '✅'}`,
    )
    ...
    console.log(
      `  Inv→Comp: ${invokedMatchedCompleted}/${invoked} = ${invToCompRate}%  ${invokedNotCompleted.length > 0 ? '⚠️ ' + invokedNotCompleted.length + ' incomplete' : '✅'}`,
    )
    ...
    if (perCapList.length === 0) {
      ...
      console.log(
        `  ${'Capability'.padEnd(20)} ${'Sel'.padEnd(6)} ${'Inv'.padEnd(6)} ${'OK'.padEnd(6)} ${'Fail'.padEnd(6)} ${'S→I%'.padEnd(7)} ${'I→C%'.padEnd(7)} ${'S→C%'.padEnd(7)}`,
      )
      console.log(
        `  ${'─'.repeat(20)} ${'─'.repeat(6)} ${'─'.repeat(6)} ${'─'.repeat(6)} ${'─'.repeat(6)} ${'─'.repeat(7)} ${'─'.repeat(7)} ${'─'.repeat(7)}`,
      )
    ...
    for (const [name, pass, val] of gates) {
      console.log(
        `  ${pass ? '✅' : hasData || name === 'Selected events >0' || name === 'LLM health ≥50%' ? '⏸' : '⏸'} ${name.padEnd(32)} ${val}`,
      )
    }
