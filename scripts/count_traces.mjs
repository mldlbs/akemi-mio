...
const traceCount = db.prepare
  ? db
      .prepare(
        "SELECT COUNT(DISTINCT trace_id) as c FROM evaluation_events WHERE session_id != '' AND session_id IS NOT NULL AND trace_id != '' AND trace_id IS NOT NULL",
      )
      .get()
  : null
...
const mixedTraces = db.prepare
  ? db
      .prepare(
        `SELECT COUNT(*) as c FROM (SELECT trace_id FROM evaluation_events WHERE session_id != '' AND session_id IS NOT NULL AND trace_id != '' AND trace_id IS NOT NULL GROUP BY trace_id HAVING SUM(CASE WHEN type LIKE 'model.%' THEN 1 ELSE 0 END) > 0 AND SUM(CASE WHEN type LIKE 'tool.%' THEN 1 ELSE 0 END) > 0)`,
      )
      .get()
  : null
...
const sizes = db.prepare
  ? db
      .prepare(
        "SELECT COUNT(*) as cnt FROM evaluation_events WHERE session_id != '' AND session_id IS NOT NULL AND trace_id != '' AND trace_id IS NOT NULL GROUP BY trace_id ORDER BY cnt",
      )
      .all()
  : []
