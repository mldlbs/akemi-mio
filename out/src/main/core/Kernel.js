...
export class Kernel {
    ...
    verifyIntegrity() {
        ...
        if (this._frozen !== true)
            issues.push('Module registry not frozen');
    ...
    async handleSyscall(method, params) {
        switch (method) {
            ...
            case 'get_module_info': {
                const p = params;
                if (!p?.name)
                    throw new Error('get_module_info requires "name" param');
                const mod = this.modules.get(p.name);
                if (!mod)
                    throw new Error(`Module "${p.name}" not found`);
                return {
                    name: mod.name,
                    state: mod.state,
                    hotReloadable: mod.hotReloadable,
            ...
            case 'reload_module': {
                const p = params;
                if (!p?.name)
                    throw new Error('reload_module requires "name" param');
