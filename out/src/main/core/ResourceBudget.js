...
export class ResourceBudget {
    constructor(config, budgetFilePath) {
        this.taskBudgets = new Map();
        this.processBudgets = new Map();
    ...
    /** 检查是否有任何预算已耗尽 */
    isExhausted() {
        return (this.state.contextExhausted.size > 0 ||
    ...
    consumeToolLoopTurn() {
        if (this.state.toolLoopTurnsThisCycle >= this.config.maxToolLoopTurns) {
            ...
            if (extraTurns >= this.config.softOverrideTurns) {
                ...
                throw new BudgetExceededError('toolLoop', this.state.toolLoopTurnsThisCycle, this.config.maxToolLoopTurns + this.config.softOverrideTurns);
            }
    ...
    /** 获取软 override 剩余轮次 */
    getSoftOverrideRemaining() {
        if (!this.isInSoftOverride())
            return 0;
    ...
    consumeTaskCpuMs(taskId, durationMs) {
        const tb = this.taskBudgets.get(taskId);
        if (!tb)
            return `Task "${taskId}" not found`;
    ...
    // ==================== 进程级预算 ====================
    /** 为子进程分配预算 */
    allocateProcessBudget(processName, config) {
    ...
    /** 消耗子进程内存（返回是否超限） */
    consumeProcessMemory(processName, mb) {
        const pb = this.processBudgets.get(processName);
        if (!pb)
            return true; // 未跟踪进程不计入预算
    ...
    /** 记录子进程重启 */
    recordProcessRestart(processName) {
        const pb = this.processBudgets.get(processName);
        if (!pb)
            return true;
    ...
    /** 获取子进程利用率 */
    getProcessUtilization(processName) {
        return this.processBudgets.get(processName)?.utilization ?? null;
    }
    /** 释放子进程预算 */
    releaseProcessBudget(processName) {
        this.processBudgets.delete(processName);
    }
    emitProcessExhausted(processName, resource, used, limit) {
    ...
    autoTune(stabilityScore) {
        if (stabilityScore >= 90) {
            ...
            const reduce = (current, min) => Math.max(min, Math.floor(current * 0.5));
            this.config = {
                ...this.config,
            ...
            const reduce = (current, min) => Math.max(min, Math.floor(current * 0.8));
            this.config = {
                ...this.config,
