...
export class BudgetRebalancer {
    constructor(resourceBudget, bus) {
        this.lastRebalance = 0;
        this.DEBOUNCE_MS = 60000;
        this.STEAL_AMOUNT = 5;
        this.unsubscribers = [];
    ...
    start(bus) {
        ...
        this.unsubscribers.push(eb.on('budget.exhausted', (p) => {
    ...
    handleExhausted(resource) {
        ...
        if (now - this.lastRebalance < this.DEBOUNCE_MS)
            return;
