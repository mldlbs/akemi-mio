...
export function insertOutbox(row) {
    const db = getRawDb();
    const now = Date.now();
    const hash = row.hash || simpleHash(`${row.chatId}:${row.msgType}:${row.message}`);
    // 去重：相同 hash 的 pending/sent 已存在则跳过
    if (outboxExists(hash))
        return 0;
    db.run(`INSERT INTO telegram_outbox (chat_id, bot, msg_type, category, message, target_message_id, hash, status, retry_count, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', 0, ?)`, [row.chatId, row.bot || 'chat', row.msgType, row.category || 'dialogue', row.message, row.targetMessageId ?? null, hash, now]);
    markDirty();
    return db.exec('SELECT last_insert_rowid()')[0]?.values?.[0]?.[0] ?? 0;
}
/** 拉取待发送消息（最多 limit 条） */
export function getPendingOutbox(limit = 10) {
    const db = getRawDb();
    const rows = db.exec(`SELECT id, chat_id, bot, msg_type, message, target_message_id, hash, status, retry_count, last_error, created_at, updated_at
...
export function markOutboxFailed(id, error) {
    ...
    if (retryCount >= 3) {
        ...
        if (msgType === 'edit' && chatId && lastMessage) {
            db.run(`INSERT INTO telegram_outbox (chat_id, msg_type, category, message, hash, status, retry_count, created_at)
         VALUES (?, 'send', 'dialogue', ?, ?, 'pending', 0, ?)`, [chatId, lastMessage, simpleHash(`${chatId}:send:${lastMessage}:fallback`), Date.now()]);
        }
        markDirty();
        return true;
    }
...
export function outboxExists(hash) {
...
export function cleanupOutbox() {
    ...
    const result = db.exec(`DELETE FROM telegram_outbox WHERE (status = 'sent' AND created_at < ?) OR (status = 'failed' AND retry_count >= 3 AND created_at < ?)`, [cutoff, cutoff]);
