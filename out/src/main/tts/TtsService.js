...
export class TtsService {
    ...
    flushBuffer() {
        ...
        if (!this.sentenceBuf.trim() && this.ttsQueue.length === 0)
            return;
    ...
    async _synthesize(text, outputFile, attempt = 1) {
        ...
        try {
            ...
            const edgeTts = execFile('edge-tts', ['--voice', 'zh-CN-XiaoxiaoNeural', '--text', text, '--write-media', outputFile, '--rate', '+10%', '--pitch', '+8Hz'], { timeout: 30000, windowsHide: true });
    ...
    async processQueue() {
        if (this.isProcessing || this.ttsQueue.length === 0 || this.stopped)
            return;
        ...
        try {
            do {
                while (this.ttsQueue.length > 0) {
                    const batch = [];
                    while (this.ttsQueue.length > 0)
                        batch.push(this.ttsQueue.shift());
