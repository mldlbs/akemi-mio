...
export class ServerManager {
    constructor() {
        ...
        this.capabilityEngine = null;
        /** 每个 MCP 服务器的独立熔断器 */
        this.circuitBreakers = new Map();
        /** 重启预算：每小时最多 RESTART_BUDGET_MAX 次重启，超限后自动禁用 */
        this.restartBudgets = new Map();
        this.RESTART_BUDGET_WINDOW = 3600000; // 1 小时
        this.RESTART_BUDGET_MAX = 10; // 每小时最多 10 次
    ...
    async callTool(name, args) {
        ...
        if (!client)
            throw new Error(`MCP 服务器不可用: ${meta.serverName}`);
        const result = await client.callTool(name, args);
        return this.formatResult(result);
    }
    async handleMgrTool(name, args) {
        switch (name) {
    ...
    registerCapabilities(name, client) {
        const toolNames = client.getToolDefinitions().map((t) => t.name);
    ...
    /** 带指数退避的重启 */
    async restartServerWithBackoff(name) {
    ...
    async restartServer(name) {
        ...
        if (!existsSync(SERVERS_CONFIG_PATH))
            return;
        try {
            const entries = JSON.parse(readFileSync(SERVERS_CONFIG_PATH, 'utf-8'));
    ...
    toolNameToCapability(toolName) {
        if (toolName === 'write_file' || toolName === 'edit_file')
            return 'file.write';
        if (toolName === 'read_file' || toolName === 'list_files' || toolName === 'grep')
            return 'file.read';
        if (toolName === 'delete_file')
            return 'file.delete';
        if (toolName === 'run_command')
            return 'shell.execute';
        if (toolName.startsWith('llm.') || toolName === 'chat')
            return 'llm.call';
        if (toolName.startsWith('memory_'))
            return 'memory.read';
        if (toolName.startsWith('mcp_'))
            return 'mcp.call';
        if (toolName.startsWith('evolution_'))
            return 'evolution.analyze';
        return 'mcp.call';
    }
}
