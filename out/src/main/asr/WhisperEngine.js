...
import { ASR_HOTWORDS, ASR_SAMPLE_RATE, ASR_MAX_AUDIO_SECONDS, ASR_INITIAL_PROMPT } from '../config';
...
const hotwordPatterns = ASR_HOTWORDS.filter((hw) => hasChinese(hw)).map((hw) => ({
    original: hw,
            ...
            for (const hwp of hotwordPatterns) {
                ...
                if (match) {
                    ...
                    if (existing)
                        ...
                        hits.push({ hotword: hwp.original, count: 1 });
                    break;
                }
            }
            if (!matched) {
...
export class WhisperEngine {
    constructor() {
        this.transcribeFn = null;
        this.loading = false;
        this.loadError = null;
        this.loadPromise = null;
        this.currentModel = '';
        this.firstInferenceDone = false;
        this.initialPrompt = (() => {
    ...
    async _doInit(model, onProgress) {
        const t0 = Date.now();
    ...
    async _createPipeline(modelId, onProgress) {
    ...
    _logInitComplete(model, t0, memBefore, memAfter) {
        const modelSizes = { 'large-v3': 1500, medium: 800, small: 500, base: 300, tiny: 150 };
    ...
    async transcribe(audioBuffer, timeoutMs = 10000, requestId) {
        if (!this.transcribeFn)
