...
export class CreativityService {
    constructor(store, deps, chatJson, chatJsonWithCode, temperature = 0.3, seed, bus, reportDir = '', taskRunner, observerDir) {
        ...
        this.evolutionDisposer = null;
        /** 轮换的策略序列：每次 cycle 按顺序切换 */
        this.strategyCycle = ['explore', 'signal', 'stable'];
    ...
    start() {
        if (this.taskRunner) {
            this.taskRunner.register('creativity.cycle', async () => {
                await this.cycle();
                return { success: true };
            }, NORMAL_CYCLE_INTERVAL_MS);
            this.taskRunner.register('creativity.dream', async () => {
    ...
    async cycle() {
        ...
        for (const idea of ideas) {
            const verdict = evaluateNovelty({ title: idea.hypothesis.title, idea: idea.hypothesis.idea, novelty: idea.hypothesis.novelty }, recent.map((h) => ({ title: h.title, idea: h.idea, novelty: h.novelty })), rejected.map((h) => ({ title: h.title, idea: h.idea })));
        ...
        const topIdea = deduped.reduce((best, i) => {
    ...
    async dreamCycle() {
        ...
        const insightSources = insights.map((i) => ({
            name: `洞察:${i.title}`,
        ...
        const ideas = await this.generator.dreamIdeas(allSources, historicalCombos, failedHypotheses.map((h) => ({
            title: h.title,
            idea: h.idea,
            risk: h.risk,
            status: 'rejected',
