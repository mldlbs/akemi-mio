...
import { insertOutbox } from '../db/outbox';
// ── 路由表：消息分类 → 使用的 bot ──
...
export class DebouncedEditor {
    constructor() {
        this.baseUrl = '';
        this.timer = null;
        this.pendingChatId = 0;
        this.pendingText = '';
        this.pendingTargetMsgId = 0;
        this.pendingBot = 'chat';
    }
    /** 在 TelegramService 初始化后设置真实的 baseUrl */
    setBaseUrl(url) {
        this.baseUrl = url;
    }
    schedule(chatId, targetMessageId, text, bot = 'chat') {
...
export class TelegramService {
        ...
        if (progressMsgId === null) {
            ...
            try {
                ...
                if (result.reply) {
                    ...
                    this.enqueueReply(msg.chatId, `👤 ${userText}\n\n⏳ 秋山澪正在处理其他请求，你的消息已加入队列，处理完成后会自动回复`, 'dialogue', botName);
    ...
    subscribePushEvents() {
        ...
        if (!chatId)
            return;
        // === 💬 对话 ===
        eventBus.on('agent.input.received', (p) => {
            if (p.source !== 'electron')
                return;
            // ★ 修复：透传 requestId 用于响应精确匹配
            this.handlePushInput(chatId, p.text, p.requestId);
        });
        eventBus.on('agent.response.generated', (p) => {
            if (p.source !== 'electron')
                return;
        ...
        eventBus.on('evolution.cycle.started', (p) => {
            ...
            lines.push(`━━━ 分析开始 ━━━`);
            if (p.mode)
                lines.push(`状态: ${p.mode}`);
            if (p.failures !== undefined && p.failures > 0)
                lines.push(`连续失败: ${p.failures} 次`);
            if (p.strategyName)
                lines.push(`策略: ${p.strategyName}`);
            if (p.historyCount !== undefined)
                lines.push(`历史记录: ${p.historyCount} 条`);
        ...
        eventBus.on('evolution.rollback.completed', (p) => {
        ...
        eventBus.on('insight.analysis.completed', (p) => {
            if (p.count > 0) {
        ...
        eventBus.on('insight.found', (p) => {
            ...
            if (p.insights?.length > 5)
                lines.push(`  ... 还有 ${p.insights.length - 5} 条`);
            this.enqueueReply(chatId, lines.join('\n'), 'insight');
        });
        // === 💡 创意 ===
        ...
        eventBus.on('creativity.cycle.completed', (p) => {
        ...
        eventBus.on('creativity.dream.completed', (p) => {
            this.enqueueReply(chatId, `💡 梦境完成: ${p.count} 个组合, 最高新颖度 ${p.topNovelty}%`, 'creativity');
        });
        eventBus.on('creativity.ideas.generated', (p) => {
    ...
    async handlePushInput(chatId, text, requestId) {
        ...
        const d1 = eventBus.on('agent.tool.invoked', (p) => {
        ...
        const d2 = eventBus.on('agent.tool.completed', (p) => {
            const session = this.pushSessions.get(requestId);
            if (!session)
                return;
        ...
        const d3 = eventBus.on('agent.tool.failed', (p) => {
            const session = this.pushSessions.get(requestId);
            if (!session)
                return;
            const line = session.toolLines.find((l) => l.name === p.tool && l.status === 'running');
    ...
    async handlePushResponse(chatId, reply, requestId) {
        const session = this.pushSessions.get(requestId);
        if (session) {
    ...
    refreshPushMessage(chatId, msgId, userText, toolLines, editor) {
        const lines = [`👤 你: ${userText}\n`];
        ...
        if (!hasRunning && toolLines.length > 0)
            lines.push('\n✍️ 正在生成回复...');
        editor.schedule(chatId, msgId, lines.join('\n'), 'chat');
    }
    cleanupPushSession(requestId) {
        const session = this.pushSessions.get(requestId);
        if (!session)
            return;
    ...
    // ── Outbox 写入 ──
    enqueueReply(chatId, text, category, bot) {
        insertOutbox({
            chatId: String(chatId),
            bot: (bot || (category && ROUTING[category]) || 'chat'),
            msgType: 'reply',
            category,
            message: text,
        });
    ...
    enqueueEdit(chatId, targetMessageId, text, bot = 'chat') {
        const inserted = insertOutbox({ chatId: String(chatId), bot: bot, msgType: 'edit', message: text, targetMessageId });
        log('DEBUG', 'telegram_enqueue_edit', { chatId, targetMessageId, inserted, textLen: text.length });
    }
    enqueueAction(chatId, action, bot = 'chat') {
        insertOutbox({ chatId: String(chatId), bot: bot, msgType: 'action', message: action });
    }
    ...
    refreshProgressMessage(chatId, msgId, userText, toolLines, editor) {
        const lines = [`👤 你: ${userText}\n`];
        let hasRunning = false;
        for (const tl of toolLines) {
            if (tl.status === 'running') {
    ...
    async handleGenBotMessage(chatId, prompt) {
        ...
        try {
            ...
            const { prompt_id } = (await wfRes.json());
        ...
        for (const filePath of filePaths) {
            try {
                ...
                fetch(`${this.baseUrl}/photo`, {
                    ...
                    .then((r) => {
                    if (r.ok)
                        log('INFO', 'telegram_photo_sent', { chatId, filePath });
                    else
                        r.text().then((t) => log('WARN', 'telegram_photo_send_failed', { status: r.status, error: t.slice(0, 200) }));
                })
    ...
    cleanupSession(chatId, bot) {
        this.cleanupSessionByKey(sessionKey(chatId, bot));
    }
    cleanupSessionByKey(key) {
...
function sessionKey(chatId, bot) {
    return `${chatId}:${bot}`;
}
