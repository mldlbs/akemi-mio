...
function getDefaultImmutablePaths() {
 ...
 * 加载宪法文档。如果不存在则创建默认文档。
 */
export function loadConstitution(dir) {
...
export function validateConstitution(doc) {
    const errors = [];
    if (!doc)
        return ['constitution document is null/undefined'];
    if (typeof doc.version !== 'string')
        errors.push('version must be a string');
    if (!Array.isArray(doc.immutablePaths))
        errors.push('immutablePaths must be an array');
    if (!Array.isArray(doc.mutablePaths))
        errors.push('mutablePaths must be an array');
    return errors;
}
