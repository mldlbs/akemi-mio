...
import { EVOLUTION_WORKSPACE_DIR } from '../tool/utils/workspace';
// ── Guardrail ──
export class Guardrail {
    constructor(deps, readOnlyTools) {
        this.deps = { ...deps };
        this.readOnlyTools =
            readOnlyTools ??
                new Set([
     ...
     */
    apply(toolResults, toolCalls, messages, ctx) {
        ...
        // 4. plan 操作失败恢复
        if (this.checkPlanErrors(toolResults, messages))
            injected = true;
        // 5. 连续工具错误 → 诊断模式
        if (this.checkConsecutiveToolErrors(toolResults, messages, ctx))
            injected = true;
        // 6. 只读错误 > 3 次 → 切换模式
        if (this.checkConsecutiveReadOnlyErrors(toolResults, messages, ctx))
            injected = true;
        // 7. writing 工具全部失败检测 → 切换提示
        if (this.checkWritingToolFailure(toolResults, messages))
            injected = true;
        // 8. 流程记忆自动建议
        if (this.checkProcedureSuggestion(toolCalls, messages))
            injected = true;
        return { injected };
    }
    // ── 各 guardrail 实现 ──
     ...
     */
    checkReadOnlyStuck(allReadOnly, toolResults, messages, ctx) {
        ...
        if (hasFailedRemote && allReadOnly && ctx.consecutiveReadOnlyRounds >= 1) {
            ...
            messages.push({
                role: 'user',
                content: '【系统提示】远程工具连续失败，请立即切换到本地工具（grep/read_file/list_files@project 或 writing_system）。不要继续使用远程工具。',
            });
            return true;
        }
        if (allReadOnly) {
        ...
        if (ctx.consecutiveReadOnlyRounds >= 2) {
            ...
            if (ctx.readonlyStuckCount >= 4) {
                messages.push({
                    role: 'user',
                    content: '【系统强制】已连续多次只读卡死，当前状态无法继续推进。立即停止所有读取操作，基于已获取的信息给出当前最佳回答或总结。不要继续尝试读取新文件。',
                });
                ...
                messages.push({
                    role: 'user',
                    content: '【系统强制】这已经是第 3 次检测到只读卡死。请不要继续读取文件。如果已有足够信息就总结输出，如果信息不足就基于现有信息给出阶段性结论。如果是远程工具卡住，请切换为本地工具（writing_system）。',
                });
            }
            else {
                messages.push({
                    role: 'user',
                    content: '【系统提示】你已经连续读取多个文件了。如果信息足够，请直接给出回答或开始执行。如果还需要继续查阅才能决策，可以继续读取。注意：远程服务器工具（centos_*）较慢，如需快速查看小说内容请用 writing_system 本地工具。',
                });
            }
            return true;
        }
     ...
     * list_files 路径兜底
     */
    checkListFilesPathFallback(toolResults) {
     ...
     * plan 操作失败恢复
     */
    checkPlanErrors(toolResults, messages) {
        ...
        if (activePlan) {
            messages.push({
                role: 'user',
                content: `【系统强制】你刚操作的开发计划不存在或 ID 不对。数据库中的活跃计划是「${activePlan.title}」（plan_id=${activePlan.id}，已完成 ${activePlan.steps.filter((s) => s.status === 'done').length}/${activePlan.steps.length} 步）。` +
                    `直接使用 plan_id=${activePlan.id} 继续，不要 list_plans，不要猜测。下一步未完成的步骤名：${activePlan.steps
     ...
     */
    checkConsecutiveToolErrors(toolResults, messages, ctx) {
        if (ctx.guardrailStop)
            return false;
        ...
        if (!ctx.suppressForceContinue && !ctx.guardrailStop) {
            ...
            if (ctx.consecutiveToolErrors >= 3) {
                ...
                const hasReadTools = failingTools.some((n) => ['read_file', 'list_files', 'grep', 'centos_read_file', 'centos_grep', 'centos_search_files'].includes(n));
     ...
     * 只读错误 > 3 次 → 切换模式
     */
    checkConsecutiveReadOnlyErrors(toolResults, messages, ctx) {
     ...
     */
    checkWritingToolFailure(toolResults, messages) {
        const writingFailures = toolResults.filter((r) => !r.success && r.name.startsWith('writing_'));
        if (writingFailures.length === 0)
            return false;
        const allWriting = toolResults.length > 0 && toolResults.every((r) => r.name.startsWith('writing_'));
        if (allWriting) {
            messages.push({
                role: 'user',
                content: '【系统提示】写作系统（writing_* 工具）全部不可用，可能是后台服务未启动。请停止调用写作工具，转用本地文件工具直接操作文件。如果用户需要写作帮助，请告知写作系统暂时不可用。',
            });
            return true;
        }
        return false;
     ...
     */
    checkProcedureSuggestion(toolCalls, messages) {
