...
import { trimOrphanedToolCallsFrom } from './context';
import { rollbackToLastKnownGood } from './ContextIntegrityChecker';
import { eventBus } from '../core/EventBus';
import { WorkingMemory } from './WorkingMemory';
import { createMessageId, createSessionId, insertMessage, getLastSessionId, getLastMessageTime, getMessagesBySession, } from '../db/messages';
...
export class ChatExecutor {
    constructor(llmService, ttsService, mainWindow, toolScheduler, guardrail, goalGuardrail, planManager, resourceBudget, memoryService, skillManager, recoveryManager, tokenAccount, subAgentPool, reflectLoop) {
        ...
        this.thinkStageCount = 0;
        this.obsLogger = null;
        this.lastUserText = '';
        /** 人格仲裁管理器 */
        this.personaManager = new PersonaStateManager();
        /** 人格漂移控制系统 */
        this.driftControl = new PersonaDriftControlSystem();
        /** 上轮 drift 评估产生的待注入消息信号 */
        this.pendingDriftSignal = null;
        /** 当前轮 user 消息的 content category */
        this.currentCategory = 'chat';
        /** 当前加载的 session，用于切换 session 时重建上下文 */
        this.currentSessionId = null;
        /** 执行决策门 — 每轮 tool batch 后强制决策 */
        this.executionGovernor = new ExecutionGovernor();
        this.noTts = false;
    ...
    updateDeps(deps) {
        ...
        if (deps.recoveryManager !== undefined)
            this.recoveryManager = deps.recoveryManager;
        if (deps.tokenAccount !== undefined)
            this.tokenAccount = deps.tokenAccount;
        if (deps.mainWindow !== undefined)
            this.mainWindow = deps.mainWindow;
        if (deps.identityContext !== undefined)
            this.identityContext = deps.identityContext;
        if (deps.proceduralMemory !== undefined)
            this.proceduralMemory = deps.proceduralMemory;
        if (deps.failureAnalyzer !== undefined)
    ...
    refreshMemory() {
        ...
        const extraModules = [...skillModules];
    ...
    persistAssistantMessage(reply, source) {
        const msg = {
            id: createMessageId(),
            source: source,
            role: 'assistant',
    ...
    async run(text, requestId, source = 'electron', extra, sessionId, noTts) {
    ...
    async toolLoop(messages, ctx, requestId, source) {
        ...
        try {
            for (let i = 0; i < MAX_TURNS; i++) {
                ...
                this.obsLogger?.logLlmTrace('result', `ms=${llmMs} reply_len=${(result.reply || '').length} tools=${result.toolCalls?.length || 0} error=${result.error || 'null'}`);
                ...
                if (result.toolCalls && result.toolCalls.length > 0) {
                    ...
                    if (guardDecision.status === 'denied') {
                        ...
                        if (!guardDecision.canRetry)
                            return ''; // 硬拒绝 → 终止本轮
                        continue; // 软拒绝 → 不 spend/不执行，LLM 重试
                    }
                    // ── 守卫通过，继续执行 ──
                    if (this.tokenAccount)
                        this.tokenAccount.spend(500, 'llm_call_tool', `chat step ${i}`);
                    if (!result.reply?.trim())
                        this.ttsService.stop();
                    if (result.reply) {
                        this.persistAssistantMessage(result.reply, source);
                        const b = result.reply.trim().replace(/[。！？\n].*$/s, '');
                        if (b.length >= 4 && !ctx.spokenReplies.some((r) => r.includes(b)))
                            ctx.spokenReplies.push(b);
                    }
                    ctx.transition(RunState.WAIT_TOOL);
                    eventBus.emit('agent.progress', { requestId, step: i + 1, toolNames: result.toolCalls.map((t) => t.name) });
                    ...
                    this.workingMemory.trimToTokenBudget(600000);
                    ...
                    if (gd.action === 'stop') {
                        ...
                        if (gd.message)
                            messages.push({ role: 'user', content: gd.message });
                ...
                if (!finalReply)
                    this.obsLogger?.logExit('empty_llm_reply', `step=${i}`);
                return finalReply;
            }
        }
        finally {
    ...
    handleLlmError(e, step, m, ctx) {
        ...
        eventBus.emit('recovery.error.classified', { category: c.category });
        ...
        if (c.category === 'CONTEXT_OVERFLOW') {
            ...
            this.workingMemory.trimToTokenBudget(300000);
    ...
    async handlePlanForceContinue(r, m, ctx) {
        const a = this.planManager.getActivePlan();
        if (!a || !this.sessionPlanIds.has(a.id))
            return 'done';
        ...
        m.push({
            role: 'user',
            content: ctx.forceContinueCount <= 3
    ...
    checkMilestone(step, tr, ctx, rid) {
        ...
        if (!ms.shouldCheckpoint)
            return;
        const pd = ap ? ap.steps.filter((s) => s.status !== 'done').map((s) => s.description) : [];
