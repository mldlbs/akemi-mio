...
import { fallbackEmbed, cosineSimilarity } from '../memory/embedding';
let idCounter = 0;
...
export class ProceduralMemory {
    save(params) {
        ...
        db.run(`INSERT INTO procedures (id, name, description, steps, trigger_keywords, embedding, success_count, fail_count, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?)`, [
            id,
            params.name,
            params.description,
    ...
    /** 按触发词搜索相关流程 */
    findByKeywords(keywords, topK = 3) {
        const db = getRawDb();
        const all = this.listAll();
        const scored = all.map((p) => {
            ...
            for (const kw of keywords) {
                ...
                if (p.name.toLowerCase().includes(kwLower))
                    score += 3;
                if (p.description.toLowerCase().includes(kwLower))
                    score += 2;
                if (p.triggerKeywords.some((t) => t.toLowerCase().includes(kwLower)))
                    score += 2;
                if (p.steps.some((s) => s.toLowerCase().includes(kwLower)))
                    score += 1;
            }
    ...
    listAll() {
        ...
        if (!rows)
            return [];
        return rows.values.map((v) => this.rowToProc(rows.columns, v));
    }
    recordHit(name) {
        const db = getRawDb();
