...
/** 采集风格熵指标 */
export function evaluateStyleEntropy(text) {
...
/** 评估熵报告，判断是否漂移 */
export function evaluateDrift(report, personaLevel) {
...
/** 根据 drift 原因生成修正信号 */
export function buildCorrectionSignal(reason) {
    if (!reason)
        return null;
...
export class PersonaDriftControlSystem {
    constructor() {
        this.styleAnchorMemory = new StyleAnchorMemory();
     ...
     */
    evaluateOutput(text, personaLevel) {
        if (personaLevel === 'core') {
            this.correctionActive = false;
