...
export function buildSystemPrompt(memoryContext, extraModules, reflectionContext, identityContext) {
...
export function getBasePromptTokens() {
...
export class ConversationContext {
    constructor(memoryContext, maxTokens = 2000, extraModules, customSystemPrompt, reflectionContext, identityContext) {
    ...
    trimToTokenBudget(maxTokens = this.maxTokens) {
        const systemTokens = estimateTokens(this.systemPrompt);
        const totalTokens = () => {
            let t = systemTokens;
            for (let i = 1; i < this._context.length; i++)
                t += estimateMessageTokens(this._context[i]);
            return t;
        };
    ...
    addSystemMessage(content) {
        for (let i = this._context.length - 1; i >= 0; i--) {
            if (this._context[i].role === 'user') {
                this._context.splice(i + 1, 0, { role: 'system', content });
                return;
            }
        }
        this._context.push({ role: 'system', content });
    }
    getShortTermMemoryPairs() {
...
export function trimOrphanedToolCallsFrom(messages) {
