...
import { PROMPT_WRITER_IDENTITY, PROMPT_WRITER_STYLE, detectWritingIntent, resolvePersona } from './writing-prompt';
...
export class PersonaStateManager {
    ...
    update(text) {
        ...
        if (resolved === this.level)
            return { changed: false, transitionSignal: null };
    ...
    getExtraModules() {
        if (this.level === 'writer')
            return [PROMPT_WRITER_IDENTITY];
        if (this.level === 'hybrid')
            return [PROMPT_WRITER_STYLE];
        return [];
    }
    /** 重置到 core（跨会话/测试清理） */
    reset() {
