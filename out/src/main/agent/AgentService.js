...
export class AgentService {
    constructor(llmService, asrService, ttsService, bus, mcpManager, planManager) {
        ...
        this.reflectLoop = new ReflectLoop();
        ...
        this.subAgentPool = new SubAgentPool(this.mcpManager, this.eventBus, this.llmService['chatApiKey'] || undefined, this.llmService['codeApiKey'] || undefined);
        this.registerDefaultHandlers();
        // 监听计划创建，追踪本次会话的活跃计划
        this.eventBus.on('agent.plan.created', (p) => {
        ...
        this.chatExecutor = new ChatExecutor(this.llmService, this.ttsService, this.mainWindow, this.toolScheduler, this.guardrail, this.goalGuardrail, this.planManager, this.resourceBudget, this.memoryService, this.skillManager, this.recoveryManager, this.tokenAccount, this.subAgentPool, this.reflectLoop);
        // 注册流程记忆到工具依赖
    ...
    refreshMemoryInContext(lastUserText) {
    ...
    async executeIntentCommand(intent, requestId) {
    ...
    async processTextInput(text, requestId, source = 'electron', extra, sessionId, noTts) {
        // 熔断检查
    ...
    pause() {
        ...
        this.eventBus.emit('agent.pause', {});
    }
    /** 恢复 Chat 处理 */
    resume() {
        if (!this._paused)
    ...
    getSelfTaskAge() {
        if (!this.inSelfTask || this.selfTaskStartTime === 0)
            return 0;
    ...
    /** 设置会话恢复管理器 */
    setRecoveryManager(rm) {
        this.recoveryManager = rm;
        this.updateRuntimeDeps();
    }
    /** 保存恢复快照（检查点） */
    async saveRecoverySnapshot(trigger, error) {
        if (!this.recoveryManager)
            return;
        try {
            const activePlan = this.planManager?.getActivePlan?.();
            const pendingDescriptions = activePlan ? activePlan.steps.filter((s) => s.status !== 'done').map((s) => s.description) : [];
    ...
    tryRestoreSession() {
        ...
        this.eventBus.emit('recovery.recovery.started', {
        ...
        this.eventBus.emit('recovery.session.restored', {
    ...
    async runSelfTask(task, systemPrompt) {
        if (this.inSelfTask)
            return { success: false, summary: '自进化已经在运行' };
