...
<text x="${x + 40}" y="365" fill="${style.badgeText}" font-size="11" font-family="${fontName}" text-anchor="middle" dominant-baseline="middle">${escapeXml(t)}</text>`;
  ...
  ${descLines
        .map((line, i) => `<text x="${pad}" y="${descY + i * lineH}" fill="${style.textColor}" font-size="15" font-family="${fontName}" opacity="0.9">${escapeXml(line)}</text>`)
        .join('\n')}
...
export const cardGeneratorTool = buildTool({
    ...
    handler: async (args) => {
        try {
            ...
            if (!title)
                return formatToolError('title 不能为空');
            const description = String(args.description).trim();
            if (!description)
                return formatToolError('description 不能为空');
            const severity = args.severity || 'info';
            if (!STYLES[severity])
                return formatToolError(`不支持的 severity: ${severity}，可选: ${Object.keys(STYLES).join(', ')}`);
