...
export const rememberProcedureTool = buildTool({
    ...
    handler: async (args) => {
        try {
            ...
            if (!pm)
                return formatToolError('流程记忆服务暂不可用');
...
export const listProceduresTool = buildTool({
    ...
    handler: async () => {
        try {
            ...
            if (all.length === 0)
                return formatToolResult('当前没有保存的流程');
            const lines = all.map((p) => `- ${p.name}: ${p.description.slice(0, 60)} (成功${p.successCount}/失败${p.failCount}, ${p.steps.length}步)`);
