...
export const writeFileTool = buildTool({
    name: 'write_file',
    description: '写入文件到工作区。重要：必须使用子目录路径。正确: "my-server/src/index.ts"。错误: "index.ts"。workspace="project" 可直接写项目源码（用于 bug 修复），workspace="evolution" 写进化分析/实验代码，默认 workspace="mcp" 写 MCP 服务器沙箱。',
    inputJSONSchema: {
        type: 'object',
        properties: {
        ...
        try {
            ...
            if (ws === 'project') {
                ...
                if (!cleanPath.includes('/') && !cleanPath.includes('\\')) {
                    return formatToolError(`禁止在项目源码根目录写文件「${args.path}」。代码写入 projects/ 下，临时文件写入 evolution_workspace/tmp/。`);
                }
