...
export const analyzeTaskTool = buildTool({
    ...
    handler: async (args) => {
        try {
            ...
            const matched = defs.filter((d) => d.tags?.includes(tier) || d.name?.toLowerCase().includes(tier));
            // fallback: 按 pipeline ID 推荐
            const fallbackId = tier === 'simple' ? 'preset_dev_pipeline_simple' : tier === 'medium' ? 'preset_dev_pipeline_medium' : 'preset_dev_pipeline_large';
            const wfList = matched.length
                ? matched.map((d) => `  - ${d.name} (${d.id})`).join('\n')
                : `  - ${tier === 'simple' ? '简单' : tier === 'medium' ? '中等' : '大型'}开发管线 (${fallbackId})`;
            const wfHint = `\n\n推荐启动: start_workflow workflowId="${fallbackId}"`;
            return formatToolResult(`复杂度分析: ${tier}（${reason}）\n推荐工作流:\n${wfList}${wfHint}\n\n或用 create_workflow 创建自定义工作流。`);
        }
        catch (err) {
...
export const listWorkflowsTool = buildTool({
    ...
    handler: async () => {
        try {
            const defs = workflowStore.listDefinitions();
            if (defs.length === 0)
                return formatToolResult('暂无工作流定义。');
...
export const createWorkflowTool = buildTool({
    ...
    description: '创建一个新的工作流定义。工作流由多个步骤组成，步骤之间可以有依赖关系（DAG），支持子 agent、工具调用、API 调用、prompt 注入和 plan 五种 handler 类型。',
    inputJSONSchema: {
        type: 'object',
        properties: {
...
export const startWorkflowTool = buildTool({
    ...
    handler: async (args) => {
        try {
            ...
            if (!def)
                return formatToolError(`工作流 ${args.workflowId} 不存在`);
    ...
    handler: async (args) => {
        try {
            if (args.runId) {
                ...
                if (!run)
                    return formatToolError(`运行 ${args.runId} 不存在`);
            ...
            if (active.length === 0)
                return formatToolResult('当前无活跃工作流运行。');
            return formatToolResult(active
...
function analyzeComplexity(taskDescription) {
    for (const p of LARGE_PATTERNS) {
        if (p.test(taskDescription))
            return { tier: 'large', reason: '检测到架构级变更关键词' };
    }
    for (const p of SIMPLE_PATTERNS) {
        if (p.test(taskDescription))
            return { tier: 'simple', reason: '检测到简单修改关键词' };
    }
    const len = taskDescription.length;
    if (len > 80)
        return { tier: 'medium', reason: '描述较长，需要规划' };
    if (len > 30)
        return { tier: 'medium', reason: '中等复杂度任务' };
