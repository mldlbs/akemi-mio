...
let _comfyUI = null;
export function setComfyUIManager(mgr) {
    _comfyUI = mgr;
}
export const generateImageTool = buildTool({
    ...
    handler: async (args) => {
        ...
        if (!prompt)
            return formatToolError('prompt 不能为空');
        // ComfyUI 可用 → 优先本地生图
        if (_comfyUI?.isReady) {
...
// ─── ComfyUI 本地生图 ───
async function generateWithComfyUI(prompt, size, negativePrompt, refImage) {
...
async function generateWithCogView(prompt, args) {
    ...
    try {
        ...
        for (let i = 0; i < data.data.length; i++) {
            ...
            if (img.revised_prompt)
                line.push(`优化提示词: ${img.revised_prompt}`);
