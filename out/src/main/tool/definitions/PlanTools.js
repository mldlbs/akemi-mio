...
export const createDevPlanTool = buildTool({
    ...
    handler: async (args) => {
        try {
            ...
            if (!pm)
                return formatToolError('计划管理器尚未就绪');
    ...
    handler: async (args) => {
        try {
            ...
            if (!ok && args.plan_id) {
                ...
                const match = allPlans.find((p) => p.title === args.plan_id);
                if (match) {
                    ok = pm.updateStep(match.id, args.step_index, args.status, args.result);
                }
            }
            if (!ok)
                return formatToolError('更新失败：计划或步骤不存在');
...
export const listPlansTool = buildTool({
    ...
    handler: async () => {
        try {
            ...
            if (plans.length === 0)
                return formatToolResult('暂无开发计划');
            ...
            for (const p of plans) {
                ...
                if (isActive) {
                    for (let i = 0; i < p.steps.length; i++) {
                        const s = p.steps[i];
                        if (s.status !== 'done')
                            text += `  ${i}: [${s.status === 'in_progress' ? '→' : ' '}] ${s.description}\n`;
                    }
    ...
    handler: async (args) => {
        try {
            ...
            if (!ok && args.plan_id) {
                ...
                const match = allPlans.find((p) => p.title === args.plan_id);
                if (match)
                    ok = pm.completePlan(match.id, args.reflection);
            }
            if (!ok)
                return formatToolError('计划不存在');
    ...
    handler: async (args) => {
        try {
            ...
            if (!ok && args.plan_id) {
                ...
                const match = allPlans.find((p) => p.title === args.plan_id);
                if (match)
                    ok = pm.abandonPlan(match.id, args.reason);
            }
            if (!ok)
                return formatToolError('计划不存在');
