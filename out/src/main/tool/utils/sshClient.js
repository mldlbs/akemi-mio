...
function getSSHAuth() {
    ...
    const keyPath = (() => {
        ...
        if (customKey)
            return customKey.startsWith('~') ? join(homedir(), customKey.slice(1)) : customKey;
...
function connectSSH(host, port, username, auth) {
...
function execCommand(client, command, timeout) {
    return new Promise((resolve, reject) => {
...
/** SSH 执行远程命令 */
export async function sshExec(host, command, timeout) {
    if (!command)
        throw new Error('command 参数缺失');
...
/** SSH 读取远程文件内容 */
export async function sshReadFile(remotePath) {
    if (!remotePath)
        ...
        return await new Promise((resolve, reject) => {
            client.sftp((err, sftp) => {
                ...
                sftp.readFile(remotePath, (err2, data) => {
                    sftp.end();
                    if (err2)
                        return reject(new Error(`远程读取文件失败: ${err2.message}`));
...
/** SSH 写入远程文件 */
export async function sshWriteFile(remotePath, content) {
    if (!remotePath)
        throw new Error('path 参数缺失');
    if (content === undefined || content === null)
        throw new Error('content 参数缺失');
    const cfg = getDefaultSSHConfig();
    const auth = getSSHAuth();
    const client = await connectSSH(cfg.host, cfg.port, cfg.username, auth);
    try {
        await new Promise((resolve, reject) => {
            client.sftp((err, sftp) => {
                ...
                sftp.writeFile(remotePath, Buffer.from(content, 'utf-8'), (err2) => {
                    sftp.end();
                    if (err2)
                        return reject(new Error(`远程写入文件失败: ${err2.message}`));
                    resolve();
                });
            });
        });
...
/** SSH grep 远程文件内容 */
export async function sshGrep(pattern, path) {
    if (!pattern)
        throw new Error('pattern 参数缺失');
...
/** SSH glob 搜索远程文件 */
export async function sshSearchFiles(pattern) {
    if (!pattern)
        throw new Error('pattern 参数缺失');
