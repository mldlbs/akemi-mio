...
import { TIER_STAGES } from './skills/types';
...
export function getPipeline(tier) {
    const names = TIER_STAGES[tier];
    if (!names)
        return [];
    return names.map((n) => STAGE_SKILLS.find((s) => s.name === n)).filter(Boolean);
}
export function getPipelineModule(tier) {
...
export function matchStandaloneSkills(taskDescription) {
    const matched = [];
...
export function analyzeComplexity(taskDescription) {
...
/** 检查项目是否已有初始化知识资产 */
export function checkColdStart(projectDir) {
    const kbDir = resolve(projectDir, 'knowledge-base');
    if (!existsSync(kbDir))
        return { isColdStart: true, existing: [] };
...
export function getColdStartPrompt() {
...
export function getIncrementalPrompt(kbDir, existing) {
