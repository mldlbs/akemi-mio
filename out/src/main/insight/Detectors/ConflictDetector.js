const CONTRADICTION_PAIRS = [
    [/(\d+)\s*轮.*对话|连续对话.*(\d+)\s*轮|(\d+).*context/i, /(\d+)\s*轮.*窗口|窗口.*(\d+)\s*轮|只保[留].*\d+/i, '对话轮次目标与窗口限制冲突', 'high'],
...
export class ConflictDetector {
    detect(ctx) {
        const results = [];
        const allText = [
            ...ctx.memoryEntries.map(e => e.content),
            ...ctx.summaries
        ].join('\n');
        ...
        if (ctx.interactionCount > 20) {
            const stalled = ctx.plans.filter(p => p.status === 'active' &&
                (Date.now() - p.updatedAt) > 7 * 24 * 60 * 60 * 1000);
            if (stalled.length > 0) {
