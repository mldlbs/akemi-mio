...
export class FrictionDetector {
    detect(ctx) {
        ...
        if (!reworkMentions || reworkMentions.length < 3)
            return results;
        ...
        const modules = reworkTokens.map(t => {
        ...
        const hasThrashing = modules.length >= 3 && ((modules[0] === modules[2]) ||
            new Set(modules.slice(0, 4)).size <= 2);
        if (hasThrashing) {
            results.push({
                detector: 'FrictionDetector',
                type: 'friction',
