...
export class DrizzleInsightStore {
    ...
    addMany(items) {
        ...
        for (const item of items) {
            db.run('INSERT OR IGNORE INTO insights (id, detector, title, description, evidence, score, confidence, reported, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)', [item.id, item.detector, item.title, item.description, JSON.stringify(item.evidence), item.score, item.confidence, item.createdAt]);
        }
        markDirty();
    ...
    getHighValueUnreported(scoreThreshold = 50, confidenceThreshold = 0.7) {
        const db = getRawDb();
        const result = db.exec(`SELECT * FROM insights WHERE reported = 0 AND score >= ${scoreThreshold} AND confidence >= ${confidenceThreshold} ORDER BY score DESC`);
        if (!result || result.length === 0 || result[0].values.length === 0)
            return [];
        return this.rowsToInsights(result[0].columns, result[0].values);
    }
