...
export class VectorMemory {
    ...
    async store(content, confidence, source) {
        ...
        const entry = { id, content, embedding, confidence, source, createdAt: Date.now(), updatedAt: Date.now() };
    ...
    saveToDb(e) {
        try {
            const db = getRawDb();
            db.run('INSERT OR IGNORE INTO memory_vectors (id, content, embedding, confidence, source, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [e.id, e.content, JSON.stringify(e.embedding), e.confidence, e.source, e.createdAt, e.updatedAt]);
            markDirty();
        }
        catch (err) {
