...
export class SummaryMemory {
    ...
    addSummary(summary, turnStart, turnEnd, options) {
        ...
        if (this.entries.length > MAX_SUMMARIES)
            this.entries = this.entries.slice(-MAX_SUMMARIES);
        this.dirty = true;
        this.saveToDb(entry);
    ...
    saveToDb(entry) {
        try {
            ...
            const columns = info?.[0]?.values?.map((v) => v[1]) || [];
            const hasTopics = columns.includes('topics');
            if (hasTopics) {
                db.run(`INSERT OR IGNORE INTO memory_summaries (id, summary, turn_start, turn_end, topics, decisions, key_entities, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                    entry.id,
                    entry.summary,
                    entry.turnStart,
