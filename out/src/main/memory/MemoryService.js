...
export class MemoryService {
    ...
    load() {
        try {
            ...
            if (result && result.length > 0) {
                ...
                this.entries = result[0].values.map((v) => {
                    const obj = {};
    ...
    addEntry(type, content, confidence, options) {
        ...
        // 2. 前缀模糊去重（近似内容合并）
        if (type === 'user_fact') {
            const similarIdx = this.entries.findIndex((e) => e.type === 'user_fact' &&
                content.length > 10 &&
                e.content.length > 10 &&
    ...
    upsertInDb(entry) {
        try {
            const db = getRawDb();
            db.run('INSERT OR REPLACE INTO memories (id, type, content, confidence, tier, reinforce_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [entry.id, entry.type, entry.content, entry.confidence, entry.tier, entry.reinforceCount, entry.createdAt, entry.updatedAt]);
            markDirty();
        }
        catch (err) {
