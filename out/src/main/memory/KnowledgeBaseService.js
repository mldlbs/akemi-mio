...
export class KnowledgeBaseService {
        ...
        if (!options.includeStores || options.includeStores.includes('memory')) {
            for (const entry of ms.getEntries?.() || []) {
                if (entry.confidence < minConfidence)
                    continue;
                results.push(this.toResult(entry.content, 'memory', entry.confidence, entry.tier === 'permanent' ? 100 : PRIORITY_MAP.memory, entry.updatedAt));
            }
        }
        // 2. SummaryMemory
    ...
    toResult(content, source, confidence, priority, timestamp) {
        return {
    ...
    crossReference(results) {
        for (let i = 0; i < results.length; i++) {
            for (let j = i + 1; j < results.length; j++) {
                ...
                if (shared.length >= 2) {
                    if (!a.crossReferences)
                        a.crossReferences = [];
                    if (!a.crossReferences.includes(b.source))
                        a.crossReferences.push(b.source);
                    if (!b.crossReferences)
                        b.crossReferences = [];
                    if (!b.crossReferences.includes(a.source))
                        b.crossReferences.push(a.source);
                }
            }
        }
        return results;
    ...
    getFormattedContext(options) {
        ...
        for (const r of results) {
            ...
            if (r.crossReferences?.length) {
                prefix += `(关联:${r.crossReferences.map((s) => this.storeLabel(s)).join(',')})`;
            }
