...
export class CognitiveService {
    constructor() {
        this.metaCycle = new MetaCycle();
        this.initialized = false;
        this.defaultStrategies = false;
    ...
    async initialize(constitutionPath, deps) {
        if (this.initialized)
            return;
        ...
        if (deps?.engineeringMemory && deps?.proceduralMemory && deps?.llmService && this.identity.getCoreIdentity()) {
            ...
            scheduler.interval(7 * 24 * 60 * 60 * 1000, async () => {
                await this.metaCycle.run();
                return '';
            }, '@meta');
            // 首次启动 60s 后跑一次初始评估
            scheduler.once(60 * 1000, async () => {
                await this.metaCycle.run();
                return '';
            }, '@meta');
        }
    ...
    async adjustByToken(failureLogs) {
    ...
    inferGoalFromError(recommendation) {
