...
export class MetaCycle {
    ...
    applyReview(review) {
        ...
        const db = getRawDb();
        const id = `meta_${now}`;
        db.run(`INSERT INTO meta_reviews (id, period_start, period_end, summary, patterns, improvements, trait_deltas, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            id,
            this.periodStart,
            now,
