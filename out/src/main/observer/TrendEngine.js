...
export class TrendEngine {
    ...
    async detectTrends(days = 3) {
        ...
        const signals = deduped.map(({ keyword, obsList }) => {
            ...
            const ageH = (nowMs - latestTs) / 3600000;
...
[{"keyword": "xxx", "matched_ids": ["id1","id2"]}]`;
        ...
        for (const entry of result.data) {
            if (!entry.keyword || !Array.isArray(entry.matched_ids))
                continue;
            const obs = entry.matched_ids.map((id) => obsById.get(id)).filter((o) => o !== undefined);
            if (obs.length === 0)
                continue;
