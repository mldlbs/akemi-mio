...
import { INSIGHT_SECTION_TITLES } from './types';
/**
...
export class InsightComposer {
    ...
    async compose(topic, research, brainOutputs, mode) {
        ...
        if (s1)
            sections.push({ title: INSIGHT_SECTION_TITLES[1], content: s1 });
        else
            missingSections.push(INSIGHT_SECTION_TITLES[1]);
    ...
    assembleSection1(research) {
        ...
        if (research.facts.length > 0)
            lines.push(...research.facts.slice(0, 8).map((f) => `- ${f}`));
