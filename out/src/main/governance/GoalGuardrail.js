...
export class GoalGuardrail {
    constructor(goalEngine, constitutionEngine, config) {
        this.systemBus = null;
     ...
     */
    async checkBatch(toolCalls, messages, ctx) {
        if (!toolCalls.length) {
        ...
        if (softScore < 0.5) {
            ...
            messages.push({ role: 'user', content: denial.message });
            return denial;
        }
        // ── 守卫通过 ──
        this.previousTurnDenied = false;
     ...
     */
    async evaluateConsistency(toolCalls) {
        // 如果 SystemBus 在线，使用多因子 utility score
        if (this.systemBus) {
            const result = await this.systemBus.query('utility-score', {
     ...
     */
    detectGoalDrift(toolNames, goalCategories) {
     ...
     * 根据偏离分数生成不同强度的对齐提示。
     */
    buildGoalDriftDecision(score) {
        const severity = score <= 0.3 ? 'severe' : score < 0.5 ? 'moderate' : 'mild';
        const messages = {
