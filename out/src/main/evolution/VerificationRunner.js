...
export class VerificationRunner {
    ...
    async verify(changedFiles) {
        ...
        if (workerAvailable && this.workerPool) {
            try {
                const result = await this.workerPool.sendTaskAndWait('verification', 'verify', { config: this.config, changedFiles }, Math.max(this.config.timeout, 120000) + 5000);
    ...
    async _doVerify(changedFiles) {
        ...
        if (this.config.compileCheck)
            checks.compile = await this.runCompileCheck();
        if (this.config.testRun)
            checks.test = await this.runTestCheck(changedFiles);
        if (this.config.lintCheck)
            checks.lint = await this.runLintCheck(changedFiles);
        const passed = Object.values(checks).every((c) => c?.passed !== false);
