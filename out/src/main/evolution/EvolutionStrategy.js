...
export class EvolutionStrategyLearner {
    ...
    loadScores() {
        try {
            if (!existsSync(this.scoreFilePath))
                return;
            const raw = JSON.parse(readFileSync(this.scoreFilePath, 'utf-8'));
    ...
    learn() {
        ...
        if (recent.length < 5)
            return { insight: '数据不足，继续收集', adjustments };
        ...
        for (const h of recent) {
            if (!successRates.has(h.strategy))
                successRates.set(h.strategy, []);
...
export class StrategyMutator {
    ...
    prune() {
        ...
        for (const [name, score] of this.scores) {
            if (DEFAULT_STRATEGIES.some((ds) => ds.name === name))
                continue; // never prune defaults
            if (score.samples >= 3 && score.score < 40)
                toRemove.push(name);
            if (score.lastUsed > 0 && score.lastUsed < maxAge && score.samples < 3)
                toRemove.push(name);
        }
    ...
    // ==================== 内部方法 ====================
    getViableStrategies() {
        return Array.from(this.strategies.values()).filter((s) => !DEFAULT_STRATEGIES.some((ds) => ds.name === s.name));
    }
