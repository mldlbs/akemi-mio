...
export class PromptEvolutionManager {
    ...
    resetToBase(mode) {
        const slot = this.registry.prompts[mode];
        if (!slot || slot.currentVersion === slot.baseVersion)
            return null;
    ...
    async llmEvolvePrompt(mode, reason, failureSummary, agentRunner) {
        ...
        try {
            const result = await withTimeout(() => agentRunner.runSelfTask(llmPrompt, '你是 prompt 优化器。只输出反模式指令列表。'), 5000, 'prompt_llm_evolve_timeout');
    ...
    pruneStaleRules(mode) {
        ...
        if (!current || !current.promptOverlay)
            return 0;
        let removedCount = 0;
        for (let v = slot.baseVersion; v <= slot.latestVersion; v++) {
            const pv = this.versions.get(`${mode}_v${v}`);
            if (!pv || v === slot.currentVersion)
                continue;
            ...
            if (avg !== undefined && avg < 40) {
                ...
                for (const word of reasonWords) {
                    ...
                    if (current.promptOverlay.length < before)
                        removedCount++;
                }
            }
        }
    ...
    shouldCompact(mode) {
        const slot = this.registry.prompts[mode];
        if (!slot)
            return { needSummarize: false, needPrune: false };
        const current = this.versions.get(`${mode}_v${slot.currentVersion}`);
    ...
    load() {
        try {
            ...
            const raw = JSON.parse(readFileSync(idxFile, 'utf-8'));
            this.registry = { version: raw.version, prompts: raw.prompts || {} };
            for (const [mode, slot] of Object.entries(this.registry.prompts)) {
                for (let v = slot.baseVersion; v <= slot.latestVersion; v++) {
                    const vf = join(this.promptsDir, `${mode}_v${v}.json`);
                    if (existsSync(vf)) {
                        this.versions.set(`${mode}_v${v}`, JSON.parse(readFileSync(vf, 'utf-8')));
                    }
                }
            }
    ...
    save() {
        if (!this.persistenceDirty && this.initialized)
            return;
        try {
            if (!existsSync(this.promptsDir))
                mkdirSync(this.promptsDir, { recursive: true });
            writeFileSync(join(this.promptsDir, 'index.json'), JSON.stringify({ version: this.registry.version, prompts: this.registry.prompts }, null, 2), 'utf-8');
