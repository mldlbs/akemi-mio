...
export class DrizzlePlanManager {
    ...
    findSimilarActivePlan(title) {
        ...
        if (!normalized)
            return undefined;
        return this.listActivePlans().find((p) => this.titleSimilarity(this.normalizeTitle(p.title), normalized) >= DrizzlePlanManager.TITLE_SIMILARITY_THRESHOLD);
    }
    ...
    createInMemoryPlan(title, description, stepDescriptions, now, priority) {
...
/** 当前活跃计划上限 */
DrizzlePlanManager.MAX_ACTIVE_PLANS = 3;
/** 标题相似度阈值 (0-1)，低于此值视为重复计划 */
DrizzlePlanManager.TITLE_SIMILARITY_THRESHOLD = 0.75;
