 ...
 */
export const ANALYSIS_PROMPT = (mode, planContext, historySummary, safetyMode, validationSummary, promptOverlay) => {
...
export const PLAN_EXECUTE_PROMPT = (planCtx, stepDesc) => [
    '你是秋山澪的自进化系统 — **步骤执行模式**。',
    '',
...
/** 构建计划上下文注入字符串 */
export function buildPlanInjection(plan, doneSteps, totalSteps, pendingSteps) {
...
/** 从多个活跃计划中选择进度最高的一个继续 */
export function pickBestPlan(plans) {
    if (plans.length === 0)
        return null;
    return plans
        .map((p) => ({ ...p, doneRatio: p.steps.filter((s) => s.status === 'done').length / Math.max(p.steps.length, 1) }))
        .sort((a, b) => b.doneRatio - a.doneRatio)[0];
}
/** 检测计划模式 */
export function detectPlanMode(planManager) {
    ...
    for (const p of activePlansPre) {
        ...
        if (doneSteps === 0 && p.createdAt < staleThreshold) {
