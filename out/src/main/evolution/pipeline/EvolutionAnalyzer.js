...
import { ANALYSIS_PROMPT, detectPlanMode } from '../EvolutionPromptBuilder';
export class EvolutionAnalyzer {
    constructor(agentService, planManager, options) {
        this.name = 'EvolutionAnalyzer';
        this.state = 'created';
        this.promptTrimMode = false;
        this.historyMaxEntries = 5;
        this.degenerationThreshold = 3;
    ...
    isActivePlanStale() {
        ...
        if (!plan)
            return true;
        const staleThreshold = Date.now() - 900000; // 15 分钟（原 1 小时）
    ...
    buildLivingPlanContext() {
        ...
        const allSections = [];
        if (mission)
            allSections.push({ label: '【Mission — 使命】', content: mission, priority: 4 });
        if (goals)
            allSections.push({ label: '【Goals — 长期目标】', content: goals, priority: 3 });
        if (planTree)
            allSections.push({ label: '【Plan Tree — 计划树】', content: planTree, priority: 2 });
        if (queue)
            allSections.push({ label: '【Execution Queue — 执行队列】', content: queue, priority: 1 });
        const template = [
            '',
            '---',
    ...
    isDegenerate() {
        if (this.recentAnalysisFingerprints.length < this.degenerationThreshold)
            return false;
    ...
    getFingerprintAgeMs() {
        if (this.recentAnalysisFingerprints.length === 0)
            return Infinity;
        const history = this.loadHistory();
