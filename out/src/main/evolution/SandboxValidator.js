...
const PATTERNS = [
    // ── p5.js 渲染顺序错误：image() 在所有粒子/形状绘制之后 ──
    {
        ...
        test: (content) => {
            ...
            lines.forEach((line, i) => {
                const l = line.trim();
                if (l.startsWith('//') || l.startsWith('if') || l.startsWith('}') || l === '}' || l === '{' || l.endsWith('{'))
                    return;
                if (/\bimage\s*\(/.test(l))
                    lastImageLine = i;
                if (/\.(update|show|display|draw|render)\s*\(/.test(l) || /\b(line|rect|ellipse|triangle|point)\s*\(/.test(l))
                    lastDrawLine = i;
            });
    ...
    // ── canvas 尺寸保护 ──
    {
        ...
        test: (content) => {
            if (!/createCanvas/.test(content))
                return { found: false };
            if (/Math\.max/.test(content) || /Math\.min/.test(content))
                return { found: false };
            if (/\b(innerWidth|innerHeight)\b/.test(content))
                return { found: false };
    ...
    // ── p5.js 脚本加载验证 ──
    {
        ...
        test: (content) => {
            if (!/cdnjs\.cloudflare\.com\/ajax\/libs\/p5\.js/.test(content))
                return { found: false };
...
export function validateSandboxHtml(filePath) {
    let content;
    try {
    ...
    if (html.errors.length)
        parts.push(`HTML 结构错误 ${html.errors.length} 项`);
    if (errors.length)
        parts.push(`渲染错误 ${errors.length} 项`);
    if (resources.missing.length)
        parts.push(`缺失本地资源 ${resources.missing.length} 项`);
 ...
 */
export function validateAllSandboxes(sandboxRoot) {
