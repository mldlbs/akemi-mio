...
export class EvolutionSelfEvaluator {
    ...
    getCalibration() {
        ...
        if (completed.length < 3)
            return { bias: 0, sampleSize: completed.length, isReliable: false };
        let totalDiff = 0;
    ...
    evaluate(params) {
        ...
        const strategyCompliance = this.evaluateStrategyCompliance(params.strategyName, params.promptMode, params.analysisMode, params.analysisSummary);
        ...
        const feedback = [];
        if (planQuality < 50 && params.planCreated)
            feedback.push('计划步骤缺少具体文件引用，应引用项目中的实际文件路径');
        if (analysisDiversity < 40)
            feedback.push('分析与近期结果高度重叠，需要探索新的分析方向');
        if (strategyCompliance < 50)
            feedback.push('策略选择与执行模式不匹配，检查策略配置');
        if (substantiveLength < 30)
            feedback.push('分析摘要过短，缺乏详细推理过程');
        const result = {
            score,
            timestamp: Date.now(),
    ...
    getTrend() {
        if (this.recentEvaluations.length < 6)
    ...
    evaluatePlanQuality(planSteps, planCreated) {
        if (!planCreated)
            return 50;
        ...
        if (totalRefs === 0)
            return 30;
        if (totalRefs >= planSteps.length * 2)
            return 95;
    ...
    evaluateDiversity(summary, recentHistory) {
        ...
        if (tokens.size === 0 || recentHistory.length === 0)
            return 70;
        let maxOverlap = 0;
    ...
    evaluateStrategyCompliance(strategyName, promptMode, analysisMode, analysisSummary) {
        ...
        if (promptMode !== expectedPromptMode[strategyName])
            return 60;
        if (strategyName === 'review' && /write_file|edit_file|execute/.test(analysisSummary))
            return 30;
        return 95;
    }
    /** 摘要长度：是否足够详细 */
