...
export class ObservabilityLogger {
    ...
    logPrompt(messages) {
        ...
        for (const m of messages) {
            ...
            if (m.tool_calls?.length) {
                content += ` [tool_calls: ${m.tool_calls.map((t) => t.function?.name || t.name).join(', ')}]`;
            }
        ...
        this.entries.push({
            tag: 'PROMPT',
            body: joined.length > 30000 ? joined.slice(0, 30000) + '\n  ... [truncated]' : joined,
        });
    }
    logToolBatch(results) {
        if (!isEnabled())
            return;
        const lines = results.map((r) => `  ${r.name} → ${r.success ? 'success' : 'fail'}${r.error ? `  error: ${r.error.slice(0, 200)}` : ''}${r.success ? `  (${r.latencyMs}ms)` : ''}`);
        this.entries.push({
            tag: 'TOOL',
            body: lines.length ? lines.join('\n') : '(no tools)',
        });
    }
    logOutput(text, durationMs) {
        if (!isEnabled())
            return;
        this.entries.push({
            tag: 'OUTPUT',
    ...
    /** 记录 LLM 调用断点 */
    logLlmTrace(phase, payload) {
        if (!isEnabled())
            return;
        log('INFO', 'llm_trace', { requestId: this.requestId, phase, payload });
    }
    flush() {
        if (!isEnabled() || !this.entries.length)
            return;
