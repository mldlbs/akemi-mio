...
export class RuntimeHealthManager {
    constructor() {
        this.name = 'RuntimeHealthManager';
        this.state = 'created';
        ...
        this.history = [];
        this.maxHistory = 60;
        this.lastCompositeScore = 100;
        this.modelHealth = new ModelHealthTracker();
    }
    // ── Dep injection ──
    setSessionHealthProvider(provider) {
        this.sessionProvider = provider;
    }
    setCapabilityHealthProvider(provider) {
        this.capabilityProvider = provider;
    }
    setTaskHealthProvider(provider) {
    ...
    getRecommendedActions() {
        ...
        if (session.score < 50)
            actions.push('会话健康度严重偏低，建议触发恢复流程');
        else if (session.score < 70)
            actions.push('会话健康度下降，关注工具调用成功率');
        if (capability.score < 80)
            actions.push('MCP 能力漂移，检查服务器状态');
        if (task.score < 70)
            actions.push('后台任务失败率高，存在反复失败的任务');
        if (model.score < 60)
            actions.push('LLM 错误率过高，检查 API Key 和网络状态');
        else if (model.failureCount > 10)
            actions.push(`LLM 累计失败 ${model.failureCount} 次`);
        if (composite.score < 40)
            actions.push('系统总健康度临界，建议立即检查各子系统');
        return actions;
    }
    getDiagnostics() {
        return {
    ...
    evaluateAll() {
        ...
        eventBus.emit('runtime.health.updated', {
            compositeScore: composite.score,
    ...
    computeComposite() {
        ...
        const modelLevel = getHealthLevel(modelScore);
        const compositeScore = Math.round(Math.min(100, Math.max(0, session.score * 0.35 + capability.score * 0.2 + task.score * 0.2 + modelScore * 0.25)));
        const level = getHealthLevel(compositeScore);
        const trend = this.computeTrend();
        return { score: compositeScore, level, trend };
    }
    getSessionDimension() {
        if (!this.sessionProvider)
            return { score: 100, level: 'HEALTHY', trend: 'stable' };
        ...
        const trend = failures > 5 ? 'declining' : failures === 0 ? 'improving' : 'stable';
    ...
    getCapabilityDimension() {
        ...
        const trend = drifting ? 'declining' : score >= 95 ? 'stable' : 'improving';
        return { score, level, trend };
    }
    getTaskDimension() {
        ...
        const level = getHealthLevel(score);
        const trend = failingCount > 0 && maxFailures > 3 ? 'declining' : 'stable';
        return { score, level, trend };
    }
    computeTrend() {
