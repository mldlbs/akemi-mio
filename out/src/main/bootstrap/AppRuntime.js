...
import { initEvolution, evolutionService, planManager, ProposalValidator, EvolutionGitOps, setSandboxRoot, } from '../evolution';
...
export class AppRuntime {
    ...
    async start() {
        ...
        const kernelModule = {
            ...
            getExport: (n) => (n === 'SyscallBus' ? this.syscallBus : undefined),
            handleSyscall: async (method) => {
                if (method === 'list_syscalls')
                    return { modules: this.syscallBus.getRegisteredModules() };
        ...
        this.healthChecker.register(this.syscallBus);
        ...
        let previousStabilityStatus;
        this.taskRunner.register('stability.tick', async () => {
        ...
        if (llmService) {
            llmExtractor.setLlm({
                chatJson: (prompt, opts) => llmService.chatJson(prompt, opts),
            });
        }
        memoryService.knowledgeGraph.setLLMExtractor(llmExtractor);
        // Phase 5: SystemBus 注册 & 冻结
        systemBus.registerQuery('utility-score', 'goal-engine', async () => {
        ...
        systemBus.registerQuery('utility-score', 'resource-budget', async () => {
        ...
        systemBus.registerQuery('utility-score', 'memory-activity', async () => {
            if (!memoryService)
                return 0.5;
        ...
        this.taskRunner.register('social.tick', async () => {
            try {
    ...
    registerCoreEventBus() {
        eventBus.track('agent.tool.invoked', (p) => log('INFO', 'tool_start', { tool: p.tool }), this.subs, 'runtime:tool_invoked');
        eventBus.track('agent.tool.completed', (p) => log('INFO', 'tool_success', { tool: p.tool }), this.subs, 'runtime:tool_completed');
        eventBus.track('agent.tool.failed', (p) => log('WARN', 'tool_error', { tool: p.tool, error: p.error }), this.subs, 'runtime:tool_failed');
        eventBus.track('agent.input.received', (p) => log('CHAT', 'agent_input', { request_id: p.requestId, text: sanitizeForLog(p.text) }), this.subs, 'runtime:agent_input');
        eventBus.track('agent.response.generated', (p) => log('CHAT', 'agent_output', { request_id: p.requestId, reply_len: p.text.length }), this.subs, 'runtime:agent_output');
        eventBus.track('agent.plan.created', (p) => log('INFO', 'plan_created', { plan_id: p.planId, title: p.title }), this.subs, 'runtime:plan_created');
        eventBus.track('agent.plan.step', (p) => log('INFO', 'plan_step', { plan_id: p.planId, step: p.stepIndex, status: p.status }), this.subs, 'runtime:plan_step');
        eventBus.track('agent.plan.completed', (p) => log('INFO', 'plan_done', { plan_id: p.planId }), this.subs, 'runtime:plan_completed');
        eventBus.track('evolution.cycle.started', () => log('INFO', 'evolution_cycle_start'), this.subs, 'runtime:evolution_start');
        eventBus.track('evolution.cycle.completed', (p) => log('INFO', 'evolution_cycle_end', { success: p.success }), this.subs, 'runtime:evolution_done');
        eventBus.track('recovery.checkpoint.created', (p) => log('INFO', 'checkpoint_created', { runId: p.runId, trigger: p.trigger }), this.subs, 'runtime:checkpoint');
        eventBus.track('recovery.session.restored', (p) => log('INFO', 'session_restored', { runId: p.runId, hasPlan: p.hasUnfinishedPlan }), this.subs, 'runtime:session_restored');
        eventBus.track('recovery.error.classified', (p) => log('INFO', 'error_classified', { category: p.category, strategy: p.strategy }), this.subs, 'runtime:error_classified');
        eventBus.track('recovery.context.compress', (p) => log('INFO', 'context_compressed', { before: p.beforeTokens, after: p.afterTokens }), this.subs, 'runtime:context_compress');
    }
    registerLazyServices(agentService, llmService, memoryService, memoryIndexer, stateManager, planManager, cognitiveService) {
        // 进化服务
        this.lazyInit.add({
            name: 'evolution',
            priority: 'normal',
        ...
        this.lazyInit.add({
            ...
            delayMs: 400,
            fn: async () => {
                const insight = initInsight(join(WORKSPACE.cache, 'insights.json'), {
                    ...
                    getPlans: () => {
                        try {
                            return planManager
                                .listPlans()
                                .map((p) => ({ title: p.title, status: p.status, updatedAt: p.updatedAt, steps: p.steps, createdAt: p.createdAt }));
                        }
                        catch {
                            return [];
                        }
                    },
                }, { chatJson: llmService.chatJson.bind(llmService) }, this.taskRunner);
                insight.start();
        ...
        this.lazyInit.add({
            ...
            delayMs: 600,
            fn: async () => {
                const creativity = initCreativity(join(WORKSPACE.cache, 'creativity.json'), {
                    ...
                    getInsights: () => {
                        try {
                            ...
                            return insightStore
                                .getUnreported()
                                .slice(0, 10)
                                .map((i) => ({ title: i.title, description: i.description, score: i.score }));
                        }
                        catch {
                            return [];
                        }
                    ...
                    getFailedHypotheses: () => {
                        ...
                        if (!store)
                            return [];
                        return store.getHypotheses({ status: 'rejected' }).map((h) => ({ title: h.title, idea: h.idea, risk: h.risk }));
                    },
                }, llmService.chatJson.bind(llmService), 0.3, join(WORKSPACE.evolution, 'creativity', 'reports'), this.taskRunner, join(WORKSPACE.evolution, 'observer'), llmService.chatJsonWithCode.bind(llmService));
                creativity.start();
        ...
        this.lazyInit.add({
            ...
            fn: async () => {
                if (!this.workerPool)
                    return;
                try {
                    await this.workerPool.sendTaskAndWait('observer', 'init', { baseDir: join(WORKSPACE.evolution, 'observer') }, 10000);
                    const llmResult = await this.workerPool.sendTaskAndWait('observer', 'initLlm', null, 15000);
                    if (llmResult?.loaded) {
                        await this.workerPool.sendTaskAndWait('observer', 'start', null, 5000);
        ...
        this.lazyInit.add({
            ...
            delayMs: 100,
            fn: async () => {
                eventBus.track('insight.detector.completed', (p) => log('INFO', 'insight_detector', { detector: p.detector, findings: p.findings }), this.subs, 'runtime:insight_detector');
                eventBus.track('insight.candidate.generated', (p) => log('INFO', 'insight_candidates', { count: p.count }), this.subs, 'runtime:insight_candidates');
                eventBus.track('insight.found', (p) => log('INFO', 'insight_found', { count: p.count, top: p.insights?.[0]?.title }), this.subs, 'runtime:insight_found');
                eventBus.track('insight.analysis.completed', (p) => log('INFO', 'insight_analysis_end', { count: p.count, hasValue: p.hasValue }), this.subs, 'runtime:insight_analysis');
                eventBus.track('creativity.cycle.started', () => log('INFO', 'creativity_cycle_start'), this.subs, 'runtime:creativity_start');
                eventBus.track('creativity.ideas.generated', (p) => log('INFO', 'creativity_ideas', { count: p.count, top: p.ideas?.[0]?.title, novelty: p.ideas?.[0]?.novelty }), this.subs, 'runtime:creativity_ideas');
                eventBus.track('creativity.dream.completed', (p) => log('INFO', 'creativity_dream_end', { count: p.count, top_novelty: p.topNovelty }), this.subs, 'runtime:creativity_dream');
            },
        });
    }
    logModelConfig(llmKey) {
    ...
    buildCreativitySources(memoryService, pm) {
        const recentTopics = [];
        const entryCount = 0;
    ...
    initGpuAsync(stateManager, gpuEngine, asrService) {
